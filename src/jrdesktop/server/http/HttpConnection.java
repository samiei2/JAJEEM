...
public class HttpConnection extends Thread {
	...
	public void run() {
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (line.startsWith("GET / ")) {
					...
					InputStream is = main.class
							.getResourceAsStream("server/http/index.html");
					...
					// send page content
					out.writeBytes("<html><head><title>jrdesktop - Java Remote Desktop</title></head>"
							+ "<body style='margin:0; padding: 0;'><link href='http://"
							+ RMIServer.serverConfig.server_address
							+ ":"
							+ RMIServer.serverConfig.http_port
							+ "/idle.png' "
					...
					// initiat input stream
					InputStream is = main.class
							.getResourceAsStream("images/idle.png");
					BufferedInputStream bis = new BufferedInputStream(is);
					...
					// initiat input stream
					InputStream is = main.class
							.getResourceAsStream("images/application.png");
					BufferedInputStream bis = new BufferedInputStream(is);
					...
					// initiat input stream
					InputStream is = main.class
							.getResourceAsStream("images/applet.png");
					BufferedInputStream bis = new BufferedInputStream(is);
					...
					// initiat input stream
					InputStream is = main.class
							.getResourceAsStream("images/jws.png");
					BufferedInputStream bis = new BufferedInputStream(is);
					...
					// initiat input stream
					InputStream is = main.class
							.getResourceAsStream("images/extension.png");
					BufferedInputStream bis = new BufferedInputStream(is);
					...
					// initiat input stream
					InputStream is = main.class
							.getResourceAsStream("images/portable.png");
					BufferedInputStream bis = new BufferedInputStream(is);
			...
			// Close the streams and socket, breaking the connection to the
			// client
			out.close();
			in.close();
			client.close();
	 ...
	 * @param url
	 * @param isApplet
	 *            if <code>true</code>, generate a JWS applet, otherwise a JWS
	 *            application
	...
	public String getJNLP(String url, boolean isApplet) {
		return "<?xml version='1.0' encoding='UTF-8' standalone='yes'?>"
				+
				// "<jnlp codebase='" + url + "' href='jrdesktop" + (isApplet ?
				// "_applet" : "") +
				// ".jnlp' spec='1.0+'>" +
				"<jnlp spec='1.0+'>"
				+ "<information>"
				...
				+ "  <jar eager='true' href='"
				+ url
				+ "jrdesktop.jar' main='true'/>"
				+ "</resources>"
				+ (isApplet ? "<applet-desc " + "	width='460'"
						+ "	height='320'"
