...
public class Server extends Thread {
	...
	public static HashMap<String, byte[]> getChangedScreenBlocks(int index,
		...
		if (viewers.containsKey(index)) {
			viewers.get(index).connectionInfos.incReceivedData(ScreenCapture
					.getChangedBlocksSize(blocks));
		}
		return blocks;
	}
	...
	public static byte[] ReceiveFile(String fileName, int index) {
		try {
			...
			/*
			 * if (Config.reverseConnection) ((Recorder)
			 * viewers.get(index)).viewerOptions.connectionInfos.
			 * incSentData(buffer.length); else
			 */
	...
	public void getOption(int option) {
		try {
			recorder.viewerOptions.setOption(
					client.rmiServer.getOption(index, option), option);
	...
	public void getOptions() {
		try {
			recorder.viewerOptions.setOptions(client.rmiServer
					.getOptions(index));
		} catch (RemoteException re) {
			re.printStackTrace();
		}
	...
	public void setScreenRect() {
		try {
			client.rmiServer.setScreenRect(
					recorder.viewerOptions.getScreenRect(), index);
		} catch (RemoteException re) {
			re.printStackTrace();
		}
	}
	...
	public void setScreenCapture() {
		try {
			if (recorder.viewerOptions.isScreenCompressionEnabled()) {
				client.rmiServer
						.setChangedScreenBlocks(rt.getChangedScreenBlocks(
								recorder.viewerOptions, true), index);
				// recorder.viewerOptions.getCapture().getChangedBlocks(),
				// index);
			} else {
				client.rmiServer.setScreenCapture(
						rt.CaptureScreenByteArray(recorder.viewerOptions),
						index);
			}
		} catch (RemoteException re) {
			re.printStackTrace();
