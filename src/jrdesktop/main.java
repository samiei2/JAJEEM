...
public class main {
		...
		if (args.length > 0) {
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (arg.startsWith("--hide")) {
					...
					proxy_port = Integer.valueOf(arg.substring(9)); // get proxy
																	// port
	...
	public static void displayHelp(boolean full) {
		System.out
				.println("jrdesktop - Java Remote Desktop.\n"
						+ "A cross-platform software that provides remote view and control of a computer.\n"
						+ "http://jrdesktop.net/\n\n");
		if (full) {
			System.out
					.println("Usage: java -jar jrdesktop.jar <command> [options]\n\n"
							+
			...
			System.out.println(Commons.jrdesktop_version + " "
					+ Commons.jrdesktop_build_date);
		}
		exit();
	}
	...
	public static void close() {
		if (Settings.displayMode == Commons.DISPLAY_MODE_APPLET) {
			JOptionPane
					.showConfirmDialog(
							null,
							"Please close the browser's window to exit this application",
	...
	public static void quit() {
		if (Settings.displayMode != Commons.DISPLAY_MODE_FRAME
				&& !Settings.exitDisabled) {
			if (JOptionPane
					.showConfirmDialog(
							null,
							"jrdesktop is running as a "
									+ (Settings.displayMode == Commons.DISPLAY_MODE_APPLET ? "web applet"
											: "Firefox extension")
							...
							"Warning Dialog", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
				main.exit();
			}
		} else if (JOptionPane.showConfirmDialog(null, "Exit application ?",
