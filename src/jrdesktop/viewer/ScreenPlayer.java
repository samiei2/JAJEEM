...
public class ScreenPlayer extends JLabel {
	...
	/*
	 * public void updateScreenRect(Rectangle rect) { if
	 * (rect.equals(Commons.emptyRect)) screenRect = new
	 * Rectangle(Toolkit.getDefaultToolkit().getScreenSize()); else screenRect =
	 * rect; if (!capture.getScreenRect().equals(screenRect)) {
	 * capture.updateScreenSize(screenRect); setNewScreenImage(screenRect,
	 * colorQuality); } }
	 */
	...
	public void updateScreenRect() {
		...
		if (!PartialScreenMode) {
			...
			if (!screenRect.equals(oldScreenRect)) {
				...
				if (!recorder.viewerOptions.capture.getScreenRect().equals(
						screenRect)) {
			...
			if (!isSelecting)
				if (!selectionRect.equals(oldselectionRect)) {
					...
					if (!recorder.viewerOptions.capture.getScreenRect().equals(
							selectionRect)) {
						recorder.viewerOptions.capture
								.updateScreenSize(selectionRect);
	...
	public void paint(Graphics g) {
		if (this.thumb) {
			g.drawImage(screenImage, 0, 0, 200, 200, this);
			DrawSelectingRect(g);
		} else {
	...
	public void UpdateScreen(HashMap<String, byte[]> changedBlocks) {
		...
		screenImage = recorder.viewerOptions.getCapture().setChangedBlocks(
				screenImage, changedBlocks);
	...
	/*
	 * public void clearScreen() { setSize(new Dimension(1, 1));
	 * setPreferredSize(new Dimension(1, 1));
	 * //recorder.viewerOptions.getCapture().clearScreen(); screenRect =
	 * Commons.emptyRect; oldScreenRect = Commons.diffRect;
	 * recorder.viewerOptions.setScreenRect(Commons.emptyRect);
	 * recorder.viewer.setOption(Commons.RECT_OPTION); repaint(); }
	 */
	...
	public void doneSelecting() {
		if (isSelecting) {
			...
			if (PartialScreenMode) {
				float screenScale = 1.0f / recorder.viewerOptions
						.getScreenScale();
