...
public class Config {
	...
	public Config(boolean side, String name, String Address,
			...
			boolean Ssl_enabled, boolean reverse_connection) {
		setConfiguration(side, name, Address, default_address,
				Multihomed_enabled, Port, httpPort, Username, Password,
	...
	public void loadData(String filename) {
		try {
			...
			server_port = Integer.valueOf(properties.get("server-port")
					.toString());
			http_port = Integer.valueOf(properties.get("http-port").toString());
			username = properties.get("username").toString();
			password = PasswordUtility.decodeString(properties.get("password")
					.toString());
			ssl_enabled = Boolean
					.valueOf(properties.getProperty("ssl-enabled"));
			multihomed_enabled = Boolean.valueOf(properties
					.getProperty("multihomed-enabled"));
			default_address = Boolean.valueOf(properties
					.getProperty("default-address"));
			reverseConnection = Boolean.valueOf(properties
					.getProperty("reverse-connection"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void storeConfiguration(boolean side, String name) {
		...
		try {
			...
			properties.put("multihomed-enabled",
					String.valueOf(multihomed_enabled));
			properties.put("default-address", String.valueOf(default_address));
			properties.put("reverse-connection",
					String.valueOf(reverseConnection));

			properties.store(new FileOutputStream(config_file), "jrdesktop "
					+ FileUtility.getSide(side) + " configuration file");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
