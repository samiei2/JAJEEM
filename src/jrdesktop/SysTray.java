...
public class SysTray {
	...
	public static void updateServerStatus(byte msgType, String msg) {
		...
		case Commons.SERVER_RUNNING:
			...
			if (Server.isRunning()) {
				if (Server.getViewersCount() != 0) {
					trayIcon.setImage(new ImageIcon(Commons.ALIVE_ICON)
							.getImage());
				} else {
					trayIcon.setImage(new ImageIcon(Commons.WAIT_ICON)
							.getImage());
				}
			}
		...
		case Commons.CONNECTION_FAILED:
			trayIcon.displayMessage("Connection status",
					"Connection Failed !!", MessageType.ERROR);
			break;
	...
	public static void displayViewer(String viewer, int size, boolean connected) {
		...
		if (connected) {
			...
			trayIcon.displayMessage("Viewer details", viewer
					+ " disconnected !!", MessageType.INFO);
			if (size == 0) {
				trayIcon.setImage(new ImageIcon(Commons.WAIT_ICON).getImage());
			}
		}
		...
		Runnable runner = new Runnable() {
			...
			public void run() {
				...
				trayIcon = new TrayIcon(
						new ImageIcon(Commons.IDLE_ICON).getImage(),
						"jrdesktop", popup);
