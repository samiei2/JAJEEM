...
public class ScreenCapture {
	...
	public void clearScreen() {
		...
		for (int y = 0; y < screen.getRowScreenBlocks(); y++) {
			for (int x = 0; x < screen.getColumnScreenBlocks(); x++) {
				screen.addScreenBlock(new ScreenBlock(screen
						.getScreenBlockName(y, x)));
	...
	public int getTrafficPercent() {
		return ((modified * 100) / (screen.getRowScreenBlocks() * screen
				.getColumnScreenBlocks()));
	}

	public void takeAndSaveImage(Robot rt, float compressionQuality,
			int colorQuality, double scaleAbsolute, Rectangle screenRect)
			throws IOException {
		...
		for (int y = 0; y < screen.getRowScreenBlocks(); y++) {
			for (int x = 0; x < screen.getColumnScreenBlocks(); x++) {
				...
				subImageGraphics.drawImage(screenImage, 0, 0, newW, newH,
						startx, starty, startx + pieceWidth, starty
								+ pieceHeight, null);
	...
	public HashMap<String, byte[]> getChangedBlocks() {
		...
		for (int i = 0; i < screen.getChangedScreenBlocks().size(); i++) {
			...
			blocks.put(blockName, screen.getScreenBlockImage(blockName)
					.getData());
		}
	...
	public BufferedImage insertMosaicPiece(BufferedImage imageTotal, String name) {
		...
		imageTotalGraphics.drawImage(imgPiece.getImage(), x, y, pieceWidth,
				pieceHeight, null);
