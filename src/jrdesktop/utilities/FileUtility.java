...
public class FileUtility {
	...
	public static final String[] BYTES = { " B", " kB", " MB", " GB", " TB",
			" PB", " EB", " ZB", " YB" };
	// measurement units for speed formatting, from "bytes per second" to
	// yottabytes.
	...
	public static ArrayList<File> getFiles(File[] files) {
		...
		for (int i = 0; i < files.length; i++) {
			if (files[i].isDirectory()) {
				allFiles.addAll(getFiles(new File(files[i].toString())
						.listFiles()));
			} else {
	...
	public static String[] getSideConfigFiles(final boolean side) {
		...
		for (int i = 0; i < files.length; i++) {
			String name = files[i];
			files[i] = name.substring((getSide(side) + "_").length(),
	...
	public static String getJarnameURI() {
		try {
			String appName = main.class.getProtectionDomain().getCodeSource()
					.getLocation().toString();
			return new File(new URL(appName).getFile()).toURI().normalize()
					.toString();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String getJarnameURL() {
		try {
			return new URL(URLDecoder.decode(getJarnameURI(), "UTF-8"))
					.getFile();
		} catch (Exception e) {
			e.printStackTrace();
