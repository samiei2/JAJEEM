...
public class RMIServer {
	...
	public static boolean Start() {
		...
		try {
			...
			if (serverConfig.ssl_enabled && serverConfig.multihomed_enabled) {
				serverImpl = new ServerImpl(
						...
						new SslRMIServerSocketFactory(null, null, true)/*
																		 ...
																		 * server_port
																		 */);
			} else if (serverConfig.ssl_enabled
					&& !serverConfig.multihomed_enabled) {
				serverImpl = new ServerImpl(new SslRMIClientSocketFactory(),
						new SslRMIServerSocketFactory(null, null, true),
						serverConfig.server_port);
			} else if (!serverConfig.ssl_enabled
					&& serverConfig.multihomed_enabled) {
			 ...
			 * serverConfig.server_port + "/jrdesktop", (ServerInterface)
			 * serverImpl);
			 */
...
//					JOptionPane.ERROR_MESSAGE);
	...
	public static String getStatus() {
		...
		String status = "Running ..."
				...
				+ serverConfig.server_port
				+ "\nauthentication: "
				+ (auth == true ? "enabled" : "disabled")
				+ "\nencryption: "
				+ (serverConfig.ssl_enabled == true ? "enabled" : "disabled")
				+ "\nmultihomed: "
				+ (serverConfig.multihomed_enabled == true ? "enabled"
						: "disabled")
				+ "\nreverse connection: "
				+ (serverConfig.reverseConnection == true ? "enabled"
						: "disabled");
		return status;
	}
}
