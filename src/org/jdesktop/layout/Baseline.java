...
public class Baseline {
	...
	private static int getProgressBarBaselineResizeBehavior(JProgressBar pb) {
		if (pb.isStringPainted()
				&& pb.getOrientation() == SwingConstants.HORIZONTAL) {
			return BRB_CENTER_OFFSET;
		}
		return BRB_OTHER;
	}
	...
	private static int getTabbedPaneBaseline(JTabbedPane tp, int height) {
		if (tp.getTabCount() > 0) {
			...
			SwingUtilities.layoutCompoundLabel(tp, metrics, "A", null,
					...
					SwingConstants.CENTER, SwingConstants.TRAILING, viewRect,
					iconRect, textRect, 0);
		...
		if (slider.getPaintLabels() && !isGTK()) {
			...
			if (slider.getOrientation() == SwingConstants.HORIZONTAL) {
				...
				if (value != null) {
					...
					getMaxSliderValue(slider).intValue();
	 ...
	 * <code>getDefaults</code>. If you want to override the baseline suport for
	 * a look and feel place the instance in the defaults returned from
