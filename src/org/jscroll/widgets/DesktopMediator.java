...
public class DesktopMediator implements DesktopConstants {
	 ...
	 * 
	 * @param mainPane
	 *            a reference to the JScrollDesktopPane that this object is to
	 *            mediate.
	 */
	...
	public JInternalFrame add(String title, ImageIcon icon,
		...
		if (getDesktopScrollpane().getNumberOfFrames() < MAX_FRAMES) {
			frame = getDesktopScrollpane().add(dListener, title, icon,
	...
	public void add(JInternalFrame frame, int x, int y) {
		if (getDesktopScrollpane().getNumberOfFrames() < MAX_FRAMES) {
			getDesktopScrollpane().add(dListener, frame, x, y);
			createFrameAssociates(frame);
		}
	}
	...
	private void createFrameAssociates(JInternalFrame frame) {
		...
		if (getDesktopScrollpane().getAutoTile()) {
			getDesktopScrollpane().tileInternalFrames();
		}
	}
	...
	public void removeAssociatedComponents(JScrollInternalFrame f) {
		...
		getDesktopScrollpane().selectNextFrame();
	}
	...
	public JInternalFrame getSelectedFrame() {
		return getDesktopScrollpane().getSelectedFrame();
	}
	...
	public void setSelectedFrame(JInternalFrame f) {
		getDesktopScrollpane().setSelectedFrame(f);
	}
	...
	public void flagContentsChanged(JInternalFrame f) {
		getDesktopScrollpane().flagContentsChanged(f);
	}
	...
	public void revalidateViewport() {
		getDesktopScrollpane().revalidate();
	}
	...
	public void centerView(JScrollInternalFrame f) {
		getDesktopScrollpane().centerView(f);
	}
	...
	public void closeSelectedFrame() {
		getDesktopScrollpane().closeSelectedFrame();
	}
	...
	public void cascadeInternalFrames() {
		getDesktopScrollpane().cascadeInternalFrames();
	}
	...
	public void setAutoTile(boolean tileMode) {
		getDesktopScrollpane().setAutoTile(tileMode);
	}
