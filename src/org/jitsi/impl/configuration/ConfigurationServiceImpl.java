...
public class ConfigurationServiceImpl implements ConfigurationService {
	 ...
	 * as necessary.
	 */
	private static final String DEFAULT_CONFIGURATION_STORE_CLASS_NAME = "net.java.sip.communicator.impl.configuration"
	...
	/**
	 * Name of the file containing overrides (possibly set by the deployer) for
	 * any of the default properties.
	 */
	 ...
	 */
	private final ChangeEventDispatcher changeEventDispatcher = new ChangeEventDispatcher(
			this);
	...
	@Override
	public void setProperty(String propertyName, Object property,
			boolean isSystem) throws ConfigPropertyVetoException {
		...
		if (changeEventDispatcher.hasVetoableChangeListeners(propertyName)) {
			changeEventDispatcher.fireVetoableChange(propertyName, oldValue,
					property);
		}
		...
		if (changeEventDispatcher.hasPropertyChangeListeners(propertyName)) {
			changeEventDispatcher.firePropertyChange(propertyName, oldValue,
					property);
		}
	}
	...
	public void setProperties(Map<String, Object> properties)
		...
		for (Map.Entry<String, Object> property : properties.entrySet()) {
			...
			if (changeEventDispatcher.hasVetoableChangeListeners(propertyName)) {
				changeEventDispatcher.fireVetoableChange(propertyName,
		...
		for (Map.Entry<String, Object> property : properties.entrySet()) {
			...
			if (changeEventDispatcher.hasPropertyChangeListeners(propertyName)) {
				changeEventDispatcher.firePropertyChange(propertyName,
						oldValues.get(propertyName), property.getValue());
			}
		}
	}
	 ...
	 */
	private void doSetProperty(String propertyName, Object property,
			boolean isSystem) {
	...
	public void removeProperty(String propertyName) {
		List<String> childPropertyNames = getPropertyNamesByPrefix(
				propertyName, false);
	...
	public List<String> getPropertyNamesByPrefix(String prefix,
		...
		if (immutableDefaultProperties.size() > 0) {
			...
			namesArray = propertyNameSet.toArray(new String[propertyNameSet
					.size()]);

			getPropertyNamesByPrefix(prefix, exactPrefixMatch, namesArray,
					resultKeySet);
		}

		// now get property names from the current store.
		getPropertyNamesByPrefix(prefix, exactPrefixMatch,
				store.getPropertyNames(), resultKeySet);
	 ...
	 */
	private Set<String> getPropertyNamesByPrefix(String prefix,
			boolean exactPrefixMatch, String[] names, Set<String> resultSet) {
	...
	public void removePropertyChangeListener(String propertyName,
			PropertyChangeListener listener) {
		changeEventDispatcher.removePropertyChangeListener(propertyName,
				listener);
	}
	...
	public void reloadConfiguration() throws IOException {
		...
		if ((file != null) && (faService != null)) {
			...
			FailSafeTransaction trans = faService
	 ...
	 * 
	 * @param file
	 *            a reference to the configuration file where properties should
	 *            be stored.
	 * @throws IOException
	 *             if there was a problem writing to the specified file.
	 */
	private void storeConfiguration(File file) throws IOException {
		 ...
		 * write it.
		 */
		String readOnly = System
				.getProperty(PNAME_CONFIGURATION_FILE_IS_READ_ONLY);
		...
		// write the file.
		FailSafeTransaction trans = (file == null) ? null : faService
				.createFailSafeTransaction(file);
		...
		try {
			...
			OutputStream stream = (file == null) ? null : new FileOutputStream(
					file);
	...
	/**
	 * Returns the configuration file currently used by the implementation. If
	 * there is no such file or this is the first time we reference it a new one
	 * is created.
	 * 
	...
	private void createConfigurationFile() throws IOException {
		...
		if (configurationFile == null) {
			...
			String extension = (extensionBeginIndex > -1) ? name
					.substring(extensionBeginIndex) : null;
			...
			if (".properties".equalsIgnoreCase(extension)) {
				...
				File newConfigurationFile = new File(
						...
						((extensionBeginIndex > -1) ? name.substring(0,
								extensionBeginIndex) : name) + ".properties");
				 ...
				 */
					 ...
					 */
					this.configurationFile = configurationFile.exists() ? configurationFile
	 ...
	 *         user specific data such as configuration files, message and call
	 *         history as well as is bundle repository.
	 */
	@Override
	 ...
	 * specific data such as configuration files, message and call history as
	 * well as is bundle repository.
	 * 
	 * @return the name of the directory where SIP Communicator is to store user
	 *         specific data such as configuration files, message and call
	 *         history as well as is bundle repository.
	 */
	@Override
	public String getScHomeDirName() {
		...
		if (scHomeDirName == null) {
			...
			// available for other bundles to consult.
			if (store != null) {
				store.setNonSystemProperty(PNAME_SC_HOME_DIR_NAME,
						scHomeDirName);
			}
		}
	...
	private File getConfigurationFile(String extension, boolean create)
		...
		File configDir = new File(getScHomeDirLocation() + File.separator
				+ getScHomeDirName());
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Copying config file from JAR into "
					+ configFileInUserHomeDir.getAbsolutePath());
		}
		configDir.mkdirs();
		try {
	 ...
	 *         default when no specific <tt>ConfigurationStore</tt>
	 *         <tt>Class</tt> is determined as necessary
	 */
	...
	/**
	 * Determines whether the property with the specified <tt>propertyName</tt>
	 * has been previously declared as System
	 * 
	 * @param propertyName
	 ...
	 *         setProperty(string, true)) or by setting the system attribute in
	 *         the xml conf file to true.
	 */
	 ...
	 * other than DEBUG or TRACE (FINE or FINEST). * Changed that system
	 * properties are printed in INFO level and this way they are included in
	 * the beginning of every users log file.
	 */
	 ...
	 * loads their contents as system properties. All such files have to be in a
	 * location that's in the classpath.
	 */
	private void preloadSystemPropertyFiles() {
		String propertyFilesListStr = System
				.getProperty(SYS_PROPS_FILE_NAME_PROPERTY);

		if (propertyFilesListStr == null
				|| propertyFilesListStr.trim().length() == 0) {
			return;
		}

		StringTokenizer tokenizer = new StringTokenizer(propertyFilesListStr,
				";,", false);
	 ...
	 * 
	 * @param clazz
	 *            the {@link ConfigurationStore} that this configuration service
	 *            instance instance has to use.
	 * 
	 * @throws IOException
	 *             if loading properties from the specified store fails.
	 */
	private void setConfigurationStore(Class<? extends ConfigurationStore> clazz)
		...
		this.configurationFile = (extension == null) ? null
	 ...
	 * 
	 * @param fileName
	 *            the name of the file we need to load.
	 */
