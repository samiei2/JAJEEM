...
class InputDataSourceDesc {
	...
	private static final Logger logger = Logger
			.getLogger(InputDataSourceDesc.class);
	 ...
	 *            a <tt>DataSourc</tt> for which additional information is to be
	 *            described by the new instance
	 * @param outputDataSource
	 *            the <tt>AudioMixingPushBufferDataSource</tt> in which the mix
	 *            contributions of <tt>inputDataSource</tt> are to not be
	 *            included
	 */
	public InputDataSourceDesc(DataSource inputDataSource,
	...
	synchronized void connect(final AudioMixer audioMixer) throws IOException {
		final DataSource effectiveInputDataSource = (transcodingDataSource == null) ? inputDataSource
		...
		if (effectiveInputDataSource instanceof TranscodingDataSource) {
			if (connectThread == null) {
				connectThread = new Thread() {
					...
					public void run() {
						try {
							audioMixer.connect(effectiveInputDataSource,
									inputDataSource);
	...
	synchronized boolean createTranscodingDataSource(Format outputFormat) {
		if (transcodingDataSource == null) {
			setTranscodingDataSource(new TranscodingDataSource(inputDataSource,
					outputFormat));
			return true;
		} else {
			return false;
	...
	public synchronized Object getControl(String controlType) {
		...
		return (effectiveInputDataSource == null) ? null
	...
	public synchronized DataSource getEffectiveInputDataSource() {
		return (transcodingDataSource == null) ? inputDataSource
