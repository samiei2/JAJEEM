...
public class AudioMixingPushBufferDataSource extends PushBufferDataSource
	...
	private static final Logger logger = Logger
			.getLogger(AudioMixingPushBufferDataSource.class);
	...
	public Object[] getControls() {
		...
		if (bufferControl == null) {
			...
			System.arraycopy(formatControls, 0, controls, 1,
	...
	public synchronized PushBufferStream[] getStreams() {
		if (connected && (outputStream == null)) {
			AudioMixerPushBufferStream audioMixerOutputStream = audioMixer
					.getOutputStream();

			if (audioMixerOutputStream != null) {
				outputStream = new AudioMixingPushBufferStream(
						audioMixerOutputStream, this);
				if (started) {
					try {
						outputStream.start();
					} catch (IOException ioex) {
						logger.error("Failed to start "
								...
								+ " with hashCode " + outputStream.hashCode(),
								ioex);
					}
				}
			}
		}
		return (outputStream == null) ? new PushBufferStream[0]
