 ...
 */
public class AudioMixingPushBufferStream extends ControlsAdapter implements
		PushBufferStream {
	...
	private static final Logger logger = Logger
			.getLogger(AudioMixingPushBufferStream.class);
	 ...
	 *             if the specified <tt>outputFormat</tt> is not supported by
	...
	private static int getMaxOutputSample(AudioFormat outputFormat)
		...
		default:
			throw new UnsupportedFormatException(
					"Format.getSampleSizeInBits()", outputFormat);
		}
	}
	 ...
	 */
	private static int[] mix(int[][] inputSamples, AudioFormat outputFormat,
		...
		if (inputSamples.length == 1) {
			...
			if (inputStreamSamples != null) {
				System.arraycopy(inputStreamSamples, 0, outputSamples, 0,
		...
		for (int[] inputStreamSamples : inputSamples) {
			...
			for (int i = 0; i < inputStreamSampleCount; i++) {
				...
				outputSamples[i] = inputStreamSample
						+ outputSample
						- Math.round(inputStreamSample
								* (outputSample / (float) maxOutputSample));
			}
		}
		return outputSamples;
	}
	...
	public void read(Buffer buffer) throws IOException {
		...
		int[] outputSamples = mix(inputSamples, outputFormat,
				maxInputSampleCount);
		...
		if (Format.byteArray.equals(outputDataType)) {
			...
			case 16:
				...
				for (int i = 0; i < outputSamples.length; i++) {
					ArrayIOUtils
							.writeInt16(outputSamples[i], outputData, i * 2);
				}
				break;
			case 32:
				if (outputData == null
	...
	synchronized void start() throws IOException {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Started " + getClass().getSimpleName()
	...
	synchronized void stop() throws IOException {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Stopped " + getClass().getSimpleName()
					+ " with hashCode " + hashCode());
		}
	}
	 ...
	 * @param outputOffset
	 *            the offset in <tt>output</tt> at which the writing of the
