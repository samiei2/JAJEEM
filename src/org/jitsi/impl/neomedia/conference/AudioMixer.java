...
public class AudioMixer {
	...
	public AudioMixer(CaptureDevice captureDevice) {
		...
		if (captureDevice instanceof PullBufferDataSource) {
			captureDevice = new PushBufferDataSourceAdapter(
					(PullBufferDataSource) captureDevice);
		}

		// Try to enable tracing on captureDevice.
		if (logger.isTraceEnabled()) {
			captureDevice = MediaDeviceImpl.createTracingCaptureDevice(
					captureDevice, logger);
		}

		this.captureDevice = captureDevice;

		this.localOutputDataSource = createOutputDataSource();
		addInputDataSource((DataSource) this.captureDevice,
	 ...
	 *            the audio contributions of <tt>inputDataSource</tt> in the mix
	 *            it outputs
	 */
	void addInputDataSource(DataSource inputDataSource,
		...
		synchronized (inputDataSources) {
			...
			InputDataSourceDesc inputDataSourceDesc = new InputDataSourceDesc(
					inputDataSource, outputDataSource);
					...
					try {
						...
						logger.error(
								"Failed to connect to inputDataSource "
										+ MediaStreamImpl
												.toString(inputDataSourceDesc.inputDataSource),
								ioe);
						throw ioe;
					}
				}
	 ...
	 * 
	 * @param inputDataSource
	 *            the <tt>InputDataSourceDesc</tt> of the input
	 *            <tt>DataSource</tt> which has finished its connecting
	 *            procedure
	 * @throws IOException
	...
	void connected(InputDataSourceDesc inputDataSource) throws IOException {
		synchronized (inputDataSources) {
			if (inputDataSources.contains(inputDataSource) && (connected > 0)) {
				if (started > 0) {
	 ...
	 *            <tt>InputDataSourceDesc</tt> to be described by the new
	 *            instance
	 * @return a new <tt>InputStreamDesc</tt> instance which describes the
	 *         specified input <tt>SourceStream</tt> and <tt>DataSource</tt>
	 */
	private InputStreamDesc createInputStreamDesc(SourceStream inputStream,
	 ...
	 * 
	 * @param inputDataSourceDesc
	 *            the <tt>InputDataSourceDesc</tt> describing the input
	...
	private boolean createTranscodingDataSource(
			InputDataSourceDesc inputDataSourceDesc, Format outputFormat)
			throws IOException {
	...
	BufferControl getBufferControl() {
		if ((bufferControl == null) && (captureDevice instanceof Controls)) {
			...
			if (captureDeviceBufferControl != null) {
				bufferControl = new ReadOnlyBufferControlDelegate(
	 ...
	 * 
	 * @param inputStream
	 *            the <tt>SourceStream</tt> to locate an
	 *            <tt>InputStreamDesc</tt> for in
	 *            <tt>existingInputStreamDescs</tt>
	 * @param existingInputStreamDescs
	 *            the list of existing <tt>InputStreamDesc</tt>s in which an
	 *            <tt>InputStreamDesc</tt> for <tt>inputStream</tt> is to be
	 *            located
	 * @return an <tt>InputStreamDesc</tt> from
	 *         <tt>existingInputStreamDescs</tt> which describes
	 *         <tt>inputStream</tt> if such an <tt>InputStreamDesc</tt> exists;
	 *         otherwise, <tt>null</tt>
	 */
	private InputStreamDesc getExistingInputStreamDesc(
			SourceStream inputStream, InputStreamDesc[] existingInputStreamDescs) {
	...
	private static Format getFormat(DataSource dataSource) {
		FormatControl formatControl = (FormatControl) dataSource
	...
	FormatControl[] getFormatControls() {
		...
		if (!OSUtils.IS_ANDROID && (formatControls != null)) {
			for (int i = 0; i < formatControls.length; i++) {
				formatControls[i] = new ReadOnlyFormatControlDelegate(
						formatControls[i]);
			}
		}
	 ...
	 *            the <tt>InputStreamDesc</tt> instances which already exist and
	 *            which are used to avoid creating multiple
	 *            <tt>InputStreamDesc</tt>s for input <tt>SourceStream</tt>s
	 *            which already have ones
	 * @param inputStreams
	 *            the <tt>List</tt> of <tt>InputStreamDesc</tt> in which the
	...
	private boolean getInputStreamsFromInputDataSource(
			InputDataSourceDesc inputDataSourceDesc, AudioFormat outputFormat,
			InputStreamDesc[] existingInputStreams,
			List<InputStreamDesc> inputStreams) {
		SourceStream[] inputDataSourceStreams = inputDataSourceDesc
				.getStreams();
		...
		if (inputDataSourceStreams != null) {
			...
			for (SourceStream inputStream : inputDataSourceStreams) {
				...
				if ((inputFormat != null) && matches(inputFormat, outputFormat)) {
					InputStreamDesc inputStreamDesc = getExistingInputStreamDesc(
							inputStream, existingInputStreams);

					if (inputStreamDesc == null) {
						inputStreamDesc = createInputStreamDesc(inputStream,
								inputDataSourceDesc);
					}
		...
		DataSource inputDataSource = inputDataSourceDesc
		...
		if ((inputFormat != null) && !matches(inputFormat, outputFormat)) {
			if (inputDataSource instanceof PushDataSource) {
				for (PushSourceStream inputStream : ((PushDataSource) inputDataSource)
						.getStreams()) {
					InputStreamDesc inputStreamDesc = getExistingInputStreamDesc(
							inputStream, existingInputStreams);
					...
					if (inputStreamDesc == null) {
						inputStreamDesc = createInputStreamDesc(
								new PushBufferStreamAdapter(inputStream,
										inputFormat), inputDataSourceDesc);
					}
			...
			if (inputDataSource instanceof PullDataSource) {
				for (PullSourceStream inputStream : ((PullDataSource) inputDataSource)
						.getStreams()) {
					InputStreamDesc inputStreamDesc = getExistingInputStreamDesc(
							inputStream, existingInputStreams);
					...
					if (inputStreamDesc == null) {
						inputStreamDesc = createInputStreamDesc(
								new PullBufferStreamAdapter(inputStream,
										inputFormat), inputDataSourceDesc);
					}
					inputStreams.add(inputStreamDesc);
				}
	...
	private Collection<InputStreamDesc> getInputStreamsFromInputDataSources(
			AudioFormat outputFormat, InputStreamDesc[] existingInputStreams)
			throws IOException {
		List<InputStreamDesc> inputStreams = new ArrayList<InputStreamDesc>();

		synchronized (inputDataSources) {
			for (InputDataSourceDesc inputDataSourceDesc : inputDataSources) {
				boolean got = getInputStreamsFromInputDataSource(
						inputDataSourceDesc, outputFormat,
						existingInputStreams, inputStreams);

				if (!got
						&& createTranscodingDataSource(inputDataSourceDesc,
								outputFormat)) {
					getInputStreamsFromInputDataSource(inputDataSourceDesc,
							outputFormat, existingInputStreams, inputStreams);
				}
			}
		}
		return inputStreams;
	...
	private AudioFormat getOutputFormatFromInputDataSources() {
		...
		synchronized (inputDataSources) {
			for (InputDataSourceDesc inputDataSource : inputDataSources) {
				DataSource effectiveInputDataSource = inputDataSource
						.getEffectiveInputDataSource();
				...
				FormatControl formatControl = (FormatControl) effectiveInputDataSource
						.getControl(formatControlType);

				if (formatControl != null) {
					AudioFormat format = (AudioFormat) formatControl
							.getFormat();
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Determined outputFormat of AudioMixer"
					+ " from inputDataSources to be " + outputFormat);
		}
		return outputFormat;
	}
	...
	AudioMixerPushBufferStream getOutputStream() {
		synchronized (inputDataSources) {
			AudioFormat outputFormat = (outputStream == null) ? getOutputFormatFromInputDataSources()
			...
			try {
				inputStreams = getInputStreamsFromInputDataSources(
						outputFormat, (outputStream == null) ? null
			...
			if (outputStream == null) {
				outputStream = new AudioMixerPushBufferStream(this,
						outputFormat);
			}
	 ...
	 *         <tt>input<tt> matches the specified <tt>pattern</tt> in the sense
	 *         of JMF <tt>Format</tt> matching; otherwise, <tt>false</tt>
	 */
	 ...
	 *             <tt>stream</tt>
	 */
	protected void read(PushBufferStream stream, Buffer buffer,
	...
	public void removeInputDataSources(DataSourceFilter dataSourceFilter) {
		synchronized (inputDataSources) {
			Iterator<InputDataSourceDesc> inputDataSourceIter = inputDataSources
					.iterator();
	...
	private void setOutputFormatToInputDataSources(AudioFormat outputFormat) {
		...
		synchronized (inputDataSources) {
			for (InputDataSourceDesc inputDataSourceDesc : inputDataSources) {
				FormatControl formatControl = (FormatControl) inputDataSourceDesc
						.getControl(formatControlType);
				...
				if (formatControl != null) {
					...
					if ((inputFormat == null)
						...
						Format setFormat = formatControl
								.setFormat(outputFormat);

						if (setFormat == null) {
							logger.error("Failed to set format of inputDataSource to "
									+ outputFormat);
						} else if (setFormat != outputFormat) {
							logger.warn("Failed to change format of inputDataSource from "
									+ setFormat + " to " + outputFormat);
						} else if (logger.isTraceEnabled()) {
							logger.trace("Set format of inputDataSource to "
									+ setFormat);
						}
					}
				}
	 ...
	 *            <tt>AudioMixer</tt> to start. If <tt>outputStream</tt> is the
	 ...
	 * @param outputStream
	 *            the <tt>AudioMixerPushBufferStream</tt> which requests this
	 *            <tt>AudioMixer</tt> to stop. If <tt>outputStream</tt> is the
	 *            current one and only <tt>AudioMixerPushBufferStream</tt> of
	 ...
	 *         <tt>inputDataSource</tt>, if we can find one, <tt>null</tt>
	 *         otherwise.
	 */
