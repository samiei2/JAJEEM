...
public class QTCaptureDevice extends NSObject {
	 ...
	 * 
	 * @param ptr
	 *            the pointer to the QTKit <tt>QTCaptureDevice</tt> object which
	 ...
	 * 
	 * @param ptr
	 *            the pointer to the QTKit <tt>QTCaptureDevice</tt> object to
	 *            close
	 */
	 ...
	 * 
	 * @param deviceUID
	 *            the unique identifier of the <tt>QTCaptureDevice</tt> to be
	 *            retrieved
	 * @return the <tt>QTCaptureDevice</tt> with the specified unique identifier
	 *         if such a <tt>QTCaptureDevice</tt> exists; otherwise,
	 *         <tt>null</tt>
	 */
	public static QTCaptureDevice deviceWithUniqueID(String deviceUID) {
		...
		QTCaptureDevice deviceWithUniqueID = deviceWithUniqueID(deviceUID,
				inputDevices);
		...
		if (formatDescriptionPtrs == null) {
			...
			if (formatDescriptionCount == 0) {
				...
				for (int i = 0; i < formatDescriptionCount; i++) {
					formatDescriptions[i] = new QTFormatDescription(
							formatDescriptionPtrs[i]);
				}
			}
		}
	...
	public static QTCaptureDevice[] inputDevicesWithMediaType(
		...
		int inputDeviceCount = (inputDevicePtrs == null) ? 0
				: inputDevicePtrs.length;
		...
		if (inputDeviceCount == 0) {
			...
			List<QTCaptureDevice> cachedInputDevicesWithMediaType = inputDevices
					.get(mediaType);
			...
			Iterator<QTCaptureDevice> cachedInputDeviceIter = cachedInputDevicesWithMediaType
					.iterator();

			while (cachedInputDeviceIter.hasNext()) {
				long cachedInputDevicePtr = cachedInputDeviceIter.next()
						.getPtr();
				boolean remove = true;
	 ...
	 *             if this device was not opened successfully and carries an
	 *             <tt>NSError</tt> describing why this device could not be
	 *             opened
	 */
	 ...
	 *         <tt>false</tt>
	 * @throws NSErrorException
	 *             if the device was not opened successfully and carries an
	 *             <tt>NSError</tt> describing why the device could not be
	 *             opened
	 */
