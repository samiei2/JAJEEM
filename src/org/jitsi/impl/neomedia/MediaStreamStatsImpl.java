...
public class MediaStreamStatsImpl implements MediaStreamStats {
	...
	private static final Logger logger = Logger
			.getLogger(MediaStreamStatsImpl.class);
	 ...
	 */
	private void updateStreamDirectionStats(StreamDirection streamDirection,
			long currentTimeMs) {
		...
		if (streamDirection == StreamDirection.DOWNLOAD) {
			...
			// of this stream.
			long newNbLost = this.getDownloadNbPDULost()
					- this.nbLost[streamDirectionIndex];
		...
		double newRateKiloBitPerSec = MediaStreamStatsImpl
				.computeRateKiloBitPerSec(newNbByte
						- this.nbByte[streamDirectionIndex], currentTimeMs
						- this.updateTimeMs);
		this.rateKiloBitPerSec[streamDirectionIndex] = MediaStreamStatsImpl
				.computeEWMA(nbSteps,
	...
	public String getLocalIPAddress() {
		InetSocketAddress mediaStreamLocalDataAddress = mediaStreamImpl
				.getLocalDataAddress();

		return (mediaStreamLocalDataAddress == null) ? null
	...
	public int getLocalPort() {
		...
		return (mediaStreamLocalDataAddress == null) ? -1
	...
	public String getRemoteIPAddress() {
		...
		// disconnected.
		return (mediaStreamTarget == null) ? null : mediaStreamTarget
				.getDataAddress().getAddress().getHostAddress();
	}
	...
	public int getRemotePort() {
		...
		// disconnected.
		return (mediaStreamTarget == null) ? -1 : mediaStreamTarget
	...
	private VideoFormat getUploadVideoFormat() {
		...
		return (deviceSession instanceof VideoMediaDeviceSession) ? ((VideoMediaDeviceSession) deviceSession)
				.getSentVideoFormat() : null;
	}
	 ...
	 *            which this function retrieve the jitter.
	 */
	private void updateJitterRTPTimestampUnits(RTCPFeedback feedback,
		...
		this.jitterRTPTimestampUnits[streamDirection.ordinal()] = feedback
				.getJitter();
	}
	...
	public void updateNewReceivedFeedback(RTCPFeedback feedback) {
		...
		long newNbLost = feedback.getNumLost()
				- this.nbLost[streamDirection.ordinal()];
	 ...
	 *            update.
	 */
	private void updateNbLoss(StreamDirection streamDirection, long newNbLost,
			long nbSteps) {
		int streamDirectionIndex = streamDirection.ordinal();

		double newPercentLoss = MediaStreamStatsImpl.computePercentLoss(
				nbSteps, newNbLost);
		this.percentLoss[streamDirectionIndex] = MediaStreamStatsImpl
				.computeEWMA(nbSteps, this.percentLoss[streamDirectionIndex],
						newPercentLoss);
	 ...
	 * 
	 * @param nbByteRecv
	 *            The number of Byte received.
	 * @param callNbTimeMsSpent
	 *            The time spent since the mediaStreamImpl is connected to the
	 *            endpoint.
	 * 
	 * @return the bandwidth rate computed in Kilo bits per seconds.
	 */
	private static double computeRateKiloBitPerSec(long nbByteRecv,
	 ...
	 */
	private static double computeEWMA(long nbStepSinceLastUpdate,
			double lastValue, double newValue) {
	...
	private long getNbPDU(StreamDirection streamDirection) {
		...
		if (rtpManager != null) {
			...
			case DOWNLOAD:
				GlobalReceptionStats globalReceptionStats = rtpManager
						.getGlobalReceptionStats();

				nbPDU = globalReceptionStats.getPacketsRecd()
	...
	private long getNbBytes(StreamDirection streamDirection) {
		...
		if (rtpManager != null) {
			...
			case DOWNLOAD:
				...
				break;
			case UPLOAD:
				nbBytes = rtpManager.getGlobalTransmissionStats()
						.getBytesSent();
				break;
			}
		}
	...
	private void updateNbFec() {
		...
		if (devSession != null) {
			for (ReceiveStream receiveStream : devSession.getReceiveStreams()) {
				for (FECDecoderControl fecDecoderControl : devSession
	...
	private void updateNbDiscarded(long newNbDiscarded, long nbSteps) {
		...
		double newPercentDiscarded = MediaStreamStatsImpl.computePercentLoss(
				nbSteps, newNbDiscarded);
		this.percentDiscarded = MediaStreamStatsImpl.computeEWMA(nbSteps,
				this.percentDiscarded, newPercentDiscarded);
	...
	private Set<PacketQueueControl> getPacketQueueControls() {
		...
		if (mediaStreamImpl.isStarted()) {
			MediaDeviceSession devSession = mediaStreamImpl.getDeviceSession();
			if (devSession != null) {
				for (ReceiveStream receiveStream : devSession
						.getReceiveStreams()) {
					DataSource ds = receiveStream.getDataSource();
					if (ds instanceof net.sf.fmj.media.protocol.rtp.DataSource) {
						for (PushBufferStream pbs : ((net.sf.fmj.media.protocol.rtp.DataSource) ds)
								.getStreams()) {
							PacketQueueControl pqc = (PacketQueueControl) pbs
									.getControl(PacketQueueControl.class
											.getName());
							if (pqc != null) {
								set.add(pqc);
							}
