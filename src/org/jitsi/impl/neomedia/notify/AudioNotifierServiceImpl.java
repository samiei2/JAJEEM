 ...
 */
public class AudioNotifierServiceImpl implements AudioNotifierService,
	...
	public AudioNotifierServiceImpl() {
		this.deviceConfiguration = NeomediaServiceUtils.getMediaServiceImpl()
	...
	public boolean audioOutAndNotificationsShareSameDevice() {
		...
		CaptureDeviceInfo notify = audioSystem
				.getSelectedDevice(AudioSystem.DataFlow.NOTIFY);
		CaptureDeviceInfo playback = audioSystem
				.getSelectedDevice(AudioSystem.DataFlow.PLAYBACK);

		return (notify == null) ? (playback == null) : notify.getLocator()
				.equals(playback.getLocator());
	}
	...
	public SCAudioClip createAudio(String uri, boolean playback) {
		...
		synchronized (audiosSyncRoot) {
			...
			if (audio == null) {
				try {
					AudioSystem audioSystem = getDeviceConfiguration()
							.getAudioSystem();

					if (audioSystem == null) {
						audio = new JavaSoundClipImpl(uri, this);
					} else if (NoneAudioSystem.LOCATOR_PROTOCOL
							.equalsIgnoreCase(audioSystem.getLocatorProtocol())) {
						audio = null;
					} else {
						audio = new AudioSystemClipImpl(uri, this, audioSystem,
								playback);
					}
				} catch (Throwable t) {
			...
			if (audio != null) {
				...
				audio = new SCAudioClip() {
					...
					private Boolean evaluateLoopCondition(
							Callable<Boolean> loopCondition) throws Exception {
						/*
						 ...
						 */
						return (loopCondition == null) ? Boolean.FALSE
								: loopCondition.call();
					}
					...
					@Override
					public void play(int loopInterval,
	 ...
	 * 
	 * @param mute
	 *            when TRUE disables the sound, otherwise enables the sound.
	 */
	@Override
