...
public class ZrtpFortunaEntropyGatherer {
	...
	private static final Logger logger = Logger
			.getLogger(ZrtpFortunaEntropyGatherer.class);
	...
	private class GatherAudio extends Thread implements BufferTransferHandler {
		...
		private boolean prepareAudioEntropy() {
			CaptureDeviceInfo audioCaptureDevice = deviceConfiguration
					.getAudioCaptureDevice();
			if (audioCaptureDevice == null) {
				return false;
			}

			MediaLocator audioCaptureDeviceLocator = audioCaptureDevice
					.getLocator();
			...
			try {
				dataSource = Manager
						.createDataSource(audioCaptureDeviceLocator);
			...
			try {
				...
				while (gatheredEntropy < bytesToGather) {
					...
					if (i < 32) {
						...
						fortuna.addSeedMaterial((i % 3), entropy, 0,
								entropy.length);
					}
				...
				if (logger.isInfoEnabled()) {
					logger.info("GatherEntropy got: " + gatheredEntropy
							+ " bytes");
				}
