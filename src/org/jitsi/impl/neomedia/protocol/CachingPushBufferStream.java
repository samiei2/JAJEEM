...
public class CachingPushBufferStream implements PushBufferStream {
	...
	private long getBufferLength() {
		synchronized (bufferControlSyncRoot) {
			return (bufferControl == null) ? BufferControl.DEFAULT_VALUE
		...
		if (controls == null) {
			...
			if (!bufferControlExists) {
				...
				if (bufferControl != null) {
					...
					System.arraycopy(controls, 0, newControls, 1,
							controls.length);
				}
			}
		}
		...
		synchronized (cache) {
			...
			if (!cache.isEmpty()) {
				...
				while (!cache.isEmpty()) {
					...
					int nextBufferOffset = read(cacheBuffer, buffer,
							bufferOffset);
	 ...
	 *            specified <tt>input</tt>
	 * @param outputOffset
	 *            the offset in <tt>output</tt> at which the data read from
	 *            <tt>input</tt> is to be written
	 * @return the offset in <tt>output</tt> at which a next round of writing is
	 *         to continue; <tt>-1</tt> if no more writing in <tt>output</tt> is
	 *         to be performed and <tt>output</tt> is to be returned to the
	 *         caller
	 * @throws IOException
	 *             if reading from <tt>input</tt> into <tt>output</tt> fails
	 *             including if either of the formats of <tt>input</tt> and
	...
	private int read(Buffer input, Buffer output, int outputOffset)
		...
		if (outputData != null) {
			...
			if (inputData.getClass().equals(dataType)
				...
				System.arraycopy(inputData, inputOffset, outputBytes,
	...
	public void setTransferHandler(BufferTransferHandler transferHandler) {
		BufferTransferHandler substituteTransferHandler = (transferHandler == null) ? null
				: new StreamSubstituteBufferTransferHandler(transferHandler,
						stream, this) {
					@Override
