 ...
 */
public class PushBufferDataSourceAdapter extends
		PushBufferDataSourceDelegate<PullBufferDataSource> {
		...
		void start() {
			synchronized (streamReadThreadSyncRoot) {
				...
				if (streamReadThread == null) {
					streamReadThread = new Thread(getClass().getName()
							+ ".streamReadThread") {
						@Override
						public void run() {
							try {
	...
	private static final Logger logger = Logger
			.getLogger(PushBufferDataSourceAdapter.class);
		...
		synchronized (streams) {
			...
			for (int dataSourceStreamIndex = 0; dataSourceStreamIndex < dataSourceStreamCount; dataSourceStreamIndex++) {
				...
				if (dataSourceStream != null) {
					PushBufferStreamAdapter stream = new PushBufferStreamAdapter(
							dataSourceStream);
