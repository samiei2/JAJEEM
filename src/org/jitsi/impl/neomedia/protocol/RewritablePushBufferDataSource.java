 ...
 */
public class RewritablePushBufferDataSource extends
		PushBufferDataSourceDelegate<PushBufferDataSource> implements
		MuteDataSource, InbandDTMFDataSource {
	...
	public static void sendDTMF(Buffer buffer, DTMFInbandTone tone) {
		...
		if (data != null && (buffer.getFormat() instanceof AudioFormat)) {
			...
			int[] sampleData = tone.getAudioSamples(samplingFrequency,
					sampleSizeInBits);
			IntBuffer.wrap(sampleData);

			int toIndex = fromIndex + sampleData.length
					* (sampleSizeInBits / 8);
	 ...
	 * support for the wrapped instance.
	 */
	private class MutePushBufferStream extends
			SourceStreamDelegate<PushBufferStream> implements PushBufferStream {
