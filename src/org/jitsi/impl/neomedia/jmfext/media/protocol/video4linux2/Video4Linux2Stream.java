...
public class Video4Linux2Stream extends AbstractVideoPullBufferStream {
	...
	public Video4Linux2Stream(DataSource dataSource, FormatControl formatControl) {
		...
		v4l2_buffer = Video4Linux2
				.v4l2_buffer_alloc(Video4Linux2.V4L2_BUF_TYPE_VIDEO_CAPTURE);
		if (0 == v4l2_buffer) {
			throw new OutOfMemoryError("v4l2_buffer_alloc");
		}
		Video4Linux2.v4l2_buffer_setMemory(v4l2_buffer,
	...
	protected void doRead(Buffer buffer) throws IOException {
		...
		try {
			...
			if ((nativePixelFormat == Video4Linux2.V4L2_PIX_FMT_JPEG)
				...
				if (avctx == 0) {
					long avcodec = FFmpeg
							.avcodec_find_decoder(FFmpeg.CODEC_ID_MJPEG);
				...
				if (FFmpeg
						.avcodec_decode_video(avctx, avframe, mmap, bytesused) != -1) {
	...
	private Format getFdFormat() {
		...
		if (-1 != fd) {
			...
			if (v4l2_format == 0) {
				...
				try {
					if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_G_FMT,
							v4l2_format) != -1) {
						long fmtPix = Video4Linux2
								.v4l2_format_getFmtPix(v4l2_format);
						int pixelformat = Video4Linux2
								.v4l2_pix_format_getPixelformat(fmtPix);
						int ffmpegPixFmt = DataSource
								.getFFmpegPixFmt(pixelformat);

						if (FFmpeg.PIX_FMT_NONE != ffmpegPixFmt) {
							int width = Video4Linux2
									.v4l2_pix_format_getWidth(fmtPix);
							int height = Video4Linux2
									.v4l2_pix_format_getHeight(fmtPix);

							format = new AVFrameFormat(new Dimension(width,
									height), Format.NOT_SPECIFIED,
									ffmpegPixFmt, pixelformat);
						}
					}
				} finally {
	...
	private void negotiateFdInputMethod() throws IOException {
		...
		try {
			if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_QUERYCAP,
					v4l2_capability) == -1) {
				throw new IOException("ioctl: request= VIDIOC_QUERYCAP");
			}
			capabilities = Video4Linux2
					.v4l2_capability_getCapabilities(v4l2_capability);
		} finally {
		...
		try {
			...
			Video4Linux2.v4l2_requestbuffers_setMemory(v4l2_requestbuffers,
					requestbuffersMemory);
			Video4Linux2.v4l2_requestbuffers_setCount(v4l2_requestbuffers, 2);
			if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_REQBUFS,
					v4l2_requestbuffers) == -1) {
				throw new IOException(
						"ioctl: request= VIDIOC_REQBUFS, memory= "
								+ requestbuffersMemory);
			}
			requestbuffersCount = Video4Linux2
					.v4l2_requestbuffers_getCount(v4l2_requestbuffers);
		} finally {
		...
		try {
			...
			try {
				for (int i = 0; i < requestbuffersCount; i++) {
					...
					if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_QUERYBUF,
							v4l2_buffer) == -1) {
						throw new IOException("ioctl: request= VIDIOC_QUERYBUF");
					}

					int length = Video4Linux2
							.v4l2_buffer_getLength(v4l2_buffer);
					long offset = Video4Linux2
							.v4l2_buffer_getMOffset(v4l2_buffer);
					long mmap = Video4Linux2.mmap(0, length,
							Video4Linux2.PROT_READ | Video4Linux2.PROT_WRITE,
							Video4Linux2.MAP_SHARED, fd, offset);
	...
	private void setFdFormat(Format format) throws IOException {
		...
		try {
			...
			if (size == null) {
				...
				size = NeomediaServiceUtils.getMediaServiceImpl()
						.getDeviceConfiguration().getVideoSize();
			}

			if ((size != null)
					&& ((size.width != width) || (size.height != height))) {
				Video4Linux2.v4l2_pix_format_setWidthAndHeight(fmtPix,
						size.width, size.height);
				setFdFormat = true;
			}
			if (Video4Linux2.v4l2_pix_format_getPixelformat(v4l2_format) != pixelformat) {
				Video4Linux2
						.v4l2_pix_format_setPixelformat(fmtPix, pixelformat);
	 ...
	 *             instance
	 */
	private void setFdFormat(long v4l2_format, long fmtPix, Dimension size,
			int pixelformat) throws IOException {
		Video4Linux2.v4l2_pix_format_setField(fmtPix,
	...
	public void start() throws IOException {
		...
		try {
			...
			for (int i = 0; i < requestbuffersCount; i++) {
				Video4Linux2.v4l2_buffer_setIndex(v4l2_buffer, i);
				if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_QBUF,
						v4l2_buffer) == -1) {
					throw new IOException(
	...
	public void stop() throws IOException {
		try {
			...
			try {
				if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_STREAMOFF,
						v4l2_buf_type) == -1) {
