 ...
 */
public class ImageStream extends AbstractVideoPullBufferStream {
	/**
	...
	public byte[] readScreen(byte[] output, Dimension dim) {
		...
		/*
		 * OK native grabber failed or is not available, try with AWT Robot and
		 * convert it to the right format
		 * 
		 * Note that it is very memory consuming since memory are allocated to
		 * capture screen (via Robot) and then for converting to raw bytes
		 * Moreover support for multiple display has not yet been investigated
		 * 
		 * Normally not of our supported platform (Windows (x86, x64), Linux
		 * (x86, x86-64), Mac OS X (i386, x86-64, ppc) and FreeBSD (x86, x86-64)
		 * should go here.
		 */
		...
		if (screen != null) {
			...
			scaledScreen = ImgStreamingUtils.getScaledImage(screen, width,
	...
	private ByteBuffer readScreenNative(Dimension dim) {
		...
		if (desktopInteract.captureScreen(displayIndex, x, y, dim.width,
				dim.height, data.getPtr(), data.getLength())) {
			return data;
		} else {
			data.free();
			return null;
	 ...
	 * 
	 * @param x
	 *            the x coordinate of the origin to be set on this instance
	 * @param y
	 *            the y coordinate of the origin to be set on this instance
	 */
