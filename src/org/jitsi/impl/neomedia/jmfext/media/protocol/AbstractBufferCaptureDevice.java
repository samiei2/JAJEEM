...
public abstract class AbstractBufferCaptureDevice<AbstractBufferStreamT extends AbstractBufferStream>
	...
	private static final Logger logger = Logger
			.getLogger(AbstractBufferCaptureDevice.class);
	...
	protected FormatControl createFormatControl(final int streamIndex) {
		return new AbstractFormatControl() {
			...
			public Format getFormat() {
				format = AbstractBufferCaptureDevice.this.internalGetFormat(
						streamIndex, format);
				return format;
			}
			...
			public Format[] getSupportedFormats() {
				return AbstractBufferCaptureDevice.this
			...
			public Format setFormat(Format format) {
				...
				if (setFormat != null) {
					setFormat = AbstractBufferCaptureDevice.this
							.internalSetFormat(streamIndex, setFormat, format);
					if (setFormat != null) {
	...
	protected FormatControl[] createFormatControls() {
		...
		return (formatControl == null) ? EMPTY_FORMAT_CONTROLS
	...
	final Object[] defaultGetControls() {
		...
		int formatControlCount = (formatControls == null) ? 0
				: formatControls.length;
		FrameRateControl[] frameRateControls = internalGetFrameRateControls();
		int frameRateControlCount = (frameRateControls == null) ? 0
				: frameRateControls.length;
		RTPInfo[] rtpInfos = internalGetRTPInfos();
		int rtpInfoCount = (rtpInfos == null) ? 0 : rtpInfos.length;

		if ((formatControlCount == 0) && (frameRateControlCount == 0)
				&& (rtpInfoCount == 0)) {
			return ControlsAdapter.EMPTY_CONTROLS;
		} else {
			Object[] controls = new Object[formatControlCount
					+ frameRateControlCount + rtpInfoCount];
			int offset = 0;

			if (formatControlCount != 0) {
				System.arraycopy(formatControls, 0, controls, offset,
						formatControlCount);
				offset += formatControlCount;
			}
			if (frameRateControlCount != 0) {
				System.arraycopy(frameRateControls, 0, controls, offset,
	...
	final Format defaultGetFormat(int streamIndex, Format oldValue) {
		...
		return ((supportedFormats == null) || (supportedFormats.length < 1)) ? null
				: supportedFormats[0];
	}
	...
	final Format[] defaultGetSupportedFormats(int streamIndex) {
		...
		return (captureDeviceInfo == null) ? null : captureDeviceInfo
				.getFormats();
	}
	...
	public static CaptureDeviceInfo getCaptureDeviceInfo(
		...
		Vector<CaptureDeviceInfo> captureDeviceInfos = CaptureDeviceManager
				.getDeviceList(null);
		MediaLocator locator = captureDevice.getLocator();

		for (CaptureDeviceInfo captureDeviceInfo : captureDeviceInfos) {
			if (captureDeviceInfo.getLocator().toString()
					.equals(locator.toString())) {
	 ...
	 *         data
	 */
	public synchronized <SourceStreamT extends SourceStream> SourceStreamT[] getStreams(
		...
		@SuppressWarnings("unchecked")
		SourceStreamT[] clone = (SourceStreamT[]) Array.newInstance(clz,
				streamCount);
	...
	private synchronized FrameRateControl[] internalGetFrameRateControls() {
		if (frameRateControls == null) {
			...
			frameRateControls = (frameRateControl == null) ? new FrameRateControl[0]
	 ...
	 */
	private Format internalSetFormat(int streamIndex, Format oldValue,
			Format newValue) {
		synchronized (this) {
			if (streams != null) {
				AbstractBufferStream stream = streams[streamIndex];
	 ...
	 *         <tt>Format</tt> is to be left in effect
	 */
	protected abstract Format setFormat(int streamIndex, Format oldValue,
			Format newValue);
