...
public class WASAPIStream extends AbstractPushBufferStream {
	 ...
	 */
	private static void throwNewIOException(String message, HResultException hre)
			throws IOException {
	...
	public WASAPIStream(DataSource dataSource, FormatControl formatControl) {
		...
		audioSystem = (WASAPISystem) AudioSystem
				.getAudioSystem(AudioSystem.LOCATOR_PROTOCOL_WASAPI);
		if (audioSystem == null) {
	 ...
	 *             connects to the audio endpoint device identified by
	 *             <tt>locator</tt>
	 */
	private void connect() throws IOException {
		...
		try {
			...
			AudioFormat[] formats = WASAPISystem
					.getFormatsToInitializeIAudioClient(thisFormat);
			...
			try {
				...
				long iAudioClient = audioSystem.initializeIAudioClient(locator,
						dataFlow, eventHandle, hnsBufferDuration, formats);
				...
				try {
					...
					long iAudioCaptureClient = IAudioClient_GetService(
							iAudioClient, IID_IAudioCaptureClient);
					...
					try {
						...
						long bufferDuration = numBufferFrames * 1000
								/ sampleRate;
						...
						if (devicePeriod <= 1) {
							...
							if ((devicePeriod > WASAPISystem.DEFAULT_DEVICE_PERIOD)
									|| (devicePeriod <= 1)) {
						...
						srcSampleSize = WASAPISystem
								.getSampleSizeInBytes(format);

						dstChannels = thisFormat.getChannels();
						dstSampleSize = WASAPISystem
								.getSampleSizeInBytes(thisFormat);

						dstFrameSize = dstSampleSize * dstChannels;
						bufferFrames = (int) (DEFAULT_BUFFER_DURATION
								* sampleRate / 1000);
			...
			if (t instanceof ThreadDeath) {
				...
				logger.error("Failed to connect a WASAPIStream"
						+ " to an audio endpoint device.", t);
	 ...
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> to read the next data packet from the
	...
	private int doRead(Buffer buffer) throws IOException {
		...
		if (toRead == 0) {
			...
			byte[] data = AbstractCodec2.validateByteArraySize(buffer, offset
					+ toRead, true);
	 ...
	 * 
	 * @param length
	 *            the number of bytes to pop from <tt>remainder</tt>
	 */
	private void popFromRemainder(int length) {
		remainderLength = WASAPIRenderer
				.pop(remainder, remainderLength, length);
	}
	...
	private BufferTransferHandler readInEventHandleCmd() {
		...
		if (numFramesInNextPacket != 0) {
			...
			try {
				int read = IAudioCaptureClient_Read(iAudioCaptureClient,
						remainder, remainderLength, toRead, srcSampleSize,
						srcChannels, dstSampleSize, dstChannels);
		...
		return (push && (remainderLength >= bufferSize)) ? transferHandler
				: null;
	}
		...
		try {
			...
			do {
				...
				if (transferHandler != null) {
					try {
						Object o = this;
						PushBufferStream pushBufferStream = (PushBufferStream) o;
