...
public class DirectShowStream extends AbstractPushBufferStream {
	...
	private static final Logger logger = Logger
			.getLogger(DirectShowStream.class);
	...
	/**
	 * The indicator which determines whether {@link #grabber} automatically
	 ...
	 * media data captured by {@link #grabber} out of this instance through the
		...
		if (isSupportedFormat(format)) {
			if (device == null) {
				...
				try {
					...
					logger.error("Failed to set format on DirectShowStream: "
							+ format, ioe);
					/*
			...
			if (transferData) {
				...
				if (transferHandler != null) {
					...
					if (frameRateControl != null) {
						float frameRate;
						long newTransferDataTimeStamp = System
								.currentTimeMillis();
						...
						if ((transferDataTimeStamp != -1)
							...
							if (minimumVideoFrameInterval > 0) {
								long t = newTransferDataTimeStamp
								...
								if ((t > 0) && (t < minimumVideoFrameInterval)) {
									...
									try {
										Thread.sleep(minimumVideoFrameInterval
												- t);
	...
	private void SampleCB(long source, long ptr, int length) {
		...
		synchronized (dataSyncRoot) {
			if (!automaticallyDropsLateVideoFrames && (data != null)) {
				...
				if (nextData != null) {
					nextData.setLength(DSCaptureDevice.samplecopy(source, ptr,
							nextData.getPtr(), length));
			...
			if (data != null) {
				data.setLength(DSCaptureDevice.samplecopy(source, ptr,
						data.getPtr(), length));
	...
	private void setDeviceFormat(Format format) throws IOException {
		if (format == null) {
			...
			if (size == null) {
				...
				int hresult = device.setFormat(new DSFormat(size.width,
						size.height, avFrameFormat.getDeviceSystemPixFmt()));
				...
				case DSCaptureDevice.S_OK:
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Set format on DirectShowStream: "
								+ format);
					}
					break;
				default:
	...
	private void throwNewHResultException(int hresult) throws IOException {
		throw new IOException("HRESULT 0x"
				+ Long.toHexString(hresult & 0xffffffffL));
	}
}
