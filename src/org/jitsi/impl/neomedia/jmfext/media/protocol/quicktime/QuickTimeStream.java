 ...
 */
public class QuickTimeStream extends AbstractPushBufferStream {
	...
	QuickTimeStream(DataSource dataSource, FormatControl formatControl) {
		...
		captureOutput
				.setDelegate(new QTCaptureDecompressedVideoOutput.Delegate() {
					 ...
					 * 
					 * @param videoFrame
					 *            the <tt>CVImageBuffer</tt> which represents
					 *            the output video frame
					 * @param sampleBuffer
					 *            the <tt>QTSampleBuffer</tt> which represents
					 *            additional details about the output video
					 *            samples
					 */
					@Override
					public void outputVideoFrameWithSampleBuffer(
						...
						captureOutputDidOutputVideoFrameWithSampleBuffer(
								captureOutput, videoFrame, sampleBuffer);
					}
				});
	 ...
	 * @param videoFrame
	 *            the <tt>CVImageBuffer</tt> which represents the output video
	 *            frame
	 * @param sampleBuffer
	...
	private void captureOutputDidOutputVideoFrameWithSampleBuffer(
			QTCaptureOutput captureOutput, CVImageBuffer videoFrame,
			QTSampleBuffer sampleBuffer) {
		...
		synchronized (dataSyncRoot) {
			...
			if (data != null) {
				data.setLength(pixelBuffer.getBytes(data.getPtr(),
						data.getCapacity()));
		...
		if (this.format == null) {
			...
			if (format == null) {
				...
				if (videoFormat.getSize() != null) {
					...
					Dimension defaultSize = NeomediaServiceUtils
							.getMediaServiceImpl().getDeviceConfiguration()
							.getVideoSize();

					format = videoFormat.intersects(new VideoFormat(
					/* encoding */null, new Dimension(defaultSize.width,
							defaultSize.height),
	...
	private Format getCaptureOutputFormat() {
		NSDictionary pixelBufferAttributes = captureOutput
		...
		if (pixelBufferAttributes != null) {
			...
			case CVPixelFormatType.kCVPixelFormatType_32ARGB:
				if (captureOutputFormat instanceof AVFrameFormat) {
					...
					return new RGBFormat(((width == 0) && (height == 0) ? null
							: new Dimension(width, height)),
	 ...
	 * 
	 * @param videoFrame
	 *            the <tt>CVPixelBuffer</tt> which provides details about the
	 *            <tt>Format</tt> of the media data made available by this
	 *            <tt>PushBufferStream</tt>
	 * @return the <tt>Format</tt> of the media data made available by this
	...
	private Format getVideoFrameFormat(CVPixelBuffer videoFrame) {
		...
		if ((size == null) || ((size.width == 0) && (size.height == 0))) {
			...
			/* encoding */null, new Dimension(videoFrame.getWidth(), videoFrame
					.getHeight()),
			/* maxDataLength */Format.NOT_SPECIFIED,
			/* dataType */null,
			/* frameRate */Format.NOT_SPECIFIED));
	 ...
	 *            <tt>PushBufferStream</tt> to be set to {@link #captureOutput}
	 */
	private void setCaptureOutputFormat(Format format) {
		 ...
		 * QTCaptureDevice in its formatDescriptions early in its creation. The
		 * workaround presented here is to just force a specific size.
		 */
		if (size == null) {
