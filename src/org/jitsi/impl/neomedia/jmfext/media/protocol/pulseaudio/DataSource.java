...
public class DataSource extends AbstractPullBufferCaptureDevice {
			...
			if (libraryVersion != null) {
				...
				if ((major >= 1) && (minor >= 0)) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Will control the volume"
	...
	private class PulseAudioStream extends AbstractPullBufferStream {
		...
		public void read(Buffer buffer) throws IOException {
			...
			try {
				...
				while (toRead > 0) {
					...
					System.arraycopy(this.buffer, this.offset, data, offset,
							toCopy);
							...
							if (overflow > 0) {
								if (overflow >= this.length) {
									if (DEBUG && logger.isDebugEnabled()) {
										logger.debug("Dropping " + this.length
												+ " bytes!");
									}
									this.offset = 0;
									this.length = 0;
									...
									if (DEBUG && logger.isDebugEnabled()) {
										logger.debug("Dropping " + overflow
												+ " bytes!");
									}
		...
		private void connectWithMainloopLock() throws IOException {
			...
			try {
				stream = pulseAudioSystem
						.createStream(sampleRate, channels, getClass()
								.getName(), PulseAudioSystem.MEDIA_ROLE_PHONE);
			...
			try {
				int bytesPerTenMillis = (sampleRate / 100) * channels
						* (sampleSizeInBits / 8);
				...
				try {
					...
					try {
						...
						int state = pulseAudioSystem.waitForStreamState(stream,
								PA.STREAM_READY);
		...
		private void setStreamVolume(long stream, float level) {
			...
			long o = PA.context_set_source_output_volume(
					pulseAudioSystem.getContext(), PA.stream_get_index(stream),
					cvolume, null);
