...
public class DataSource extends AbstractVideoPushBufferCaptureDevice {
	...
	protected FrameRateControl createFrameRateControl() {
		return new FrameRateControlAdapter() {
			...
			public float getFrameRate() {
				...
				return frameRateFromQuickTimeStream ? frameRate
						: this.frameRate;
			}
	...
	protected void doConnect() throws IOException {
		...
		QTCaptureDeviceInput deviceInput = QTCaptureDeviceInput
			...
			if (streams != null) {
				for (Object stream : streams) {
					if (stream != null) {
						try {
							captureSession
									.addOutput(((QuickTimeStream) stream).captureOutput);
						} catch (NSErrorException nseex) {
							logger.error(
	 ...
	 *            the <tt>Format</tt>s from which more specific are to be
	 *            derived
	...
	private static synchronized Format[] getSupportedFormats(
		...
		supportedFormats = specificFormats.toArray(new Format[specificFormats
				.size()]);
		return supportedFormats.clone();
	}
	 ...
	 * 
	 * @param device
	 *            the <tt>QTCaptureDevice</tt> which represents the media source
	 ...
	 * 
	 * @param locator
	 *            the <tt>MediaLocator</tt> which specifies the media source of
	 *            this <tt>DataSource</tt>
	...
	public void setLocator(MediaLocator locator) {
		...
		if ((locator != null)
				&& DeviceSystem.LOCATOR_PROTOCOL_QUICKTIME
