...
public class PortAudioStream extends AbstractPullBufferStream {
	...
	private static final Logger logger = Logger
			.getLogger(PortAudioStream.class);
	...
	public PortAudioStream(DataSource dataSource, FormatControl formatControl,
		...
		MediaServiceImpl mediaServiceImpl = NeomediaServiceUtils
				.getMediaServiceImpl();

		gainControl = (mediaServiceImpl == null) ? null
	...
	private void connect() throws IOException {
		...
		if (deviceIndex == Pa.paNoDevice) {
			throw new IOException("The audio device " + deviceID
		...
		try {
			inputParameters = Pa.StreamParameters_new(deviceIndex, channels,
					sampleFormat, Pa.getSuggestedLatency());

			stream = Pa
					.OpenStream(inputParameters, 0 /* outputParameters */,
							sampleRate, framesPerBuffer,
							Pa.STREAM_FLAGS_CLIP_OFF
									| Pa.STREAM_FLAGS_DITHER_OFF, null /* streamCallback */);
		 ...
		 */
		this.format = new AudioFormat(AudioFormat.LINEAR, sampleRate,
				sampleSizeInBits, channels, AudioFormat.LITTLE_ENDIAN,
		...
		if (mediaServiceImpl != null) {
			DeviceConfiguration devCfg = mediaServiceImpl
					.getDeviceConfiguration();

			if (devCfg != null) {
				denoise = devCfg.isDenoise();
				echoCancel = devCfg.isEchoCancel();
				echoCancelFilterLengthInMillis = devCfg
		...
		Pa.setEchoFilterLengthInMillis(
				stream,
				(audioQualityImprovement && echoCancel) ? echoCancelFilterLengthInMillis
						: 0);
	...
	public void read(Buffer buffer) throws IOException {
		...
		try {
			 ...
			 * allocations.
			 */
			byte[] data = AbstractCodec2.validateByteArraySize(buffer,
					bytesPerBuffer, false);
			 ...
			 */
			if (gainControl != null) {
				BasicVolumeControl.applyGain(gainControl, data, 0,
						bytesPerBuffer);
			}
			...
			if (stream != 0) {
				...
				try {
					...
					if ((errorCode == Pa.paTimedOut)
							|| (Pa.HostApiTypeId.paMME.equals(pae
									.getHostApiType()) && (errorCode == Pa.MMSYSERR_NODRIVER))) {
						closed = true;
					}

					if (!closed) {
						logger.error("Failed to close "
								+ getClass().getSimpleName(), pae);
	...
	public synchronized void start() throws IOException {
		if (stream != 0) {
			...
			try {
				...
				logger.error("Failed to start " + getClass().getSimpleName(),
						paex);
	...
	public synchronized void stop() throws IOException {
		if (stream != 0) {
			...
			try {
				...
				logger.error("Failed to stop " + getClass().getSimpleName(),
						paex);
