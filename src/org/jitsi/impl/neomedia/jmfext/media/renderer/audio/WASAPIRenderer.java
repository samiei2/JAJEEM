...
public class WASAPIRenderer extends AbstractAudioRenderer<WASAPISystem> {
	...
	public WASAPIRenderer(boolean playback) {
		this(playback ? AudioSystem.DataFlow.PLAYBACK
		...
		try {
			...
			try {
				...
				try {
					...
					long iAudioRenderClient = IAudioClient_GetService(
							iAudioClient, IID_IAudioRenderClient);
					...
					try {
						 ...
						 */
						if (devicePeriod <= 1) {
							long bufferDuration = numBufferFrames * 1000L
									/ sampleRate;
						...
						devicePeriodInFrames = (int) (devicePeriod * sampleRate / 1000L);
			...
			if (t instanceof ThreadDeath) {
				...
				logger.error("Failed to open a WASAPIRenderer"
						+ " on an audio endpoint device.", t);
				...
				if (numFramesRequested == 0) {
					if (eventHandle == 0) {
						...
						if (toCopy > 0) {
							...
							System.arraycopy(data, offset, remainder,
					...
					int toWrite = Math.min(effectiveLength, numFramesRequested
							* srcFrameSize);
					byte[] effectiveData;
					int effectiveOffset;
					...
					if ((toWrite / srcFrameSize) == 0) {
						...
						if (gainControl != null) {
							BasicVolumeControl.applyGain(gainControl,
									effectiveData, effectiveOffset, toWrite);
						}

						try {
							written = IAudioRenderClient_Write(
									iAudioRenderClient, effectiveData,
									effectiveOffset, toWrite, srcSampleSize,
			...
			do {
				...
				try {
					...
					if (numFramesRequested > 0) {
						...
						if (silence > 0) {
							Arrays.fill(remainder, remainderLength, toWrite,
									(byte) 0);
							remainderLength = toWrite;
						}
						...
						try {
							written = IAudioRenderClient_Write(
									iAudioRenderClient, remainder, 0, toWrite,
									srcSampleSize, srcChannels, dstSampleSize,
									dstChannels);
						} catch (HResultException hre) {
							written = 0;
							logger.error("IAudioRenderClient_Write", hre);
		...
		if (iAudioClient != 0) {
			...
			if (remainder != null) {
				if (remainderLength > 0) {
					 ...
					 */
					for (int i = remainder.length - 1, j = remainderLength - 1; j >= 0; i--, j--) {
