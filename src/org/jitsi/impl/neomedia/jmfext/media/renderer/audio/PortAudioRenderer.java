...
public class PortAudioRenderer extends AbstractAudioRenderer<PortAudioSystem> {
	...
	private static final Logger logger = Logger
			.getLogger(PortAudioRenderer.class);
	 ...
	 * input.
	 */
	private static final double[] SUPPORTED_INPUT_SAMPLE_RATES = new double[] {
	...
	static {
		...
		for (int i = 0; i < count; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR,
					SUPPORTED_INPUT_SAMPLE_RATES[i], 16 /* sampleSizeInBits */,
	...
	public PortAudioRenderer(boolean enableVolumeControl) {
		super(AudioSystem.LOCATOR_PROTOCOL_PORTAUDIO,
				enableVolumeControl ? AudioSystem.DataFlow.PLAYBACK
		 ...
		 */
		PortAudioSystem
				.addPaUpdateAvailableDeviceListListener(paUpdateAvailableDeviceListListener);
	}
	...
	public Format[] getSupportedInputFormats() {
		if (supportedInputFormats == null) {
			...
			if ((locator == null)
					...
					/* minInputChannels */0,
					/* minOutputChannels */1)) == Pa.paNoDevice)
				 ...
				 * at most 2.
				 */
				int maxOutputChannels = Math.min(
						Pa.DeviceInfo_getMaxOutputChannels(deviceInfo), 2);
				List<Format> supportedInputFormats = new ArrayList<Format>(
						SUPPORTED_INPUT_FORMATS.length);

				for (Format supportedInputFormat : SUPPORTED_INPUT_FORMATS) {
					getSupportedInputFormats(supportedInputFormat, deviceIndex,
							minOutputChannels, maxOutputChannels,
							supportedInputFormats);
				}
				this.supportedInputFormats = supportedInputFormats.isEmpty() ? EMPTY_SUPPORTED_INPUT_FORMATS
						: supportedInputFormats
	...
	private void getSupportedInputFormats(Format format, int deviceIndex,
		...
		for (int channels = minOutputChannels; channels <= maxOutputChannels; channels++) {
			long outputParameters = Pa.StreamParameters_new(deviceIndex,
					channels, sampleFormat, Pa.LATENCY_UNSPECIFIED);

			if (outputParameters != 0) {
				try {
					if (Pa.IsFormatSupported(0, outputParameters, sampleRate)) {
						supportedInputFormats.add(new AudioFormat(audioFormat
								.getEncoding(), sampleRate, sampleSizeInBits,
								channels, audioFormat.getEndian(), audioFormat
										.getSigned(),
								Format.NOT_SPECIFIED /* frameSizeInBits */,
	...
	private void doOpen() throws ResourceUnavailableException {
		if (stream == 0) {
			...
			if (deviceIndex == Pa.paNoDevice) {
				throw new ResourceUnavailableException("The audio device "
						+ deviceID + " appears to be disconnected.");
			}
			...
			long sampleFormat = Pa.getPaSampleFormat(inputFormat
					.getSampleSizeInBits());
			...
			try {
				outputParameters = Pa.StreamParameters_new(deviceIndex,
						channels, sampleFormat, Pa.getSuggestedLatency());

				stream = Pa.OpenStream(0 /* inputParameters */,
						outputParameters, sampleRate, framesPerBuffer,
			...
			bytesPerBuffer = Pa.GetSampleSize(sampleFormat) * channels
					* framesPerBuffer;
	...
	public int process(Buffer buffer) {
		...
		try {
			process((byte[]) buffer.getData(), buffer.getOffset(),
					buffer.getLength());
			...
			synchronized (this) {
				...
				if (errorCode == Pa.paNoError) {
					...
					if (writeIsMalfunctioningSince != NEVER) {
						...
						|| (Pa.HostApiTypeId.paMME.equals(hostApiType) && (Pa.MMSYSERR_NODRIVER == errorCode))) {
					if (writeIsMalfunctioningSince == NEVER) {
	...
	private void process(byte[] buffer, int offset, int length)
		...
		if ((bufferLeft != null) && (bufferLeftLength > 0)) {
			int numberOfBytesInBufferLeftToBytesPerBuffer = bytesPerBuffer
					- bufferLeftLength;
			int numberOfBytesToCopyToBufferLeft = (numberOfBytesInBufferLeftToBytesPerBuffer < length) ? numberOfBytesInBufferLeftToBytesPerBuffer
					: length;
		...
		if (numberOfWrites > 0) {
			...
			if (gainControl != null) {
				BasicVolumeControl.applyGain(gainControl, buffer, offset,
						length);
			}

			Pa.WriteStream(stream, buffer, offset, framesPerBuffer,
					numberOfWrites);
