...
public class PulseAudioRenderer extends AbstractAudioRenderer<PulseAudioSystem> {
	...
	private static final Format[] SUPPORTED_INPUT_FORMATS = new Format[] { new AudioFormat(
			AudioFormat.LINEAR, Format.NOT_SPECIFIED /* sampleRate */, 16,
			Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN,
	...
	public PulseAudioRenderer(String mediaRole) {
		super(PulseAudioSystem.getPulseAudioSystem(),
				((mediaRole == null) || PulseAudioSystem.MEDIA_ROLE_PHONE
						.equals(mediaRole)) ? AudioSystem.DataFlow.PLAYBACK
		...
		this.mediaRole = (mediaRole == null) ? PulseAudioSystem.MEDIA_ROLE_PHONE
				: mediaRole;
	}
	...
	private void openWithMainloopLock() throws ResourceUnavailableException {
		...
		try {
			stream = audioSystem.createStream(sampleRate, channels, getClass()
					.getName(), mediaRole);
		...
		try {
			...
			try {
				...
				try {
					...
					int state = audioSystem.waitForStreamState(stream,
							PA.STREAM_READY);
		...
		if (writableSize <= 0) {
			...
			if (gainControl != null) {
				if (SOFTWARE_GAIN || (cvolume == 0)) {
					if (length > 0) {
						BasicVolumeControl.applyGain(gainControl, data, offset,
								writableSize);
					}
				} else {
					float gainControlLevel = gainControl.getLevel();
			...
			int writtenSize = PA.stream_write(stream, data, offset,
					writableSize, null, 0, PA.SEEK_RELATIVE);
