 ...
 */
public class JAWTRenderer extends AbstractRenderer<VideoFormat> implements
		VideoRenderer {
	/**
	 ...
	 */
	private static final Format[] SUPPORTED_INPUT_FORMATS = new Format[] { OSUtils.IS_LINUX ? new YUVFormat(
			null /* size */, Format.NOT_SPECIFIED /* maxDataLength */,
			Format.intArray, Format.NOT_SPECIFIED /* frameRate */,
			YUVFormat.YUV_420, Format.NOT_SPECIFIED /* strideY */,
			Format.NOT_SPECIFIED /* strideUV */,
			Format.NOT_SPECIFIED /* offsetY */,
			Format.NOT_SPECIFIED /* offsetU */, Format.NOT_SPECIFIED /* offsetV */)
			: OSUtils.IS_ANDROID ? new RGBFormat(null, Format.NOT_SPECIFIED,
					Format.intArray, Format.NOT_SPECIFIED, 32, 0x000000ff,
					0x0000ff00, 0x00ff0000) : new RGBFormat(null,
					Format.NOT_SPECIFIED, Format.intArray,
					Format.NOT_SPECIFIED, 32, 0x00ff0000, 0x0000ff00,
					0x000000ff) };
	 ...
	 */
	static native boolean paint(long handle, Component component, Graphics g,
			int zOrder);
	 ...
	 */
	static native boolean process(long handle, Component component, int[] data,
			int offset, int length, int width, int height);
	...
	public synchronized Component getComponent() {
		if (component == null) {
			...
			componentClassName
					.append("org.jitsi.impl.neomedia.jmfext.media.renderer.video"
							+ ".JAWTRenderer");
			...
			try {
				...
				Constructor<?> componentConstructor = componentClass
						.getConstructor(JAWTRenderer.class);
			 ...
			 */
			component.setSize(DEFAULT_COMPONENT_HEIGHT_OR_WIDTH,
	...
	public synchronized int process(Buffer buffer) {
		...
		if ((format != null) && (format != this.inputFormat)
		...
		if (handle == 0) {
			...
			if ((size.width >= SwScale.MIN_SWS_SCALE_HEIGHT_OR_WIDTH)
				...
				boolean repaint = process(handle, component,
						(int[]) buffer.getData(), buffer.getOffset(),
	...
	private void reflectInputFormatOnComponent() {
		if (SwingUtilities.isEventDispatchThread()) {
			...
			SwingUtilities
					.invokeLater(reflectInputFormatOnComponentInEventDispatchThread);
		}
	}
	...
	private void reflectInputFormatOnComponentInEventDispatchThread() {
		...
		if ((component != null) && (width > 0) && (height > 0)) {
			...
			if (component.isPreferredSizeSet()
				...
				if ((size.width < 1)
						|| (size.height < 1)
						|| !VideoLayout.areAspectRatiosEqual(size,
	...
	public synchronized Format setInputFormat(Format format) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug(getClass().getName() + " 0x"
					+ Integer.toHexString(hashCode()) + " set to input in "
					+ inputFormat);
		}
