...
public abstract class AbstractRTPConnector implements RTPConnector {
	...
	public void addTarget(SessionAddress target) throws IOException {
		...
		if (controlAddress != null) {
			getControlOutputStream().addTarget(controlAddress,
					target.getControlPort());
		}

		getDataOutputStream().addTarget(target.getDataAddress(),
				target.getDataPort());
	}
	...
	public void removeTarget(SessionAddress target) {
		if (controlOutputStream != null) {
			controlOutputStream.removeTarget(target.getControlAddress(),
					target.getControlPort());
		}

		if (dataOutputStream != null) {
			dataOutputStream.removeTarget(target.getDataAddress(),
					target.getDataPort());
		}
	}
