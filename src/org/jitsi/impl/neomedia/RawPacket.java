...
public class RawPacket {
	...
	public short readShort(int off) {
		return (short) ((this.buffer[this.offset + off + 0] << 8) | (this.buffer[this.offset
				+ off + 1] & 0xff));
	}
	...
	public void append(byte[] data, int len) {
		...
		if ((this.length + len) > (buffer.length - this.offset)) {
			...
			System.arraycopy(this.buffer, this.offset, newBuffer, 0,
					this.length);
			this.offset = 0;
	...
	public void setCsrcList(long[] newCsrcList) {
		...
		byte[] newBuffer = new byte[length + offset + csrcBuff.length
				- oldCsrcCount * 4];
		...
		System.arraycopy(csrcBuff, 0, newBuffer, offset + FIXED_HEADER_SIZE,
				csrcBuff.length);

		// now copy the payload from the old buff and make sure we don't copy
		// the CSRC list if there was one in the old packet
		int payloadOffsetForOldBuff = offset + FIXED_HEADER_SIZE + oldCsrcCount
				* 4;

		int payloadOffsetForNewBuff = offset + FIXED_HEADER_SIZE + newCsrcCount
				* 4;

		System.arraycopy(oldBuffer, payloadOffsetForOldBuff, newBuffer,
				payloadOffsetForNewBuff, length - payloadOffsetForOldBuff);
	...
	public int getHeaderLength() {
		if (getExtensionBit()) {
			return FIXED_HEADER_SIZE + 4 * getCsrcCount() + EXT_HEADER_SIZE
					+ getExtensionLength();
		} else {
	...
	public void addExtension(byte[] extBuff, int newExtensionLen) {
		 ...
		 * header if any.
		 */
		System.arraycopy(buffer, bufferOffset, newBuffer, newBufferOffset,
				lengthToCopy);
		...
		System.arraycopy(extBuff, 0, newBuffer, newBufferOffset,
				newExtensionLen);
	...
	public void removeExtension() {
		...
		System.arraycopy(buffer, payloadOffset, buffer, payloadOffset
				- extHeaderLen, getPayloadLength());
	...
	private int findExtension(int extensionID) {
		...
		int extOffset = offset + FIXED_HEADER_SIZE + getCsrcCount() * 4
				+ EXT_HEADER_SIZE;
	...
	public int getHeaderExtensionType() {
		...
		return readUnsignedShortAsInt(offset + FIXED_HEADER_SIZE
				+ getCsrcCount() * 4);
	}
}
