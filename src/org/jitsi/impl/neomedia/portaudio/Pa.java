...
public final class Pa {
	...
	public static enum HostApiTypeId {
		paAL(9), paALSA(8), paASIO(3), paAudioScienceHPI(14), paBeOS(10), paCoreAudio(
				5), paDirectSound(1), paInDevelopment(0) /*
														 * use while developing
														 * support for a new
														 * host API
														 */, paJACK(12), paMME(
				2), paOSS(7), paSoundManager(4), paWASAPI(13), paWDMKS(11);
	...
	public static String DeviceInfo_getTransportType(long deviceInfo) {
		return StringUtils
				.newString(DeviceInfo_getTransportTypeBytes(deviceInfo));
	}
	 ...
	 *         exists
	 */
	public static int getDeviceIndex(String deviceID, int minInputChannels,
		...
		if (deviceID != null) {
			...
			for (int deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
				...
				if (deviceID
						.equals(((deviceUID == null) || (deviceUID.length() == 0)) ? Pa
								.DeviceInfo_getName(deviceInfo) : deviceUID)) {
					/*
					...
					if ((minInputChannels > 0)
							&& (Pa.DeviceInfo_getMaxInputChannels(deviceInfo) < minInputChannels)) {
						continue;
					}
					if ((minOutputChannels > 0)
							&& (Pa.DeviceInfo_getMaxOutputChannels(deviceInfo) < minOutputChannels)) {
						continue;
					}
	...
	public static double getSuggestedLatency() {
		...
		if (cfg != null) {
			String suggestedLatencyString = cfg
					.getString(PROP_SUGGESTED_LATENCY);

			if (suggestedLatencyString != null) {
				try {
					double suggestedLatency = Double
							.parseDouble(suggestedLatencyString);

					if (suggestedLatency != LATENCY_UNSPECIFIED) {
						return suggestedLatency;
					}
				} catch (NumberFormatException nfe) {
					logger.error("Failed to parse configuration property "
							+ PROP_SUGGESTED_LATENCY + " value as a double",
							nfe);
				}
			}
		}
	 ...
	 */
	public static native boolean IsFormatSupported(long inputParameters,
			long outputParameters, double sampleRate);
	 ...
	 * @throws PortAudioException
	 */
	public static native long OpenStream(long inputParameters,
			long outputParameters, double sampleRate, long framesPerBuffer,
			long streamFlags, PortAudioStreamCallback streamCallback)
			throws PortAudioException;
	 ...
	 * @throws PortAudioException
	 */
	public static native void ReadStream(long stream, byte[] buffer, long frames)
			throws PortAudioException;
	 ...
	 */
	public static native void setEchoFilterLengthInMillis(long stream,
	 ...
	 */
	public static native long StreamParameters_new(int deviceIndex,
			int channelCount, long sampleFormat, double suggestedLatency);
	 ...
	 */
	public static native void WriteStream(long stream, byte[] buffer,
			int offset, long frames, int numberOfWrites)
			throws PortAudioException;
