...
public class RTPTranslatorImpl implements ReceiveStreamListener, RTPTranslator {
	...
	private static final Logger logger = Logger
			.getLogger(RTPTranslatorImpl.class);
	...
	public synchronized void addFormat(StreamRTPManager streamRTPManager,
			Format format, int payloadType) {
		manager.addFormat(format, payloadType);

		getStreamRTPManagerDesc(streamRTPManager, true).addFormat(format,
				payloadType);
	}

	public synchronized void addReceiveStreamListener(
			StreamRTPManager streamRTPManager, ReceiveStreamListener listener) {
	...
	private synchronized void closeFakeSendStreamIfNotNecessary() {
		...
		try {
			...
			if (t instanceof ThreadDeath) {
				...
				logger.debug("Failed to close the fake SendStream of this"
						+ " RTPTranslator.", t);
			}
		}
	}
	...
	private synchronized void createFakeSendStreamIfNecessary() {
		 ...
		 * and RTCP to.
		 */
		if (CREATE_FAKE_SEND_STREAM_IF_NECESSARY && (fakeSendStream == null)
				&& sendStreams.isEmpty() && (streamRTPManagers.size() > 1)) {
			...
			if (supportedFormat != null) {
				try {
					fakeSendStream = manager.createSendStream(
							new FakePushBufferDataSource(supportedFormat), 0);
				} catch (Throwable t) {
					if (t instanceof ThreadDeath) {
						throw (ThreadDeath) t;
					} else {
	...
	public synchronized SendStream createSendStream(
			StreamRTPManager streamRTPManager, DataSource dataSource,
			int streamIndex) throws IOException, UnsupportedFormatException {
		...
		if (sendStreamDesc == null) {
			SendStream sendStream = manager.createSendStream(dataSource,
					streamIndex);

			if (sendStream != null) {
				sendStreamDesc = new SendStreamDesc(dataSource, streamIndex,
						sendStream);
		...
		return (sendStreamDesc == null) ? null : sendStreamDesc.getSendStream(
				streamRTPManager, true);
	}
	...
	public synchronized void dispose(StreamRTPManager streamRTPManager) {
		Iterator<StreamRTPManagerDesc> streamRTPManagerIter = streamRTPManagers
				.iterator();

		while (streamRTPManagerIter.hasNext()) {
			StreamRTPManagerDesc streamRTPManagerDesc = streamRTPManagerIter
					.next();
	...
	private synchronized StreamRTPManagerDesc findStreamRTPManagerDescByReceiveSSRC(
			long receiveSSRC, StreamRTPManagerDesc exclusion) {
	...
	public synchronized Vector<ReceiveStream> getReceiveStreams(
			StreamRTPManager streamRTPManager) {
		StreamRTPManagerDesc streamRTPManagerDesc = getStreamRTPManagerDesc(
				streamRTPManager, false);
		...
		if (streamRTPManagerDesc != null) {
			...
			if (managerReceiveStreams != null) {
				receiveStreams = new Vector<ReceiveStream>(
						managerReceiveStreams.size());
				for (Object s : managerReceiveStreams) {
					ReceiveStream receiveStream = (ReceiveStream) s;

					if (streamRTPManagerDesc.containsReceiveSSRC(receiveStream
							.getSSRC())) {
		...
		if (managerSendStreams != null) {
			...
			for (SendStreamDesc sendStreamDesc : this.sendStreams) {
				if (managerSendStreams.contains(sendStreamDesc.sendStream)) {
					SendStream sendStream = sendStreamDesc.getSendStream(
							streamRTPManager, false);
	...
	private synchronized StreamRTPManagerDesc getStreamRTPManagerDesc(
			StreamRTPManager streamRTPManager, boolean create) {
		for (StreamRTPManagerDesc s : streamRTPManagers) {
	...
	public synchronized void initialize(StreamRTPManager streamRTPManager,
	 ...
	 *            valid data
	 */
	private static void logRTCP(Object obj, String methodName, byte[] buffer,
		...
		if (length > 8) {
			...
			if (v == 2) {
				...
				if (pt == 203 /* BYE */) {
					...
					logger.trace(obj.getClass().getName() + '.' + methodName
	 ...
	 */
	private int read(PushSourceStreamDesc streamDesc, byte[] buffer,
			int offset, int length, int read) throws IOException {
		...
		if (data) {
			...
			if ((length >= 12)
				...
				if (!streamRTPManagerDesc.containsReceiveSSRC(ssrc)) {
					if (findStreamRTPManagerDescByReceiveSSRC(ssrc,
		...
		OutputDataStreamImpl outputStream = data ? connector
				.getDataOutputStream() : connector.getControlOutputStream();

		if (outputStream != null) {
			outputStream.write(buffer, offset, read, format,
	 ...
	 * @param offset
	 *            the zero-based offset in <tt>buffer</tt> to start reading an
	 *            <tt>int</tt> from
	...
	public void update(ReceiveStreamEvent event) {
		...
		if (event != null) {
			...
			if (receiveStream != null) {
				StreamRTPManagerDesc streamRTPManagerDesc = findStreamRTPManagerDescByReceiveSSRC(
						receiveStream.getSSRC(), null);

				if (streamRTPManagerDesc != null) {
					for (ReceiveStreamListener listener : streamRTPManagerDesc
							.getReceiveStreamListeners()) {
	...
	private static class OutputDataStreamDesc {
		...
		public OutputDataStreamDesc(RTPConnectorDesc connectorDesc,
	...
	private static class OutputDataStreamImpl implements OutputDataStream,
			Runnable {
		...
		public synchronized void addStream(RTPConnectorDesc connectorDesc,
				OutputDataStream stream) {
		...
		private synchronized int doWrite(byte[] buffer, int offset, int length,
				...
				if (streamRTPManagerDesc != exclusion) {
					if (data) {
						if ((format != null) && (length > 0)) {
							Integer payloadType = streamRTPManagerDesc
									.getPayloadType(format);
					...
					int streamWrite = streamDesc.stream.write(buffer, offset,
							length);
		...
		public synchronized void write(byte[] buffer, int offset, int length,
			...
			if (writeQueueLength < writeQueue.length) {
				writeIndex = (writeQueueHead + writeQueueLength)
						% writeQueue.length;
			} else {
				writeIndex = writeQueueHead;
				writeQueueHead++;
	...
	private static class PushSourceStreamDesc {
		...
		public PushSourceStreamDesc(RTPConnectorDesc connectorDesc,
				PushSourceStream stream, boolean data) {
			this.connectorDesc = connectorDesc;
			this.stream = stream;
			this.data = data;
		}
	...
	private class PushSourceStreamImpl implements PushSourceStream,
		...
		public synchronized void addStream(RTPConnectorDesc connectorDesc,
			...
			streams.add(new PushSourceStreamDesc(connectorDesc, stream,
					this.data));
		...
		public synchronized int getMinimumTransferSize() {
			...
			for (PushSourceStreamDesc streamDesc : streams) {
				int streamMinimumTransferSize = streamDesc.stream
		...
		public int read(byte[] buffer, int offset, int length)
			...
			synchronized (this) {
				...
				read = (streamDesc == null) ? 0 : streamDesc.stream.read(
						buffer, offset, length);
			}
			if (read > 0) {
				read = RTPTranslatorImpl.this.read(streamDesc, buffer, offset,
						length, read);
			}
			return read;
		}
	...
	private static class RTPConnectorDesc {
		...
		public RTPConnectorDesc(StreamRTPManagerDesc streamRTPManagerDesc,
				RTPConnector connector) {
	...
	private class RTPConnectorImpl implements RTPConnector {
		...
		public synchronized void addConnector(RTPConnectorDesc connector) {
			if (!connectors.contains(connector)) {
				...
				if (this.controlInputStream != null) {
					...
					try {
						controlInputStream = connector.connector
								.getControlInputStream();
					} catch (IOException ioe) {
						throw new UndeclaredThrowableException(ioe);
					}
					if (controlInputStream != null) {
						this.controlInputStream.addStream(connector,
								controlInputStream);
					}
				}
				if (this.controlOutputStream != null) {
					...
					try {
						controlOutputStream = connector.connector
								.getControlOutputStream();
					} catch (IOException ioe) {
						throw new UndeclaredThrowableException(ioe);
					}
					if (controlOutputStream != null) {
						this.controlOutputStream.addStream(connector,
								controlOutputStream);
					}
				}
				if (this.dataInputStream != null) {
					...
					try {
						dataInputStream = connector.connector
								.getDataInputStream();
					} catch (IOException ioe) {
						throw new UndeclaredThrowableException(ioe);
					}
					if (dataInputStream != null) {
						this.dataInputStream.addStream(connector,
								dataInputStream);
					}
				}
				if (this.dataOutputStream != null) {
					...
					try {
						dataOutputStream = connector.connector
								.getDataOutputStream();
					} catch (IOException ioe) {
						throw new UndeclaredThrowableException(ioe);
					}
					if (dataOutputStream != null) {
						this.dataOutputStream.addStream(connector,
								dataOutputStream);
					}
				}
			}
		...
		public synchronized PushSourceStream getControlInputStream()
			...
			if (this.controlInputStream == null) {
				...
				for (RTPConnectorDesc connectorDesc : connectors) {
					PushSourceStream controlInputStream = connectorDesc.connector
							.getControlInputStream();

					if (controlInputStream != null) {
						this.controlInputStream.addStream(connectorDesc,
								controlInputStream);
					}
				}
			}
		...
		public synchronized OutputDataStreamImpl getControlOutputStream()
			...
			if (this.controlOutputStream == null) {
				...
				for (RTPConnectorDesc connectorDesc : connectors) {
					OutputDataStream controlOutputStream = connectorDesc.connector
							.getControlOutputStream();

					if (controlOutputStream != null) {
						this.controlOutputStream.addStream(connectorDesc,
								controlOutputStream);
					}
				}
			}
		...
		public synchronized PushSourceStream getDataInputStream()
			...
			if (this.dataInputStream == null) {
				...
				for (RTPConnectorDesc connectorDesc : connectors) {
					PushSourceStream dataInputStream = connectorDesc.connector
							.getDataInputStream();

					if (dataInputStream != null) {
						this.dataInputStream.addStream(connectorDesc,
								dataInputStream);
					}
				}
			}
		...
		public synchronized OutputDataStreamImpl getDataOutputStream()
			...
			if (this.dataOutputStream == null) {
				...
				for (RTPConnectorDesc connectorDesc : connectors) {
					OutputDataStream dataOutputStream = connectorDesc.connector
							.getDataOutputStream();

					if (dataOutputStream != null) {
						this.dataOutputStream.addStream(connectorDesc,
								dataOutputStream);
					}
				}
			}
	...
	private class SendStreamDesc {
		...
		public SendStreamDesc(DataSource dataSource, int streamIndex,
		...
		public synchronized SendStreamImpl getSendStream(
			...
			if (create) {
				SendStreamImpl sendStream = new SendStreamImpl(
						streamRTPManager, this);
	...
	private static class SendStreamImpl implements SendStream {
		...
		public SendStreamImpl(StreamRTPManager streamRTPManager,
		...
		public void start() throws IOException {
			if (closed) {
				throw new IOException("Cannot start SendStream"
	...
	private static class StreamRTPManagerDesc {
		...
		public synchronized void addReceiveSSRC(long receiveSSRC) {
			if (!containsReceiveSSRC(receiveSSRC)) {
				...
				System.arraycopy(receiveSSRCs, 0, newReceiveSSRCs, 0,
						receiveSSRCCount);
		...
		public ReceiveStreamListener[] getReceiveStreamListeners() {
			synchronized (receiveStreamListeners) {
				return receiveStreamListeners
						.toArray(new ReceiveStreamListener[receiveStreamListeners
								.size()]);
			}
		}
