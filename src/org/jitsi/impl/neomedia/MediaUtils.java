...
public class MediaUtils {
	 ...
	 */
	private static final List<MediaFormat> rtpPayloadTypelessMediaFormats = new ArrayList<MediaFormat>();

	/**
	 * The <tt>Map</tt> of RTP payload types (expressed as <tt>String</tt>s) to
	 * <tt>MediaFormat</tt>s.
	 */
	private static final Map<String, MediaFormat[]> rtpPayloadTypeStrToMediaFormats = new HashMap<String, MediaFormat[]>();

	static {
		addMediaFormats((byte) SdpConstants.PCMU, "PCMU", MediaType.AUDIO,
				AudioFormat.ULAW_RTP, 8000);
		...
		if (OSUtils.IS_LINUX32 || OSUtils.IS_WINDOWS32) {
			Map<String, String> g723FormatParams = new HashMap<String, String>();
			g723FormatParams.put("annexa", "no");
			g723FormatParams.put("bitrate", "6.3");
			addMediaFormats((byte) SdpConstants.G723, "G723", MediaType.AUDIO,
					AudioFormat.G723_RTP, g723FormatParams, null, 8000);
		}

		addMediaFormats((byte) SdpConstants.GSM, "GSM", MediaType.AUDIO,
				AudioFormat.GSM_RTP, 8000);
		addMediaFormats((byte) SdpConstants.PCMA, "PCMA", MediaType.AUDIO,
				Constants.ALAW_RTP, 8000);
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "iLBC",
				MediaType.AUDIO, Constants.ILBC_RTP, 8000);
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "speex",
				MediaType.AUDIO, Constants.SPEEX_RTP, 8000, 16000, 32000);
		addMediaFormats((byte) SdpConstants.G722, "G722", MediaType.AUDIO,
				Constants.G722_RTP, 8000);
		if (EncodingConfigurationImpl.G729) {
			addMediaFormats((byte) SdpConstants.G729, "G729", MediaType.AUDIO,
					AudioFormat.G729_RTP, 8000);
		}
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN,
				"telephone-event", MediaType.AUDIO, Constants.TELEPHONE_EVENT,
				8000);

		ConfigurationService cfg = LibJitsi.getConfigurationService();

		boolean advertiseFEC = cfg.getBoolean(
				Constants.PROP_SILK_ADVERSISE_FEC, false);
		Map<String, String> silkFormatParams = new HashMap<String, String>();
		if (advertiseFEC) {
			silkFormatParams.put("useinbandfec", "1");
		}
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "SILK",
				MediaType.AUDIO, Constants.SILK_RTP, silkFormatParams, null,
		...
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "opus",
				MediaType.AUDIO, Constants.OPUS_RTP, opusFormatParams, null,
				48000);

		/*
		 * We don't really support these.
		 * 
		 * addMediaFormats( (byte) SdpConstants.JPEG, "JPEG", MediaType.VIDEO,
		 * VideoFormat.JPEG_RTP); addMediaFormats( (byte) SdpConstants.H263,
		 * "H263", MediaType.VIDEO, VideoFormat.H263_RTP); addMediaFormats(
		 * (byte) SdpConstants.H261, "H261", MediaType.VIDEO,
		...
		if ((cfg == null)
			...
			addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "H264",
					MediaType.VIDEO, Constants.H264_RTP, h264FormatParams,
		...
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "H263-1998",
				MediaType.VIDEO, Constants.H263P_RTP, h263FormatParams,
				h263AdvancedAttributes);

		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "VP8",
				MediaType.VIDEO, Constants.VP8_RTP, null, null);

		// Calculate the values of the MAX_AUDIO_* static fields of MediaUtils.
		List<MediaFormat> audioMediaFormats = new ArrayList<MediaFormat>(
				rtpPayloadTypeStrToMediaFormats.size()
						+ rtpPayloadTypelessMediaFormats.size());

		for (MediaFormat[] mediaFormats : rtpPayloadTypeStrToMediaFormats
				.values()) {
		...
		for (MediaFormat mediaFormat : audioMediaFormats) {
			...
			int sampleSizeInBits = audioMediaFormat.getFormat()
					.getSampleSizeInBits();
	 ...
	 */
	private static void addMediaFormats(byte rtpPayloadType, String encoding,
			MediaType mediaType, String jmfEncoding, double... clockRates) {
		addMediaFormats(rtpPayloadType, encoding, mediaType, jmfEncoding, null,
				null, clockRates);
	}
	...
	private static void addMediaFormats(byte rtpPayloadType, String encoding,
			MediaType mediaType, String jmfEncoding,
			Map<String, String> formatParameters,
			Map<String, String> advancedAttributes, double... clockRates) {
		...
		if (clockRateCount > 0) {
			for (double clockRate : clockRates) {
				...
				case AUDIO:
					...
					break;
				case VIDEO:
					format = new ParameterizedVideoFormat(jmfEncoding,
							formatParameters);
					break;
				default:
				...
				MediaFormat mediaFormat = MediaFormatImpl
			...
			MediaFormat mediaFormat = MediaFormatImpl.createInstance(format,
					clockRate, formatParameters, advancedAttributes);
	 ...
	 *            the clock rate of the <tt>MediaFormat</tt> to get
	 * @param fmtps
	 *            the format parameters of the <tt>MediaFormat</tt> to get
	 * @return the <tt>MediaFormat</tt> known to <tt>MediaUtils</tt> and having
	 *         the specified <tt>encoding</tt> (name), <tt>clockRate</tt> and
	 *         matching format parameters
	 */
	public static MediaFormat getMediaFormat(String encoding, double clockRate,
	...
	/**
	 * Gets the <tt>MediaFormat</tt>s (expressed as an array) corresponding to a
	 * specific RTP payload type.
	 * 
	 * @param rtpPayloadType
	 *            the RTP payload type to retrieve the corresponding
	 *            <tt>MediaFormat</tt>s for
	...
	public static MediaFormat[] getMediaFormats(byte rtpPayloadType) {
		MediaFormat[] mediaFormats = rtpPayloadTypeStrToMediaFormats.get(Byte
				.toString(rtpPayloadType));

		return (mediaFormats == null) ? EMPTY_MEDIA_FORMATS : mediaFormats
				.clone();
	}
	...
	public static List<MediaFormat> getMediaFormats(String encoding) {
		...
		for (Map.Entry<String, String> jmfEncodingToEncoding : jmfEncodingToEncodings
				.entrySet()) {
		...
		if (jmfEncoding != null) {
			for (MediaFormat[] rtpPayloadTypeMediaFormats : rtpPayloadTypeStrToMediaFormats
					.values()) {
