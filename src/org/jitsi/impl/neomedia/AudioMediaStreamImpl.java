 ...
 * @author Emil Ivov
 */
public class AudioMediaStreamImpl extends MediaStreamImpl implements
		AudioMediaStream, PropertyChangeListener {
	/**
	...
	private static final AudioFormat[] CUSTOM_CODEC_FORMATS = new AudioFormat[] {
			 ...
			 * parameters.
			 */
			new AudioFormat(Constants.ALAW_RTP, 8000, 8, 1,
					Format.NOT_SPECIFIED, AudioFormat.SIGNED),
			new AudioFormat(Constants.G722_RTP, 8000,
	...
	private static final Logger logger = Logger
			.getLogger(AudioMediaStreamImpl.class);
	...
	public void addRTPExtension(byte extensionID, RTPExtension rtpExtension) {
		...
		if (RTPExtension.CSRC_AUDIO_LEVEL_URN.equals(rtpExtension.getURI()
				.toString())) {
	...
	protected void deviceSessionChanged(MediaDeviceSession oldValue,
		...
		try {
			...
			if (newValue != null) {
				...
				if (localUserAudioLevelListener != null) {
					deviceSession
							.setLocalUserAudioLevelListener(localUserAudioLevelListener);
				}
				if (streamAudioLevelListener != null) {
					deviceSession
	...
	protected void registerCustomCodecFormats(StreamRTPManager rtpManager) {
		...
		for (AudioFormat format : CUSTOM_CODEC_FORMATS) {
			...
			rtpManager.addFormat(
					format,
					MediaUtils.getRTPPayloadType(format.getEncoding(),
	...
	public void startSendingDTMF(DTMFTone tone, DTMFMethod dtmfMethod,
			int minimalToneDuration, int maximalToneDuration) {
		switch (dtmfMethod) {
		case INBAND_DTMF:
