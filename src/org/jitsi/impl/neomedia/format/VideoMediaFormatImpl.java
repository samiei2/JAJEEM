...
public class VideoMediaFormatImpl extends MediaFormatImpl<VideoFormat>
	 ...
	 * 
	 * @param encoding
	 *            the encoding of the new <tt>VideoMediaFormatImpl</tt> instance
	 */
	 ...
	 *            the encoding of the new <tt>VideoMediaFormatImpl</tt> instance
	 * @param clockRate
	 *            the clock rate of the new <tt>VideoMediaFormatImpl</tt>
	 *            instance
	 */
	 ...
	 * provide an implementation of <tt>VideoMediaFormat</tt> for a specific JMF
	 * <tt>VideoFormat</tt>.
	 * 
	 * @param format
	 *            the JMF <tt>VideoFormat</tt> the new instance is to wrap and
	 *            provide an implementation of <tt>VideoMediaFormat</tt> for
	 */
	 ...
	 * Initializes a new <tt>VideoMediaFormatImpl</tt> instance which is to
	 * provide an implementation of <tt>VideoMediaFormat</tt> for a specific JMF
	 * <tt>VideoFormat</tt> and to have a specific clock rate.
	 * 
	 * @param format
	 *            the JMF <tt>VideoFormat</tt> the new instance is to wrap and
	 *            provide an implementation of <tt>VideoMediaFormat</tt> for
	 ...
	 * Initializes a new <tt>VideoMediaFormatImpl</tt> instance which is to
	 * provide an implementation of <tt>VideoMediaFormat</tt> for a specific JMF
	 * <tt>VideoFormat</tt> and to have specific clock rate and set of
	 ...
	 *            instance
	 * @param frameRate
	 *            the frame rate of the new <tt>VideoMediaFormatImpl</tt>
	 *            instance
	...
	VideoMediaFormatImpl(VideoFormat format, double clockRate, float frameRate,
		...
		super(new ParameterizedVideoFormat(format.getEncoding(),
				format.getSize(), format.getMaxDataLength(),
				format.getDataType(), frameRate, formatParameters),
	 ...
	 * 
	 * @param mediaFormat
	 *            the object that we'd like to compare <tt>this</tt> one to
	...
	public boolean equals(Object mediaFormat) {
		...
		double videoMediaFormatImplClockRate = videoMediaFormatImpl
				.getClockRate();
	...
	public static boolean formatParametersAreEqual(String encoding,
		...
		return MediaFormatImpl.formatParametersAreEqual(encoding, fmtps1,
				fmtps2);
	}
	 ...
	 * 
	 * @param fmtps
	 *            the set of format parameters to match to the format parameters
	 *            of this <tt>MediaFormat</tt>
	 * @return <tt>true</tt> if this <tt>MediaFormat</tt> considers
	 *         <tt>fmtps</tt> matching its format parameters; otherwise,
	 *         <tt>false</tt>
	 */
	@Override
	public boolean formatParametersMatch(Map<String, String> fmtps) {
		return formatParametersMatch(getEncoding(), getFormatParameters(),
				fmtps) && super.formatParametersMatch(fmtps);
	}
	 ...
	 * @param fmtps1
	 *            the first set of format parameters which is to be matched
	 ...
	 *         context of the specified <tt>encoding</tt>; otherwise,
	 *         <tt>false</tt>
	 */
	public static boolean formatParametersMatch(String encoding,
		...
		if ("H264".equalsIgnoreCase(encoding)
				|| "h264/rtp".equalsIgnoreCase(encoding)) {
			String packetizationMode = "packetization-mode";
			String pm1 = (fmtps1 == null) ? null : fmtps1
					.get(packetizationMode);
			String pm2 = (fmtps2 == null) ? null : fmtps2
					.get(packetizationMode);
	 ...
	 *         <tt>MediaType.VIDEO</tt> for <tt>AudioMediaFormatImpl</tt>
	 *         instances
