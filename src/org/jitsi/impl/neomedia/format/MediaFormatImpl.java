...
public abstract class MediaFormatImpl<T extends Format> implements MediaFormat {
	 ...
	 */
	static final Map<String, String> EMPTY_FORMAT_PARAMETERS = Collections
			.emptyMap();
	 ...
	 *            implementation of <tt>MediaFormat</tt> for
	 * @param clockRate
	 *            the clock rate of the new instance
	 * @param formatParameters
	 *            the set of format-specific parameters of the new instance
	 * @param advancedAttributess
	...
	/**
	 * The advanced parameters of this instance which have been received via
	 ...
	 *            implementation of <tt>MediaFormat</tt> for
	 * @param formatParameters
	 *            any codec-specific parameters that have been received via
	 *            SIP/SDP or XMPP/Jingle
	 * @param advancedAttributes
	 *            any parameters that have been received via SIP/SDP or
	 *            XMPP/Jingle
	 */
	protected MediaFormatImpl(T format, Map<String, String> formatParameters,
		...
		this.format = format;
		this.formatParameters = ((formatParameters == null) || formatParameters
				.isEmpty()) ? EMPTY_FORMAT_PARAMETERS
				: new HashMap<String, String>(formatParameters);
		this.advancedAttributes = ((advancedAttributes == null) || advancedAttributes
				.isEmpty()) ? EMPTY_FORMAT_PARAMETERS
	 ...
	 *         Modifications to the returned Map do no affect the format
	 *         properties of this instance.
	 */
	@Override
	public Map<String, String> getAdvancedAttributes() {
		return (advancedAttributes == EMPTY_FORMAT_PARAMETERS) ? EMPTY_FORMAT_PARAMETERS
				: new HashMap<String, String>(advancedAttributes);
	}
	 ...
	 *         to the returned Map do no affect the format properties of this
	 *         instance.
	 */
	@Override
	public Map<String, String> getFormatParameters() {
		return (formatParameters == EMPTY_FORMAT_PARAMETERS) ? EMPTY_FORMAT_PARAMETERS
				: new HashMap<String, String>(formatParameters);
	}
	...
	public int hashCode() {
		 ...
		 * covering the two methods in question and causes problems, for
		 * example, with Map. While jmfEncoding is more generic than format, it
		 * still provides a relatively good distribution given that we do not
	...
	public boolean matches(MediaFormat format) {
		...
		int channels = MediaType.AUDIO.equals(mediaType) ? ((AudioMediaFormat) format)
				.getChannels() : MediaFormatFactory.CHANNELS_NOT_SPECIFIED;
		Map<String, String> formatParameters = format.getFormatParameters();

		return matches(mediaType, encoding, clockRate, channels,
				formatParameters);
	}
	 ...
	 */
	@Override
	public boolean matches(MediaType mediaType, String encoding,
			double clockRate, int channels, Map<String, String> formatParameters) {
		// mediaType
		// encoding
	...
	public void setAdditionalCodecSettings(Map<String, String> settings) {
		codecSettings = ((settings == null) || settings.isEmpty()) ? EMPTY_FORMAT_PARAMETERS
				: settings;
	}
	...
	public String toString() {
		...
		if (!formatParameters.isEmpty()) {
			...
			for (Map.Entry<String, String> formatParameter : formatParameters
					.entrySet()) {
				if (prependSeparator) {
					str.append(';');
				} else {
