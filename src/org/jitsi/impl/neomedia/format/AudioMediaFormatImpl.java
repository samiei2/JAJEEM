...
public class AudioMediaFormatImpl extends MediaFormatImpl<AudioFormat>
	 ...
	 * provide an implementation of <tt>AudioMediaFormat</tt> for a specific JMF
	 * <tt>AudioFormat</tt>.
	 * 
	 * @param format
	 *            the JMF <tt>AudioFormat</tt> the new instance is to wrap and
	 *            provide an implementation of <tt>AudioMediaFormat</tt> for
	 */
	 ...
	 * Initializes a new <tt>AudioMediaFormatImpl</tt> instance which is to
	 * provide an implementation of <tt>AudioMediaFormat</tt> for a specific JMF
	 * <tt>AudioFormat</tt> and to have a specific set of format-specific
	 * parameters.
	 * 
	 * @param format
	 *            the JMF <tt>AudioFormat</tt> the new instance is to wrap and
	 ...
	 * 
	 * @param encoding
	 *            the encoding of the new <tt>AudioMediaFormatImpl</tt> instance
	 */
	 ...
	 *            <tt>AudioMediaFormatImpl</tt> instance
	 * @param channels
	 *            the number of available channels (1 for mono, 2 for stereo)
	 */
	 ...
	 * @param advancedParameters
	 *            set of advanced parameters that have been received by SIP/SDP
	 *            or XMPP/Jingle
	 */
	AudioMediaFormatImpl(String encoding, double clockRate,
			Map<String, String> formatParameters,
	...
	AudioMediaFormatImpl(String encoding, double clockRate, int channels,
			Map<String, String> formatParameters,
			Map<String, String> advancedParameters) {
		this(new AudioFormat(encoding, clockRate, Format.NOT_SPECIFIED,
				channels), formatParameters, advancedParameters);
	}
	 ...
	 * 
	 * @param format
	 *            the <tt>AudioFormat</tt> to get a match of
	 * @return if the specified <tt>format</tt> has a specific number of
	 *         channels, <tt>format</tt>; otherwise, a new <tt>AudioFormat</tt>
	 *         instance which matches <tt>format</tt> and has 1 channel
	 */
	private static AudioFormat fixChannels(AudioFormat format) {
		if (Format.NOT_SPECIFIED == format.getChannels()) {
			format = (AudioFormat) format.intersects(new AudioFormat(format
					.getEncoding(), format.getSampleRate(), format
	 ...
	 *         <tt>MediaType.AUDIO</tt> for <tt>AudioMediaFormatImpl</tt>
	 *         instances
