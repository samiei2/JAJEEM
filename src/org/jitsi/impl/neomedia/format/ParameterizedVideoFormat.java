...
public class ParameterizedVideoFormat extends VideoFormat {
	 ...
	 *            format parameters
	 */
	public ParameterizedVideoFormat(String encoding, Dimension size,
			int maxDataLength, Class<?> dataType, float frameRate,
			Map<String, String> fmtps) {
		super(encoding, size, maxDataLength, dataType, frameRate);

		this.fmtps = ((fmtps == null) || fmtps.isEmpty()) ? MediaFormatImpl.EMPTY_FORMAT_PARAMETERS
	...
	public Object clone() {
		ParameterizedVideoFormat f = new ParameterizedVideoFormat(
				getEncoding(), getSize(), getMaxDataLength(), getDataType(),
				getFrameRate(),
				/*
	...
	protected void copy(Format f) {
		...
		if (f instanceof ParameterizedVideoFormat) {
			...
			fmtps = ((pvfFmtps == null) || pvfFmtps.isEmpty()) ? MediaFormatImpl.EMPTY_FORMAT_PARAMETERS
	...
	public boolean equals(Object obj) {
		...
		return VideoMediaFormatImpl.formatParametersAreEqual(getEncoding(),
	...
	public boolean formatParametersMatch(Format format) {
		...
		if (format instanceof ParameterizedVideoFormat) {
			formatFmtps = ((ParameterizedVideoFormat) format)
					.getFormatParameters();
		}
		return VideoMediaFormatImpl.formatParametersMatch(getEncoding(),
	...
	public Format intersects(Format format) {
		...
		((ParameterizedVideoFormat) intersection).fmtps = fmtps.isEmpty() ? MediaFormatImpl.EMPTY_FORMAT_PARAMETERS
				: getFormatParameters();
		return intersection;
	}
	 ...
	 *            the new <tt>Map</tt>
	 * @param entries
	 *            the associations to be created in the new <tt>Map</tt> where
	 *            the key and value of an association are expressed as
