...
public class MediaFormatFactoryImpl implements MediaFormatFactory {
	...
	private static final Logger logger = Logger
			.getLogger(MediaFormatFactoryImpl.class);
	 ...
	 *         is either an <tt>AudioMediaFormat</tt> or a
	 ...
	 *            the RTP payload type of the <tt>MediaFormat</tt> to create
	 * @return a <tt>MediaFormat</tt> with the specified <tt>rtpPayloadType</tt>
	 *         which is either an <tt>AudioMediaFormat</tt> or a
	 *         <tt>VideoMediaFormat</tt> instance if <tt>rtpPayloadType</tt> is
	...
	public MediaFormat createMediaFormat(byte rtpPayloadType) {
		 ...
		 * disabled.
		 */
		for (MediaFormat rtpPayloadTypeMediaFormat : MediaUtils
				.getMediaFormats(rtpPayloadType)) {
	...
	private MediaFormat createMediaFormat(String encoding, double clockRate,
			int channels, Map<String, String> fmtps) {
		for (MediaFormat format : getSupportedMediaFormats(encoding, clockRate)) {
			 ...
			 */
			if (format.matches(format.getMediaType(), format.getEncoding(),
					format.getClockRate(), channels, fmtps)) {
				return format;
			}
		}
		return null;
	...
	public MediaFormat createMediaFormat(String encoding, double clockRate,
		...
		return createMediaFormat(encoding, clockRate, 1, -1, formatParams,
				advancedParams);
	}
	...
	public MediaFormat createMediaFormat(String encoding, double clockRate,
		...
		if ((formatParameters != null) || (advancedParameters != null)) {
			...
			case VIDEO:
				 ...
				 */
				mediaFormat = new VideoMediaFormatImpl(
						videoMediaFormatImpl.getFormat(),
						videoMediaFormatImpl.getClockRate(), frameRate,
						formatParameters, advancedParameters);
				break;
			default:
				mediaFormat = null;
	 ...
	 * 
	 * @param rtpPayloadType
	 *            the RTP payload type to create a <tt>MediaFormat</tt> for;
	 *            {@link MediaFormat#RTP_PAYLOAD_TYPE_UNKNOWN} if
	 *            <tt>encoding</tt> is not <tt>null</tt>. If
	 *            <tt>rtpPayloadType</tt> is not
	 *            <tt>MediaFormat#RTP_PAYLOAD_TYPE_UNKNOWN</tt> and
	 *            <tt>encoding</tt> is not <tt>null</tt>,
	 *            <tt>rtpPayloadType</tt> is ignored
	 * @param encoding
	 *            the well-known encoding (name) to create a
	...
	public MediaFormat createMediaFormat(byte rtpPayloadType, String encoding,
			double clockRate, int channels, float frameRate,
			Map<String, String> formatParams, Map<String, String> advancedParams) {
		...
		if ((MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN != rtpPayloadType)
			...
			MediaFormat[] rtpPayloadTypeMediaFormats = MediaUtils
					.getMediaFormats(rtpPayloadType);
			...
			if (rtpPayloadTypeMediaFormats.length > 0) {
				...
				if (CLOCK_RATE_NOT_SPECIFIED == clockRate) {
					...
					for (MediaFormat rtpPayloadTypeMediaFormat : rtpPayloadTypeMediaFormats) {
						if (rtpPayloadTypeMediaFormat.getEncoding().equals(
								encoding)
		...
		return createMediaFormat(encoding, clockRate, channels, frameRate,
				formatParams, advancedParams);
	}
	...
	private List<MediaFormat> getMatchingMediaFormats(
		...
		for (MediaFormat mediaFormat : mediaFormats) {
			if (mediaFormat.getEncoding().equalsIgnoreCase(encoding)
					&& ((CLOCK_RATE_NOT_SPECIFIED == clockRate) || (mediaFormat
							.getClockRate() == clockRate))) {
	 ...
	 */
	private List<MediaFormat> getSupportedMediaFormats(String encoding,
			double clockRate) {
		EncodingConfiguration encodingConfiguration = NeomediaServiceUtils
				.getMediaServiceImpl().getCurrentEncodingConfiguration();
		List<MediaFormat> supportedMediaFormats = getMatchingMediaFormats(
				encodingConfiguration.getAllEncodings(MediaType.AUDIO),
				encoding, clockRate);

		if (supportedMediaFormats.isEmpty()) {
			supportedMediaFormats = getMatchingMediaFormats(
					encodingConfiguration.getAllEncodings(MediaType.VIDEO),
					encoding, clockRate);
		}
		return supportedMediaFormats;
	}
