...
public class FMJPlugInConfiguration {
	...
	private static final String[] CUSTOM_CODECS = {
			...
			EncodingConfigurationImpl.G729 ? "org.jitsi.impl.neomedia.codec.audio.g729.JavaDecoder"
					: null,
			EncodingConfigurationImpl.G729 ? "org.jitsi.impl.neomedia.codec.audio.g729.JavaEncoder"
					: null,
	...
	private static final Logger logger = Logger
			.getLogger(FMJPlugInConfiguration.class);
	...
	public static void registerCustomCodecs() {
		...
		String[] gsmCodecClasses = new String[] { "JavaDecoder",
				"JavaDecoder_ms", "JavaEncoder", "JavaEncoder_ms",
				"NativeDecoder", "NativeDecoder_ms", "NativeEncoder",
				"NativeEncoder_ms", "Packetizer" };
		for (String gsmCodecClass : gsmCodecClasses) {
			PlugInManager.removePlugIn(gsmCodecPackage + gsmCodecClass,
					PlugInManager.CODEC);
		}
		 ...
		 */
		PlugInManager.removePlugIn("net.sf.fmj.media.codec.JavaSoundCodec",
				PlugInManager.CODEC);
		...
		for (String className : CUSTOM_CODECS) {
			...
			if (registeredPlugins.contains(className)) {
				...
				try {
					Codec codec = (Codec) Class.forName(className)
							.newInstance();

					registered = PlugInManager.addPlugIn(className,
				...
				if (registered) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Codec " + className
								+ " is NOT successfully registered", exception);
					}
				}
			}
		}
