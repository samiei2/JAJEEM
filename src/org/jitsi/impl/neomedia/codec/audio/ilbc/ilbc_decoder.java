...
class ilbc_decoder {
	...
	void SimplelsfDEQ(float lsfdeq[], /* (o) dequantized lsf coefficients */
		...
		for (i = 0; i < ilbc_constants.LSF_NSPLIT; i++) {
			...
			cb_pos += ilbc_constants.size_lsfCbTbl[i]
					* ilbc_constants.dim_lsfCbTbl[i];
		}
			...
			for (i = 0; i < ilbc_constants.LSF_NSPLIT; i++) {
				for (j = 0; j < ilbc_constants.dim_lsfCbTbl[i]; j++) {
					lsfdeq[ilbc_constants.LPC_FILTERORDER + pos + j] = ilbc_constants.lsfCbTbl[cb_pos
							+ (int) ((long) (index[ilbc_constants.LSF_NSPLIT
									+ i]) * ilbc_constants.dim_lsfCbTbl[i]) + j];
				}
				pos += ilbc_constants.dim_lsfCbTbl[i];
				cb_pos += ilbc_constants.size_lsfCbTbl[i]
						* ilbc_constants.dim_lsfCbTbl[i];
			...
			int seq2_idx, int dim2) /* (i) dimension seq2 */
	{
		...
		for (i = 0; i <= dim1 - dim2; i++) {
			...
			for (j = 0; j < dim2; j++) {
				corr[corr_idx + i] += seq1[seq1_idx + i + j]
						* seq2[seq2_idx + j];
			}
		}
	}
			...
			float period) /* (i) estimated pitch period */
	{
		...
		float maxv;
		float[] corrVecUps = new float[ilbc_constants.ENH_CORRDIM
				* ilbc_constants.ENH_UPS0];
		...
		mycorr1(corrVec, 0, idata, searchSegStartPos, corrdim
				+ ilbc_constants.ENH_BLOCKL - 1, idata, centerStartPos,
		...
		updStartPos = searchSegStartPos + (float) tloc
				/ (float) ilbc_constants.ENH_UPS0 + 1.0f;
		...
		if (st < 0) {
			...
			System.arraycopy(idata, 0, vect, -st,
					(ilbc_constants.ENH_VECTL + st));
			// memcpy(&vect[-st],idata,
			// (ilbc_constants.ENH_VECTL+st)*sizeof(float));
		} else {
			en = st + ilbc_constants.ENH_VECTL;

			if (en > idatal) {
				System.arraycopy(idata, st, vect, 0,
						(ilbc_constants.ENH_VECTL - (en - idatal)));
				...
				// memcpy(vect, &idata[st],
				// ilbc_constants.ENH_VECTL*sizeof(float));
			}
		}
		...
		// System.out.println("dimension 2 : " + (2 *
		// ilbc_constants.ENH_FL0+1));
		// System.out.println("correlations de dimension " + seg.length);
		mycorr1(seg, seg_idx, vect, 0, ilbc_constants.ENH_VECTL,
				ilbc_constants.polyphaserTbl, (2 * ilbc_constants.ENH_FL0 + 1)
						* fraction, 2 * ilbc_constants.ENH_FL0 + 1);
			...
			float alpha0)/* (i) max smoothing energy fraction */
	{
		...
		for (i = 1; i <= 2 * hl + 1; i++) {
			wt[i - 1] = (float) 0.5
					* (1 - (float) Math.cos(2 * ilbc_constants.PI * i
							/ (2 * hl + 2)));
		}
		...
		if (errs > alpha0 * w00) {
			...
			if (denom > 0.0001f) { /*
									 * eliminates numerical problems for if
									 * smooth
									 */
				...
				B = B + 1;
					 ...
					 * essentially no difference between cycles; smoothing not
					 * needed
					 */
				A = 0.0f;
				B = 1.0f;
			...
			int hl) /* (i) 2*hl+1 is the number of sequences */
	{
		...
		lagBlock[hl] = NearestNeighbor(plocs, (float) 0.5
				* (centerStartPos + centerEndPos), periodl);

		blockStartPos[hl] = centerStartPos;

		psseq = ilbc_constants.ENH_BLOCKL * hl;
		// psseq=sseq+ENH_BLOCKL*hl;
		System.arraycopy(idata, centerStartPos, sseq, psseq,
				ilbc_constants.ENH_BLOCKL);
		...
		for (q = hl - 1; q >= 0; q--) {
			...
			lagBlock[q] = NearestNeighbor(plocs, blockStartPos[q]
					+ ilbc_constants.ENH_BLOCKL_HALF - period[lagBlock[q + 1]],
					periodl);

			if (blockStartPos[q] - ilbc_constants.ENH_OVERHANG >= 0) {
				blockStartPos[q] = refiner(sseq, q * ilbc_constants.ENH_BLOCKL,
						idata, idatal, centerStartPos, blockStartPos[q],
						period[lagBlock[q + 1]]);
			} else {
		...
		for (q = hl + 1; q <= 2 * hl; q++) {
			...
			if (blockStartPos[q] + ilbc_constants.ENH_BLOCKL
					+ ilbc_constants.ENH_OVERHANG < idatal) {
				blockStartPos[q] = refiner(sseq, q * ilbc_constants.ENH_BLOCKL,
						idata, idatal, centerStartPos, blockStartPos[q],
						period[lagBlock[q]]);
			} else {
	...
	public void enhancer(float odata[], /* (o) smoothed block, dimension blockl */
		...
		float[] sseq = new float[(2 * ilbc_constants.ENH_HL + 1)
				* ilbc_constants.ENH_BLOCKL];

		/* get said second sequence of segments */

		getsseq(sseq, idata, idatal, centerStartPos, period, plocs, periodl,
				ilbc_constants.ENH_HL);
			...
			float in[]) /* (i) unenhanced signal */
	{
		...
		float[] downsampled = new float[(ilbc_constants.ENH_NBLOCKS
				* ilbc_constants.ENH_BLOCKL + 120) / 2];
		int inLen = ilbc_constants.ENH_NBLOCKS * ilbc_constants.ENH_BLOCKL
				+ 120;
		...
		System.arraycopy(enh_buf, this.ULP_inst.blockl, enh_buf, 0,
				ilbc_constants.ENH_BUFL - this.ULP_inst.blockl);
		// memmove(enh_buf, &enh_buf[iLBCdec_inst->blockl],
		// (ENH_BUFL-iLBCdec_inst->blockl)*sizeof(float));

		System.arraycopy(in, 0, enh_buf, ilbc_constants.ENH_BUFL
				- this.ULP_inst.blockl, this.ULP_inst.blockl);
		...
		i = 3 - ioffset;
		System.arraycopy(enh_period, i, enh_period, 0,
				ilbc_constants.ENH_NBLOCKS_TOT - i);
		...
		DownSample(enh_buf, (ilbc_constants.ENH_NBLOCKS_EXTRA + ioffset)
				* ilbc_constants.ENH_BLOCKL - 120,
				ilbc_constants.lpFilt_coefsTbl, inLen - ioffset
		...
		for (iblock = 0; iblock < ilbc_constants.ENH_NBLOCKS - ioffset; iblock++) {
			...
			maxcc = xCorrCoef(downsampled, 60 + iblock
					* ilbc_constants.ENH_BLOCKL_HALF, downsampled, 60 + iblock
					* ilbc_constants.ENH_BLOCKL_HALF - lag,
					ilbc_constants.ENH_BLOCKL_HALF);
			for (ilag = 11; ilag < 60; ilag++) {
				cc = xCorrCoef(downsampled, 60 + iblock
						* ilbc_constants.ENH_BLOCKL_HALF, downsampled, 60
						+ iblock * ilbc_constants.ENH_BLOCKL_HALF - ilag,
						ilbc_constants.ENH_BLOCKL_HALF);
		...
		if (this.prev_enh_pl == 1) {
			...
			for (i = 0; i < plc_blockl; i++) {
				ftmp2 += enh_buf[ilbc_constants.ENH_BUFL - 1
						- this.ULP_inst.blockl - i]
						* enh_buf[ilbc_constants.ENH_BUFL - 1
								- this.ULP_inst.blockl - i];
		...
		if (this.ULP_inst.mode == 20) {
			...
			for (iblock = 0; iblock < 2; iblock++) {
				enhancer(out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf,
						ilbc_constants.ENH_BUFL, (5 + iblock)
								* ilbc_constants.ENH_BLOCKL + 40,
						ilbc_constants.ENH_ALPHA0, enh_period,
						ilbc_constants.enh_plocsTbl,
			...
			for (iblock = 0; iblock < 3; iblock++) {
				enhancer(out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf,
						ilbc_constants.ENH_BUFL, (4 + iblock)
								* ilbc_constants.ENH_BLOCKL,
						ilbc_constants.ENH_ALPHA0, enh_period,
						ilbc_constants.enh_plocsTbl,
						ilbc_constants.ENH_NBLOCKS_TOT);
			...
			int lpc_idx, int inlag) /* (i) pitch lag */
	{
		...
		if (PLI == 1) {
			...
			if (this.prevPLI != 1) {
				...
				a_gain[0] = gain;
				a_per[0] = max_per;
				compCorr(a_comp, a_gain, a_per, this.prevResidual, lag,
						this.ULP_inst.blockl, 60);
				maxcc = a_comp[0];
				gain = a_gain[0];
				max_per = a_per[0];
				...
				for (i = inlag - 2; i <= inlag + 3; i++) {
					...
					a_gain[0] = gain_comp;
					a_per[0] = per;
					compCorr(a_comp, a_gain, a_per, this.prevResidual, i,
							this.ULP_inst.blockl, 60);
					maxcc_comp = a_comp[0];
					gain_comp = a_gain[0];
					per = a_per[0];
			...
			for (i = 0; i < this.ULP_inst.blockl; i++) {
				...
				if (pick < 0) {
					PLCresidual[i] = this.prevResidual[this.ULP_inst.blockl
							+ pick];
				} else {
				...
				if (i < 80) {
					PLCresidual[i] = use_gain
							* (pitchfact * PLCresidual[i] + (1.0f - pitchfact)
									* randvec[i]);
				} else if (i < 160) {
					PLCresidual[i] = (float) 0.95
							* use_gain
							* (pitchfact * PLCresidual[i] + (1.0f - pitchfact)
									* randvec[i]);
				} else {
					PLCresidual[i] = (float) 0.9
							* use_gain
							* (pitchfact * PLCresidual[i] + (1.0f - pitchfact)
									* randvec[i]);
				}
			...
			System.arraycopy(this.prevLpc, 0, PLClpc, 0,
		...
		/* no packet loss, copy input */
			...
			// memcpy(PLCresidual,
			// decresidual,this.ULP_inst.blockl*sizeof(float));
			System.arraycopy(decresidual, 0, PLCresidual, 0,
					this.ULP_inst.blockl);
			// memcpy(PLClpc, lpc, (LPC_FILTERORDER+1)*sizeof(float));
			System.arraycopy(lpc, lpc_idx, PLClpc, 0,
					ilbc_constants.LPC_FILTERORDER + 1);
			this.consPLICount = 0;
		}
		...
		System.arraycopy(PLClpc, 0, this.prevLpc, 0,
				ilbc_constants.LPC_FILTERORDER + 1);
		// memcpy(this.prevResidual, PLCresidual,
		// this.ULP_inst.blockl*sizeof(float));
		System.arraycopy(PLCresidual, 0, this.prevResidual, 0,
				this.ULP_inst.blockl);
	}
			...
			short mode) /* (i) 0=PL, 1=Normal */
	{
		...
		bitstream en_data = new bitstream(encoded, encodedOffset,
				this.ULP_inst.no_of_bytes);
	...
	public void Decode(float decresidual[], /* (o) decoded residual frame */
			...
			int idxForMax, /*
							 ...
							 */
			int idxVec[], /*
						 * (i) codebook indexes for the samples in the start
						 * state
						 */
			float syntdenum[], /*
			...
			int extra_gain_index[], /*
									 * (i) the indexes for the corresponding
									 * gains
									 */
			int state_first) /*
							 * (i) 1 if non adaptive part of start state comes
							 ...
							 */
	{
		...
		if (state_first != 0) { /* put adaptive part in the end */
			...
			for (k = 0; k < diff; k++) {
				reverseDecresidual[k] = decresidual[(start + 1)
						* ilbc_constants.SUBL - 1
					 ...
					 */
	{
		...
		// int check;
		int[] gain_index = new int[ilbc_constants.NASUB_MAX
				* ilbc_constants.CB_NSTAGES];
		...
		if (mode > 0) { /* the data are good */
			...
			for (k = 0; k < ilbc_constants.LSF_NSPLIT
					* ilbc_constants.LPC_N_MAX; k++) {
				lsf_i[k] = 0;
			}
			start = 0;
			state_first = 0;
			...
			for (ulp = 0; ulp < 3; ulp++) {
				...
				for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
					lastpart = bytes
							.unpack(this.ULP_inst.extra_cb_index[k][ulp]);
					// unpack( &pbytes, &lastpart,
					// this.ULP_inst.extra_cb_index[k][ulp],
					// &pos);
					extra_cb_index[k] = bytes.packcombine(extra_cb_index[k],
							lastpart, this.ULP_inst.extra_cb_index[k][ulp]);
				...
				for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
					lastpart = bytes
							.unpack(this.ULP_inst.extra_cb_gain[k][ulp]);
					// unpack( &pbytes, &lastpart,
					// this.ULP_inst.extra_cb_gain[k][ulp],
					// &pos);
					extra_gain_index[k] = bytes.packcombine(
							extra_gain_index[k], lastpart,
							this.ULP_inst.extra_cb_gain[k][ulp]);
				...
				for (i = 0; i < this.ULP_inst.nasub; i++) {
					for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
						lastpart = bytes
								.unpack(this.ULP_inst.cb_index[i][k][ulp]);
						// unpack( &pbytes, &lastpart,
						// this.ULP_inst.cb_index[i][k][ulp],
						// &pos);
						cb_index[i * ilbc_constants.CB_NSTAGES + k] = bytes
								.packcombine(cb_index[i
				...
				for (i = 0; i < this.ULP_inst.nasub; i++) {
					for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
						lastpart = bytes
								.unpack(this.ULP_inst.cb_gain[i][k][ulp]);
						gain_index[i * ilbc_constants.CB_NSTAGES + k] = bytes
								.packcombine(gain_index[i
										* ilbc_constants.CB_NSTAGES + k],
										lastpart,
			...
			if (mode == 1) { /*
				...
				ilbc_common.LSF_check(lsfdeq, ilbc_constants.LPC_FILTERORDER,
						this.ULP_inst.lpc_n);
				// check=ilbc_common.LSF_check(lsfdeq,
				...
				// System.out.println("syntdenum[" + li + "] = " +
				// syntdenum[li]);
				...
				doThePLC(PLCresidual, PLClpc, 0, decresidual, syntdenum,
						(ilbc_constants.LPC_FILTERORDER + 1)
								* (this.ULP_inst.nsub - 1), last_lag);

				System.arraycopy(PLCresidual, 0, decresidual, 0,
						this.ULP_inst.blockl);
				// for (int li = 0; li < decresidual.length; li++)
				// System.out.println("decresidual[" + li + "] = " +
				// decresidual[li]);
		...
		if (mode == 0) {
			...
			for (i = 0; i < this.ULP_inst.nsub; i++) {
				System.arraycopy(PLClpc, 0, syntdenum, (i * order_plus_one),
						order_plus_one);
		...
		if (this.use_enhancer == 1) {
			...
			if (this.ULP_inst.mode == 20) {
				...
				syntFilter(data, i * ilbc_constants.SUBL, this.old_syntdenum,
						(i + this.ULP_inst.nsub - 1)
								* (ilbc_constants.LPC_FILTERORDER + 1),
						ilbc_constants.SUBL, this.syntMem);
				// System.out.println("runs 2");
				for (i = 1; i < this.ULP_inst.nsub; i++) {
					// System.out.println("pass " + i);
					syntFilter(data, i * ilbc_constants.SUBL, syntdenum,
							(i - 1) * (ilbc_constants.LPC_FILTERORDER + 1),
							ilbc_constants.SUBL, this.syntMem);
				...
				for (i = 0; i < 2; i++) {
					syntFilter(data, i * ilbc_constants.SUBL,
							this.old_syntdenum, (i + this.ULP_inst.nsub - 2)
									* (ilbc_constants.LPC_FILTERORDER + 1),
							ilbc_constants.SUBL, this.syntMem);
				}
				for (i = 2; i < this.ULP_inst.nsub; i++) {
					...
					syntFilter(data, i * ilbc_constants.SUBL, syntdenum,
							(i - 2) * (ilbc_constants.LPC_FILTERORDER + 1),
							ilbc_constants.SUBL, this.syntMem);
				}
			}
			...
			for (ilag = 21; ilag < 120; ilag++) {
				cc = xCorrCoef(decresidual, ilbc_constants.BLOCKL_MAX
						- ilbc_constants.ENH_BLOCKL, decresidual,
						ilbc_constants.BLOCKL_MAX - ilbc_constants.ENH_BLOCKL
								- ilag, ilbc_constants.ENH_BLOCKL);
	...
	public ilbc_decoder(int init_mode, int init_enhancer) {
		...
		old_syntdenum = new float[(ilbc_constants.LPC_FILTERORDER + 1)
				* ilbc_constants.NSUB_MAX];
		hpomem = new float[4];
