...
public class JavaDecoder extends AbstractCodec2 {
	 ...
	 */
	public JavaDecoder() {
		super("iLBC Decoder", AudioFormat.class,
				new Format[] { new AudioFormat(AudioFormat.LINEAR) });

		inputFormats = new Format[] { new AudioFormat(Constants.ILBC_RTP, 8000,
				16, 1, Format.NOT_SPECIFIED /* endian */, Format.NOT_SPECIFIED /* signed */) };

		addControl(new com.sun.media.controls.SilenceSuppressionAdapter(this,
				false, false));
	}
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		dec.decode(output, outputOffset, input, inputBuffer.getOffset()
				+ offsetToAdd, (short) 1);

		updateOutput(outputBuffer, getOutputFormat(), outputLength,
				outputOffset);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR,
	 ...
	 * 
	 * @param inputLength
	 *            the number of bytes of input to be processed by {@link #dec}
	 */
