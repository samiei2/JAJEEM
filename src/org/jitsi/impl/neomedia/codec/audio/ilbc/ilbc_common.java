...
class ilbc_common {
	...
	public static void StateConstructW(int idxForMax, /*
		...
		for (k = 0; k < len; k++) {
			...
			/* maxVal = 1/scal */
			tmpbuf[tmp + k] = maxVal
					* ilbc_constants.state_sq3Tbl[idxVec[tmpi]];
		...
		ilbc_common.ZeroPoleFilter(tmpbuf, tmp, numerator, syntDenum,
				...
				foutbuf, fout);
		for (k = 0; k < len; k++) {
			out[out_idx + k] = foutbuf[fout + len - 1 - k]
	...
	public static void AllPoleFilter(float InOut[], /*
													 ...
													 * on en exit
													 * InOut[-orderCoef] to
													 * InOut[-1] is unchanged
													 * and InOut[0] to
													 * InOut[lengthInOut-1]
													 * contain filtered samples
													 */
			int InOut_idx, float Coef[],/*
										 * (i) filter coefficients, Coef[0] is
										 * assumed to be 1.0f
										 */
	...
	public static void AllZeroFilter(float In[], /*
			...
			int orderCoef, /* (i) number of filter coefficients */
			float Out[], /*
						 * (i/o) on entrance Out[-orderCoef] to Out[-1] contain
						 * the filter state, on exit Out[0] to
	...
	public static void ZeroPoleFilter(float In[], /*
												 ...
												 * In[lengthInOut-1] contain
												 * filter input samples
												 * In[-orderCoef] to In[-1]
												 * contain state of all-zero
												 * section
												 */
			int In_idx, float ZeroCoef[],/*
										 * (i) filter coefficients for all-zero
										 * section (ZeroCoef[0] is assumed to be
										 * 1.0f)
										 */
			float PoleCoef[],/*
			...
			int PoleCoef_idx, int lengthInOut,/*
											 * (i) number of input/output
											 * samples
											 */
			int orderCoef, /* (i) number of filter coefficients */
			float Out[], /*
						 * (i/o) on entrance Out[-orderCoef] to Out[-1] contain
						 * state of all-pole section. On exit Out[0] to
						 * Out[lengthInOut-1] contain filtered samples
						 */
			int Out_idx) {
	...
	public static void lsf2a(float a_coef[], float freq[]) {
		...
		if ((freq[0] <= 0.0f)
				|| (freq[ilbc_constants.LPC_FILTERORDER - 1] >= 0.5)) {
			...
			int cbveclen) /* (i) Codebook vector length */
	{
		...
		if (index < lMem - cbveclen + 1) {
			...
			System.arraycopy(mem, mem_idx + lMem - k + ihigh, cbvec, ihigh,
					(cbveclen - ihigh));
		...
		/* Higher codebook section based on filtering */
			...
			if (index - base_size < lMem - cbveclen + 1) {
				float[] tempbuff2 = new float[ilbc_constants.CB_MEML
						+ ilbc_constants.CB_FILTERLEN + 1];
				// float *pos;
				// float *pp, *pp1;
				int pos, pp, pp1;
				...
				System.arraycopy(mem, mem_idx, tempbuff2,
						ilbc_constants.CB_HALFFILTERLEN, lMem);
			...
			/* interpolated vectors */
				...
				alfa = 0.0f;
				for (j = ilow; j < ihigh; j++) {
					cbvec[j] = (1.0f - alfa) * tmpbuf[lMem - k / 2 + j] + alfa
							* tmpbuf[lMem - k + j];
					alfa += alfa1;
				}
				...
				System.arraycopy(tmpbuf, lMem - k + ihigh, cbvec, ihigh,
						cbveclen - ihigh);
	...
	public static void iCBConstruct(float decvector[], /* (o) Decoded vector */
		...
		if (nStages > 1) {
			for (k = 1; k < nStages; k++) {
				getCBvec(cbvec, mem, mem_idx, index[index_idx + k], lMem,
						veclen);
				for (j = 0; j < veclen; j++) {
