...
/**
 * Functions coder_ld8k and init_coder_ld8k Coder constant parameters (defined
 * in "ld8k.h")
 * 
 * <pre>
...
class CodLd8k extends Ld8k {
	...
	/*
	 * File : COD_LD8K.C Used for the floating point version of G.729 main body
	 * (not for G.729A)
	 */
	...
	private final float[/* M */] lsp_old = { 0.9595f, 0.8413f, 0.6549f,
			0.4154f, 0.1423f, -0.1423f, -0.4154f, -0.6549f, -0.8413f, -0.9595f };
	...
	/**
	 * Initialization of variables for the encoder. Initialize pointers to
	 * speech vector.
	 * 
	 * <pre>
	 *   |--------------------|-------------|-------------|------------|
	...
	void coder_ld8k(int ana[]) {
		...
		float[] A_t = new float[(MP1) * 2]; /*
											 * A(z) unquantized for the 2
											 * subframes
											 */
		float[] Aq_t = new float[(MP1) * 2]; /*
											 * A(z) quantized for the 2
											 * subframes
											 */
		...
		Filter.syn_filt(Ap2, 0, wsp, wsp_offset, wsp, wsp_offset, L_SUBFR,
				mem_w, 0, 1);

		Lpcfunc.weight_az(A_t, MP1, gamma1[1], M, Ap1);
		Lpcfunc.weight_az(A_t, MP1, gamma2[1], M, Ap2);
		Filter.residu(Ap1, 0, speech, speech_offset + L_SUBFR, wsp, wsp_offset
				+ L_SUBFR, L_SUBFR);
		Filter.syn_filt(Ap2, 0, wsp, wsp_offset + L_SUBFR, wsp, wsp_offset
				+ L_SUBFR, L_SUBFR, mem_w, 0, 1);
		...
		for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
			...
			Filter.syn_filt(Aq, Aq_offset, ai_zero, 0, h1, 0, L_SUBFR, zero,
					zero_offset, 0);
			...
			Filter.residu(Aq, Aq_offset, speech, speech_offset + i_subfr, exc,
					exc_offset + i_subfr, L_SUBFR); /* LPC residual */

			Filter.syn_filt(Aq, Aq_offset, exc, exc_offset + i_subfr, error,
					error_offset, L_SUBFR, mem_err, 0, 0);
			...
			t0 = Pitch.pitch_fr3(exc, exc_offset + i_subfr, xn, h1, L_SUBFR,
					t0_min.value, t0_max.value, i_subfr, t0_frac);

			index = Pitch.enc_lag3(t0, t0_frac.value, t0_min, t0_max, PIT_MIN,
					PIT_MAX, i_subfr);
			...
			PredLt3.pred_lt_3(exc, exc_offset + i_subfr, t0, t0_frac.value,
					L_SUBFR);
			...
			iRef.value = i;
			index = acelpCo.ACELP_codebook(xn2, h1, t0, sharp, i_subfr, code,
					y2, iRef);
			i = iRef.value;
			...
			ana[ana_offset] = quaGain.qua_gain(code, g_coeff, L_SUBFR,
					_gain_pit, _gain_code, taming);
			...
			for (i = 0; i < L_SUBFR; i++) {
				exc[exc_offset + i + i_subfr] = gain_pit
						* exc[exc_offset + i + i_subfr] + gain_code * code[i];
			}

			this.taming.update_exc_err(gain_pit, t0);

			Filter.syn_filt(Aq, Aq_offset, exc, exc_offset + i_subfr, synth,
					i_subfr, L_SUBFR, mem_syn, 0, 1);

			for (i = L_SUBFR - M, j = 0; i < L_SUBFR; i++, j++) {
				mem_err[j] = speech[speech_offset + i_subfr + i]
						- synth[i_subfr + i];
