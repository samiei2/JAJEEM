...
class Pwf {
	...
	/*
	 * File : PWF.C Used for the floating point version of G.729 main body (not
	 * for G.729A)
	 */
	 ...
	 *            output: gamma2 value
	 * @param lsfint
	 *            input : Interpolated lsf vector : 1st subframe
	 * @param lsfnew
	 *            input : lsf vector : 2nd subframe
	 * @param r_c
	 *            input : Reflection coefficients
	 */
	void perc_var(float[] gamma1, float[] gamma2, float[] lsfint,
			float[] lsfnew, float[] r_c) {
		...
		for (i = 0; i < 2; i++) {
			lar_new[lar_new_offset + i] = (float) Math.log10((1.0f + r_c[i])
					/ (1.0f - r_c[i]));
		}
