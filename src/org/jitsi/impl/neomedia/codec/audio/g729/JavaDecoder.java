...
public class JavaDecoder extends AbstractCodec2 {
	...
	public JavaDecoder() {
		super("G.729 Decoder", AudioFormat.class,
				new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR, 8000,
						16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED) });

		inputFormats = new AudioFormat[] { new AudioFormat(
				AudioFormat.G729_RTP, 8000, Format.NOT_SPECIFIED, 1) };
	}

	private void depacketize(byte[] inputFrame, int inputFrameOffset,
			short[] serial) {
		serial[0] = SYNC_WORD;
		serial[1] = SIZE_WORD;
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		byte[] output = validateByteArraySize(outputBuffer,
				outputBuffer.getOffset() + OUTPUT_FRAME_SIZE_IN_BYTES, true);
	...
	private static void writeShorts(short[] input, byte[] output,
			int outputOffset) {
