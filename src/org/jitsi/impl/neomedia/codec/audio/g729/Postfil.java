...
class Postfil extends Ld8k {
	...
	/*
	 * ITU-T G.729 Annex C - Reference C code for floating point implementation
	 * of G.729 Version 1.01 of 15.September.98
	 */
	 ...
	 * COPYRIGHT NOTICE
	 * ----------------------------------------------------------------------
	 * ITU-T G.729 Annex C ANSI C source code Copyright (C) 1998, AT&T, France
	 * Telecom, NTT, University of Sherbrooke. All rights reserved.
	 * 
	 * ----------------------------------------------------------------------
	 */

	/*
	 * File : POSTFIL.C Used for the floating point version of G.729 main body
	 * (not for G.729A)
	 */
	 ...
	 * 
	 * @param t0
	 *            input : pitch delay given by coder
	 * @param signal_ptr
	 *            input : input signal (pointer to current subframe)
	 * @param signal_ptr_offset
	 *            input : input signal offset
	 * @param coeff
	 *            input : LPC coefficients for current subframe
	 * @param coeff_offset
	 *            input : LPC coefficients offset
	 * @param sig_out
	 *            output: postfiltered output
	 * @param sig_out_offset
	 *            input: postfiltered output offset
	 * @return voicing decision 0 = uv, > 0 delay
	 */
	int post(int t0, float[] signal_ptr, int signal_ptr_offset, float[] coeff,
			int coeff_offset, float[] sig_out, int sig_out_offset) {
		...
		Filter.residu(apond2, 0, signal_ptr, signal_ptr_offset, res2, res2_ptr,
				L_SUBFR);
		...
		/* gain control */
		gain_prec = scale_st(signal_ptr, signal_ptr_offset, sig_out,
				sig_out_offset, gain_prec);
	 ...
	 * @param ptr_sig_in_offset
	 *            input : postfilter input filter offset
	 * @param ptr_sig_pst0
	 *            output: harmonic postfilter output
	 * @param ptr_sig_pst0_offset
	 *            input: harmonic postfilter offset
	 * @return voicing decision 0 = uv, > 0 delay
	 */
	private int pst_ltp(int t0, float[] ptr_sig_in, int ptr_sig_in_offset,
			float[] ptr_sig_pst0, int ptr_sig_pst0_offset) {
		int vo;
		...
		search_del(t0, ptr_sig_in, ptr_sig_in_offset, _ltpdel, _phase,
				_num_gltp, _den_gltp, y_up, _off_yup);
		...
		if (num_gltp == 0.f) {
			Util.copy(ptr_sig_in, ptr_sig_in_offset, ptr_sig_pst0,
					ptr_sig_pst0_offset, L_SUBFR);
		} else {
			...
			if (phase == 0) {
				...
				compute_ltp_l(ptr_sig_in, ptr_sig_in_offset, ltpdel, phase,
						ptr_sig_pst0, ptr_sig_pst0_offset, _num2_gltp,
						_den2_gltp);
			...
			filt_plt(ptr_sig_in, ptr_sig_in_offset, ptr_y_up, ptr_y_up_offset,
					ptr_sig_pst0, ptr_sig_pst0_offset, gain_plt);
		}
		return vo;
	}
	 ...
	 *            input : input signal offset
	 * @param ltpdel
	 *            output: delay = *ltpdel - *phase / f_up
	 * @param phase
	 *            output: phase
	 * @param num_gltp
	 *            output: numerator of LTP gain
	 * @param den_gltp
	 *            output: denominator of LTP gain
	 * @param y_up
	 * @param off_yup
	 */
	private void search_del(int t0, float[] ptr_sig_in, int ptr_sig_in_offset,
			IntReference ltpdel, IntReference phase, FloatReference num_gltp,
			FloatReference den_gltp, float[] y_up, IntReference off_yup) {
		...
		ener = 0.f;
		for (i = 0; i < L_SUBFR; i++) {
			ener += ptr_sig_in[ptr_sig_in_offset + i]
					* ptr_sig_in[ptr_sig_in_offset + i];
		}
		if (ener < 0.1f) {
			num_gltp.value = 0.f;
			den_gltp.value = 1.f;
		...
		for (i = 0; i < 3; i++) {
			num = 0.f;
			for (n = 0; n < L_SUBFR; n++) {
				num += ptr_sig_in[ptr_sig_in_offset + n]
						* ptr_sig_in[ptr_sig_past + n];
			}
			if (num > num_int) {
				i_max = i;
				num_int = num;
		...
		den_int = 0.f;
		for (n = 0; n < L_SUBFR; n++) {
			den_int += ptr_sig_in[ptr_sig_past + n]
					* ptr_sig_in[ptr_sig_past + n];
		}
		if (den_int < 0.1f) {
			num_gltp.value = 0.f;
		...
		for (phi = 1; phi < F_UP_PST; phi++) {
			...
			if (Math.abs(y_up[ptr_y_up + 0]) > Math
					.abs(y_up[ptr_y_up + L_SUBFR])) {
				if (den0 > den_max) {
					den_max = den0;
				}
			} else {
	 ...
	 *            input : input signal offset
	 * @param s_ltp
	 *            input : filtered signal with gain 1
	 * @param s_ltp_offset
	 *            input : filtered signal offset
	 * @param s_out
	 ...
	 *            input : filter gain
	 */
	private void filt_plt(float[] s_in, int s_in_offset, float[] s_ltp,
			int s_ltp_offset, float[] s_out, int s_out_offset, float gain_plt) {
	...
	/**
	 * Compute delayed signal, num & den of gain for fractional delay with long
	 ...
	 *            delayed signal
	 * @param y_up_offset
	 *            delayed signal offset
	 * @param num
	 *            numerator of LTP gain
	 * @param den
	 *            denominator of LTP gain
	 */
	private void compute_ltp_l(float[] s_in, int s_in_offset, int ltpdel,
			int phase, float[] y_up, int y_up_offset, FloatReference num,
			FloatReference den) {
	...
	/**
	 * Selects best of (gain1, gain2) with gain1 = num1 / den1 and gain2 = num2
	 * / den2
	 * 
	 * @param num1
	 *            input : numerator of gain1
	 * @param den1
	 *            input : denominator of gain1
	 * @param num2
	 *            input : numerator of gain2
	 * @param den2
	 *            input : denominator of gain2
	...
	/**
	 * Computes impulse response of A(gamma2) / A(gamma1). controls gain :
	 * computation of energy impulse response as SUMn (abs (h[n])) and computes
	 * parcor0
	 * 
	 * @param apond2
	 *            input : coefficients of numerator
	 * @param apond1
	 *            input : coefficients of denominator
	 * @param sig_ltp_ptr
	 *            in/out: input of 1/A(gamma1) : scaled by 1/g0
	 * @param sig_ltp_ptr_offset
	 *            input : input of 1/A(gamma1) ... offset
	 * @return 1st parcor calcul. on composed filter
	 */
	private float calc_st_filt(float[] apond2, float[] apond1,
			float[] sig_ltp_ptr, int sig_ltp_ptr_offset) {
		...
		if (g0 > 1.f) {
			temp = 1.f / g0;
			for (int i = sig_ltp_ptr_offset, toIndex = sig_ltp_ptr_offset
					+ L_SUBFR; i < toIndex; i++) {
	 ...
	 * 
	 * @param h
	 *            input : impulse response of composed filter
	 * @return 1st parcor
	 */
	...
	/**
	 * Tilt filtering with : (1 + mu z-1) * (1/1-|mu|). computes y[n] =
	 * (1/1-|mu|) (x[n]+mu*x[n-1])
	 * 
	 * @param sig_in
	 *            input : input signal (beginning at sample -1)
	 * @param sig_out
	 *            output: output signal
	 * @param sig_out_offset
	 *            input: output signal offset
	 * @param parcor0
	 *            input : parcor0 (mu = parcor0 * gamma3)
	 */
	private void filt_mu(float[] sig_in, float[] sig_out, int sig_out_offset,
			float parcor0) {
		int n;
		float mu, ga, temp;
		int ptrs;
	...
	/**
	 * Control of the subframe gain. gain[n] = AGC_FAC * gain[n-1] + (1 -
	 * AGC_FAC) g_in/g_out
	 * 
	 * @param sig_in
	 *            input : postfilter input signal
	 * @param sig_in_offset
	 *            input : postfilter input signal offset
	 * @param sig_out
	 *            in/out: postfilter output signal
	 * @param sig_out_offset
	 *            input: postfilter output signal offset
	 * @param gain_prec
	 *            input : last value of gain for subframe
	 * @return gain_prec last value of gain for subframe
	 */
	private float scale_st(float[] sig_in, int sig_in_offset, float[] sig_out,
			int sig_out_offset, float gain_prec) {
