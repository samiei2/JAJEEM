...
class AcelpCo {
	...
	/*
	 * File : ACELP_CO.C Used for the floating point version of G.729 main body
	 * (not for G.729A)
	 */
	 ...
	 *            (i) :Pitch lag
	 * @param pitch_sharp
	 *            (i) :Last quantized pitch gain
	 * @param i_subfr
	 *            (i) :Indicator of 1st subframe,
	 * @param code
	 *            (o) :Innovative codebook
	 * @param y
	 *            (o) :Filtered innovative codebook
	 * @param sign
	 *            (o) :Signs of 4 pulses
	 * @return index of pulses positions
	 */
	int ACELP_codebook(float x[], float h[], int t0, float pitch_sharp,
			int i_subfr, float code[], float y[], IntReference sign) {
	 ...
	 * 
	 * @param dn
	 *            (i) : backward filtered target vector
	 * @param rr
	 *            (i) : autocorrelations of impulse response h[]
	 * @param h
	 *            (i) : impulse response of filters
	 * @param cod
	 *            (o) : selected algebraic codeword
	 * @param y
	 *            (o) : output: selected algebraic codeword
	 * @param signs
	 *            (o) : signs of 4 pulses
	 * @param i_subfr
	 *            (i) subframe flag
	 ...
	 */
	private int d4i40_17(float dn[], float rr[], float h[], float cod[],
			float y[], IntReference signs, int i_subfr) {
		int L_SUBFR = Ld8k.L_SUBFR;
		 ...
		 * with pulse spacings of step = 5 Each pulses can have 8 possible
		 * positions (positive or negative):
		 * 
		 * i0 (+-1) : 0, 5, 10, 15, 20, 25, 30, 35 i1 (+-1) : 1, 6, 11, 16, 21,
		 * 26, 31, 36 i2 (+-1) : 2, 7, 12, 17, 22, 27, 32, 37 i3 (+-1) : 3, 8,
		 * 13, 18, 23, 28, 33, 38 4, 9, 14, 19, 24, 29, 34, 39
		 * --------------------
		 * -------------------------------------------------------
		 */
		...
		end_search: for (i0 = 0; i0 < L_SUBFR; i0 += STEP) /* first pulse loop */
		{
			...
			for (i1 = 1; i1 < L_SUBFR; i1 += STEP) /* second pulse loop */
			{
				...
				for (i2 = 2; i2 < L_SUBFR; i2 += STEP) {
					ps2 = ps1 + dn[i2];
					alp2 = alp1 + rr[ptr_ri2i2] + 2.0f
							* (rr[ptr_ri0i2] + rr[ptr_ri1i2]);
					ptr_ri2i2++;
					ptr_ri0i2++;
					ptr_ri1i2++;
					...
					if (ps2 > thres) {
						ptr_ri3i3 = rri3i3; /*
											 * Init. pointers that depend on 4th
											 * loop
											 */
						...
						for (i3 = 3; i3 < L_SUBFR; i3 += STEP) {
							...
							alp3 = alp2
									+ rr[ptr_ri3i3]
									+ 2.0f
									* (rr[ptr_ri1i3] + rr[ptr_ri0i3] + rr[ptr_ri2i3]);
							ptr_ri3i3++;
							ptr_ri1i3++;
							ptr_ri0i3++;
							ptr_ri2i3++;
						...
						for (i3 = 4; i3 < L_SUBFR; i3 += STEP) {
							...
							alp3 = alp2
									+ rr[ptr_ri4i4]
									+ 2.0f
									* (rr[ptr_ri1i4] + rr[ptr_ri0i4] + rr[ptr_ri2i4]);
							ptr_ri4i4++;
							ptr_ri1i4++;
							ptr_ri0i4++;
							ptr_ri2i4++;
