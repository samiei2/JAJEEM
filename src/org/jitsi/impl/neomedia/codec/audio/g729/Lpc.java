...
/**
 * Lpc analysis routines. NOTE: these routines are assuming that the order is
 * defined as M and that NC is defined as M/2. M has to be even
 * 
...
class Lpc {
	...
	/*
	 * File : LPC.C Used for the floating point version of G.729 main body (not
	 * for G.729A)
	 */
	 ...
	 * 
	 * @param x
	 *            (i) input signal x[0:L_WINDOW]
	 * @param x_offset
	 *            (i) input signal offset
	 * @param m
	 *            (i) LPC order
	 * @param r
	 *            (o) auto-correlation vector r[0:M]
	 */
	 ...
	 * 
	 * @param r
	 *            (i) auto correlation coefficients r[0:M]
	 * @param a
	 *            (o) lpc coefficients a[0] = 1
	 * @param a_offset
	 *            (i) lpc coefficients offset
	 * @param rc
	 *            (o) reflection coefficients rc[0:M-1]
	 ...
	 * polynomials. The found LSPs are in the cosine domain with values in the
	 * range from 1 down to -1. The table grid[] contains the points (in the
	 * cosine domain) at which the polynomials are evaluated. The table
	 * corresponds to NO_POINTS frequencies uniformly spaced between 0 and pi.
	 * 
	 * @param a
	 *            (i) LP filter coefficients
	 * @param a_offset
	 *            (i) LP filter coefficients offset
	 * @param lsp
	 *            (o) Line spectral pairs (in the cosine domain)
	 * @param old_lsp
	 ...
	 * 
	 * The polynomial order is n = m/2 (m is the prediction order) The
	 * polynomial is given by C(x) = T_n(x) + f(1)T_n-1(x) + ... +f(n-1)T_1(x) +
	 * f(n)/2
	 * 
	 * @param x
	 *            (i) value of evaluation; x=cos(freq)
	 * @param f
	 *            (i) coefficients of sum or diff polynomial
	 * @param n
