...
class Lspdec {
	...
	/*
	 * File : LSPDEC.C Used for the floating point version of both G.729 main
	 * body and G.729A
	 */
	...
	private void lsp_iqua_cs(int prm[], int prm_offset, float lsp_q[], int erase) {
		...
		if (erase == 0) /* Not frame erasure */
		{
			...
			Lspgetq.lsp_get_quant(lspcb1, lspcb2, code0, code1, code2,
		...
		} else /* Frame erased */
		{
			...
			/* update freq_prev */
			Lspgetq.lsp_prev_extract(prev_lsp, buf, fg[prev_ma], freq_prev,
					fg_sum_inv[prev_ma]);
	 ...
	 *            input : indexes
	 * @param index_offset
	 *            input : indexes offset
	 * @param lsp_q
	 *            output: decoded lsp
	 * @param bfi
	 *            input : frame erase information
	 */
	void d_lsp(int index[], int index_offset, float lsp_q[], int bfi) {
		int i;
