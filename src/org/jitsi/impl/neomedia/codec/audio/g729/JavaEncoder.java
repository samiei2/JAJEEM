...
public class JavaEncoder extends AbstractCodec2 {
	 ...
	 * nanosecond. We packetize 2 audio frames in one G729 packet.
	 */
	...
	public JavaEncoder() {
		super("G.729 Encoder", AudioFormat.class,
				new AudioFormat[] { new AudioFormat(AudioFormat.G729_RTP, 8000,
						Format.NOT_SPECIFIED, 1) });

		inputFormats = new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR,
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if ((prevInputLength + inputLength) < INPUT_FRAME_SIZE_IN_BYTES) {
			System.arraycopy(input, inputOffset, prevInput, prevInputLength,
					inputLength);
		...
		readShorts = readShorts(input, inputOffset, sp16, readShorts,
		...
		byte[] output = validateByteArraySize(outputBuffer,
				outputBuffer.getOffset() + 2 * OUTPUT_FRAME_SIZE_IN_BYTES, true);

		packetize(serial, output, outputBuffer.getOffset()
				+ OUTPUT_FRAME_SIZE_IN_BYTES * outputFrameCount);
		outputBuffer.setLength(outputBuffer.getLength()
	...
	private void packetize(short[] serial, byte[] outputFrame,
			int outputFrameOffset) {
		Arrays.fill(outputFrame, outputFrameOffset, outputFrameOffset + L_FRAME
				/ 8, (byte) 0);
	...
	private static int readShorts(byte[] input, int inputOffset,
			short[] output, int outputOffset, int outputLength) {
