...
class DecLd8k extends Ld8k {
	...
	/*
	 * File : DEC_LD8K.C Used for the floating point version of G.729 main body
	 * (not for G.729A)
	 */
	 ...
	 * @param voicing
	 *            input : voicing decision from previous frame
	 * @param synth
	 *            output: synthesized speech
	 * @param synth_offset
	 *            input : synthesized speech offset
	 * @param A_t
	 *            output: two sets of A(z) coefficients length=2*MP1
	 * @return output: integer delay of first subframe
	 */
	int decod_ld8k(int parm[], int voicing, float synth[], int synth_offset,
			float A_t[]) {
		int parm_offset = 0;
		...
		for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
			...
			if (i_subfr == 0) { /* if first subframe */
				...
				bad_pitch = bfi + i;
				if (bad_pitch == 0) {
					DecLag3.dec_lag3(index, PIT_MIN, PIT_MAX, i_subfr, t0,
							t0_frac);
					old_t0 = t0.value;
			...
			PredLt3.pred_lt_3(exc, exc_offset + i_subfr, t0.value,
					t0_frac.value, L_SUBFR);
			...
			if (bfi != 0) { /* Bad Frame Error Concealment */
				parm[parm_offset + 0] = (Util.random_g729() & 0x1fff); /*
																		 * 13
																		 * bits
																		 * random
																		 */
				parm[parm_offset + 1] = (Util.random_g729() & 0x000f); /*
																		 * 4
																		 * bits
																		 * random
																		 */
			...
			DeAcelp.decod_ACELP(parm[parm_offset + 1], parm[parm_offset + 0],
					code);
			parm_offset += 2;
			...
			if (bfi != 0) {
				if (voicing == 0) { /* for unvoiced frame */
					for (i = 0; i < L_SUBFR; i++) {
						exc[exc_offset + i + i_subfr] = gain_code.value
								* code[i];
					}
				} else { /* for voiced frame */
					for (i = 0; i < L_SUBFR; i++) {
						exc[exc_offset + i + i_subfr] = gain_pitch.value
			...
			Filter.syn_filt(Az, Az_offset, exc, exc_offset + i_subfr, synth,
					synth_offset + i_subfr, L_SUBFR, mem_syn, 0, 1);
