...
public class JNIDecoder extends AbstractCodec2 implements FECDecoderControl {
	...
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(
			AudioFormat.LINEAR, 48000, 16, 1, AudioFormat.LITTLE_ENDIAN,
			AudioFormat.SIGNED,
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if (decodeFEC) {
			...
			byte[] out = validateByteArraySize(outBuffer, outOffset
					+ lastFrameSizeInSamplesPerChannel * outputFrameSize,
					outOffset != 0);
			int frameSizeInSamplesPerChannel = Opus.decode(decoder, in,
					inOffset, inLength, out, /* outOffset, */
					lastFrameSizeInSamplesPerChannel,
					/* decodeFEC */1);

			if (frameSizeInSamplesPerChannel > 0) {
				int frameSizeInBytes = frameSizeInSamplesPerChannel
						* outputFrameSize;
				...
				outBuffer.setFlags(outBuffer.getFlags()
						| (((in == null) || (inLength == 0)) ? BUFFER_FLAG_PLC
								: BUFFER_FLAG_FEC));
				nbDecodedFec++;
			}
			...
			int frameSizeInSamplesPerChannel = Opus.decoder_get_nb_samples(
					decoder, in, inOffset, inLength);
			byte[] out = validateByteArraySize(outBuffer, outOffset
					+ frameSizeInSamplesPerChannel * outputFrameSize,
					outOffset != 0);

			frameSizeInSamplesPerChannel = Opus.decode(decoder, in, inOffset,
					inLength, out, /* outOffset, */
		...
		if (outLength > 0) {
			outBuffer.setDuration(totalFrameSizeInSamplesPerChannel * channels
					* 1000L * 1000L / outputSampleRate);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AudioFormat(AudioFormat.LINEAR,
				inputAudioFormat.getSampleRate(), 16, 1,
	...
	public Format setInputFormat(Format format) {
		...
		if (inFormat != null) {
			...
			if ((outSampleRate != inSampleRate) || (outChannels != inChannels)) {
				setOutputFormat(new AudioFormat(AudioFormat.LINEAR,
						inSampleRate, 16, inChannels,
						AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
						/* frameSizeInBits */Format.NOT_SPECIFIED,
						/* frameRate */Format.NOT_SPECIFIED, Format.byteArray));
			}
