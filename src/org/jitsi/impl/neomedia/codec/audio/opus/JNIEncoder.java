 ...
 */
public class JNIEncoder extends AbstractCodec2 implements
		FormatParametersAwareCodec, PacketLossAwareEncoder {
	/**
	...
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(
			...
			/* sampleSizeInBits */Format.NOT_SPECIFIED, 1,
	...
	static {
		...
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i + supportedInputCount] = new AudioFormat(
					AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i], 16, 2,
					AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
					/* frameSizeInBits */Format.NOT_SPECIFIED,
					/* frameRate */Format.NOT_SPECIFIED, Format.byteArray);
		}
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if ((prevIn != null) && (prevInLength > 0)) {
			if (prevInLength < frameSizeInBytes) {
				...
				int bytesToCopyFromInToPrevInput = Math.min(frameSizeInBytes
		...
		int outLength = Opus.encode(encoder, in, inOffset,
				frameSizeInSamplesPerChannel, out, /* 0, */out.length);
	...
	public Format getOutputFormat() {
		...
		if ((f != null) && (f.getClass() == AudioFormat.class)) {
			...
			f = setOutputFormat(new AudioFormat(af.getEncoding(),
					af.getSampleRate(), af.getSampleSizeInBits(),
					af.getChannels(), af.getEndian(), af.getSigned(),
					af.getFrameSizeInBits(), af.getFrameRate(),
					af.getDataType()) {
	...
	public void setFormatParameters(Map<String, String> fmtps) {
		...
		Opus.encoder_set_bitrate(encoder,
