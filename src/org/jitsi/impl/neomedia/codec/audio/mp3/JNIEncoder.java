...
public class JNIEncoder extends AbstractCodec2 {
	...
	private static final Format[] SUPPORTED_INPUT_FORMATS = new Format[] { new AudioFormat(
			AudioFormat.LINEAR,
			/* sampleRate */Format.NOT_SPECIFIED, 16,
			/* channels */Format.NOT_SPECIFIED, AudioFormat.LITTLE_ENDIAN,
			AudioFormat.SIGNED,
	...
	protected synchronized void doOpen() throws ResourceUnavailableException {
		...
		try {
			...
			frameSizeInBytes = FFmpeg.avcodeccontext_get_frame_size(avctx)
					* (inputFormat.getSampleSizeInBits() / 8) * channels;
		} finally {
			if (avcodec_open < 0) {
				FFmpeg.av_free(avctx);
				avctx = 0;
	...
	protected synchronized int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if ((prevInputLength > 0) || (inputLength < frameSizeInBytes)) {
			int newPrevInputLength = Math.min(frameSizeInBytes
			...
			if (newPrevInputLength > 0) {
				...
				System.arraycopy(input, inputOffset, prevInput,
		...
		byte[] output = (outputData instanceof byte[]) ? (byte[]) outputData
				: null;
		...
		int outputLength = FFmpeg.avcodec_encode_audio(avctx, output,
				outputOffset, output.length - outputOffset, input, inputOffset);
