...
public class JNIDecoder extends AbstractCodec2 {
	...
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(
			AudioFormat.LINEAR, Format.NOT_SPECIFIED, 16, 1,
	...
	static {
		...
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(Constants.SPEEX_RTP,
					SUPPORTED_INPUT_SAMPLE_RATES[i], Format.NOT_SPECIFIED, 1,
					AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (state == 0) {
			...
			if (Speex.speex_decoder_ctl(state, Speex.SPEEX_SET_SAMPLING_RATE,
					inputSampleRate) != 0) {
				return BUFFER_PROCESSED_FAILED;
			}

			int frameSize = Speex.speex_decoder_ctl(state,
		...
		if (inputFormat != null) {
			...
			if ((outputSampleRate != inputSampleRate)
				...
				setOutputFormat(new AudioFormat(AudioFormat.LINEAR,
						inputSampleRate, 16, inputChannels,
						AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
						Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
						Format.byteArray));
			}
