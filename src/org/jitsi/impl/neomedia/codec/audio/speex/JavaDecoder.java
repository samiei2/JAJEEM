...
public class JavaDecoder extends AbstractCodec {
	 ...
	 */
	protected Format[] DEFAULT_OUTPUT_FORMATS = new Format[] { new AudioFormat(
			AudioFormat.LINEAR) };
	...
	public JavaDecoder() {
		this.inputFormats = new AudioFormat[] {
				new AudioFormat(Constants.SPEEX_RTP, 8000, 8, 1,
						Format.NOT_SPECIFIED, Format.NOT_SPECIFIED),
				new AudioFormat(Constants.SPEEX_RTP, 16000, 8, 1,
						Format.NOT_SPECIFIED, Format.NOT_SPECIFIED),
				new AudioFormat(Constants.SPEEX_RTP, 32000, 8, 1,
						Format.NOT_SPECIFIED, Format.NOT_SPECIFIED) };
	}
	...
	private Format[] getMatchingOutputFormats(Format in) {
		...
		return new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR,
				af.getSampleRate(), 16, af.getChannels(),
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer) {
		...
		try {
			int fullyProcessed = decoder.processData(inData, inOffset,
					inpLength);
