...
public class JNIEncoder extends AbstractCodec2 {
	 ...
	 * <tt>JNIEncoder</tt> instances.
	 */
	static final double[] SUPPORTED_INPUT_SAMPLE_RATES = new double[] { 8000,
			16000, 32000 };
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (state == 0) {
			long mode = Speex
					.speex_lib_get_mode((inputSampleRate == 16000) ? Speex.SPEEX_MODEID_WB
							: (inputSampleRate == 32000) ? Speex.SPEEX_MODEID_UWB
			...
			if (Speex.speex_encoder_ctl(state, Speex.SPEEX_SET_SAMPLING_RATE,
					inputSampleRate) != 0) {
				return BUFFER_PROCESSED_FAILED;
			}

			int frameSize = Speex.speex_encoder_ctl(state,
					Speex.SPEEX_GET_FRAME_SIZE);
		...
		if ((previousInput != null) && (previousInputLength > 0)) {
			if (previousInputLength < this.frameSize) {
				if (previousInput.length < this.frameSize) {
					...
					System.arraycopy(previousInput, 0, newPreviousInput, 0,
							previousInput.length);
					previousInput = newPreviousInput;
				}

				int bytesToCopyFromInputToPreviousInput = Math.min(
						this.frameSize - previousInputLength, inputLength);

				if (bytesToCopyFromInputToPreviousInput > 0) {
					System.arraycopy(input, inputOffset, previousInput,
							previousInputLength,
			...
			if (previousInputLength == this.frameSize) {
				...
				System.arraycopy(previousInput, input.length, previousInput, 0,
						previousInputLength);
			} else {
		...
		if (outputLength > 0) {
			...
			outputLength = Speex.speex_bits_write(bits, output, 0,
					output.length);
			if (outputLength > 0) {
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AudioFormat(Constants.SPEEX_RTP,
				inputAudioFormat.getSampleRate(), Format.NOT_SPECIFIED, 1,
				AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
	...
	public Format setInputFormat(Format format) {
		...
		if (inputFormat != null) {
			...
			if ((outputSampleRate != inputSampleRate)
				...
				setOutputFormat(new AudioFormat(Constants.SPEEX_RTP,
						inputSampleRate, Format.NOT_SPECIFIED, inputChannels,
						AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
						Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
						Format.byteArray));
			}
