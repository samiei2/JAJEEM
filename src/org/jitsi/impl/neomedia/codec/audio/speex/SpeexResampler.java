...
public class SpeexResampler extends AbstractCodec2 {
	...
	static {
		...
		for (int i = 0; i < supportedCount; i++) {
			...
			SUPPORTED_FORMATS[j] = new AudioFormat(AudioFormat.LINEAR,
					SUPPORTED_SAMPLE_RATES[i], 16 /* sampleSizeInBits */,
					1 /* channels */, AudioFormat.LITTLE_ENDIAN,
					AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */,
					Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
			SUPPORTED_FORMATS[j + 1] = new AudioFormat(AudioFormat.LINEAR,
					SUPPORTED_SAMPLE_RATES[i], 16 /* sampleSizeInBits */,
					1 /* channels */, AudioFormat.LITTLE_ENDIAN,
					AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */,
					Format.NOT_SPECIFIED /* frameRate */, Format.shortArray);
			SUPPORTED_FORMATS[j + 2] = new AudioFormat(AudioFormat.LINEAR,
					SUPPORTED_SAMPLE_RATES[i], 16 /* sampleSizeInBits */,
					2 /* channels */, AudioFormat.LITTLE_ENDIAN,
					AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */,
					Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
			SUPPORTED_FORMATS[j + 3] = new AudioFormat(AudioFormat.LINEAR,
					SUPPORTED_SAMPLE_RATES[i], 16 /* sampleSizeInBits */,
					2 /* channels */, AudioFormat.LITTLE_ENDIAN,
					AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */,
		...
		if (inSampleRate == outSampleRate) {
			...
			if (Format.byteArray.equals(inDataType)) {
				...
				if (Format.byteArray.equals(outDataType)) {
					...
					byte[] output = validateByteArraySize(outBuffer, length,
							false);
					...
					short[] output = validateShortArraySize(outBuffer,
							outLength);
				...
				if (Format.byteArray.equals(outDataType)) {
					...
					int outOffset = 0;
					int outLength = inLength * 2;
					byte[] output = validateByteArraySize(outBuffer, outLength,
							false);
			...
			if (channelsHaveChanged || (this.inputSampleRate != inSampleRate)
				...
				if (resampler == 0) {
					resampler = Speex.speex_resampler_init(channels,
							inSampleRate, outSampleRate,
							Speex.SPEEX_RESAMPLER_QUALITY_VOIP, 0);
				} else {
					Speex.speex_resampler_set_rate(resampler, inSampleRate,
							outSampleRate);
				}
				if (resampler != 0) {
			...
			int frameSize = channels
					* (inAudioFormat.getSampleSizeInBits() / 8);
			/*
			...
			byte[] output = validateByteArraySize(outBuffer, outSampleCount
					* frameSize, false);

			outSampleCount = Speex.speex_resampler_process_interleaved_int(
					resampler, input, inBuffer.getOffset(), inSampleCount,
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		if (inputFormat instanceof AudioFormat) {
			...
			for (Format supportedFormat : SUPPORTED_FORMATS) {
				...
				if ((Format.byteArray.equals(supportedFormat.getDataType()) && Format.byteArray
						.equals(inDataType))
		...
		return matchingOutputFormats.toArray(new Format[matchingOutputFormats
				.size()]);
	}
	...
	public Format setInputFormat(Format format) {
		...
		if (inFormat != null) {
			...
			setOutputFormat(new AudioFormat(inFormat.getEncoding(),
					outSampleRate, inFormat.getSampleSizeInBits(),
					inFormat.getChannels(), inFormat.getEndian(),
					inFormat.getSigned(), Format.NOT_SPECIFIED,
					Format.NOT_SPECIFIED, outDataType));
		}
		return inFormat;
	}
}
