...
public class JavaEncoder extends AbstractCodec2 {
	 ...
	 */
	public JavaEncoder() {
		super("Speex Java Encoder", AudioFormat.class,
		...
		inputFormats = new AudioFormat[] {
				...
				new AudioFormat(AudioFormat.LINEAR, 16000, 16, 1,
						AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED),
				new AudioFormat(AudioFormat.LINEAR, 32000, 16, 1,
						AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED) };
	}
	...
	private void initConverter(AudioFormat inFormat) {
		...
		outFormat = new AudioFormat(oFormat.getEncoding(),
				oFormat.getSampleRate(), oFormat.getSampleSizeInBits(),
				oFormat.getChannels(), oFormat.getEndian(),
				oFormat.getSigned(), oFormat.getFrameSizeInBits(),
				oFormat.getFrameRate(), oFormat.getDataType()) {
			/**
			 * Serial version UID.
			 */
