...
public class ApplySineWindowFLP {
	/**
	 * Apply sine window to signal vector. Window types: 0 -> sine window from 0
	 * to pi. 1 -> sine window from 0 to pi/2. 2 -> sine window from pi/2 to pi.
	 * 
	 * @param px_win
	...
	static void SKP_Silk_apply_sine_window_FLP(float px_win[], /*
																 ...
																 * signal
																 */
			int px_win_offset, float px[], /* I Pointer to input signal */
			int px_offset, final int win_type, /* I Selects a window type */
		...
		for (k = 0; k < length; k += 4) {
			px_win[px_win_offset + k + 0] = px[px_offset + k + 0] * 0.5f
					* (S0 + S1);
			px_win[px_win_offset + k + 1] = px[px_offset + k + 1] * S1;
			S0 = c * S1 - S0;
			px_win[px_win_offset + k + 2] = px[px_offset + k + 2] * 0.5f
					* (S1 + S0);
