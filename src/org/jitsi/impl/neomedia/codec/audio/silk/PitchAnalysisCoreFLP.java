...
public class PitchAnalysisCoreFLP {
	 ...
	 */
	static int SKP_Silk_pitch_analysis_core_FLP( /*
												 * O voicing estimate: 0 voiced,
												 * 1 unvoiced
												 */
	float[] signal, /* I signal of length PITCH_EST_FRAME_LENGTH_MS*Fs_kHz */
			...
			float[] LTPCorr, /*
							 * I/O normalized correlation; input: value from
							 * previous frame
							 */
			int prevLag, /* I last lag of previous frame; set to zero is unvoiced */
			final float search_thres1, /*
										 * I first stage threshold for lag
										 * candidates 0 - 1
										 */
			final float search_thres2, /*
										 * I final threshold for lag candidates
										 * 0 - 1
										 */
		...
		float[][] C = new float[CommonPitchEstDefines.PITCH_EST_NB_SUBFR][(CommonPitchEstDefines.PITCH_EST_MAX_LAG >> 1) + 5]; /*
																																 * use
																																 * to
																																 * be
																																 * +
		...
		if (Fs_kHz == 12) {
			...
			ResamplerDown23.SKP_Silk_resampler_down2_3(R23, 0, signal_8, 0,
					signal_12, 0,
					CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS * 12);
			SigProcFLP.SKP_short2float_array(signal_8kHz, 0, signal_8, 0,
					frame_length_8kHz);
		} else if (Fs_kHz == 16) {
			if (complexity == SigProcFIXConstants.SKP_Silk_PITCH_EST_MAX_COMPLEX) {
				...
				Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP(signal, 0,
						filt_state, 0, signal_8kHz, 0, scratch_mem, 0,
						frame_length_8kHz);
			} else {
				...
				Decimate2CoarsestFLP.SKP_Silk_decimate2_coarsest_FLP(signal, 0,
						filt_state, 0, signal_8kHz, 0, scratch_mem, 0,
						frame_length_8kHz);
			}
		} else if (Fs_kHz == 24) {
			...
			ResamplerDown3.SKP_Silk_resampler_down3(filt_state_fix, 0,
					signal_8, 0, signal_24, 0,
					24 * CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS);
			SigProcFLP.SKP_short2float_array(signal_8kHz, 0, signal_8, 0,
					frame_length_8kHz);
		} else {
		...
		if (complexity == SigProcFIXConstants.SKP_Silk_PITCH_EST_MAX_COMPLEX) {
			...
			Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP(signal_8kHz, 0,
					filt_state, 0, signal_4kHz, 0, scratch_mem, 0,
					frame_length_4kHz);
		} else {
			assert (2 <= CommonPitchEstDefines.PITCH_EST_MAX_DECIMATE_STATE_LENGTH);
			for (int i_djinn = 0; i_djinn < 4; i_djinn++) {
			...
			Decimate2CoarsestFLP.SKP_Silk_decimate2_coarsest_FLP(signal_8kHz,
					0, filt_state, 0, signal_4kHz, 0, scratch_mem, 0,
					frame_length_4kHz);
		}
		...
		for (k = 0; k < 2; k++) {
			...
			cross_corr = InnerProductFLP.SKP_Silk_inner_product_FLP(target_ptr,
					target_ptr_offset, basis_ptr, basis_ptr_offset,
					sf_length_8kHz);
			normalizer = EnergyFLP.SKP_Silk_energy_FLP(basis_ptr,
					basis_ptr_offset, sf_length_8kHz) + 1000.0f;
			...
			for (d = min_lag_4kHz + 1; d <= max_lag_4kHz; d++) {
				...
				cross_corr = InnerProductFLP.SKP_Silk_inner_product_FLP(
						target_ptr, target_ptr_offset, basis_ptr,
						basis_ptr_offset, sf_length_8kHz);

				/*
				 * Add contribution of new sample and remove contribution from
				 * oldest sample
				 */
				// normalizer +=
				// basis_ptr[ 0 ] * basis_ptr[ 0 ] -
				// basis_ptr[ sf_length_8kHz ] * basis_ptr[ sf_length_8kHz ];
				normalizer += basis_ptr[basis_ptr_offset + 0]
						* basis_ptr[basis_ptr_offset + 0]
		...
		SortFLP.SKP_Silk_insertion_sort_decreasing_FLP(C[0], min_lag_4kHz,
				d_srch, max_lag_4kHz - min_lag_4kHz + 1, length_d_srch);
		...
		for (i = 0; i < frame_length_4kHz >> 1; i++) {
			energy += target_ptr[target_ptr_offset + i]
					* target_ptr[target_ptr_offset + i];
		}
		threshold = Cmax * Cmax;
		...
		for (i = 0; i < length_d_srch; i++) {
			/*
			 * Convert to 8 kHz indices for the sorted correlation that exceeds
			 * the threshold
			 */
		...
		for (k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++) {
			...
			energy_tmp = EnergyFLP.SKP_Silk_energy_FLP(target_ptr,
					target_ptr_offset, sf_length_8kHz);
			for (j = 0; j < length_d_comp; j++) {
				...
				cross_corr = InnerProductFLP.SKP_Silk_inner_product_FLP(
						basis_ptr, basis_ptr_offset, target_ptr,
						target_ptr_offset, sf_length_8kHz);
				energy = EnergyFLP.SKP_Silk_energy_FLP(basis_ptr,
						basis_ptr_offset, sf_length_8kHz);
				if (cross_corr > 0.0f) {
					C[k][d] = (float) (cross_corr * cross_corr / (energy
							* energy_tmp + eps));
				} else {
					C[k][d] = 0.0f;
				}
		...
		if (Fs_kHz == 8
				&& complexity > SigProcFIXConstants.SKP_Silk_PITCH_EST_MIN_COMPLEX) {
		...
		for (k = 0; k < length_d_srch; k++) {
			...
			for (j = 0; j < nb_cbks_stage2; j++) {
				...
				for (i = 0; i < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; i++) {
					/* Try all codebooks */
					CC[j] += C[i][d
							+ PitchEstTables.SKP_Silk_CB_lags_stage2[i][j]];
				}
			}
			/* Find best codebook */
			CCmax_new = -1000.0f;
			...
			CCmax_new_b -= PitchEstDefinesFLP.PITCH_EST_FLP_SHORTLAG_BIAS
					* CommonPitchEstDefines.PITCH_EST_NB_SUBFR * lag_log2;

			/* Bias towards previous lag */
			if (prevLag > 0) {
				delta_lag_log2_sqr = lag_log2 - prevLag_log2;
				delta_lag_log2_sqr *= delta_lag_log2_sqr;
				CCmax_new_b -= PitchEstDefinesFLP.PITCH_EST_FLP_PREVLAG_BIAS
						* CommonPitchEstDefines.PITCH_EST_NB_SUBFR * LTPCorr[0]
						* delta_lag_log2_sqr / (delta_lag_log2_sqr + 0.5f);
			}

			if (CCmax_new_b > CCmax_b
					&& CCmax_new > CommonPitchEstDefines.PITCH_EST_NB_SUBFR
		...
		if (Fs_kHz > 8) {
			...
			assert (CCmax >= 0.0f);
			LTPCorr[0] = (float) Math.sqrt(CCmax
					/ CommonPitchEstDefines.PITCH_EST_NB_SUBFR); // Output
																	// normalized
																	// correlation
			...
			SKP_P_Ana_calc_corr_st3(cross_corr_st3, signal, 0, start_lag,
					sf_length, complexity);
			SKP_P_Ana_calc_energy_st3(energies_st3, signal, 0, start_lag,
					sf_length, complexity);

			lag_counter = 0;
			assert (lag == SigProcFIX.SKP_SAT16(lag));
			contour_bias = PitchEstDefinesFLP.PITCH_EST_FLP_FLATCONTOUR_BIAS
					/ lag;
			...
			for (d = start_lag; d <= end_lag; d++) {
				for (j = cbk_offset; j < (cbk_offset + cbk_size); j++) {
					...
					if (cross_corr > 0.0) {
						...
						diff = j
								- (CommonPitchEstDefines.PITCH_EST_NB_CBKS_STAGE3_MAX >> 1);
			...
			for (k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++) {
				pitch_out[k] = lag_new
						+ PitchEstTables.SKP_Silk_CB_lags_stage3[k][CBimax];
			}
			...
			assert (CCmax >= 0.0f);
			LTPCorr[0] = (float) Math.sqrt(CCmax
					/ CommonPitchEstDefines.PITCH_EST_NB_SUBFR); /*
																 * Output
																 * normalized
																 * correlation
																 */
			for (k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++) {
				pitch_out[k] = lag
						+ PitchEstTables.SKP_Silk_CB_lags_stage2[k][CBimax];
			}
	...
	static void SKP_P_Ana_calc_corr_st3(float[][][] cross_corr_st3,
			...
			int signal_offset, int start_lag, /* I start lag */
	 ...
	 **********************************************************************/
	{
		...
		for (k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++) {
			...
			for (j = PitchEstTables.SKP_Silk_Lag_range_stage3[complexity][k][0]; j <= PitchEstTables.SKP_Silk_Lag_range_stage3[complexity][k][1]; j++) {
				...
				scratch_mem[lag_counter] = (float) InnerProductFLP
						.SKP_Silk_inner_product_FLP(target_ptr,
								target_ptr_offset, basis_ptr, basis_ptr_offset,
								sf_length);
				lag_counter++;
			}
	 ...
	 ****************************************************************/
	{
		...
		for (k = 0; k < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; k++) {
			...
			energy = EnergyFLP.SKP_Silk_energy_FLP(basis_ptr, basis_ptr_offset,
					sf_length) + 1e-3;
			assert (energy >= 0.0);
			scratch_mem[lag_counter] = (float) energy;
			lag_counter++;

			for (i = 1; i < (PitchEstTables.SKP_Silk_Lag_range_stage3[complexity][k][1]
					- PitchEstTables.SKP_Silk_Lag_range_stage3[complexity][k][0] + 1); i++) {
				/* remove part outside new window */
				energy -= basis_ptr[basis_ptr_offset + sf_length - i]
						* basis_ptr[basis_ptr_offset + sf_length - i];
				assert (energy >= 0.0);

				/* add part that comes into window */
				energy += basis_ptr[basis_ptr_offset - i]
						* basis_ptr[basis_ptr_offset - i];
				assert (energy >= 0.0);
				assert (lag_counter < SCRATCH_SIZE);
				scratch_mem[lag_counter] = (float) energy;
				lag_counter++;
