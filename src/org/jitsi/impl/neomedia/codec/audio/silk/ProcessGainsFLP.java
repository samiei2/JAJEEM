...
public class ProcessGainsFLP {
	 ...
	 *            Encoder control FLP
	 */
	static void SKP_Silk_process_gains_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
																			 * I/
																			 * O
																			 * Encoder
																			 * state
		...
		if (psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
			s = 1.0f - 0.5f * SigProcFLP
					.SKP_sigmoid(0.25f * (psEncCtrl.LTPredCodGain - 12.0f));
		...
		InvMaxSqrVal = (float) (Math.pow(2.0f,
				0.33f * (21.0f - psEncCtrl.current_SNR_dB)) / psEnc.sCmn.subfr_length);

		for (k = 0; k < Define.NB_SUBFR; k++) {
			/* Soft limit on ratio residual energy and squared gains */
			gain = psEncCtrl.Gains[k];
			gain = (float) Math.sqrt(gain * gain + psEncCtrl.ResNrg[k]
					* InvMaxSqrVal);
		...
		/*
		 * Overwrite unquantized gains with quantized gains and convert back to
		 * Q0 from Q16
		 */
		for (k = 0; k < Define.NB_SUBFR; k++) {
		...
		/*
		 * Set quantizer offset for voiced signals. Larger offset when LTP
		 * coding gain is low or tilt is high (ie low-pass)
		 */
		if (psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
		...
		if (psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
			psEncCtrl.Lambda = 1.2f - 0.4f * psEnc.speech_activity - 0.3f
					* psEncCtrl.input_quality + 0.2f
					* psEncCtrl.sCmn.QuantOffsetType - 0.1f
					* psEncCtrl.coding_quality;
		} else {
			psEncCtrl.Lambda = 1.2f - 0.4f * psEnc.speech_activity - 0.4f
					* psEncCtrl.input_quality + 0.4f
					* psEncCtrl.sCmn.QuantOffsetType - 0.1f
					* psEncCtrl.coding_quality;
		}
