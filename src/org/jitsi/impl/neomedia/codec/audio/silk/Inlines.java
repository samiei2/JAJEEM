...
public class Inlines extends InlinesConstants {
	...
	/**
	 * Approximation of square root Accuracy: < +/- 10% for output values > 15 <
	...
	/**
	 * returns the number of left shifts before overflow for a 16 bit number
	 * (ITU definition with norm(0)=0).
	 * 
	 * @param a
	 * @return
	 */
	...
	/**
	 * returns the number of left shifts before overflow for a 32 bit number
	 * (ITU definition with norm(0)=0)
	 * 
	 * @param a
	 * @return
	 ...
	 */
	static int SKP_DIV32_varQ /*
							 * O returns a good approximation of
							 * "(a32 << Qres) / b32"
							 */
	(final int a32, /* I numerator (Q0) */
		...
		/* First approximation */
		result = Macros.SKP_SMULWB(a32_nrm, b32_inv); /*
													 * Q: 29 + a_headrm -
													 * b_headrm
													 */
		...
		/* Refinement */
		result = Macros.SKP_SMLAWB(result, a32_nrm, b32_inv); /*
															 * Q: 29 + a_headrm
															 * - b_headrm
															 */
	...
	/**
	 * Sine approximation; an input of 65536 corresponds to 2 * pi Uses
	 * polynomial expansion of the input to the power 0, 2, 4 and 6 The relative
	 * error is below 1e-5
	 * 
	 * @param x
	 * @return returns approximately 2^24 * sin(x * 2 * pi / 65536).
	 */
	static int SKP_Silk_SIN_APPROX_Q24( /*
										 * O returns approximately 2^24 * sin(x
										 * * 2 * pi / 65536)
										 */
	int x) {
		int y_Q30;
		...
		/*
		 * Keep only bottom 16 bits (the function repeats itself with period
		 * 65536)
		 */
		x &= 65535;
		...
		if (x <= 32768) {
			...
			y_Q30 = Macros.SKP_SMLAWB(SKP_SIN_APPROX_CONST2, x,
	...
	/**
	 * Cosine approximation; an input of 65536 corresponds to 2 * pi The
	 * relative error is below 1e-5
	 * 
	 * @param x
	 * @return returns approximately 2^24 * cos(x * 2 * pi / 65536).
	 */
	static int SKP_Silk_COS_APPROX_Q24( /*
										 * O returns approximately 2^24 * cos(x
										 * * 2 * pi / 65536)
										 */
	int x) {
