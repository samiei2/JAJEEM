 ...
 * @author Boris Grozev
 */
public class JavaEncoder extends AbstractCodec2 implements
		PacketLossAwareEncoder {
	/**
	 * The <tt>Logger</tt> used by this <tt>JavaEncoder</tt> instance for
	 * logging output.
	 */
	 ...
	 * <tt>JavaEncoder</tt> instances.
	 */
	private static final double[] SUPPORTED_SAMPLE_RATES = new double[] { 8000,
			12000, 16000, 24000 };
	...
	/**
	 * If <tt>alwaysExpectPacketLoss</tt> is <tt>true</tt> the expected packet
	 * loss will always be set at or above this threshold.
	 */
	...
	/**
	 * The duration an output <tt>Buffer</tt> produced by this <tt>Codec</tt> in
	 * nanosecond.
	 */
	...
	static {
		...
		for (int i = 0; i < supportedCount; i++) {
			...
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR,
					supportedSampleRate, 16, 1, AudioFormat.LITTLE_ENDIAN,
					AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */,
					Format.NOT_SPECIFIED /* frameRate */, Format.shortArray);
			SUPPORTED_OUTPUT_FORMATS[i] = new AudioFormat(Constants.SILK_RTP,
					supportedSampleRate,
					Format.NOT_SPECIFIED /* sampleSizeInBits */, 1,
	...
	public JavaEncoder() {
		...
		alwaysAssumePacketLoss = cfg.getBoolean(Constants.PROP_SILK_ASSUME_PL,
				true);
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		byte[] outputData = validateByteArraySize(outputBuffer,
				MAX_BYTES_PER_FRAME, false);
		int outputOffset = 0;

		int processed;

		outputLength[0] = MAX_BYTES_PER_FRAME;
		if (EncAPI.SKP_Silk_SDK_Encode(encState, encControl, inputData,
				inputOffset, inputLength, outputData, outputOffset,
				outputLength) == 0) {
		...
		if (processed != BUFFER_PROCESSED_FAILED) {
			if (processed == BUFFER_PROCESSED_OK) {
				updateOutput(outputBuffer, getOutputFormat(),
						outputBuffer.getLength(), outputBuffer.getOffset());
	...
	static Format[] getMatchingOutputFormats(Format inputFormat,
			Format[] supportedInputFormats, Format[] supportedOutputFormats) {
		if (inputFormat == null) {
			return supportedOutputFormats;
		} else {
			Format matchingInputFormat = matches(inputFormat,
			...
			if (matchingInputFormat == null) {
				...
				Format matchingOutputFormat = matches(outputFormat,
						supportedOutputFormats);

				if (matchingOutputFormat == null) {
					return new Format[0];
				} else {
					return new Format[] { matchingOutputFormat
	...
	public void setExpectedPacketLoss(int percentage) {
		if (opened) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Setting expected packet loss to: " + percentage);
			}
		}
	}
