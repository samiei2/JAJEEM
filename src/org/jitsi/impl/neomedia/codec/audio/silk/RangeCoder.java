...
public class RangeCoder {
	 ...
	 */
	static void SKP_Silk_range_decoder(int data[], /* O uncompressed data */
			int data_offset, SKP_Silk_range_coder_state psRC, /*
															 * I/O compressor
															 * data structure
															 */
			final int prob[], /* I cumulative density function */
			int prob_offset, int probIx /* I initial (middle) entry of cdf */
	) {
		long low_Q16, high_Q16;
		long base_tmp, range_Q32;
	 ...
	 *            cumulative density functions.
	 * @param probStartIx
	 *            initial (middle) entries of cdfs [nSymbols].
	 * @param nSymbols
	 *            number of data symbols.
	 */
	...
	static void SKP_Silk_range_dec_init(SKP_Silk_range_coder_state psRC, /*
			...
			int buffer_offset, final int bufferLength /*
													 * I buffer length (in
													 * bytes)
													 */
	) {
		...
		psRC.bufferIx = 0;
		psRC.base_Q32 = (((buffer[buffer_offset + 0] & 0xFF) << 24)
				| ((buffer[buffer_offset + 1] & 0xFF) << 16)
				| ((buffer[buffer_offset + 2] & 0xFF) << 8) | (buffer[buffer_offset + 3] & 0xFF)) & 0xFFFFFFFFL;
		psRC.range_Q16 = 0x0000FFFF;
		psRC.error = 0;
	}
	 ...
	 */
	static int SKP_Silk_range_coder_get_length( /*
												 * O returns number of BITS in
												 * stream
												 */
	SKP_Silk_range_coder_state psRC, /* I compressed data structure */
		...
		nBits = (psRC.bufferIx << 3)
				+ Macros.SKP_Silk_CLZ32((int) (psRC.range_Q16 - 1)) - 14;
	...
	/**
	 * Write shortest uniquely decodable stream to buffer, and determine its
	 * length.
	 * 
	 * @param psRC
