...
public class PrefilterFLP {
	 ...
	 */
	static void SKP_Silk_prefilter_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
																		 * I/O
																		 * Encoder
																		 * state
																		 * FLP
		...
		float[] B = new float[2];
		float[] AR1_shp = new float[Define.NB_SUBFR
		...
		float[] st_res = new float[Define.MAX_FRAME_LENGTH / Define.NB_SUBFR
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			...
			HarmShapeGain = psEncCtrl.HarmShapeGain[k]
					* (1.0f - psEncCtrl.HarmBoost[k]);
			HarmShapeFIR[0] = TablesOtherFLP.SKP_Silk_HarmShapeFIR_FLP[0]
					* HarmShapeGain;
			HarmShapeFIR[1] = TablesOtherFLP.SKP_Silk_HarmShapeFIR_FLP[1]
					* HarmShapeGain;
			HarmShapeFIR[2] = TablesOtherFLP.SKP_Silk_HarmShapeFIR_FLP[2]
					* HarmShapeGain;
			Tilt = psEncCtrl.Tilt[k];
			LF_MA_shp = psEncCtrl.LF_MA_shp[k];
			LF_AR_shp = psEncCtrl.LF_AR_shp[k];
			// TODO: copy the psEncCtrl.AR1 to a local buffer or use a
			// reference(pointer) to the struct???
			...
			System.arraycopy(psEncCtrl.AR1, k * Define.SHAPE_LPC_ORDER_MAX,
					AR1_shp, 0, psEncCtrl.AR1.length - k
							* Define.SHAPE_LPC_ORDER_MAX);

			/* Short term FIR filtering */
			LPCAnalysisFilterFLP.SKP_Silk_LPC_analysis_filter_FLP(st_res,
					AR1_shp, px, px_offset - psEnc.sCmn.shapingLPCOrder,
					psEnc.sCmn.subfr_length + psEnc.sCmn.shapingLPCOrder,
					psEnc.sCmn.shapingLPCOrder);

			pst_res = st_res;
			pst_res_offset = psEnc.sCmn.shapingLPCOrder; // Point to first
															// sample

			/* reduce (mainly) low frequencies during harmonic emphasis */
			B[0] = psEncCtrl.GainsPre[k];
			B[1] = -psEncCtrl.GainsPre[k]
					* (psEncCtrl.HarmBoost[k] * HarmShapeGain
							+ PerceptualParametersFLP.INPUT_TILT + psEncCtrl.coding_quality
							* PerceptualParametersFLP.HIGH_RATE_INPUT_TILT);
			pxw[pxw_offset + 0] = B[0] * pst_res[pst_res_offset + 0] + B[1]
					* P.sHarmHP;
			for (j = 1; j < psEnc.sCmn.subfr_length; j++) {
				pxw[pxw_offset + j] = B[0] * pst_res[pst_res_offset + j] + B[1]
						* pst_res[pst_res_offset + j - 1];
			}
			P.sHarmHP = pst_res[pst_res_offset + psEnc.sCmn.subfr_length - 1];

			SKP_Silk_prefilt_FLP(P, pxw, pxw_offset, pxw, pxw_offset,
					HarmShapeFIR, Tilt, LF_MA_shp, LF_AR_shp, lag,
	 ...
	 * @param length
	 */
	static void SKP_Silk_prefilt_FLP(SKP_Silk_prefilter_state_FLP P,/*
																	 * (I/O)
																	 * state
																	 */
			float st_res[], /* (I) */
			int st_res_offset, float xw[], /* (O) */
			int xw_offset, float[] HarmShapeFIR, /* (I) */
			float Tilt, /* (I) */
			float LF_MA_shp, /* (I) */
			float LF_AR_shp, /* (I) */
			int lag, /* (I) */
		...
		for (i = 0; i < length; i++) {
			if (lag > 0) {
				...
				n_LTP = LTP_shp_buf[(idx - Define.HARM_SHAPE_FIR_TAPS / 2 - 1)
						& Define.LTP_MASK]
						* HarmShapeFIR[0];
				n_LTP += LTP_shp_buf[(idx - Define.HARM_SHAPE_FIR_TAPS / 2)
						& Define.LTP_MASK]
						* HarmShapeFIR[1];
				n_LTP += LTP_shp_buf[(idx - Define.HARM_SHAPE_FIR_TAPS / 2 + 1)
						& Define.LTP_MASK]
