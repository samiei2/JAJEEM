...
public class GainQuant {
	...
	static void SKP_Silk_gains_quant(int ind[], /* O gain indices */
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			/*
			 * Add half of previous quantization error, convert to log scale,
			 * scale, floor()
			 */
			ind[k] = Macros.SKP_SMULWB(SCALE_Q16,
					Lin2log.SKP_Silk_lin2log(gain_Q16[k]) - OFFSET);
			...
			if (k == 0 && conditional == 0) {
				/* Full index */
				ind[k] = SigProcFIX.SKP_LIMIT_int(ind[k], 0,
						Define.N_LEVELS_QGAIN - 1);
				ind[k] = Math.max(ind[k], prev_ind[0]
						+ Define.MIN_DELTA_GAIN_QUANT);
				prev_ind[0] = ind[k];
			} else {
				/* Delta index */
				ind[k] = SigProcFIX.SKP_LIMIT_int(ind[k] - prev_ind[0],
						Define.MIN_DELTA_GAIN_QUANT,
			...
			gain_Q16[k] = Log2lin.SKP_Silk_log2lin(Math.min(
					Macros.SKP_SMULWB(INV_SCALE_Q16, prev_ind[0]) + OFFSET,
					3967)); /* 3967 = 31 in Q7 */
		}
	}
