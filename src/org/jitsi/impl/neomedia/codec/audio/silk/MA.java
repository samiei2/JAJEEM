...
public class MA {
	...
	static void SKP_Silk_MA_Prediction(short[] in, /* I: Input signal */
			int in_offset, short[] B, /*
									 * I: MA prediction coefficients, Q12
									 * [order]
									 */
		...
		for (k = 0; k < len; k++) {
			...
			for (d = 0; d < order - 1; d++) {
				S[d] = SigProcFIX.SKP_SMLABB_ovflw(S[d + 1], in16, B[B_offset
						+ d]);
			}
	...
	static void SKP_Silk_MA_Prediction_Q13(short[] in, /* I: input signal */
			int in_offset, short[] B, /*
									 * I: MA prediction coefficients, Q13
									 * [order]
									 */
			int[] S, /* I/O: state vector [order] */
			short[] out, /* O: output signal */
	...
	static void SKP_Silk_LPC_analysis_filter(short[] in, /* I: Input signal */
		...
		for (k = 0; k < len; k++) {
			...
			/* Subtract prediction */
			out32_Q12 = Macros.SKP_SUB_SAT32((in[in_offset + k] << 12),
					out32_Q12);
