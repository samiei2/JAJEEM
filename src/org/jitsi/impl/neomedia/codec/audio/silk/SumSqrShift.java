...
/**
 * compute number of bits to right shift the sum of squares of a vector of
 * int16s to make it fit in an int32
 * 
 * @author Jing Dai
 * @author Dingxin Xu
 */
public class SumSqrShift {
	...
	static void SKP_Silk_sum_sqr_shift(int[] energy, /*
			...
			int x_offset, int len /* I Length of input vector */
	) {
		int i, shft;
		...
		/*
		 * Make sure to have at least one extra leading zero (two leading zeros
		 * in total)
		 */
		if ((nrg & 0xC0000000) != 0) {
			nrg = (nrg >>> 2);
