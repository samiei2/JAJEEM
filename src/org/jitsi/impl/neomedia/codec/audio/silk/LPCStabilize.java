	...
	static void SKP_Silk_LPC_stabilize(short[] a_Q12, /*
		...
		while (true) {
			...
			if (maxabs >= LPC_STABILIZE_LPC_MAX_ABS_VALUE_Q16) {
				...
				sc_Q16 = (sc_Q16 / (idx + 1));
				sc_Q16 = 65536 - sc_Q16;
				sc_Q16 = (Macros.SKP_SMULWB(sc_Q16, 32604) << 1); // 0.995 in
																	// Q16
		...
		while (LPCInvPredGain.SKP_Silk_LPC_inverse_pred_gain(invGain_Q30_ptr,
				a_Q12, L) == 1) {
	...
	static void SKP_Silk_LPC_fit(short[] a_QQ, /*
		...
		while (true) {
			...
			if (maxabs >= Short.MAX_VALUE) {
				/* Reduce magnitude of prediction coefficients */
				maxabs = Math.min(maxabs, 98369); // ( SKP_int32_MAX / ( 65470
													// >> 2 ) ) + SKP_int16_MAX
													// = 98369
