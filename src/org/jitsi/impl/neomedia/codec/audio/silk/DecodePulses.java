	...
	static void SKP_Silk_decode_pulses(SKP_Silk_range_coder_state psRC, /*
		...
		RangeCoder
				.SKP_Silk_range_decoder(
						...
						0,
						psRC,
						TablesPulsesPerBlock.SKP_Silk_rate_levels_CDF[psDecCtrl.sigtype],
						0, TablesPulsesPerBlock.SKP_Silk_rate_levels_CDF_offset);
		...
		for (i = 0; i < iter; i++) {
			if (sum_pulses[i] > 0) {
				ShellCoder.SKP_Silk_shell_decoder(q,
						Macros.SKP_SMULBB(i, Define.SHELL_CODEC_FRAME_LENGTH),
						psRC, sum_pulses[i]);
			} else {
				Arrays.fill(
						q,
						(Macros.SKP_SMULBB(i, Define.SHELL_CODEC_FRAME_LENGTH)),
						((Macros.SKP_SMULBB(i, Define.SHELL_CODEC_FRAME_LENGTH)) + Define.SHELL_CODEC_FRAME_LENGTH),
						0);
			}
		}
		...
		for (i = 0; i < iter; i++) {
			if (nLshifts[i] > 0) {
				nLS = nLshifts[i];
				pulses_ptr = q;
				pulses_ptr_offset = Macros.SKP_SMULBB(i,
		...
		/****************************************/
		CodeSigns.SKP_Silk_decode_signs(psRC, q, frame_length,
