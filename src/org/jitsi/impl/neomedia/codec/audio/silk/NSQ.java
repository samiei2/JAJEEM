...
public class NSQ {
	 ...
	 *            LTP state scaling
	 */
	static void SKP_Silk_NSQ(SKP_Silk_encoder_state psEncC, /* I/O Encoder State */
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			...
			if (psEncCtrlC.sigtype == Define.SIG_TYPE_VOICED) {
				...
				if ((k & (3 - (LSF_interpolation_flag << 1))) == 0) {
					...
					MA.SKP_Silk_MA_Prediction(NSQ.xq, start_idx + k
							* (psEncC.frame_length >> 2), A_Q12, A_Q12_offset,
			...
			SKP_Silk_nsq_scale_states(NSQ, x_tmp, x_tmp_offset, x_sc_Q10,
					psEncC.subfr_length, sLTP, sLTP_Q16, k, LTP_scale_Q14,
					Gains_Q16, psEncCtrlC.pitchL);

			SKP_Silk_noise_shape_quantizer(NSQ, psEncCtrlC.sigtype, x_sc_Q10,
					q_tmp, q_tmp_offset, pxq, pxq_offset, sLTP_Q16, A_Q12,
					A_Q12_offset, B_Q14, B_Q14_offset, AR_shp_Q13,
					AR_shp_Q13_offset, lag, HarmShapeFIRPacked_Q14,
					Tilt_Q14[k], LF_shp_Q14[k], Gains_Q16[k], Lambda_Q10,
	...
	static void SKP_Silk_noise_shape_quantizer(SKP_Silk_nsq_state NSQ, /*
		...
		for (i = 0; i < length; i++) {
			...
			// SKP_assert( ( (SKP_int64)a_Q12 & 3 ) == 0 ); /* check that array
			// starts at 4-byte aligned address */
			assert (predictLPCOrder >= 10); /* check that unrolling works */
			/* Partially unrolled */
			LPC_pred_Q10 = Macros.SKP_SMULWB(psLPC_Q14[psLPC_Q14_offset + 0],
					a_Q12[a_Q12_offset + 0]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 1], a_Q12[a_Q12_offset + 1]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 2], a_Q12[a_Q12_offset + 2]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 3], a_Q12[a_Q12_offset + 3]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 4], a_Q12[a_Q12_offset + 4]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 5], a_Q12[a_Q12_offset + 5]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 6], a_Q12[a_Q12_offset + 6]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 7], a_Q12[a_Q12_offset + 7]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 8], a_Q12[a_Q12_offset + 8]);
			LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
					psLPC_Q14[psLPC_Q14_offset - 9], a_Q12[a_Q12_offset + 9]);

			for (j = 10; j < predictLPCOrder; j++) {
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - j],
						a_Q12[a_Q12_offset + j]);
			}
			...
			// SKP_assert( ( (SKP_int64)AR_shp_Q13 & 3 ) == 0 ); /* check that
			...
			n_AR_Q10 = Macros
					.SKP_SMLAWB(n_AR_Q10, NSQ.sLF_AR_shp_Q12, Tilt_Q14);

			n_LF_Q10 = (Macros.SKP_SMULWB(
					NSQ.sLTP_shp_Q10[NSQ.sLTP_shp_buf_idx - 1], LF_shp_Q14) << 2);
			n_LF_Q10 = Macros.SKP_SMLAWT(n_LF_Q10, NSQ.sLF_AR_shp_Q12,
					LF_shp_Q14);
			...
			/* Quantize */
			if (r_Q10 < thr1_Q10) {
				q_Q0 = SigProcFIX.SKP_RSHIFT_ROUND((r_Q10 + (Lambda_Q10 >> 1)),
						10);
				q_Q10 = (q_Q0 << 10);
			...
			q[q_offset + i] = (byte) q_Q0; /*
											 * No saturation needed because max
											 * is 64
											 */
			...
			/* Add predictions */
			LPC_exc_Q10 = (exc_Q10 + SigProcFIX.SKP_RSHIFT_ROUND(LTP_pred_Q14,
					4));
			xq_Q10 = (LPC_exc_Q10 + LPC_pred_Q10);

			/* Scale XQ back to normal level before saving */
			xq[xq_offset + i] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
					.SKP_RSHIFT_ROUND(Macros.SKP_SMULWW(xq_Q10, Gain_Q16), 10));
		...
		System.arraycopy(NSQ.sLPC_Q14, length, NSQ.sLPC_Q14, 0,
				Define.MAX_LPC_ORDER);
	}
	 ...
	 * @param pitchL
	 */
	static void SKP_Silk_nsq_scale_states(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
			final short x[], /* I input in Q0 */
			int x_offset, int x_sc_Q10[], /* O input scaled with 1/Gain */
		...
		scale_length = scale_length
				- Macros.SKP_SMULBB(Define.NB_SUBFR - (subfr + 1), length); /*
																			 ...
																			 * frame
																			 */
		scale_length = SigProcFIX.SKP_max_int(scale_length, lag
				+ Define.LTP_ORDER); /*
									 * make sure to scale whole pitch period if
									 * voiced
									 */
		...
		if (inv_gain_Q16 != NSQ.prev_inv_gain_Q16) {
			...
			NSQ.sLF_AR_shp_Q12 = Macros.SKP_SMULWW(gain_adj_Q16,
					NSQ.sLF_AR_shp_Q12);

			/* scale short term state */
			for (i = 0; i < Define.MAX_LPC_ORDER; i++) {
				NSQ.sLPC_Q14[i] = Macros.SKP_SMULWW(gain_adj_Q16,
						NSQ.sLPC_Q14[i]);
			}
		}
