...
public class LTPAnalysisFilterFLP {
	 ...
	 */
	static void SKP_Silk_LTP_analysis_filter_FLP(float[] LTP_res, /*
																 * O LTP res
																 * NB_SUBFR
																 * *(pre_lgth
																 * +subfr_lngth)
			...
			int x_offset, final float B[], /*
											 * I LTP coefficients for each
											 * subframe
											 */
			...
			for (i = 0; i < subfr_length + pre_length; i++) {
				...
				for (j = 0; j < Define.LTP_ORDER; j++) {
					LTP_res_ptr[LTP_res_ptr_offset + i] -= Btmp[j]
							* x_lag_ptr[x_lag_ptr_offset + Define.LTP_ORDER / 2
									- j];
				}
