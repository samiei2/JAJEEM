...
public class LPCSynthesisFilter {
	 ...
	 */
	static void SKP_Silk_LPC_synthesis_filter(short[] in, /* I: excitation signal */
			short[] A_Q12, /*
							 * I: AR coefficients [Order], between -8_Q0 and
							 * 8_Q0
							 */
		...
		for (k = 0; k < len; k++) {
			...
			for (j = 0; j < (Order_half - 1); j++) {
				...
				out32_Q10 = Macros.SKP_SMLAWB(out32_Q10, SB,
						A_Q12[(j << 1) + 1]);
				SA = S[Order - 2 - idx];
				S[Order - 2 - idx] = SB;
			}
			...
			out32_Q10 = Macros.SKP_ADD_SAT32(out32_Q10,
					Macros.SKP_SMULWB(Gain_Q26, in[k]));
