...
public class ResamplerUp2 {
	 ...
	 */
	static void SKP_Silk_resampler_up2(int[] S, /* I/O: State vector [ 2 ] */
		...
		for (k = 0; k < len; k++) {
			...
			out[out_offset + 2 * k] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
					.SKP_RSHIFT_ROUND(out32, 10));

			/* All-pass section for odd output sample */
			Y = in32 - S[S_offset + 1];
			X = Macros.SKP_SMLAWB(Y, Y,
					ResamplerRom.SKP_Silk_resampler_up2_lq_1);
			out32 = S[S_offset + 1] + X;
			S[S_offset + 1] = in32 + X;

			/* Convert back to int16 and store to output */
			out[out_offset + 2 * k + 1] = (short) SigProcFIX
					.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(out32, 10));
		}
	}
}
