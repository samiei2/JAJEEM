...
public class LPVariableCutoff {
	 ...
	 * @param fac_Q16
	 */
	static void SKP_Silk_LP_interpolate_filter_taps(int[] B_Q28, int[] A_Q28,
		...
		if (ind < Define.TRANSITION_INT_NUM - 1) {
			if (fac_Q16 > 0) {
				if (fac_Q16 == SigProcFIX.SKP_SAT16(fac_Q16)) { /*
					...
					assert (((1 << 16) - fac_Q16) == SigProcFIX
	...
	/**
	 * Low-pass filter with variable cutoff frequency based on piece-wise linear
	 * interpolation between elliptic filters Start by setting
	 * psEncC->transition_frame_no = 1; Deactivate by setting
	 * psEncC->transition_frame_no = 0;
	 * 
	 * @param psLP
	 *            LP filter state
	 * @param out
	...
	static void SKP_Silk_LP_variable_cutoff(SKP_Silk_LP_state psLP, /*
			...
			int in_offset, final int frame_length /* I Frame length */
	) {
		...
		if (psLP.transition_frame_no > 0) {
			if (psLP.mode == 0) {
				if (psLP.transition_frame_no < Define.TRANSITION_FRAMES_DOWN) {
					/*
					 * Calculate index and interpolation factor for
					 * interpolation
					 */
					if (Define.TRANSITION_INT_STEPS_DOWN == 32) {
						fac_Q16 = psLP.transition_frame_no << (16 - 5);
					} else {
						fac_Q16 = (psLP.transition_frame_no << 16)
								/ Define.TRANSITION_INT_STEPS_DOWN;
					}
					...
					SKP_Silk_LP_interpolate_filter_taps(B_Q28, A_Q28, ind,
							fac_Q16);
					...
					SKP_Silk_LP_interpolate_filter_taps(B_Q28, A_Q28,
							Define.TRANSITION_INT_NUM - 1, 0);
				}
			} else if (psLP.mode == 1) {
				if (psLP.transition_frame_no < Define.TRANSITION_FRAMES_UP) {
					...
					if (Define.TRANSITION_INT_STEPS_UP == 64) {
						...
						fac_Q16 = ((Define.TRANSITION_FRAMES_UP - psLP.transition_frame_no) << 16)
		...
		if (psLP.transition_frame_no > 0) {
			...
			BiquadAlt.SKP_Silk_biquad_alt(in, in_offset, B_Q28, A_Q28,
					psLP.In_LP_State, out, out_offset, frame_length);
		} else {
