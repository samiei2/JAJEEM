...
public class ProcessNLSFsFLP {
	 ...
	 */
	static void SKP_Silk_process_NLSFs_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
																			 * I/
																			 * O
																			 * Encoder
																			 * state
		...
		NLSFVQWeightsLaroiaFLP.SKP_Silk_NLSF_VQ_weights_laroia_FLP(pNLSFW,
				pNLSF, psEnc.sCmn.predictLPCOrder);
		...
		if (doInterpolate) {
			...
			WrappersFLP.SKP_Silk_interpolate_wrapper_FLP(pNLSF0_temp,
					psEnc.sPred.prev_NLSFq, pNLSF, NLSF_interpolation_factor,
					psEnc.sCmn.predictLPCOrder);

			/* Calculate first half NLSF weights for the interpolated NLSFs */
			NLSFVQWeightsLaroiaFLP.SKP_Silk_NLSF_VQ_weights_laroia_FLP(
					pNLSFW0_temp, pNLSF0_temp, psEnc.sCmn.predictLPCOrder);
		...
		/*
		 * Set pointer to the NLSF codebook for the current signal type and LPC
		 * order
		 */
		psNLSF_CB_FLP = psEnc.psNLSF_CB_FLP[psEncCtrl.sCmn.sigtype];

		/* Quantize NLSF parameters given the trained NLSF codebooks */
		NLSFMSVQEncodeFLP.SKP_Silk_NLSF_MSVQ_encode_FLP(
				psEncCtrl.sCmn.NLSFIndices, pNLSF, psNLSF_CB_FLP,
				psEnc.sPred.prev_NLSFq, pNLSFW, NLSF_mu, NLSF_mu_fluc_red,
		...
		if (doInterpolate) {
			/*
			 * Calculate the interpolated, quantized NLSF vector for the first
			 * half
			 */
			WrappersFLP.SKP_Silk_interpolate_wrapper_FLP(pNLSF0_temp,
					psEnc.sPred.prev_NLSFq, pNLSF, NLSF_interpolation_factor,
			...
			WrappersFLP.SKP_Silk_NLSF2A_stable_FLP(psEncCtrl.PredCoef[0],
					pNLSF0_temp, psEnc.sCmn.predictLPCOrder);

		} else {
			/* Copy LPC coefficients for first half from second half */
			System.arraycopy(psEncCtrl.PredCoef[1], 0, psEncCtrl.PredCoef[0],
					0, psEnc.sCmn.predictLPCOrder);
		}
	}
}
