...
public class NLSFMSVQDecodeFLP {
	 ...
	 * @param LPC_order
	 */
	static void SKP_Silk_NLSF_MSVQ_decode_FLP(float[] pNLSF, /*
															 * O Decoded output
															 * vector [
															 * LPC_ORDER ]
															 */
		...
		for (s = 1; s < psNLSF_CB_FLP.nStages; s++) {
			/* Check that each index is within valid range */
			assert (0 <= NLSFIndices[NLSFIndices_offset + s] && NLSFIndices[NLSFIndices_offset
					+ s] < psNLSF_CB_FLP.CBStages[s].nVectors);
			...
			if (LPC_order == 16) {
				...
				/* Point to the first vector element */
				pCB_element = psNLSF_CB_FLP.CBStages[s].CB;
				pCB_element_offset = NLSFIndices[NLSFIndices_offset + s]
						* LPC_order;
		...
		/* NLSF stabilization */
		WrappersFLP.SKP_Silk_NLSF_stabilize_FLP(pNLSF, psNLSF_CB_FLP.NDeltaMin,
				LPC_order);
	}
}
