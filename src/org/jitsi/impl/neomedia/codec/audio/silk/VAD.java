...
public class VAD {
	...
	SKP_Silk_VAD_state psSilk_VAD /* I/O Pointer to Silk VAD state */
		...
		/* init noise levels */
		/*
		 * Initialize array with approx pink noise levels (psd proportional to
		 * inverse of frequency)
		 */
		for (b = 0; b < Define.VAD_N_BANDS; b++) {
			psSilk_VAD.NoiseLevelBias[b] = Math.max(
					Define.VAD_NOISE_LEVELS_BIAS / (b + 1), 1);
		}
	...
	SKP_Silk_VAD_state psSilk_VAD, /* I/O Silk VAD state */
		...
		AnaFiltBank1.SKP_Silk_ana_filt_bank_1(pIn, pIn_offset,
				psSilk_VAD.AnaState, 0, X[0], 0, X[3], 0, scratch, framelength);

		/* 0-4 kHz to 0-2 kHz and 2-4 kHz */
		AnaFiltBank1.SKP_Silk_ana_filt_bank_1(X[0], 0, psSilk_VAD.AnaState1, 0,
				X[0], 0, X[2], 0, scratch, framelength >> 1);

		/* 0-2 kHz to 0-1 kHz and 1-2 kHz */
		AnaFiltBank1.SKP_Silk_ana_filt_bank_1(X[0], 0, psSilk_VAD.AnaState2, 0,
				X[0], 0, X[1], 0, scratch, framelength >> 2);
		...
		for (b = 0; b < Define.VAD_N_BANDS; b++) {
			...
			decimated_framelength = framelength >> Math.min(Define.VAD_N_BANDS
					- b, Define.VAD_N_BANDS - 1);
			...
			for (s = 0; s < Define.VAD_INTERNAL_SUBFRAMES; s++) {
				...
				for (i = 0; i < dec_subframe_length; i++) {
					/*
					 * The energy will be less than dec_subframe_length * (
					 * SKP_int16_MIN / 8 )^2.
					 */
					/*
					 * Therefore we can accumulate with no risk of overflow
					 * (unless dec_subframe_length > 128)
					 */
				...
				if (s < Define.VAD_INTERNAL_SUBFRAMES - 1) {
					...
					/* look-ahead subframe */
					Xnrg[b] = SigProcFIX.SKP_ADD_POS_SAT32(Xnrg[b],
							sumSquared >> 1);
				}
		...
		for (b = 0; b < Define.VAD_N_BANDS; b++) {
			...
			if (speech_nrg > 0) {
				...
				if ((Xnrg[b] & 0xFF800000) == 0) {
					NrgToNoiseRatio_Q8[b] = (Xnrg[b] << 8)
							/ (psSilk_VAD.NL[b] + 1);
				} else {
					NrgToNoiseRatio_Q8[b] = Xnrg[b]
							/ ((psSilk_VAD.NL[b] >> 8) + 1);
				}
				...
				if (speech_nrg < (1 << 20)) {
					...
					SNR_Q7 = Macros.SKP_SMULWB(
							Inlines.SKP_Silk_SQRT_APPROX(speech_nrg) << 6,
							SNR_Q7);
				}
				input_tilt = Macros.SKP_SMLAWB(input_tilt, tiltWeights[b],
						SNR_Q7);
			} else {
		...
		/*
		 * Root-mean-square approximation, scale to dBs, and write to output
		 * pointer
		 */
		...
		/*********************************/
		SA_Q15 = SigmQ15.SKP_Silk_sigm_Q15(Macros.SKP_SMULWB(
				Define.VAD_SNR_FACTOR_Q16, pSNR_dB_Q7[0])
		...
		smooth_coef_Q16 = Macros.SKP_SMULWB(Define.VAD_SNR_SMOOTH_COEF_Q18,
				Macros.SKP_SMULWB(SA_Q15, SA_Q15));
		for (b = 0; b < Define.VAD_N_BANDS; b++) {
			/* compute smoothed energy-to-noise ratio per band */
			psSilk_VAD.NrgRatioSmth_Q8[b] = Macros.SKP_SMLAWB(
					psSilk_VAD.NrgRatioSmth_Q8[b], NrgToNoiseRatio_Q8[b]
							- psSilk_VAD.NrgRatioSmth_Q8[b], smooth_coef_Q16);

			/* signal to noise ratio in dB per band */
			SNR_Q7 = 3 * (Lin2log
					.SKP_Silk_lin2log(psSilk_VAD.NrgRatioSmth_Q8[b]) - 8 * 128);
			/* quality = sigmoid( 0.25 * ( SNR_dB - 16 ) ); */
			pQuality_Q15[b] = SigmQ15
					.SKP_Silk_sigm_Q15((SNR_Q7 - 16 * 128) >> 4);
		}
	...
	static void SKP_Silk_VAD_GetNoiseLevels(int[] pX, /* I subband energies */
		...
		for (k = 0; k < Define.VAD_N_BANDS; k++) {
			...
			/* Add bias */
			nrg = SigProcFIX.SKP_ADD_POS_SAT32(pX[k],
					psSilk_VAD.NoiseLevelBias[k]);
			assert (nrg > 0);
			...
			if (nrg > nl << 3) {
				...
				coef = Macros.SKP_SMULWB(Macros.SKP_SMULWW(inv_nrg, nl),
						Define.VAD_NOISE_LEVEL_SMOOTH_COEF_Q16 << 1);
			}
			...
			psSilk_VAD.inv_NL[k] = Macros.SKP_SMLAWB(psSilk_VAD.inv_NL[k],
