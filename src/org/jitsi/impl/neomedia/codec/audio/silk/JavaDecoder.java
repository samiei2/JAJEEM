...
public class JavaDecoder extends AbstractCodec2 {
	...
	/**
	 * Temporary buffer used when decoding FEC. Defined here to avoid using
	...
	/**
	 * The <tt>Logger</tt> used by this <tt>JavaDecoder</tt> instance for
	 * logging output.
	 */
	...
	protected void doClose() {
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Packets lost (subsequent missing):" + nbPacketsLost);
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if (decodeFEC) /* Decode with FEC. */
		{
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Packet loss detected. Last seen " + lastSeqNo
						+ ", current " + seqNo);
				logger.trace("Looking for FEC data, found " + lbrrBytes[0]
						+ " bytes");
			}
			...
			if (lbrrBytes[0] == 0) {
				...
				lostFlag = 1;
			} else if (DecAPI.SKP_Silk_SDK_Decode(decState, decControl, 0,
					lbrrData, 0, lbrrBytes[0], out, outOffset, outputLength) == 0) {
		...
		{
			outputLength[0] = frameLength;
			if (DecAPI.SKP_Silk_SDK_Decode(decState, decControl, lostFlag, in,
					inOffset, inLength, out, outOffset, outputLength) == 0) {
				outBuffer.setDuration(FRAME_DURATION * 1000000);
				outBuffer.setLength(outputLength[0]);
				outBuffer.setOffset(outOffset);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		return JavaEncoder.getMatchingOutputFormats(inputFormat,
