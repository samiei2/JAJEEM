...
public class LTPScaleCtrlFLP {
	 ...
	 */
	static final float LTPScaleThresholds[] = { 0.95f, 0.8f, 0.50f, 0.400f,
			0.3f, 0.2f, 0.15f, 0.1f, 0.08f, 0.075f, 0.0f };
	...
	static void SKP_Silk_LTP_scale_ctrl_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
		...
		// g_HP(n) = g(n) - g(n-1) + 0.5 * g_HP(n-1); // tune the 0.5: higher
		// means longer impact of jump
		psEnc.HPLTPredCodGain = Math.max(psEncCtrl.LTPredCodGain
				- psEnc.prevLTPredCodGain, 0.0f)
				+ 0.5f * psEnc.HPLTPredCodGain;

		psEnc.prevLTPredCodGain = psEncCtrl.LTPredCodGain;

		/* combine input and filtered input */
		g_out = 0.5f * psEncCtrl.LTPredCodGain + (1.0f - 0.5f)
				* psEnc.HPLTPredCodGain;
		...
		if (psEnc.sCmn.nFramesInPayloadBuf == 0) {
			...
			frames_per_packet = psEnc.sCmn.PacketSize_ms
					/ Define.FRAME_LENGTH_MS;

			round_loss += (frames_per_packet - 1);
			// thrld1 = LTPScaleThresholds[ Math.min( round_loss, NB_THRESHOLDS
			// - 1 ) ];
			// thrld2 = LTPScaleThresholds[ Math.min( round_loss + 1,
			// NB_THRESHOLDS - 1 ) ];
			thrld1 = LTPScaleThresholds[round_loss < (NB_THRESHOLDS - 1) ? round_loss
					: (NB_THRESHOLDS - 1)];
			thrld2 = LTPScaleThresholds[(round_loss + 1) < (NB_THRESHOLDS - 1) ? (round_loss + 1)
					: (NB_THRESHOLDS - 1)];
