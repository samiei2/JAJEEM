...
/**
 * Calculates the reflection coefficients from the input vector Input vector
 * contains nb_subfr sub vectors of length L_sub + D
 * 
 * @author Dingxin Xu
 */
public class BurgModifiedFLP {
	static final int MAX_FRAME_SIZE = 544; // subfr_length * nb_subfr = ( 0.005
											// * 24000 + 16 ) * 4 = 544
	...
	float A[], /* O prediction coefficients (length order) */
			...
			int x_offset, final int subfr_length, /*
												 * I input signal subframe
												 * length (including D
												 * preceeding samples)
												 */
			final int nb_subfr, /* I number of subframes stacked in x */
			final float WhiteNoiseFrac, /*
										 * I fraction added to zero-lag
										 * autocorrelation
										 */
			final int D /* I order */
	) {
		...
		float[] x_ptr;
		int x_ptr_offset;
		double C_first_row[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC], C_last_row[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC];
		double CAf[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC + 1], CAb[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC + 1];
		double Af[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC];
		...
		C0 = EnergyFLP
				.SKP_Silk_energy_FLP(x, x_offset, nb_subfr * subfr_length);
		for (s = 0; s < nb_subfr; s++) {
			x_ptr = x;
			x_ptr_offset = x_offset + s * subfr_length;
			for (n = 1; n < D + 1; n++) {
				C_first_row[n - 1] += InnerProductFLP
						.SKP_Silk_inner_product_FLP(x_ptr, x_ptr_offset, x_ptr,
								x_ptr_offset + n, subfr_length - n);
			}
		}
		System.arraycopy(C_first_row, 0, C_last_row, 0,
				SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC);
		...
		for (n = 0; n < D; n++) {
			...
			/*
			 * Update last row of correlation matrix (without last element,
			...
			for (s = 0; s < nb_subfr; s++) {
				...
				for (k = 0; k < n; k++) {
					C_first_row[k] -= x_ptr[x_ptr_offset + n]
							* x_ptr[x_ptr_offset + n - k - 1];
				...
				for (k = 0; k <= n; k++) {
					...
					CAb[k] -= tmp2
							* x_ptr[x_ptr_offset + subfr_length - n + k - 1];
				}
			}
			tmp1 = C_first_row[n];
			tmp2 = C_last_row[n];
