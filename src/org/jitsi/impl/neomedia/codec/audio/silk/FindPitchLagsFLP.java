...
public class FindPitchLagsFLP {
	 ...
	 */
	static void SKP_Silk_find_pitch_lags_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
																				 * I
																				 * /
																				 * O
																				 * Encoder
		...
		x_buf_ptr = x_buf;
		x_buf_ptr_offset = x_buf_offset + buf_len
				- psPredSt.pitch_LPC_win_length;
		Wsig_ptr = Wsig;
		Wsig_ptr_offset = 0;
		ApplySineWindowFLP.SKP_Silk_apply_sine_window_FLP(Wsig_ptr,
				Wsig_ptr_offset, x_buf_ptr, x_buf_ptr_offset, 1,
				psEnc.sCmn.la_pitch);
		...
		// psEnc->sCmn.la_pitch << 1 ) ) * sizeof( SKP_float ) );
		for (int i_djinn = 0; i_djinn < psPredSt.pitch_LPC_win_length
				- (psEnc.sCmn.la_pitch << 1); i_djinn++) {
			Wsig_ptr[Wsig_ptr_offset + i_djinn] = x_buf_ptr[x_buf_ptr_offset
					+ i_djinn];
		}

		/* Last LA_LTP samples */
		Wsig_ptr_offset += psPredSt.pitch_LPC_win_length
				- (psEnc.sCmn.la_pitch << 1);
		x_buf_ptr_offset += psPredSt.pitch_LPC_win_length
				- (psEnc.sCmn.la_pitch << 1);
		ApplySineWindowFLP.SKP_Silk_apply_sine_window_FLP(Wsig_ptr,
				Wsig_ptr_offset, x_buf_ptr, x_buf_ptr_offset, 2,
				psEnc.sCmn.la_pitch);
		...
		auto_corr[0] += auto_corr[0]
				* DefineFLP.FIND_PITCH_WHITE_NOISE_FRACTION;

		/* Calculate the reflection coefficients using Schur */
		SchurFLP.SKP_Silk_schur_FLP(refl_coef, 0, auto_corr, 0,
				psEnc.sCmn.pitchEstimationLPCOrder);

		/* Convert reflection coefficients to prediction coefficients */
		K2aFLP.SKP_Silk_k2a_FLP(A, refl_coef,
				psEnc.sCmn.pitchEstimationLPCOrder);

		/* Bandwidth expansion */
		BwexpanderFLP.SKP_Silk_bwexpander_FLP(A, 0,
				psEnc.sCmn.pitchEstimationLPCOrder,
		...
		/*****************************************/
		LPCAnalysisFilterFLP.SKP_Silk_LPC_analysis_filter_FLP(res, A, x_buf,
				x_buf_offset, buf_len, psEnc.sCmn.pitchEstimationLPCOrder);
		// SKP_memset( res, 0, psEnc->sCmn.pitchEstimationLPCOrder * sizeof(
		// SKP_float ) );
		...
		psEncCtrl.sCmn.sigtype = PitchAnalysisCoreFLP
				.SKP_Silk_pitch_analysis_core_FLP(res, psEncCtrl.sCmn.pitchL,
						lagIndex_djinnaddress, contourIndex_djinnaddress,
						LTPCorr_djinnaddress, psEnc.sCmn.prevLag,
						psEnc.pitchEstimationThreshold, thrhld,
