...
/**
 * Compute inverse of LPC prediction gain, and test if LPC coefficients are
 * stable (all poles within unit circle)
 * 
 * @author Jing Dai
 * @author Dingxin Xu
 */
public class LPCInvPredGain {
	...
	int[] invGain_Q30, /* O: Inverse prediction gain, Q30 energy domain */
		...
		int[][] Atmp_QA = new int[2][SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC];
		int tmp_QA;
		int[] Aold_QA, Anew_QA;
		...
		for (k = order - 1; k > 0; k--) {
			...
			rc_mult1_Q30 = (Typedef.SKP_int32_MAX >> 1)
					- SigProcFIX.SKP_SMMUL(rc_Q31, rc_Q31);
			Typedef.SKP_assert(rc_mult1_Q30 > (1 << 15)); /*
														 * reduce A_LIMIT if
														 * fails
														 */
			...
			rc_mult2_Q16 = Inlines.SKP_INVERSE32_varQ(rc_mult1_Q30, 46); /*
																		 * 16 =
																		 * 46 -
																		 * 30
																		 */
			...
			invGain_Q30[0] = (SigProcFIX
					.SKP_SMMUL(invGain_Q30[0], rc_mult1_Q30) << 2);
			...
			for (n = 0; n < k; n++) {
				tmp_QA = Aold_QA[n]
						- (SigProcFIX.SKP_SMMUL(Aold_QA[k - n - 1], rc_Q31) << 1);
	 ...
	 * @return Returns 1 if unstable, otherwise 0.
	 */
	static int SKP_Silk_LPC_inverse_pred_gain_Q13( /*
													 * O: Returns 1 if unstable,
													 * otherwise 0
													 */
	int[] invGain_Q30, /* O: Inverse prediction gain, Q30 energy domain */
