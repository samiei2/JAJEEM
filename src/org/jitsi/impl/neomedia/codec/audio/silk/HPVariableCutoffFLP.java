...
public class HPVariableCutoffFLP {
	...
	static void SKP_Silk_HP_variable_cutoff_FLP(
		...
		if (psEnc.sCmn.prev_sigtype == Define.SIG_TYPE_VOICED) {
			...
			pitch_freq_log -= quality
					* quality
					* (pitch_freq_log - MainFLP
							.SKP_Silk_log2(DefineFLP.VARIABLE_HP_MIN_FREQ));
			pitch_freq_log += 0.5f * (0.6f - quality);

			delta_freq = pitch_freq_log - psEnc.variable_HP_smth1;
			if (delta_freq < 0.0) {
				/*
				 * Less smoothing for decreasing pitch frequency, to track
			...
			delta_freq = SigProcFLP.SKP_LIMIT_float(delta_freq,
					-DefineFLP.VARIABLE_HP_MAX_DELTA_FREQ,
					DefineFLP.VARIABLE_HP_MAX_DELTA_FREQ);

			/* Update smoother */
			smth_coef = DefineFLP.VARIABLE_HP_SMTH_COEF1
		...
		psEncCtrl.pitch_freq_low_Hz = (float) Math.pow(2.0,
				psEnc.variable_HP_smth2);

		/* Limit frequency range */
		psEncCtrl.pitch_freq_low_Hz = SigProcFLP.SKP_LIMIT_float(
				psEncCtrl.pitch_freq_low_Hz, DefineFLP.VARIABLE_HP_MIN_FREQ,
				DefineFLP.VARIABLE_HP_MAX_FREQ);
		...
		B_Q28[2] = B_Q28[0];
		A_Q28[0] = SigProcFLP.SKP_float2int((1 << 28) * -2.0f * r
				* (1.0f - 0.5f * Fc * Fc));
		...
		BiquadAlt.SKP_Silk_biquad_alt(in, in_offset, B_Q28, A_Q28,
				psEnc.sCmn.In_HP_State, out, out_offset,
