...
/**
 * Downsample by a factor 4. Note: very low quality, only use with input
 * sampling rates above 96 kHz.
 * 
 * @author Jing Dai
 * @author Dingxin Xu
 */
public class ResamplerPrivateDown4 {
	 ...
	 */
	static void SKP_Silk_resampler_private_down4(int[] S, /*
														 * I/O: State vector [ 2
														 * ]
														 */
			int S_offset, short[] out, /* O: Output signal [ floor(len/2) ] */
		...
		for (k = 0; k < len4; k++) {
			...
			Y = in32 - S[S_offset];
			X = Macros
					.SKP_SMLAWB(Y, Y, ResamplerRom.SKP_Silk_resampler_down2_1);
			out32 = S[S_offset] + X;
			S[S_offset] = in32 + X;
			...
			out[out_offset + k] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
					.SKP_RSHIFT_ROUND(out32, 11));
		}
	}
}
