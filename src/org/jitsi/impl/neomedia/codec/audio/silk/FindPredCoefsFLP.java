...
public class FindPredCoefsFLP {
	 ...
	 */
	static void SKP_Silk_find_pred_coefs_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
																				 * I
																				 * /
																				 * O
																				 * Encoder
		...
		int i;
		float[] WLTP = new float[Define.NB_SUBFR * Define.LTP_ORDER
				* Define.LTP_ORDER];
		float[] invGains = new float[Define.NB_SUBFR], Wght = new float[Define.NB_SUBFR];
		float[] NLSF = new float[Define.MAX_LPC_ORDER];
		float[] x_ptr;
		int x_ptr_offset;
		float[] x_pre_ptr, LPC_in_pre = new float[Define.NB_SUBFR
				* Define.MAX_LPC_ORDER + Define.MAX_FRAME_LENGTH];
		...
		if (psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
			...
			FindLTPFLP.SKP_Silk_find_LTP_FLP(psEncCtrl.LTPCoef, WLTP,
					LTPredCodGain_ptr, res_pitch, res_pitch,
					(psEnc.sCmn.frame_length >> 1), psEncCtrl.sCmn.pitchL,
					Wght, psEnc.sCmn.subfr_length, psEnc.sCmn.frame_length);
			...
			/* Create LTP residual */
			LTPAnalysisFilterFLP.SKP_Silk_LTP_analysis_filter_FLP(LPC_in_pre,
					psEnc.x_buf, psEnc.sCmn.frame_length
							- psEnc.sCmn.predictLPCOrder, psEncCtrl.LTPCoef,
			...
			x_pre_ptr_offset = 0;
			for (i = 0; i < Define.NB_SUBFR; i++) {
				ScaleCopyVectorFLP.SKP_Silk_scale_copy_vector_FLP(x_pre_ptr,
						x_pre_ptr_offset, x_ptr, x_ptr_offset, invGains[i],
						psEnc.sCmn.subfr_length + psEnc.sCmn.predictLPCOrder);
				x_pre_ptr_offset += psEnc.sCmn.subfr_length
						+ psEnc.sCmn.predictLPCOrder;
				x_ptr_offset += psEnc.sCmn.subfr_length;
			}

			Arrays.fill(psEncCtrl.LTPCoef, 0, Define.NB_SUBFR
					* Define.LTP_ORDER, 0.0f);
			psEncCtrl.LTPredCodGain = 0.0f;
		}

		/*
		 * LPC_in_pre contains the LTP-filtered input for voiced, and the
		 * unfiltered input for unvoiced
		 */
		int[] NLSFInterpCoef_Q2_ptr = new int[1];
		NLSFInterpCoef_Q2_ptr[0] = psEncCtrl.sCmn.NLSFInterpCoef_Q2;
		FindLPCFLP.SKP_Silk_find_LPC_FLP(NLSF, NLSFInterpCoef_Q2_ptr,
				psEnc.sPred.prev_NLSFq, psEnc.sCmn.useInterpolatedNLSFs
						* (1 - psEnc.sCmn.first_frame_after_reset),
		...
		ResidualEnergyFLP.SKP_Silk_residual_energy_FLP(psEncCtrl.ResNrg,
				LPC_in_pre, psEncCtrl.PredCoef, psEncCtrl.Gains,
				psEnc.sCmn.subfr_length, psEnc.sCmn.predictLPCOrder);

		/*
		 * Copy to prediction struct for use in next frame for fluctuation
		 * reduction
		 */
		System.arraycopy(NLSF, 0, psEnc.sPred.prev_NLSFq, 0,
				psEnc.sCmn.predictLPCOrder);
	}
}
