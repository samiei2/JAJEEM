 ...
 * 
 * 8 C UF U UF UF UF UF 12 AF C UF U UF UF UF 16 D AF C UF U UF UF Fs_in (kHz)
 * 24 AIF D AF C UF UF U 32 UF AF D AF C UF UF 44.1 AMI AMI AMI AMI AMI C UF 48
 * DAF DAF AF D AF UF C
 * 
 * default method: UF
 * 
 * C -> Copy (no resampling) D -> Allpass-based 2x downsampling U ->
 * Allpass-based 2x upsampling DAF -> Allpass-based 2x downsampling followed by
 * AR2 filter followed by FIR interpolation UF -> Allpass-based 2x upsampling
 * followed by FIR interpolation AMI -> ARMA4 filter followed by FIR
...
public class Resampler {
	...
	static int SKP_Silk_resampler_init(SKP_Silk_resampler_state_struct S, /*
		...
		if (ResamplerStructs.RESAMPLER_SUPPORT_ABOVE_48KHZ != 0) {
			if (Fs_Hz_in < 8000 || Fs_Hz_in > 192000 || Fs_Hz_out < 8000
					|| Fs_Hz_out > 192000) {
				assert (false);
				return -1;
			}
		} else {
			if (Fs_Hz_in < 8000 || Fs_Hz_in > 48000 || Fs_Hz_out < 8000
					|| Fs_Hz_out > 48000) {
				assert (false);
				return -1;
			}
		...
		if (ResamplerStructs.RESAMPLER_SUPPORT_ABOVE_48KHZ != 0) {
			...
			if (S.nPreDownsamplers + S.nPostUpsamplers > 0) {
				...
				/*
				 * Convert sampling rate to those after pre-downsampling and
				 * before post-upsampling
				 */
		...
		if ((S.batchSize * 100 != Fs_Hz_in) || (Fs_Hz_in % 100 != 0)) {
			/*
			 * No integer number of input or output samples with 10 ms frames,
			 * use greatest common divisor
			 */
			cycleLen = Fs_Hz_in / gcd(Fs_Hz_in, Fs_Hz_out);
			cyclesPerBatch = ResamplerPrivate.RESAMPLER_MAX_BATCH_SIZE_IN
					/ cycleLen;
			if (cyclesPerBatch == 0) {
				/*
				 * cycleLen too big, let's just use the maximum batch size. Some
	...
	/**
	 * Clear the states of all resampling filters, without resetting sampling
	 * rate ratio.
	 * 
	 * @param S
	 *            Resampler state.
				...
				while (inLen > 0) {
					...
					if (S.nPreDownsamplers > 0) {
						...
						S.resampler_function(S, out_buf, 0, in, in_offset,
								(nSamplesIn >> S.nPreDownsamplers));
						S.up_post_function(S.sUpPost, out, out_offset, out_buf,
								0, (nSamplesOut >> S.nPostUpsamplers));
					}
...
class DownPreImplDown4 implements DownPreFP {
	...
	public void down_pre_function(int[] state, short[] out, int outOffset,
		...
		ResamplerPrivateDown4.SKP_Silk_resampler_private_down4(state, 0, out,
				outOffset, in, inOffset, len);
	}
}
...
class DownPreImplDown2 implements DownPreFP {
	...
	public void down_pre_function(int[] state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerDown2.SKP_Silk_resampler_down2(state, 0, out, outOffset, in,
				inOffset, len);
	}
}
...
class UpPostImplUp4 implements UpPostFP {
	...
	public void up_post_function(int[] state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerPrivateUp4.SKP_Silk_resampler_private_up4(state, 0, out,
				outOffset, in, inOffset, len);
	}
...
class UpPostImplUp2 implements UpPostFP {
	...
	public void up_post_function(int[] state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerUp2.SKP_Silk_resampler_up2(state, 0, out, outOffset, in,
				inOffset, len);
	}
}
...
class ResamplerImplIIRFIR implements ResamplerFP {
	...
	public void resampler_function(Object state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerPrivateIIRFIR.SKP_Silk_resampler_private_IIR_FIR(state, out,
				outOffset, in, inOffset, len);
	}
}
...
class ResamplerImplDownFIR implements ResamplerFP {
	...
	public void resampler_function(Object state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerPrivateDownFIR.SKP_Silk_resampler_private_down_FIR(state, out,
				outOffset, in, inOffset, len);
	}
}
...
class ResamplerImplCopy implements ResamplerFP {
	...
	public void resampler_function(Object state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerPrivateCopy.SKP_Silk_resampler_private_copy(state, out,
				outOffset, in, inOffset, len);
	}
...
class Up2ImplHQ implements Up2FP {
	...
	public void up2_function(int[] state, short[] out, int outOffset,
			short[] in, int inOffset, int len) {
		ResamplerPrivateUp2HQ.SKP_Silk_resampler_private_up2_HQ(state, 0, out,
				outOffset, in, inOffset, len);
	}
