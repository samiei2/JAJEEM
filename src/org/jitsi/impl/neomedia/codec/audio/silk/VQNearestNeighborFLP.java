...
public class VQNearestNeighborFLP {
	...
	static void SKP_Silk_VQ_WMat_EC_FLP(int[] ind, /*
													 ...
													 * vector
													 */
			int ind_offset, float[] rate_dist, /*
												 * O Best weighted quant. error
												 * + mu * rate
												 */
		...
		for (k = 0; k < L; k++) {
			...
			diff[0] = in[in_offset + 0] - cb_row[0]
					* DefineFLP.Q14_CONVERSION_FAC;
			diff[1] = in[in_offset + 1] - cb_row[1]
					* DefineFLP.Q14_CONVERSION_FAC;
			diff[2] = in[in_offset + 2] - cb_row[2]
					* DefineFLP.Q14_CONVERSION_FAC;
			diff[3] = in[in_offset + 3] - cb_row[3]
					* DefineFLP.Q14_CONVERSION_FAC;
			diff[4] = in[in_offset + 4] - cb_row[4]
					* DefineFLP.Q14_CONVERSION_FAC;
			...
			/* first row of W */
			sum1 += diff[0]
					* (W[W_offset + 0] * diff[0] + 2.0f * (W[W_offset + 1]
							* diff[1] + W[W_offset + 2] * diff[2]
							+ W[W_offset + 3] * diff[3] + W[W_offset + 4]
							* diff[4]));

			/* second row of W */
			sum1 += diff[1]
					* (W[W_offset + 6] * diff[1] + 2.0f * (W[W_offset + 7]
							* diff[2] + W[W_offset + 8] * diff[3] + W[W_offset + 9]
							* diff[4]));

			/* third row of W */
			sum1 += diff[2]
					* (W[W_offset + 12] * diff[2] + 2.0f * (W[W_offset + 13]
							* diff[3] + W[W_offset + 14] * diff[4]));

			/* fourth row of W */
			sum1 += diff[3]
					* (W[W_offset + 18] * diff[3] + 2.0f * (W[W_offset + 19] * diff[4]));
