...
class SKP_Silk_nsq_state implements Cloneable {
	short[] xq = new short[2 * Define.MAX_FRAME_LENGTH]; /*
...
class SKP_Silk_VAD_state {
	...
	int[] NrgRatioSmth_Q8 = new int[Define.VAD_N_BANDS]; /*
	...
	int[] inv_NL = new int[Define.VAD_N_BANDS]; /*
												 * Inverse noise energy level in
												 * each band
												 */
	int[] NoiseLevelBias = new int[Define.VAD_N_BANDS]; /*
...
class SKP_Silk_NLSF_CBS {
	...
	public SKP_Silk_NLSF_CBS(int nVectors,
			short[] SKP_Silk_NLSF_MSVQ_CB0_10_Q15, int Q15_offset,
			short[] SKP_Silk_NLSF_MSVQ_CB0_10_rates_Q5, int Q5_offset) {
		this.nVectors = nVectors;
		this.CB_NLSF_Q15 = new short[SKP_Silk_NLSF_MSVQ_CB0_10_Q15.length
				- Q15_offset];
		System.arraycopy(SKP_Silk_NLSF_MSVQ_CB0_10_Q15, Q15_offset,
				this.CB_NLSF_Q15, 0, this.CB_NLSF_Q15.length);
		this.Rates_Q5 = new short[SKP_Silk_NLSF_MSVQ_CB0_10_rates_Q5.length
				- Q5_offset];
		System.arraycopy(SKP_Silk_NLSF_MSVQ_CB0_10_rates_Q5, Q5_offset,
				this.Rates_Q5, 0, this.Rates_Q5.length);
	}
...
class SKP_Silk_NLSF_CB_struct {
	public SKP_Silk_NLSF_CB_struct(int nStates, SKP_Silk_NLSF_CBS[] CBStages,
			int[] NDeltaMin_Q15, int[] CDF, int[][] StartPtr, int[] MiddleIx) {
...
class SKP_Silk_encoder_state {
	...
	int nStatesDelayedDecision; /*
								 * Number of states in delayed decision
								 * quantization
								 */
	...
	int first_frame_after_reset; /*
								 * Flag for deactivating NLSF interp. and fluc.
								 * reduction after resets
								 */

	/* Input/output buffering */
	short[] inputBuf = new short[Define.MAX_FRAME_LENGTH]; /*
															 * buffer containin
															 * input signal
															 */
	int inputBufIx;
	...
	int LBRR_GainIncreases; /*
							 * Number of shifts to Gains to get LBRR rate Voiced
							 * frames
							 */

	/* Bitrate control */
	int bitrateDiff; /*
					 * Accumulated diff. between the target bitrate and the
					 * switch bitrates
					 */
	int bitrate_threshold_up; /*
							 * Threshold for switching to a higher internal
							 * sample frequency
							 */
	int bitrate_threshold_down; /*
								 * Threshold for switching to a lower internal
								 * sample frequency
								 */
...
class SKP_Silk_PLC_struct {
	...
	short[] LTPCoef_Q14 = new short[Define.LTP_ORDER]; /*
														 * LTP coeficients to
														 * use for voiced
														 * concealment
														 */
...
class SKP_Silk_decoder_state {
	...
	short[] outBuf = new short[2 * Define.MAX_FRAME_LENGTH]; /*
															 * Buffer for output
															 * signal
															 */
	...
	int[] prevNLSF_Q15 = new int[Define.MAX_LPC_ORDER]; /*
														 * Used to interpolate
														 * LSFs
														 */
	int first_frame_after_reset; /*
	...
	int inband_FEC_offset; /*
							 * 0: no FEC, 1: FEC with 1 packet offset, 2: FEC w
							 * 2 packets offset
							 */
