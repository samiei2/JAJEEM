...
public class PLC {
	 ...
	 *            Loss flag.
	 */
	static void SKP_Silk_PLC(SKP_Silk_decoder_state psDec, /* I Decoder state */
			SKP_Silk_decoder_control psDecCtrl, /* I Decoder control */
			short signal[], /* O Concealed signal */
			int signal_offset, int length, /* I length of residual */
		...
		if (lost != 0) {
			...
			/* Generate Signal */
			/****************************/
			SKP_Silk_PLC_conceal(psDec, psDecCtrl, signal, signal_offset,
					length);
		} else {
			/****************************/
			/* Update state */
	...
	static void SKP_Silk_PLC_update(SKP_Silk_decoder_state psDec, /*
			...
			short signal[], int signal_offset, int length) {
		...
		if (psDecCtrl.sigtype == Define.SIG_TYPE_VOICED) {
			/*
			 * Find the parameters for the last subframe which contains a pitch
			 * pulse
			 */
			for (j = 0; j * psDec.subfr_length < psDecCtrl.pitchL[Define.NB_SUBFR - 1]; j++) {
				temp_LTP_Gain_Q14 = 0;
				for (i = 0; i < Define.LTP_ORDER; i++) {
					temp_LTP_Gain_Q14 += psDecCtrl.LTPCoef_Q14[(Define.NB_SUBFR - 1 - j)
							* Define.LTP_ORDER + i];
				}
				if (temp_LTP_Gain_Q14 > LTP_Gain_Q14) {
					LTP_Gain_Q14 = temp_LTP_Gain_Q14;
					System.arraycopy(psDecCtrl.LTPCoef_Q14, Macros.SKP_SMULBB(
							Define.NB_SUBFR - 1 - j, Define.LTP_ORDER),
			...
			if (LTP_Gain_Q14 < V_PITCH_GAIN_START_MIN_Q14) {
				...
				for (i = 0; i < Define.LTP_ORDER; i++) {
					psPLC.LTPCoef_Q14[i] = (short) (Macros.SKP_SMULBB(
							psPLC.LTPCoef_Q14[i], scale_Q10) >> 10);
				}
				...
				for (i = 0; i < Define.LTP_ORDER; i++) {
					psPLC.LTPCoef_Q14[i] = (short) (Macros.SKP_SMULBB(
							psPLC.LTPCoef_Q14[i], scale_Q14) << 14);
				}
			}
		} else {
		...
		System.arraycopy(psDecCtrl.PredCoef_Q12[1], 0, psPLC.prevLPC_Q12, 0,
				psDec.LPC_order);
		psPLC.prevLTP_scale_Q14 = (short) psDecCtrl.LTP_scale_Q14;

		/* Save Gains */
		System.arraycopy(psDecCtrl.Gains_Q16, 0, psPLC.prevGain_Q16, 0,
				Define.NB_SUBFR);
	...
	static void SKP_Silk_PLC_conceal(SKP_Silk_decoder_state psDec, /*
		...
		/* Update LTP buffer */
		System.arraycopy(psDec.sLTP_Q16, psDec.frame_length, psDec.sLTP_Q16, 0,
				psDec.frame_length);

		/* LPC concealment. Apply BWE to previous LPC */
		Bwexpander.SKP_Silk_bwexpander(psPLC.prevLPC_Q12, psDec.LPC_order,
				BWE_COEF_Q16);
		...
		for (k = (Define.NB_SUBFR >> 1); k < Define.NB_SUBFR; k++) {
			for (i = 0; i < psDec.subfr_length; i++) {
				exc_buf_ptr[exc_buf_ptr_offset + i] = (short) (Macros
						.SKP_SMULWW(psDec.exc_Q10[i + k * psDec.subfr_length],
		...
		/*
		 * Find the subframe with lowest energy of the last two and use that as
		 * random noise generator
		 */
		SumSqrShift.SKP_Silk_sum_sqr_shift(energy1_ptr, shift1_ptr, exc_buf, 0,
				psDec.subfr_length);
		energy1 = energy1_ptr[0];
		SumSqrShift.SKP_Silk_sum_sqr_shift(energy2_ptr, shift2_ptr, exc_buf,
				psDec.subfr_length, psDec.subfr_length);
		shift2 = shift2_ptr[0];

		if ((energy1 >> shift2) < (energy1 >> shift2)) {
			/* First sub-frame has lowest energy */
			rand_ptr = psDec.exc_Q10;
			rand_ptr_offset = Math.max(0, 3 * psDec.subfr_length
					- RAND_BUF_SIZE);
		} else {
		...
		if (psDec.prev_sigtype == Define.SIG_TYPE_VOICED) {
			rand_Gain_Q15 = PLC_RAND_ATTENUATE_V_Q15[Math.min(NB_ATT - 1,
					psDec.lossCnt)];
		} else {
			rand_Gain_Q15 = PLC_RAND_ATTENUATE_UV_Q15[Math.min(NB_ATT - 1,
					psDec.lossCnt)];
		}
		...
		if (psDec.lossCnt == 0) {
			...
			if (psDec.prev_sigtype == Define.SIG_TYPE_VOICED) {
				...
				rand_scale_Q14 = (short) (Macros.SKP_SMULBB(rand_scale_Q14,
			...
			if (psDec.prev_sigtype == Define.SIG_TYPE_UNVOICED) {
				...
				LPCInvPredGain.SKP_Silk_LPC_inverse_pred_gain(invGain_Q30_ptr,
						psPLC.prevLPC_Q12, psDec.LPC_order);
				invGain_Q30 = invGain_Q30_ptr[0];

				down_scale_Q30 = Math.min(
						((1 << 30) >> LOG2_INV_LPC_GAIN_HIGH_THRES),
						invGain_Q30);
				down_scale_Q30 = Math.max(
						((1 << 30) >> LOG2_INV_LPC_GAIN_LOW_THRES),
						down_scale_Q30);
				down_scale_Q30 = (down_scale_Q30 << LOG2_INV_LPC_GAIN_HIGH_THRES);

				rand_Gain_Q15 = (Macros.SKP_SMULWB(down_scale_Q30,
						rand_Gain_Q15) >> 14);
			}
		}
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			...
			pred_lag_ptr_offset = psDec.sLTP_buf_idx - lag + Define.LTP_ORDER
					/ 2;
			...
			for (i = 0; i < psDec.subfr_length; i++) {
				...
				/* Unrolled loop */
				// LTP_pred_Q14 = Macros.SKP_SMULWB( pred_lag_ptr[
				// pred_lag_ptr_offset + 0 ], B_Q14[ 0 ] );
				// TODO: array bonds check???
				LTP_pred_Q14 = Macros.SKP_SMULWB(
						pred_lag_ptr[pred_lag_ptr_offset + 0], B_Q14[0]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 1], B_Q14[1]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 2], B_Q14[2]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 3], B_Q14[3]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 4], B_Q14[4]);
				pred_lag_ptr_offset++;

				/* Generate LPC residual */
				LPC_exc_Q10 = (Macros.SKP_SMULWB(
						rand_ptr[rand_ptr_offset + idx], rand_scale_Q14) << 2); /*
																				 * Random
																				 * noise
																				 * part
																				 */
				LPC_exc_Q10 = (LPC_exc_Q10 + SigProcFIX.SKP_RSHIFT_ROUND(
						LTP_pred_Q14, 4)); /* Harmonic part */
			...
			psPLC.pitchL_Q8 += Macros.SKP_SMULWB(psPLC.pitchL_Q8,
					PITCH_DRIFT_FAC_Q16);
			psPLC.pitchL_Q8 = Math.min(psPLC.pitchL_Q8,
					(Macros.SKP_SMULBB(MAX_PITCH_LAG_MS, psDec.fs_kHz) << 8));
			lag = SigProcFIX.SKP_RSHIFT_ROUND(psPLC.pitchL_Q8, 8);
		}
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			for (i = 0; i < psDec.subfr_length; i++) {
				...
				LPC_pred_Q10 = Macros.SKP_SMULWB(
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 1],
						A_Q12_tmp[0]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 2],
						A_Q12_tmp[1]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 3],
						A_Q12_tmp[2]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 4],
						A_Q12_tmp[3]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 5],
						A_Q12_tmp[4]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 6],
						A_Q12_tmp[5]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 7],
						A_Q12_tmp[6]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 8],
						A_Q12_tmp[7]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 9],
						A_Q12_tmp[8]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - 10],
						A_Q12_tmp[9]);

				for (j = 10; j < psDec.LPC_order; j++) {
					LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
							psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i - j - 1],
							A_Q12_tmp[j]);
				}
				/* Add prediction to LPC residual */
				sig_Q10_ptr[sig_Q10_ptr_offset = i] = (sig_Q10_ptr[sig_Q10_ptr_offset
						+ i] + LPC_pred_Q10);

				/* Update states */
				psDec.sLPC_Q14[Define.MAX_LPC_ORDER + i] = (sig_Q10_ptr[sig_Q10_ptr_offset
						+ i] << 4);
			}
			sig_Q10_ptr_offset += psDec.subfr_length;
			/* Update LPC filter state */
			System.arraycopy(psDec.sLPC_Q14, psDec.subfr_length,
					psDec.sLPC_Q14, 0, Define.MAX_LPC_ORDER);
		}

		/* Scale with Gain */
		for (i = 0; i < psDec.frame_length; i++) {
			signal[signal_offset + i] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
					.SKP_RSHIFT_ROUND(Macros.SKP_SMULWW(sig_Q10[i],
							psPLC.prevGain_Q16[Define.NB_SUBFR - 1]), 10));
		}
	...
	static void SKP_Silk_PLC_glue_frames(SKP_Silk_decoder_state psDec, /*
		...
		if (psDec.lossCnt != 0) {
			...
			SumSqrShift.SKP_Silk_sum_sqr_shift(conc_energy_ptr,
					conc_energy_shift_ptr, signal, signal_offset, length);
			...
			if (psDec.sPLC.last_frame_lost != 0) {
				...
				SumSqrShift.SKP_Silk_sum_sqr_shift(energy_ptr,
						energy_shift_ptr, signal, signal_offset, length);
				energy = energy_ptr[0];
				energy_shift = energy_shift_ptr[0];

				/* Normalize energies */
				if (energy_shift > psPLC.conc_energy_shift) {
					psPLC.conc_energy = (psPLC.conc_energy >> energy_shift
				...
				if (energy > psPLC.conc_energy) {
					...
					for (i = 0; i < length; i++) {
						signal[signal_offset + i] = (short) ((gain_Q12 * signal[signal_offset
								+ i]) >> 12);
						gain_Q12 += slope_Q12;
