...
public class DecAPI {
	 ...
	 * @return ret
	 */
	static int SKP_Silk_SDK_InitDecoder(Object decState /* I/O: State */
	) {
		int ret = 0;
	 ...
	 * @param nSamplesOut
	 *            number of samples.
	 * @return the returned value carries the error message. 0 indicates OK;
	 *         other indicates error.
	 */
	static int SKP_Silk_SDK_Decode(Object decState, /* I/O: State */
			SKP_SILK_SDK_DecControlStruct decControl, /* I/O: Control structure */
			int lostFlag, /* I: 0: no loss, 1 loss */
			byte[] inData, /* I: Encoded input vector */
			int inData_offset, final int nBytesIn, /* I: Number of input Bytes */
			short[] samplesOut, /* O: Decoded output speech vector */
			int samplesOut_offset, short[] nSamplesOut /*
														 * I/O: Number of
														 * samples
														 * (vector/decoded)
														 */
		...
		ret += DecodeFrame.SKP_Silk_decode_frame(psDec, samplesOut,
				samplesOut_offset, nSamplesOut, inData, inData_offset,
		...
		if (used_bytes != 0) /* Only Call if not a packet loss */
		{
			if (psDec.nBytesLeft > 0
					&& psDec.FrameTermination == Define.SKP_SILK_MORE_FRAMES
		...
		if (psDec.fs_kHz * 1000 != decControl.API_sampleRate) {
			short[] samplesOut_tmp = new short[Define.MAX_API_FS_KHZ
			...
			System.arraycopy(samplesOut, samplesOut_offset + 0, samplesOut_tmp,
					0, nSamplesOut[0]);
			/*
			 * (Re-)initialize resampler state when switching internal sampling
			 * frequency
			 */
			if (prev_fs_kHz != psDec.fs_kHz
					|| psDec.prev_API_sampleRate != decControl.API_sampleRate) {
				ret = Resampler.SKP_Silk_resampler_init(psDec.resampler_state,
						psDec.fs_kHz * 1000, decControl.API_sampleRate);
			}

			/* Resample the output to API_sampleRate */
			ret += Resampler.SKP_Silk_resampler(psDec.resampler_state,
					samplesOut, samplesOut_offset, samplesOut_tmp, 0,
					nSamplesOut[0]);
		...
		/*
		 * Copy all parameters that are needed out of internal structure to the
		 * control stucture
		 */
	...
	static void SKP_Silk_SDK_search_for_LBRR(byte[] inData, /*
															 ...
															 * vector
															 */
			int inData_offset, final short nBytesIn, /* I: Number of input Bytes */
			int lost_offset, /* I: Offset from lost packet */
			byte[] LBRRData, /* O: LBRR payload */
			int LBRRData_offset, short[] nLBRRBytes /* O: Number of LBRR Bytes */
	) {
		SKP_Silk_decoder_state sDec = new SKP_Silk_decoder_state(); // Local
																	// decoder
																	// state to
																	// avoid
																	// interfering
		...
		RangeCoder.SKP_Silk_range_dec_init(sDec.sRC, inData, inData_offset,
				nBytesIn);

		while (true) {
			DecodeParameters.SKP_Silk_decode_parameters(sDec, sDecCtrl, TempQ,
					0);
			if (sDec.sRC.error != 0) {
				/* Corrupt stream */
				nLBRRBytes[0] = 0;
			...
			if (((sDec.FrameTermination - 1) & lost_offset) != 0
				...
				System.arraycopy(inData, inData_offset + nBytesIn
						- sDec.nBytesLeft, LBRRData, LBRRData_offset + 0,
						sDec.nBytesLeft);
				break;
			}
	 ...
	 *            type of content.
	 */
	static void SKP_Silk_SDK_get_TOC(byte[] inData, /* I: Encoded input vector */
