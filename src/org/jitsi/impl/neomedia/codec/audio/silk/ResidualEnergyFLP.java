...
public class ResidualEnergyFLP {
	 ...
	 */
	static float SKP_Silk_residual_energy_covar_FLP( /*
													 * O Weighted residual
													 * energy
													 */
	final float[] c, /* I Filter coefficients */
			int c_offset, float[] wXX, /*
										 * I/O Weighted correlation matrix, reg.
										 * out
										 */
		...
		for (k = 0; k < MAX_ITERATIONS_RESIDUAL_NRG; k++) {
			...
			for (i = 0; i < D; i++) {
				...
				// nrg += c[ i ] * ( 2.0f * tmp + matrix_c_ptr( wXX, i, i, D ) *
				// c[ i ] );
				nrg += c[c_offset + i]
						* (2.0f * tmp + wXX[wXX_offset + i + D * i]
								* c[c_offset + i]);
			}
			if (nrg > 0) {
				break;
	 ...
	 * LPC_order of preceeding samples
	 * 
	 * @param nrgs
	...
	static void SKP_Silk_residual_energy_FLP(float nrgs[], /*
		...
		int shift;
		// SKP_float *LPC_res_ptr, LPC_res[ ( MAX_FRAME_LENGTH + NB_SUBFR *
		// MAX_LPC_ORDER ) / 2 ];
		float LPC_res_ptr[], LPC_res[] = new float[(Define.MAX_FRAME_LENGTH + Define.NB_SUBFR
		...
		/*
		 * Filter input to create the LPC residual for each frame half, and
		 * measure subframe energies
		 */
		LPCAnalysisFilterFLP.SKP_Silk_LPC_analysis_filter_FLP(LPC_res, a[0], x,
				0 + 0 * shift, 2 * shift, LPC_order);
		nrgs[0] = (float) (gains[0] * gains[0] * EnergyFLP.SKP_Silk_energy_FLP(
				LPC_res_ptr, LPC_res_ptr_offset + 0 * shift, subfr_length));
		nrgs[1] = (float) (gains[1] * gains[1] * EnergyFLP.SKP_Silk_energy_FLP(
				LPC_res_ptr, LPC_res_ptr_offset + 1 * shift, subfr_length));

		LPCAnalysisFilterFLP.SKP_Silk_LPC_analysis_filter_FLP(LPC_res, a[1], x,
				0 + 2 * shift, 2 * shift, LPC_order);
		nrgs[2] = (float) (gains[2] * gains[2] * EnergyFLP.SKP_Silk_energy_FLP(
				LPC_res_ptr, LPC_res_ptr_offset + 0 * shift, subfr_length));
		nrgs[3] = (float) (gains[3] * gains[3] * EnergyFLP.SKP_Silk_energy_FLP(
				LPC_res_ptr, LPC_res_ptr_offset + 1 * shift, subfr_length));
	}
}
