...
public class NLSFVQRateDistortionFLP {
	...
	static void SKP_Silk_NLSF_VQ_rate_distortion_FLP(float[] pRD, /*
																 ...
																 * ->nVectors*N]
																 */
			final SKP_Silk_NLSF_CBS_FLP psNLSF_CBS_FLP, /*
														 * I NLSF codebook stage
														 * struct
														 */
		...
		/*
		 * Compute weighted quantization errors for all input vectors over one
		 * codebook stage
		 */
		NLSFVQSumErrorFLP.SKP_Silk_NLSF_VQ_sum_error_FLP(pRD, in, w,
				psNLSF_CBS_FLP.CB, N, psNLSF_CBS_FLP.nVectors, LPC_order);
		...
		for (n = 0; n < N; n++) {
			...
			for (i = 0; i < psNLSF_CBS_FLP.nVectors; i++) {
				pRD_vec[pRD_vec_offset + i] += mu
						* (rate_acc[n] + psNLSF_CBS_FLP.Rates[i]);
			}
