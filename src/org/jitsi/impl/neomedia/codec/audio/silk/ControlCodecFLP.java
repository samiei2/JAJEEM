...
public class ControlCodecFLP {
	...
	static int SKP_Silk_control_encoder_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
			...
			final int max_internal_fs_kHz,/*
										 * I Maximum internal sampling rate
										 * (kHz)
										 */
		...
		/*
		 * Only switch during low speech activity, when no frames are sitting in
		 * the payload buffer
		 */
		if (API_fs_Hz == 8000 || fs_kHz == 0 || API_fs_Hz < fs_kHz * 1000
				|| fs_kHz > max_internal_fs_kHz) {
			/*
			 * Switching is not possible, encoder just initialized, internal
			 * mode higher than external,
			 */
			/* or internal mode higher than maximum allowed internal mode */
			fs_kHz = Math.min(API_fs_Hz / 1000, max_internal_fs_kHz);
		} else {
			/*
			 * Accumulate the difference between the target rate and limit for
			 * switching down
			 */
			...
			if (psEnc.speech_activity < 0.5f
				...
				if (Define.SWITCH_TRANSITION_FILTERING != 0) {
					if ((psEnc.sCmn.sLP.transition_frame_no == 0)
							...
							(psEnc.sCmn.bitrateDiff <= -Define.ACCUM_BITS_DIFF_THRESHOLD || /*
																							 * Bitrate
																							 * threshold
																							 * is
																							 * met
																							 */
							(psEnc.sCmn.sSWBdetect.WB_detected
									* psEnc.sCmn.fs_kHz == 24))) { /*
																	 * Forced
																	 * down
																	 * -switching
																	 * due to WB
					...
					} else if ((psEnc.sCmn.sLP.transition_frame_no >= Define.TRANSITION_FRAMES_DOWN)
				...
				if (Define.SWITCH_TRANSITION_FILTERING != 0) {
					if (((psEnc.sCmn.fs_kHz * 1000 < API_fs_Hz)
							&& (TargetRate_bps >= psEnc.sCmn.bitrate_threshold_up) && (psEnc.sCmn.sSWBdetect.WB_detected
							* psEnc.sCmn.fs_kHz != 16))
							&& ((psEnc.sCmn.fs_kHz == 16)
									&& (max_internal_fs_kHz >= 24)
									|| (psEnc.sCmn.fs_kHz == 12)
									&& (max_internal_fs_kHz >= 16) || (psEnc.sCmn.fs_kHz == 8)
		...
		if (Define.SWITCH_TRANSITION_FILTERING != 0) {
			/*
			 * After switching up, stop transition filter during speech
			 * inactivity
			 */
		...
		if (psEnc.sCmn.fs_kHz != fs_kHz
			...
			/*
			 * Allocate space for worst case temporary upsampling, 8 to 48 kHz,
			 * so a factor 6
			 */
			short[] x_buf_API_fs_Hz = new short[(Define.MAX_API_FS_KHZ / 8)
					* (2 * Define.MAX_FRAME_LENGTH + Define.LA_SHAPE_MAX)];
			short[] x_bufFIX = new short[2 * Define.MAX_FRAME_LENGTH
					+ Define.LA_SHAPE_MAX];

			int nSamples_temp = 2 * psEnc.sCmn.frame_length
					+ psEnc.sCmn.la_shape;

			SigProcFLP.SKP_float2short_array(x_bufFIX, 0, psEnc.x_buf, 0, 2
					* Define.MAX_FRAME_LENGTH + Define.LA_SHAPE_MAX);
			...
			if (fs_kHz * 1000 < API_fs_Hz && psEnc.sCmn.fs_kHz != 0) {
				...
				/*
				 * Initialize resampler for temporary resampling of x_buf data
				 * to API_fs_Hz
				 */
				...
				ret += Resampler.SKP_Silk_resampler(temp_resampler_state,
						x_buf_API_fs_Hz, 0, x_bufFIX, 0, nSamples_temp);

				/*
				 * Calculate number of samples that has been temporarily
				 * upsampled
				 */
				nSamples_temp = (nSamples_temp * API_fs_Hz)
						/ (psEnc.sCmn.fs_kHz * 1000);

				/*
				 * Initialize the resampler for enc_API.c preparing resampling
				 * from API_fs_Hz to fs_kHz
				 */
				ret += Resampler.SKP_Silk_resampler_init(
						psEnc.sCmn.resampler_state, API_fs_Hz, fs_kHz * 1000);
			...
			if (1000 * fs_kHz != API_fs_Hz) {
				/*
				 * Correct resampler state (unless resampling by a factor 1) by
				 * resampling buffered data from API_fs_Hz to fs_kHz
				 */
				ret += Resampler.SKP_Silk_resampler(psEnc.sCmn.resampler_state,
						x_bufFIX, 0, x_buf_API_fs_Hz, 0, nSamples_temp);
			}
			SigProcFLP.SKP_short2float_array(psEnc.x_buf, 0, x_bufFIX, 0, 2
					* Define.MAX_FRAME_LENGTH + Define.LA_SHAPE_MAX);
		}
		...
		if (psEnc.sCmn.fs_kHz != fs_kHz) {
			...
			// SKP_memset( &psEnc->sPrefilt, 0, sizeof(
			...
			Arrays.fill(psEnc.sNSQ.xq, 0, 2 * Define.MAX_FRAME_LENGTH,
					(short) 0);
			// SKP_memset( psEnc->sNSQ_LBRR.xq, 0, ( 2 * MAX_FRAME_LENGTH ) *
			// sizeof( SKP_float ) );
			Arrays.fill(psEnc.sNSQ_LBRR.xq, (short) 0);
			// SKP_memset( psEnc->sCmn.LBRR_buffer, 0, MAX_LBRR_DELAY * sizeof(
			...
			psEnc.sCmn.TargetRate_bps = 0; /*
											 * Ensures that psEnc->SNR_dB is
											 * recomputed
											 */

			// SKP_memset( psEnc->sPred.prev_NLSFq, 0, MAX_LPC_ORDER * sizeof(
			// SKP_float ) );
			...
			psEnc.sPred.pitch_LPC_win_length = Define.FIND_PITCH_LPC_WIN_MS
					* fs_kHz;
			if (psEnc.sCmn.fs_kHz == 24) {
		...
		psEnc.sCmn.pitchEstimationLPCOrder = Math.min(
				psEnc.sCmn.pitchEstimationLPCOrder, psEnc.sCmn.predictLPCOrder);
		...
		if (psEnc.sCmn.fs_kHz == 8) {
			TargetRate_bps = Math.max(TargetRate_bps,
					Define.MIN_TARGET_RATE_NB_BPS);
		} else if (psEnc.sCmn.fs_kHz == 12) {
			TargetRate_bps = Math.max(TargetRate_bps,
					Define.MIN_TARGET_RATE_MB_BPS);
		} else if (psEnc.sCmn.fs_kHz == 16) {
			TargetRate_bps = Math.max(TargetRate_bps,
					Define.MIN_TARGET_RATE_WB_BPS);
		} else {
			TargetRate_bps = Math.max(TargetRate_bps,
					Define.MIN_TARGET_RATE_SWB_BPS);
		}
		...
		/* Set packet size */
		if ((PacketSize_ms != 20) && (PacketSize_ms != 40)
				&& (PacketSize_ms != 60) && (PacketSize_ms != 80)
				&& (PacketSize_ms != 100)) {
		...
		if (Define.USE_LBRR != 0) {
			...
			if (psEnc.sCmn.nFramesInPayloadBuf == 0) {
				...
				if (psEnc.sCmn.TargetRate_bps >= LBRRRate_thres_bps) {
					...
					/*
					 * Meaning that at 16% loss main rate and redundant rate is
					 * the same, -> G = 0
					 */
					psEnc.sCmn.LBRR_GainIncreases = Math.max(
							8 - (psEnc.sCmn.PacketLoss_perc >> 1), 0);

					/* Set main stream rate compensation */
					if (psEnc.sCmn.LBRR_enabled != 0
							&& psEnc.sCmn.PacketLoss_perc > Define.LBRR_LOSS_THRES) {
						/*
						 * Tuned to give aprox same mean / weighted bitrate as
						 * no inband FEC
						 */
	...
	static void SKP_Silk_LBRR_ctrl_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
		...
		if (psEnc.sCmn.LBRR_enabled != 0) {
			...
			/*
			 * Usage Control based on sensitivity and packet loss
			 * characteristics
			 */
			/*
...
class NSQImplNSQ implements NoiseShapingQuantizerFP {
	...
	public void NoiseShapingQuantizer(SKP_Silk_encoder_state psEnc,
		...
		NSQ.SKP_Silk_NSQ(psEnc, psEncCtrl, nsq, x, q, arg6, arg7, arg8, arg9,
...
class NSQImplNSQDelDec implements NoiseShapingQuantizerFP {
	...
	public void NoiseShapingQuantizer(SKP_Silk_encoder_state psEnc,
			...
			int arg14, int arg15) {
		// TODO Auto-generated method stub
		NSQDelDec.SKP_Silk_NSQ_del_dec(psEnc, psEncCtrl, NSQ, x, q, arg6, arg7,
