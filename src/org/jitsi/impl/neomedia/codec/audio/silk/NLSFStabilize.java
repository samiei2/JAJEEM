...
/**
 * NLSF stabilizer: - Moves NLSFs further apart if they are too close - Moves
 * NLSFs away from borders if they are too close - High effort to achieve a
 * modification with minimum Euclidean distance to input vector - Output are
 * sorted NLSF coefficients
 * 
 * @author Jing Dai
 * @author Dingxin Xu
 */
public class NLSFStabilize {
	 ...
	 * @param NDeltaMin_Q15
	 *            Normalized delta min vector in Q15, NDeltaMin_Q15[L] must be
	 *            >= 1 [L+1].
	 * @param L
	...
	static void SKP_Silk_NLSF_stabilize(int[] NLSF_Q15, /*
														 * I/O:
														 * Unstable/stabilized
														 * normalized LSF vector
														 * in Q15 [L]
														 */
			int NLSF_Q15_offset, int[] NDeltaMin_Q15, /*
													 * I: Normalized delta min
													 * vector in Q15,
													 * NDeltaMin_Q15[L] must be
													 * >= 1 [L+1]
													 */
		...
		for (loops = 0; loops < MAX_LOOPS; loops++) {
			...
			/* Last element */
			diff_Q15 = (1 << 15)
					- (NLSF_Q15[NLSF_Q15_offset + L - 1] + NDeltaMin_Q15[L]);
			if (diff_Q15 < min_diff_Q15) {
				min_diff_Q15 = diff_Q15;
				I = L;
			}
			...
			if (I == 0) {
				...
				NLSF_Q15[NLSF_Q15_offset + L - 1] = (1 << 15)
						- NDeltaMin_Q15[L];

			} else {
				/*
				 * Find the lower extreme for the location of the current center
				 * frequency
				 */
				min_center_Q15 = 0;
				for (k = 0; k < I; k++) {
				...
				/*
				 * Find the upper extreme for the location of the current center
				 * frequency
				 */
				max_center_Q15 = (1 << 15);
				for (k = L; k > I; k--) {
				...
				/*
				 * Move apart, sorted by value, keeping the same center
				 * frequency
				 */
				center_freq_Q15 = SigProcFIX.SKP_LIMIT_32(
						SigProcFIX.SKP_RSHIFT_ROUND(NLSF_Q15[NLSF_Q15_offset
								+ I - 1]
								+ NLSF_Q15[NLSF_Q15_offset + I], 1),
						min_center_Q15, max_center_Q15);
				NLSF_Q15[NLSF_Q15_offset + I - 1] = center_freq_Q15
						- (NDeltaMin_Q15[I] >> 1);
				NLSF_Q15[NLSF_Q15_offset + I] = NLSF_Q15[NLSF_Q15_offset + I
						- 1]
						+ NDeltaMin_Q15[I];
			}
		}
		...
		if (loops == MAX_LOOPS) {
			...
			Sort.SKP_Silk_insertion_sort_increasing_all_values(NLSF_Q15,
					NLSF_Q15_offset + 0, L);

			/* First NLSF should be no less than NDeltaMin[0] */
			NLSF_Q15[NLSF_Q15_offset + 0] = SigProcFIX.SKP_max_int(
					NLSF_Q15[NLSF_Q15_offset + 0], NDeltaMin_Q15[0]);

			/* Keep delta_min distance between the NLSFs */
			for (i = 1; i < L; i++) {
				NLSF_Q15[NLSF_Q15_offset + i] = SigProcFIX.SKP_max_int(
						NLSF_Q15[NLSF_Q15_offset + i], NLSF_Q15[NLSF_Q15_offset
								+ i - 1]
								+ NDeltaMin_Q15[i]);
			}

			/* Last NLSF should be no higher than 1 - NDeltaMin[L] */
			NLSF_Q15[NLSF_Q15_offset + L - 1] = SigProcFIX.SKP_min_int(
					NLSF_Q15[NLSF_Q15_offset + L - 1], (1 << 15)
							- NDeltaMin_Q15[L]);
