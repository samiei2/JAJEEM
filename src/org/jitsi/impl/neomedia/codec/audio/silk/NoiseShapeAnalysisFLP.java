...
public class NoiseShapeAnalysisFLP {
	...
	static void SKP_Silk_noise_shape_analysis_FLP(
			...
			int pitch_res_offset, float[] x, /*
											 * I Input signal [frame_length +
											 * la_shape]
											 */
			int x_offset) {
		...
		x_ptr = x;
		x_ptr_offset = x_offset + psEnc.sCmn.la_shape - Define.SHAPE_LPC_WIN_MS
				* psEnc.sCmn.fs_kHz + psEnc.sCmn.subfr_length;

		/****************/
		/* CONTROL SNR */
		/****************/
		/* Reduce SNR_dB values if recent bitstream has exceeded TargetRate */
		psEncCtrl.current_SNR_dB = psEnc.SNR_dB - 0.05f
		...
		/****************/
		/*
		 * Input quality is the average of the quality in the lowest two VAD
		 * bands
		 */
		...
		if (psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
			...
			/*
			 * Sparseness measure, based on relative fluctuations of energy per
			 * 2 milliseconds
			 */
			...
			for (k = 0; k < Define.FRAME_LENGTH_MS / 2; k++) {
				nrg = nSamples
						+ (float) EnergyFLP.SKP_Silk_energy_FLP(pitch_res_ptr,
			...
			psEncCtrl.sparseness = SigProcFLP
					.SKP_sigmoid(0.4f * (energy_variation - 5.0f));
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			/* Apply window */
			ApplySineWindowFLP.SKP_Silk_apply_sine_window_FLP(x_windowed, 0,
					x_ptr, x_ptr_offset, 0, Define.SHAPE_LPC_WIN_MS
							* psEnc.sCmn.fs_kHz);
			...
			AutocorrelationFLP.SKP_Silk_autocorrelation_FLP(auto_corr, 0,
					x_windowed, 0, Define.SHAPE_LPC_WIN_MS * psEnc.sCmn.fs_kHz,
					psEnc.sCmn.shapingLPCOrder + 1);

			/* Add white noise, as a fraction of energy */
			auto_corr[0] += auto_corr[0]
					* PerceptualParametersFLP.SHAPE_WHITE_NOISE_FRACTION;

			/*
			 * Convert correlations to prediction coefficients, and compute
			 * residual energy
			 */
			nrg = LevinsondurbinFLP.SKP_Silk_levinsondurbin_FLP(psEncCtrl.AR2,
					k * Define.SHAPE_LPC_ORDER_MAX, auto_corr,
					psEnc.sCmn.shapingLPCOrder);

			/* Bandwidth expansion for synthesis filter shaping */
			BwexpanderFLP.SKP_Silk_bwexpander_FLP(psEncCtrl.AR2, k
					* Define.SHAPE_LPC_ORDER_MAX, psEnc.sCmn.shapingLPCOrder,
					BWExp2);

			/* Make sure to fit in Q13 SKP_int16 */
			LPC_fit_int16(psEncCtrl.AR2, k * Define.SHAPE_LPC_ORDER_MAX, 1.0f,
					psEnc.sCmn.shapingLPCOrder, 3.999f);
			...
			BwexpanderFLP.SKP_Silk_bwexpander_FLP(psEncCtrl.AR1, k
					* Define.SHAPE_LPC_ORDER_MAX, psEnc.sCmn.shapingLPCOrder,
					BWExp1);

			/* Increase residual energy */
			nrg += PerceptualParametersFLP.SHAPE_MIN_ENERGY_RATIO
					* auto_corr[0];
			psEncCtrl.Gains[k] = (float) Math.sqrt(nrg);

			/* Ratio of prediction gains, in energy domain */
			float[] pre_nrg_djinnaddress = { pre_nrg };
			LPCInvPredGainFLP.SKP_Silk_LPC_inverse_pred_gain_FLP(
					pre_nrg_djinnaddress, psEncCtrl.AR2, k
							* Define.SHAPE_LPC_ORDER_MAX,
					psEnc.sCmn.shapingLPCOrder);
			pre_nrg = pre_nrg_djinnaddress[0];
			float[] nrg_djinnaddress = { nrg };
			LPCInvPredGainFLP.SKP_Silk_LPC_inverse_pred_gain_FLP(
					nrg_djinnaddress, psEncCtrl.AR1, k
							* Define.SHAPE_LPC_ORDER_MAX,
					psEnc.sCmn.shapingLPCOrder);
		...
		/*****************/
		/*
		 * Increase gains during low speech activity and put lower limit on
		 * gains
		 */
		gain_mult = (float) Math.pow(2.0f, -0.16f * SNR_adj_dB);
		gain_add = (float) Math.pow(2.0f,
				0.16f * PerceptualParametersFLP.NOISE_FLOOR_dB)
				+ (float) Math.pow(2.0f,
						0.16f * PerceptualParametersFLP.RELATIVE_MIN_GAIN_dB)
				* psEnc.avgGain;
		for (k = 0; k < Define.NB_SUBFR; k++) {
			psEncCtrl.Gains[k] *= gain_mult;
			psEncCtrl.Gains[k] += gain_add;
			psEnc.avgGain += psEnc.speech_activity
					* PerceptualParametersFLP.GAIN_SMOOTHING_COEF
		...
		/************************************************/
		gain_mult = 1.0f + PerceptualParametersFLP.INPUT_TILT
				+ psEncCtrl.coding_quality
				* PerceptualParametersFLP.HIGH_RATE_INPUT_TILT;
		if (psEncCtrl.input_tilt <= 0.0f
				&& psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_UNVOICED) {
			float essStrength = -psEncCtrl.input_tilt * psEnc.speech_activity
					* (1.0f - psEncCtrl.sparseness);
			if (psEnc.sCmn.fs_kHz == 24) {
				gain_mult *= (float) Math.pow(2.0f, -0.16f
						* PerceptualParametersFLP.DE_ESSER_COEF_SWB_dB
						* essStrength);
			} else if (psEnc.sCmn.fs_kHz == 16) {
				gain_mult *= (float) Math.pow(2.0f, -0.16f
						* PerceptualParametersFLP.DE_ESSER_COEF_WB_dB
						* essStrength);
			} else {
		...
		if (psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
			 ...
			 */
			/*
			 * f = 400; freqz([1, -0.98 + 2e-4 * f], [1, -0.97 + 7e-4 * f],
			 * 2^12, Fs); axis([0, 1000, -10, 1])
			 */
			for (k = 0; k < Define.NB_SUBFR; k++) {
			...
			Tilt = -PerceptualParametersFLP.HP_NOISE_COEF
					...
					* PerceptualParametersFLP.HARM_HP_NOISE_COEF
					* psEnc.speech_activity;
		} else {
		...
		if (Define.USE_HARM_SHAPING != 0
				&& psEncCtrl.sCmn.sigtype == Define.SIG_TYPE_VOICED) {
			...
			HarmShapeGain += PerceptualParametersFLP.HIGH_RATE_OR_LOW_QUALITY_HARMONIC_SHAPING
					* (1.0f - (1.0f - psEncCtrl.coding_quality)
	...
	static void LPC_fit_int16(float[] a, /*
										 * I/O: Unstable/stabilized LPC vector
										 * [L]
										 */
		...
		/**********************/
		for (k = 0; k < 1000; k++) {
			if (LPCInvPredGainFLP.SKP_Silk_LPC_inverse_pred_gain_FLP(invGain,
					a, a_offset, L) == 1) {
