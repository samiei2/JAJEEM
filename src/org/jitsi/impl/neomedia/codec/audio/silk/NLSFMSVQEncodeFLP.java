...
public class NLSFMSVQEncodeFLP {
	...
	static void SKP_Silk_NLSF_MSVQ_encode_FLP(int[] NLSFIndices, /*
			...
			final float[] pNLSF_q_prev, /*
										 * I Prev. quantized NLSF vector
										 * [LPC_ORDER]
										 */
		...
		if (Define.LOW_COMPLEXITY_ONLY == 1) {
			pRateDist = new float[Define
					.NLSF_MSVQ_TREE_SEARCH_MAX_VECTORS_EVALUATED_LC_MODE()];
			...
			pPath = new int[Define.MAX_NLSF_MSVQ_SURVIVORS_LC_MODE
					* Define.NLSF_MSVQ_MAX_CB_STAGES];
			pPath_new = new int[Define.MAX_NLSF_MSVQ_SURVIVORS_LC_MODE
					* Define.NLSF_MSVQ_MAX_CB_STAGES];
			pRes = new float[Define.MAX_NLSF_MSVQ_SURVIVORS_LC_MODE
					* Define.MAX_LPC_ORDER];
			pRes_new = new float[Define.MAX_NLSF_MSVQ_SURVIVORS_LC_MODE
					* Define.MAX_LPC_ORDER];
		} else {
			pRateDist = new float[Define
					.NLSF_MSVQ_TREE_SEARCH_MAX_VECTORS_EVALUATED()];
			...
			pPath = new int[Define.MAX_NLSF_MSVQ_SURVIVORS
					* Define.NLSF_MSVQ_MAX_CB_STAGES];
			pPath_new = new int[Define.MAX_NLSF_MSVQ_SURVIVORS
					* Define.NLSF_MSVQ_MAX_CB_STAGES];
			pRes = new float[Define.MAX_NLSF_MSVQ_SURVIVORS
					* Define.MAX_LPC_ORDER];
			pRes_new = new float[Define.MAX_NLSF_MSVQ_SURVIVORS
					* Define.MAX_LPC_ORDER];
		}
		...
		for (s = 0; s < psNLSF_CB_FLP.nStages; s++) {
			...
			cur_survivors = Math.min(NLSF_MSVQ_Survivors, prev_survivors
			...
			NLSFVQRateDistortionFLP.SKP_Silk_NLSF_VQ_rate_distortion_FLP(
					pRateDist, pCurrentCBStage, pRes, pW, pRate, NLSF_mu,
					prev_survivors, LPC_order);

			/* Sort the rate-distortion errors */
			SortFLP.SKP_Silk_insertion_sort_increasing_FLP(pRateDist, 0,
					pTempIndices, prev_survivors * pCurrentCBStage.nVectors,
					cur_survivors);

			/*
			 * Discard survivors with rate-distortion values too far above the
			 * best one
			 */
			rateDistThreshold = Define.NLSF_MSVQ_SURV_MAX_REL_RD * pRateDist[0];
			while (pRateDist[cur_survivors - 1] > rateDistThreshold
					&& cur_survivors > 1) {
				cur_survivors--;
			}

			/*
			 * Update accumulated codebook contributions for the 'cur_survivors'
			 * best codebook indices
			 */
			for (k = 0; k < cur_survivors; k++) {
				if (s > 0) {
					...
					if (pCurrentCBStage.nVectors == 8) {
						...
						input_index = pTempIndices[k]
								/ pCurrentCBStage.nVectors;
						cb_index = pTempIndices[k] - input_index
								* pCurrentCBStage.nVectors;
					}
				} else {
					/* Find the indices of the input and the codebook vector */
				...
				/*
				 * Subtract new contribution from the previous residual vector
				 * for each of 'cur_survivors'
				 */
				pConstFloat = pRes;
				...
				for (i = 0; i < LPC_order; i++) {
					pFloat[pFloat_offset + i] = pConstFloat[pConstFloat_offset
							+ i]
							- pCB_element[pCB_element_offset + i];
				}

				/* Update accumulated rate for stage 1 to the current */
				pRate_new[k] = pRate[input_index]
						+ pCurrentCBStage.Rates[cb_index];
				...
				/*
				 * Write the current stage indices for the 'cur_survivors' to
				 * the best path matrix
				 */
			...
			if (s < psNLSF_CB_FLP.nStages - 1) {
				...
				System.arraycopy(pRes_new, 0, pRes, 0, cur_survivors
						* LPC_order);
				...
				System.arraycopy(pPath_new, 0, pPath, 0, cur_survivors
		...
		if (Define.NLSF_MSVQ_FLUCTUATION_REDUCTION == 1) {
			...
			if (deactivate_fluc_red != 1) {
				...
				/*
				 * Search among all survivors, now taking also weighted
				 * fluctuation errors into account
				 */
				bestRateDist = Float.MAX_VALUE;
				for (s = 0; s < cur_survivors; s++) {
					/*
					 * Decode survivor to compare with previous quantized NLSF
					 * vector
					 */
					NLSFMSVQDecodeFLP.SKP_Silk_NLSF_MSVQ_decode_FLP(pNLSF,
							psNLSF_CB_FLP, pPath_new,
							s * psNLSF_CB_FLP.nStages, LPC_order);

					/*
					 * Compare decoded NLSF vector with the previously quantized
					 * vector
					 */
					wsse = 0;
					...
					/*
					 * Add the fluctuation reduction penalty to the rate
					 * distortion error
					 */
		...
		System.arraycopy(pPath_new, bestIndex * psNLSF_CB_FLP.nStages,
				NLSFIndices, 0, psNLSF_CB_FLP.nStages);

		/* Decode and stabilize the best survivor */
		NLSFMSVQDecodeFLP.SKP_Silk_NLSF_MSVQ_decode_FLP(pNLSF, psNLSF_CB_FLP,
				NLSFIndices, 0, LPC_order);
	}
}
