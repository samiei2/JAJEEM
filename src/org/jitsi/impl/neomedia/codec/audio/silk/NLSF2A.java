...
/**
 * conversion between prediction filter coefficients and LSFs order should be
 * even a piecewise linear approximation maps LSF <-> cos(LSF) therefore the
 * result is not accurate LSFs, but the two function are accurate inverses of
 * each other.
 * 
 * @author Jing Dai
 * @author Dingxin Xu
...
public class NLSF2A {
	...
	static void SKP_Silk_NLSF2A_find_poly(int[] out, /*
			...
			int cLSF_offset, int dd /* i polynomial order (= 1/2 * filter order) */
	) {
		int k, n;
		int ftmp;
		...
		for (k = 1; k < dd; k++) {
			...
			SigProcFIX.SKP_SMULL(ftmp, out[k]);

			out[k + 1] = (out[k - 1] << 1)
					- (int) SigProcFIX.SKP_RSHIFT_ROUND64(
							SigProcFIX.SKP_SMULL(ftmp, out[k]), 20);

			for (n = k; n > 1; n--) {
				out[n] += out[n - 2]
						- (int) SigProcFIX.SKP_RSHIFT_ROUND64(
								SigProcFIX.SKP_SMULL(ftmp, out[n - 1]), 20);
			}
			out[1] -= ftmp;
		}
	}
	...
	static void SKP_Silk_NLSF2A(short[] a, /*
											 * o monic whitening filter
											 * coefficients in Q12, [d]
											 */
			int[] NLSF, /* i normalized line spectral frequencies in Q15, [d] */
			final int d /* i filter order (should be even) */
	) {
		int k, i, dd;
		int[] cos_LSF_Q20 = new int[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC];
		...
		int f_frac;
		int cos_val, delta;
		int[] a_int32 = new int[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC];
		...
		/*
		 * convert LSFs to 2*cos(LSF(i)), using piecewise linear curve from
		 * table
		 */
		for (k = 0; k < d; k++) {
		...
		for (i = 0; i < 10; i++) {
			...
			if (maxabs > Typedef.SKP_int16_MAX) {
				...
				maxabs = SigProcFIX.SKP_min(maxabs, 98369); // ( SKP_int32_MAX /
															// ( 65470 >> 2 ) )
															// + SKP_int16_MAX =
															// 98369
				sc_Q16 = 65470 - (((65470 >> 2) * (maxabs - Typedef.SKP_int16_MAX)) / ((maxabs * (idx + 1)) >> 2));
