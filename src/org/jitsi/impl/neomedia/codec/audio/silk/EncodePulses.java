...
public class EncodePulses {
	...
	int[] pulses_comb, /* O */
		...
		for (k = 0; k < len; k++) {
			sum = pulses_in[pulses_in_offset + 2 * k]
					+ pulses_in[pulses_in_offset + 2 * k + 1];
			if (sum > max_pulses) {
				return 1;
			}
			pulses_comb[k] = sum;
	...
	static void SKP_Silk_encode_pulses(SKP_Silk_range_coder_state psRC, /*
		...
		for (i = 0; i < iter; i++) {
			...
			while (true) {
				...
				if (scale_down != 0) {
					...
					/*
					 * Jump out of while(1) loop and go to next shell coding
					 * frame
					 */
					break;
				}
		...
		for (i = 0; i < iter; i++) {
			if (nRshifts[i] == 0) {
				RangeCoder.SKP_Silk_range_encoder(psRC, sum_pulses[i], cdf_ptr,
						0);
			} else {
				...
				for (k = 0; k < nRshifts[i] - 1; k++) {
					RangeCoder
							.SKP_Silk_range_encoder(
									psRC,
									Define.MAX_PULSES + 1,
									TablesPulsesPerBlock.SKP_Silk_pulses_per_block_CDF[Define.N_RATE_LEVELS - 1],
									0);
				}
				RangeCoder
						.SKP_Silk_range_encoder(
		...
		for (i = 0; i < iter; i++) {
			if (sum_pulses[i] > 0) {
				ShellCoder.SKP_Silk_shell_encoder(psRC, abs_pulses, i
						* Define.SHELL_CODEC_FRAME_LENGTH);
			}
		}
			...
			if (nRshifts[i] > 0) {
				...
				for (k = 0; k < Define.SHELL_CODEC_FRAME_LENGTH; k++) {
					abs_q = pulses_ptr[pulses_ptr_offset + k] > 0 ? pulses_ptr[pulses_ptr_offset
							+ k]
							: (-pulses_ptr[pulses_ptr_offset + k]);
					for (j = nLS; j > 0; j--) {
						bit = (abs_q >> j) & 1;
						RangeCoder.SKP_Silk_range_encoder(psRC, bit,
								TablesOther.SKP_Silk_lsb_CDF, 0);
					}
					bit = abs_q & 1;
					RangeCoder.SKP_Silk_range_encoder(psRC, bit,
							TablesOther.SKP_Silk_lsb_CDF, 0);
		...
		/* Encode signs */
		/****************/
		CodeSigns.SKP_Silk_encode_signs(psRC, q, frame_length, sigtype,
				QuantOffsetType, RateLevelIndex);
	}
}
