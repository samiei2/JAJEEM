...
public class Macros {
	...
	// a32 + (b32 * (SKP_int32)((SKP_int16)(c32))) >> 16 output have to be 32bit
	// int
	...
	// (SKP_int32)((SKP_int16)(a3))) * (SKP_int32)((SKP_int16)(b32)) output have
	// to be 32bit int
	...
	static int SKP_SMULWW(int a32, int b32) {
		return SKP_SMULWB(a32, b32) + a32
				* SigProcFIX.SKP_RSHIFT_ROUND(b32, 16);
	}

	// a32 + ((b32 * c32) >> 16)
	static int SKP_SMLAWW(int a32, int b32, int c32) {
		return SKP_SMLAWB(a32, b32, c32) + b32
				* SigProcFIX.SKP_RSHIFT_ROUND(c32, 16);
	}
	...
	static int SKP_SUB_SAT32(int a, int b) {
		if (((a - b) & 0x80000000) == 0) {
			return (a & (b ^ 0x80000000) & 0x80000000) != 0 ? Integer.MIN_VALUE
					: a - b;
		} else {
			return ((a ^ 0x80000000) & b & 0x80000000) != 0 ? Integer.MAX_VALUE
					: a - b;
		}
	}
