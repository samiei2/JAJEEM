...
public class ResamplerPrivateIIRFIR {
	...
	static void SKP_Silk_resampler_private_IIR_FIR(Object SS, /*
		...
		short[] buf = new short[2
				* ResamplerPrivate.RESAMPLER_MAX_BATCH_SIZE_IN
		...
		while (true) {
			...
			if (S.input2x == 1) {
				/* Upsample 2x */
				S.up2_function(S.sIIR, buf,
						ResamplerRom.RESAMPLER_ORDER_FIR_144, in, in_offset,
						nSamplesIn);
			} else {
				/* Fourth-order ARMA filter */
				ResamplerPrivateARMA4.SKP_Silk_resampler_private_ARMA4(S.sIIR,
						0, buf, ResamplerRom.RESAMPLER_ORDER_FIR_144, in,
			...
			for (index_Q16 = 0; index_Q16 < max_index_Q16; index_Q16 += index_increment_Q16) {
				...
				res_Q15 = Macros
						.SKP_SMULBB(
								buf[buf_ptr],
								ResamplerRom.SKP_Silk_resampler_frac_FIR_144[table_index][0]);
				res_Q15 = Macros
						.SKP_SMLABB(
								res_Q15,
								buf[buf_ptr + 1],
								ResamplerRom.SKP_Silk_resampler_frac_FIR_144[table_index][1]);
				res_Q15 = Macros
						.SKP_SMLABB(
								res_Q15,
								buf[buf_ptr + 2],
								ResamplerRom.SKP_Silk_resampler_frac_FIR_144[table_index][2]);
				res_Q15 = Macros
						.SKP_SMLABB(
								res_Q15,
								buf[buf_ptr + 3],
								ResamplerRom.SKP_Silk_resampler_frac_FIR_144[143 - table_index][2]);
				res_Q15 = Macros
						.SKP_SMLABB(
								res_Q15,
								buf[buf_ptr + 4],
								ResamplerRom.SKP_Silk_resampler_frac_FIR_144[143 - table_index][1]);
				res_Q15 = Macros
						.SKP_SMLABB(
								res_Q15,
								buf[buf_ptr + 5],
								ResamplerRom.SKP_Silk_resampler_frac_FIR_144[143 - table_index][0]);
				out[out_offset++] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
						.SKP_RSHIFT_ROUND(res_Q15, 15));
			}
			...
			if (inLen > 0) {
				 ...
				 */
				// TODO:litter-endian or big-endian???
				// SKP_memcpy( buf, &buf[ nSamplesIn << S->input2x ],
				// RESAMPLER_ORDER_FIR_144 * sizeof( SKP_int32 ) );
				for (int i_djinn = 0; i_djinn < ResamplerRom.RESAMPLER_ORDER_FIR_144; i_djinn++) {
		...
		// TODO:litter-endian or big-endian???
		// SKP_memcpy( S->sFIR, &buf[nSamplesIn << S->input2x ],
		// RESAMPLER_ORDER_FIR_144 * sizeof( SKP_int32 ) );
		for (int i_djinn = 0; i_djinn < ResamplerRom.RESAMPLER_ORDER_FIR_144; i_djinn++) {
			// S.sFIR[i_djinn] = (int)buf[(nSamplesIn << S.input2x) + 2*i_djinn]
			// << 16;
			// S.sFIR[i_djinn] |= (int)buf[(nSamplesIn << S.input2x) +
			// 2*i_djinn+1] & 0x0000FFFF;
			// little-endian
			...
			S.sFIR[i_djinn] |= (((buf[(nSamplesIn << S.input2x) + 2 * i_djinn
					+ 1] >> 0) & 0xFF) << 16) & 0x00FF0000;
			S.sFIR[i_djinn] |= (((buf[(nSamplesIn << S.input2x) + 2 * i_djinn
					+ 1] >> 8) & 0xFF) << 24) & 0xFF000000;
		}
	}
}
