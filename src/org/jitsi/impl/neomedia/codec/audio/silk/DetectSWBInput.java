...
public class DetectSWBInput {
	...
	static void SKP_Silk_detect_SWB_input(
			...
			int samplesIn_offset, int nSamplesIn /* (I) length of input */
	) {
		...
		/* Cutoff around 9 khz */
		/*
		 * A = conv(conv([8192,14613, 6868], [8192,12883, 7337]), [8192,11586,
		 * 7911]);
		 */
		...
		Biquad.SKP_Silk_biquad(samplesIn, samplesIn_offset,
				...
				TablesOther.SKP_Silk_SWB_detect_A_HP_Q13[0],
				psSWBdetect.S_HP_8_kHz[0], in_HP_8_kHz, 0, HP_8_kHz_len);
		for (i = 1; i < Define.NB_SOS; i++) {
			Biquad.SKP_Silk_biquad(in_HP_8_kHz, 0,
					TablesOther.SKP_Silk_SWB_detect_B_HP_Q13[i],
					TablesOther.SKP_Silk_SWB_detect_A_HP_Q13[i],
					psSWBdetect.S_HP_8_kHz[i], in_HP_8_kHz, 0, HP_8_kHz_len);
		}

		/* Calculate energy in HP signal */
		SumSqrShift.SKP_Silk_sum_sqr_shift(energy_32, shift, in_HP_8_kHz, 0,
				HP_8_kHz_len);

		/*
		 * Count concecutive samples above threshold, after adjusting threshold
		 * for number of input samples and shift
		 */
		if (energy_32[0] > Macros.SKP_SMULBB(Define.HP_8_KHZ_THRES,
				HP_8_kHz_len) >> shift[0]) {
			...
			psSWBdetect.ConsecSmplsAboveThres -= nSamplesIn;
			psSWBdetect.ConsecSmplsAboveThres = Math.max(
					psSWBdetect.ConsecSmplsAboveThres, 0);
		}

		/*
		 * If sufficient speech activity and no SWB detected, we detect the
		 * signal as being WB
		 */
		if ((psSWBdetect.ActiveSpeech_ms > Define.WB_DETECT_ACTIVE_SPEECH_MS_THRES)
