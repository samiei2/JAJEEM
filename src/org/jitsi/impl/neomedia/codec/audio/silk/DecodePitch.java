...
public class DecodePitch {
	...
	static void SKP_Silk_decode_pitch(int lagIndex, /* I */
		...
		min_lag = Macros.SKP_SMULBB(CommonPitchEstDefines.PITCH_EST_MIN_LAG_MS,
				Fs_kHz);
		...
		if (Fs_kHz == 8) {
			...
			for (i = 0; i < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; i++) {
				pitch_lags[i] = lag
						+ PitchEstTables.SKP_Silk_CB_lags_stage2[i][contourIndex];
			}
		} else {
			for (i = 0; i < CommonPitchEstDefines.PITCH_EST_NB_SUBFR; i++) {
				pitch_lags[i] = lag
						+ PitchEstTables.SKP_Silk_CB_lags_stage3[i][contourIndex];
			}
		}
	}
}
