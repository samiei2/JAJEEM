...
public class EncodeFrameFLP {
	 ...
	 * @param pnBytesOut
	 *            Number of payload bytes; input: max length; output: used.
	 * @param pIn
	 *            Input speech frame
	 * @param pIn_offset
	 *            offset of valid data.
	...
	static int SKP_Silk_encode_frame_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
																			 ...
																			 * FLP
																			 */
			byte[] pCode, int pCode_offset, short[] pnBytesOut, /*
																 * I/O Number of
																 * payload
																 * bytes;
																 */
		...
		float[] res_pitch = new float[2 * Define.MAX_FRAME_LENGTH
				+ Define.LA_PITCH_MAX];
		...
		x_frame = psEnc.x_buf;
		x_frame_offset = psEnc.x_buf_offset + psEnc.sCmn.frame_length; // start
																		// of
																		// frame
																		// to
																		// encode
		res_pitch_frame = res_pitch;
		res_pitch_frame_offset = psEnc.sCmn.frame_length; // start of pitch LPC
															// residual frame
		...
		if (Define.HIGH_PASS_INPUT != 0) {
			...
			HPVariableCutoffFLP.SKP_Silk_HP_variable_cutoff_FLP(psEnc,
					sEncCtrl, pIn_HP, 0, pIn, pIn_offset);
		} else {
			System.arraycopy(pIn, pIn_offset, pIn_HP, 0,
		...
		if (Define.SWITCH_TRANSITION_FILTERING != 0) {
			...
			LPVariableCutoff.SKP_Silk_LP_variable_cutoff(psEnc.sCmn.sLP,
					pIn_HP_LP, 0, pIn_HP, 0, psEnc.sCmn.frame_length);
		} else {
		...
		/*******************************************/
		SigProcFLP.SKP_short2float_array(x_frame, x_frame_offset
				+ psEnc.sCmn.la_shape, pIn_HP_LP, 0, psEnc.sCmn.frame_length);
		...
		/*
		 * Add tiny signal to avoid high CPU load from denormalized floating
		 * point numbers
		 */
		for (k = 0; k < 8; k++) {
			x_frame[x_frame_offset + psEnc.sCmn.la_shape + k
					* (psEnc.sCmn.frame_length >> 3)] += (1 - (k & 2)) * 1e-6f;
		}
		/*
		 * TEST******************************************************************
		 * *********
		...
		/*****************************************/
		FindPitchLagsFLP.SKP_Silk_find_pitch_lags_FLP(psEnc, sEncCtrl,
				res_pitch, x_frame, x_frame_offset);

		/************************/
		/* Noise shape analysis */
		/************************/
		NoiseShapeAnalysisFLP.SKP_Silk_noise_shape_analysis_FLP(psEnc,
				sEncCtrl, res_pitch_frame, res_pitch_frame_offset, x_frame,
				x_frame_offset);

		/*****************************************/
		/* Prefiltering for noise shaper */
		/*****************************************/
		PrefilterFLP.SKP_Silk_prefilter_FLP(psEnc, sEncCtrl, xfw, x_frame,
				x_frame_offset);

		/***************************************************/
		/* Find linear prediction coefficients (LPC + LTP) */
		/***************************************************/
		FindPredCoefsFLP.SKP_Silk_find_pred_coefs_FLP(psEnc, sEncCtrl,
				res_pitch);
		...
		/*****************************************/
		WrappersFLP.SKP_Silk_NSQ_wrapper_FLP(psEnc, sEncCtrl, xfw, 0,
				psEnc.sCmn.q, 0, 0);
		...
		/* Encode Parameters */
		/****************************************/
		EncodeParameters.SKP_Silk_encode_parameters(psEnc.sCmn, sEncCtrl.sCmn,
				psEnc.sCmn.sRC, psEnc.sCmn.q);
		...
		/****************************************/
		/* Update input buffer */
		System.arraycopy(psEnc.x_buf, psEnc.x_buf_offset
				+ psEnc.sCmn.frame_length, psEnc.x_buf, psEnc.x_buf_offset,
				psEnc.sCmn.frame_length + psEnc.sCmn.la_shape);
		...
		if (psEnc.sCmn.nFramesInPayloadBuf * Define.FRAME_LENGTH_MS >= psEnc.sCmn.PacketSize_ms) {
			...
			RangeCoder.SKP_Silk_range_encoder(psEnc.sCmn.sRC, frame_terminator,
					FrameTermination_CDF, 0);

			/* Payload length so far */
			RangeCoder.SKP_Silk_range_coder_get_length(psEnc.sCmn.sRC, nBytes);

			/*
			 * Check that there is enough space in external output buffer, and
			 * move data
			 */
			if (pnBytesOut[0] >= nBytes[0]) {
				RangeCoder.SKP_Silk_range_enc_wrap_up(psEnc.sCmn.sRC);
				System.arraycopy(psEnc.sCmn.sRC.buffer, 0, pCode, pCode_offset,
						nBytes[0]);

				if (frame_terminator > Define.SKP_SILK_MORE_FRAMES
						&& pnBytesOut[0] >= nBytes[0]
								+ psEnc.sCmn.LBRR_buffer[LBRR_idx].nBytes) {
					/* Get old packet and add to payload. */
					System.arraycopy(psEnc.sCmn.LBRR_buffer[LBRR_idx].payload,
							0, pCode, pCode_offset + nBytes[0],
							psEnc.sCmn.LBRR_buffer[LBRR_idx].nBytes);
				...
				System.arraycopy(
						LBRRpayload,
						0,
						psEnc.sCmn.LBRR_buffer[psEnc.sCmn.oldest_LBRR_idx].payload,
						0, nBytesLBRR[0]);
		...
		/*
		 * simulate number of ms buffered in channel because of exceeding
		 * TargetRate
		 */
		psEnc.BufferedInChannel_ms += (8.0f * 1000.0f * (nBytes[0] - psEnc.sCmn.nBytesInPayloadBuf))
				/ psEnc.sCmn.TargetRate_bps;
		psEnc.BufferedInChannel_ms -= Define.FRAME_LENGTH_MS;
		psEnc.BufferedInChannel_ms = SigProcFLP.SKP_LIMIT_float(
				psEnc.BufferedInChannel_ms, 0.0f, 100.0f);
		psEnc.sCmn.nBytesInPayloadBuf = nBytes[0];

		if (psEnc.speech_activity > DefineFLP.WB_DETECT_ACTIVE_SPEECH_LEVEL_THRES) {
			psEnc.sCmn.sSWBdetect.ActiveSpeech_ms = SigProcFIX
					.SKP_ADD_POS_SAT32(psEnc.sCmn.sSWBdetect.ActiveSpeech_ms,
							Define.FRAME_LENGTH_MS);
		}
	...
	static void SKP_Silk_LBRR_encode_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
		...
		if (psEnc.sCmn.LBRR_enabled != 0) {
			/* Save original gains */
			System.arraycopy(psEncCtrl.sCmn.GainsIndices, 0, TempGainsIndices,
					0, Define.NB_SUBFR);
			System.arraycopy(psEncCtrl.Gains, 0, TempGains, 0, Define.NB_SUBFR);

			typeOffset = psEnc.sCmn.typeOffsetPrev; // Temp save as cannot be
													// overwritten
			...
			if (psEnc.sCmn.Complexity > 0
				...
				if (psEnc.sCmn.nFramesInPayloadBuf == 0) {
					...
					psEncCtrl.sCmn.GainsIndices[0] = SigProcFIX.SKP_LIMIT(
							psEncCtrl.sCmn.GainsIndices[0], 0,
							Define.N_LEVELS_QGAIN - 1);
				}
				/* Decode to get Gains in sync with decoder */
				int LBRRprevLastGainIndex_ptr[] = new int[1];
				LBRRprevLastGainIndex_ptr[0] = psEnc.sCmn.LBRRprevLastGainIndex;
				GainQuant.SKP_Silk_gains_dequant(Gains_Q16,
						psEncCtrl.sCmn.GainsIndices, LBRRprevLastGainIndex_ptr,
						psEnc.sCmn.nFramesInPayloadBuf);
				psEnc.sCmn.LBRRprevLastGainIndex = LBRRprevLastGainIndex_ptr[0];

				/*
				 * Overwrite unquantized gains with quantized gains and convert
				 * back to Q0 from Q16
				 */
				...
				/* Noise shaping quantization */
				/*****************************************/
				WrappersFLP.SKP_Silk_NSQ_wrapper_FLP(psEnc, psEncCtrl, xfw, 0,
						psEnc.sCmn.q_LBRR, 0, 1);
			} else {
			...
			/* Encode Parameters */
			/****************************************/
			EncodeParameters.SKP_Silk_encode_parameters(psEnc.sCmn,
					psEncCtrl.sCmn, psEnc.sCmn.sRC_LBRR, psEnc.sCmn.q_LBRR);
			/****************************************/
			/* Encode Parameters */
			/****************************************/
			...
			if (Macros.SKP_SMULBB(nFramesInPayloadBuf, Define.FRAME_LENGTH_MS) >= psEnc.sCmn.PacketSize_ms) {
				...
				/* Add the frame termination info to stream */
				RangeCoder.SKP_Silk_range_encoder(psEnc.sCmn.sRC_LBRR,
						frame_terminator,
						TablesOther.SKP_Silk_FrameTermination_CDF, 0);

				/* Payload length so far */
				RangeCoder.SKP_Silk_range_coder_get_length(psEnc.sCmn.sRC_LBRR,
						nBytes);

				/*
				 * Check that there is enough space in external output buffer
				 * and move data
				 */
				if (pnBytesOut[0] >= nBytes[0]) {
					RangeCoder.SKP_Silk_range_enc_wrap_up(psEnc.sCmn.sRC_LBRR);
					System.arraycopy(psEnc.sCmn.sRC_LBRR.buffer, 0, pCode, 0,
							nBytes[0]);
			...
			System.arraycopy(TempGainsIndices, 0, psEncCtrl.sCmn.GainsIndices,
					0, Define.NB_SUBFR);
