...
public class DecodeParameters {
	...
	static void SKP_Silk_decode_parameters(SKP_Silk_decoder_state psDec, /*
			...
			final int fullDecoding /*
									 * I Flag to tell if only arithmetic
									 * decoding
									 */
	) {
		...
		if (psDec.nFramesDecoded == 0) {
			...
			RangeCoder
					.SKP_Silk_range_decoder(
							...
							0,
							psRC,
							TablesTypeOffset.SKP_Silk_type_offset_joint_CDF[psDec.typeOffsetPrev],
							0, TablesTypeOffset.SKP_Silk_type_offset_CDF_offset);
			Ix = Ix_ptr[0];
		}
		...
		if (psDec.nFramesDecoded == 0) {
			...
			RangeCoder.SKP_Silk_range_decoder(GainsIndices, 0, psRC,
					...
					TablesGain.SKP_Silk_delta_gain_CDF_offset);
		}
		...
		GainQuant.SKP_Silk_gains_dequant(psDecCtrl.Gains_Q16, GainsIndices,
				LastGainIndex_ptr, psDec.nFramesDecoded);
		...
		RangeCoder.SKP_Silk_range_decoder_multi(NLSFIndices, psRC,
				psNLSF_CB.StartPtr, psNLSF_CB.MiddleIx, psNLSF_CB.nStages);

		/* From the NLSF path, decode an NLSF vector */
		NLSFMSVQDecode.SKP_Silk_NLSF_MSVQ_decode(pNLSF_Q15, psNLSF_CB,
				NLSFIndices, psDec.LPC_order);
		...
		if (fullDecoding != 0) {
			...
			NLSF2AStable.SKP_Silk_NLSF2A_stable(psDecCtrl.PredCoef_Q12[1],
					pNLSF_Q15, psDec.LPC_order);
			if (psDecCtrl.NLSFInterpCoef_Q2 < 4) {
				/*
				 * Calculation of the interpolated NLSF0 vector from the
				 * interpolation factor,
				 */
				/* the previous NLSF1, and the current NLSF1 */
				for (i = 0; i < psDec.LPC_order; i++) {
					pNLSF0_Q15[i] = psDec.prevNLSF_Q15[i]
							+ ((psDecCtrl.NLSFInterpCoef_Q2 * (pNLSF_Q15[i] - psDec.prevNLSF_Q15[i])) >> 2);
				}

				/* Convert NLSF parameters to AR prediction filter coefficients */
				NLSF2AStable.SKP_Silk_NLSF2A_stable(psDecCtrl.PredCoef_Q12[0],
						pNLSF0_Q15, psDec.LPC_order);
			} else {
				/* Copy LPC coefficients for first half from second half */
				System.arraycopy(psDecCtrl.PredCoef_Q12[1], 0,
						psDecCtrl.PredCoef_Q12[0], 0, psDec.LPC_order);
			}
		}
		...
		if (psDec.lossCnt != 0) {
			Bwexpander.SKP_Silk_bwexpander(psDecCtrl.PredCoef_Q12[0],
					psDec.LPC_order, Define.BWE_AFTER_LOSS_Q16);
			Bwexpander.SKP_Silk_bwexpander(psDecCtrl.PredCoef_Q12[1],
					psDec.LPC_order, Define.BWE_AFTER_LOSS_Q16);
		}
		...
		if (psDecCtrl.sigtype == Define.SIG_TYPE_VOICED) {
			...
			/* Decode pitch values */
			DecodePitch.SKP_Silk_decode_pitch(Ixs[0], Ixs[1], psDecCtrl.pitchL,
					psDec.fs_kHz);
			...
			cbk_ptr_Q14 = TablesLTP.SKP_Silk_LTP_vq_ptrs_Q14[psDecCtrl.PERIndex]; // set
																					// pointer
																					// to
																					// start
																					// of
			...
			for (k = 0; k < Define.NB_SUBFR; k++) {
				RangeCoder
						.SKP_Silk_range_decoder(
								...
								0,
								psRC,
								TablesLTP.SKP_Silk_LTP_gain_CDF_ptrs[psDecCtrl.PERIndex],
								0,
								TablesLTP.SKP_Silk_LTP_gain_CDF_offsets[psDecCtrl.PERIndex]);
				Ix = Ix_ptr[0];
				for (i = 0; i < Define.LTP_ORDER; i++) {
					psDecCtrl.LTPCoef_Q14[Macros
							.SKP_SMULBB(k, Define.LTP_ORDER) + i] = cbk_ptr_Q14[Macros
							.SKP_SMULBB(Ix, Define.LTP_ORDER) + i];
				}
			}
			...
			RangeCoder.SKP_Silk_range_decoder(Ix_ptr, 0, psRC,
					...
					TablesOther.SKP_Silk_LTPscale_offset);
			Ix = Ix_ptr[0];
		...
		/***************/
		RangeCoder.SKP_Silk_range_decoder(Ix_ptr, 0, psRC,
				TablesOther.SKP_Silk_Seed_CDF, 0,
		...
		/*********************************************/
		DecodePulses.SKP_Silk_decode_pulses(psRC, psDecCtrl, q,
				psDec.frame_length);
		...
		RangeCoder.SKP_Silk_range_decoder(vadFlag_ptr, 0, psRC,
				TablesOther.SKP_Silk_vadflag_CDF, 0,
				TablesOther.SKP_Silk_vadflag_offset);
