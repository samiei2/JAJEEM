...
public class CNG {
	 ...
	 * 
	 */
	static void SKP_Silk_CNG_exc(short residual[], /* O CNG residual signal Q0 */
			int residual_offset, int exc_buf_Q10[], /*
													 * I Random samples buffer
													 * Q10
													 */
			int exc_buf_Q10_offset, int Gain_Q16, /* I Gain to apply */
			int length, /* I Length */
		...
		for (i = 0; i < length; i++) {
			...
			residual[residual_offset + i] = (short) SigProcFIX
					.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(
							Macros.SKP_SMULWW(exc_buf_Q10[idx], Gain_Q16), 10));
		}
		rand_seed[0] = seed;
	}
	 ...
	 *            Length of residual.
	 */
	static void SKP_Silk_CNG(SKP_Silk_decoder_state psDec, /* I/O Decoder state */
		...
		if (psDec.lossCnt == 0 && psDec.vadFlag == Define.NO_VOICE_ACTIVITY) {
			...
			/* Smoothing of LSF's */
			for (i = 0; i < psDec.LPC_order; i++) {
				psCNG.CNG_smth_NLSF_Q15[i] += Macros.SKP_SMULWB(
						psDec.prevNLSF_Q15[i] - psCNG.CNG_smth_NLSF_Q15[i],
						Define.CNG_NLSF_SMTH_Q16);
			}
			...
			System.arraycopy(psCNG.CNG_exc_buf_Q10, 0, psCNG.CNG_exc_buf_Q10,
					psDec.subfr_length, (Define.NB_SUBFR - 1)
							* psDec.subfr_length);
			System.arraycopy(psDec.exc_Q10, subfr * psDec.subfr_length,
					psCNG.CNG_exc_buf_Q10, 0, psDec.subfr_length);
			/* Smooth gains */
			for (i = 0; i < Define.NB_SUBFR; i++) {
				psCNG.CNG_smth_Gain_Q16 += Macros.SKP_SMULWB(
		...
		if (psDec.lossCnt != 0) {// || psDec.vadFlag == NO_VOICE_ACTIVITY ) {
			...
			NLSF2AStable.SKP_Silk_NLSF2A_stable(LPC_buf,
			...
			if (psDec.LPC_order == 16) {
				LPCSynthesisOrder16.SKP_Silk_LPC_synthesis_order16(CNG_sig,
						LPC_buf, Gain_Q26, psCNG.CNG_synth_state, CNG_sig,
						length);
			} else {
				LPCSynthesisFilter.SKP_Silk_LPC_synthesis_filter(CNG_sig,
						LPC_buf, Gain_Q26, psCNG.CNG_synth_state, CNG_sig,
						length, psDec.LPC_order);
			}
			/* Mix with signal */
			for (i = 0; i < length; i++) {
				tmp_32 = signal[signal_offset + i] + CNG_sig[i];
				signal[signal_offset + i] = (short) SigProcFIX
						.SKP_SAT16(tmp_32);
			}
		} else {
