...
public class CorrMatrixFLP {
	 ...
	 */
	static void SKP_Silk_corrVector_FLP(final float[] x, /*
														 * I x vector
														 * [L+order-1] used to
														 * create X
														 */
			int x_offset, final float[] t, /* I Target vector [L] */
			int t_offset, final int L, /* I Length of vecors */
		...
		for (lag = 0; lag < Order; lag++) {
			/* Calculate X[:,lag]'*t */
			Xt[lag] = (float) InnerProductFLP.SKP_Silk_inner_product_FLP(ptr1,
					ptr1_offset, t, t_offset, L);
	 ...
	 *            offset of valid data.
	 */
	static void SKP_Silk_corrMatrix_FLP(final float[] x, /*
														 * I x vector [
														 * L+order-1 ] used to
														 * create X
														 */
			int x_offset, final int L, /* I Length of vectors */
			final int Order, /* I Max lag for correlation */
		...
		energy = EnergyFLP.SKP_Silk_energy_FLP(ptr1, ptr1_offset, L);/*
																	 * X[:,0]'*X[
																	 * :,0]
																	 */
		...
		for (j = 1; j < Order; j++) {
			/* Calculate X[:,j]'*X[:,j] */
			energy += ptr1[ptr1_offset - j] * ptr1[ptr1_offset - j]
					- ptr1[ptr1_offset + L - j] * ptr1[ptr1_offset + L - j];
		...
		for (lag = 1; lag < Order; lag++) {
			...
			energy = InnerProductFLP.SKP_Silk_inner_product_FLP(ptr1,
					ptr1_offset, ptr2, ptr2_offset, L);
			/* Calculate X[:,j]'*X[:,j + lag] */
			for (j = 1; j < (Order - lag); j++) {
				// matrix_ptr( XX, lag + j, j, Order ) = ( SKP_float )energy;
				// matrix_ptr( XX, j, lag + j, Order ) = ( SKP_float )energy;
				energy += ptr1[ptr1_offset - j] * ptr2[ptr2_offset - j]
						- ptr1[ptr1_offset + L - j] * ptr2[ptr2_offset + L - j];
