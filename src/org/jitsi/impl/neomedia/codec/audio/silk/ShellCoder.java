...
public class ShellCoder {
	 ...
	 */
	static void combine_pulses(int[] out, /* O: combined pulses vector [len] */
			int out_offset, int[] in, /* I: input vector [2 * len] */
			int in_offset, final int len /* I: number of OUTPUT samples */
	) {
		int k;
		for (k = 0; k < len; k++) {
			out[out_offset + k] = in[in_offset + 2 * k]
					+ in[in_offset + 2 * k + 1];
		}
	}
	...
	static void decode_split(int[] p_child1, /*
											 * O: pulse amplitude of first child
											 * subframe
											 */
			int p_child1_offset, int[] p_child2, /*
												 * O: pulse amplitude of second
												 * child subframe
												 */
			int p_child2_offset, SKP_Silk_range_coder_state sRC, /*
																 * I/O:
																 * compressor
																 * data
																 * structure
		...
		if (p > 0) {
			...
			RangeCoder.SKP_Silk_range_decoder(p_child1, p_child1_offset, sRC,
					cdf, cdf_offset, cdf_middle);
	...
	static void SKP_Silk_shell_encoder(SKP_Silk_range_coder_state sRC, /*
		...
		encode_split(sRC, pulses3[0], pulses4[0],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table3);

		encode_split(sRC, pulses2[0], pulses3[0],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table2);

		encode_split(sRC, pulses1[0], pulses2[0],
		...
		encode_split(sRC, pulses1[2], pulses2[1],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
		...
		encode_split(sRC, pulses2[2], pulses3[1],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table2);

		encode_split(sRC, pulses1[4], pulses2[2],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
		...
		encode_split(sRC, pulses1[6], pulses2[3],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
	...
	static void SKP_Silk_shell_decoder(int[] pulses0, /*
													 * O data: nonnegative pulse
													 * amplitudes
													 */
			int pulses0_offset, SKP_Silk_range_coder_state sRC, /*
																 * I/O
																 * compressor
																 * data
																 * structure
		...
		decode_split(pulses1, 0, pulses1, 1, sRC, pulses2[0],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
		decode_split(pulses0, pulses0_offset + 0, pulses0, pulses0_offset + 1,
				sRC, pulses1[0],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);
		decode_split(pulses0, pulses0_offset + 2, pulses0, pulses0_offset + 3,
				sRC, pulses1[1],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);

		decode_split(pulses1, 2, pulses1, 3, sRC, pulses2[1],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
		decode_split(pulses0, pulses0_offset + 4, pulses0, pulses0_offset + 5,
				sRC, pulses1[2],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);
		decode_split(pulses0, pulses0_offset + 6, pulses0, pulses0_offset + 7,
				sRC, pulses1[3],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);
		...
		decode_split(pulses1, 4, pulses1, 5, sRC, pulses2[2],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
		decode_split(pulses0, pulses0_offset + 8, pulses0, pulses0_offset + 9,
				sRC, pulses1[4],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);
		decode_split(pulses0, pulses0_offset + 10, pulses0,
				pulses0_offset + 11, sRC, pulses1[5],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);

		decode_split(pulses1, 6, pulses1, 7, sRC, pulses2[3],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table1);
		decode_split(pulses0, pulses0_offset + 12, pulses0,
				pulses0_offset + 13, sRC, pulses1[6],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);
		decode_split(pulses0, pulses0_offset + 14, pulses0,
				pulses0_offset + 15, sRC, pulses1[7],
				TablesPulsesPerBlock.SKP_Silk_shell_code_table0);
	}
}
