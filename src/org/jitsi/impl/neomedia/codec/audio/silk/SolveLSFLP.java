...
public class SolveLSFLP {
	/**
	 * Function to solve linear equation Ax = b, when A is a MxM symmetric
	 * square matrix - using LDL factorisation.
	 * 
	 * @param A
	...
	static void SKP_Silk_solve_LDL_FLP(float[] A, /*
		...
		int i;
		// float L[][] = new
		// float[Define.MAX_MATRIX_SIZE][Define.MAX_MATRIX_SIZE];
		// TODO:change L from two dimension to one dimension.
		float L_tmp[] = new float[Define.MAX_MATRIX_SIZE
				* Define.MAX_MATRIX_SIZE];
		float T[] = new float[Define.MAX_MATRIX_SIZE];
		float Dinv[] = new float[Define.MAX_MATRIX_SIZE];// inverse diagonal
															// elements of D

		assert (M <= Define.MAX_MATRIX_SIZE);

		/***************************************************
		 * Factorize A by LDL such that A = L*D*(L^T), where L is lower
		 * triangular with ones on diagonal
		 ****************************************************/
		// SKP_Silk_LDL_FLP( A, M, &L[ 0 ][ 0 ], Dinv );
		SKP_Silk_LDL_FLP(A, A_offset, M, L_tmp, Dinv);

		/****************************************************
		 * substitute D*(L^T) = T. ie: L*D*(L^T)*x = b => L*T = b <=> T =
		 * inv(L)*b
		 ******************************************************/
		...
		/****************************************************
		 * D*(L^T)*x = T <=> (L^T)*x = inv(D)*T, because D is diagonal just
		 * multiply with 1/d_i
		 ****************************************************/
		for (i = 0; i < M; i++) {
			T[i] = T[i] * Dinv[i];
		 ...
		 * x = inv(L') * inv(D) * T
		 *****************************************************/
		// SKP_Silk_SolveWithUpperTriangularFromLowerWdiagOnes_FLP( &L[ 0 ][ 0
		// ], M, T, x );
		SKP_Silk_SolveWithUpperTriangularFromLowerWdiagOnes_FLP(L_tmp, M, T, x,
				x_offset);
	 ...
	 * Matrix D (only the diagonal elements returned in a vector)such that the
	 * symmetric matric A is given by A = L*D*L'.
	 * 
	 * @param A
	...
	static void SKP_Silk_LDL_FLP(float[] A, /*
											 * (I/O) Pointer to Symetric Square
											 * Matrix
											 */
			int A_offset, int M, /* (I) Size of Matrix */
			float[] L, /* (I/O) Pointer to Square Upper triangular Matrix */
			float[] Dinv /*
						 * (I/O) Pointer to vector holding the inverse diagonal
						 * elements of D
						 */
	) {
		/*
		 * SKP_int i, j, k, loop_count, err = 1; SKP_float *ptr1, *ptr2; double
		 * temp, diag_min_value; SKP_float v[ MAX_MATRIX_SIZE ], D[
		 * MAX_MATRIX_SIZE ]; // temp arrays
		 */
		int i, j, k, loop_count, err = 1;
		float ptr1[], ptr2[];
		int ptr1_offset, ptr2_offset;
		double temp, diag_min_value;
		float v[] = new float[Define.MAX_MATRIX_SIZE], D[] = new float[Define.MAX_MATRIX_SIZE]; // temp
																								// arrays
		...
		for (loop_count = 0; loop_count < M && err == 1; loop_count++) {
			...
			for (j = 0; j < M; j++) {
				...
				for (i = j + 1; i < M; i++) {
					...
					// matrix_ptr( L, i, j, M ) = ( SKP_float )( ( ptr1[ i ] -
					// temp ) * Dinv[ j ] );
