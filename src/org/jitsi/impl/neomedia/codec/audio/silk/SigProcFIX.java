...
public class SigProcFIX extends SigProcFIXConstants {
	...
	static int SKP_SAT8(int a) {
		return ((a) > Byte.MAX_VALUE ? Byte.MAX_VALUE
				: ((a) < Byte.MIN_VALUE ? Byte.MIN_VALUE : (a)));
	}

	static int SKP_SAT16(int a) {
		return ((a) > Short.MAX_VALUE ? Short.MAX_VALUE
				: ((a) < Short.MIN_VALUE ? Short.MIN_VALUE : (a)));
	}
	...
	static long SKP_ADD_SAT64(long a, long b) {
		if (((a + b) & 0x8000000000000000L) == 0) {
			return ((a & b) & 0x8000000000000000L) != 0 ? Long.MIN_VALUE : a
					+ b;
		} else {
			return ((a | b) & 0x8000000000000000L) == 0 ? Long.MAX_VALUE : a
					+ b;
		}
	}
	...
	static long SKP_SUB_SAT64(long a, long b) {
		if (((a - b) & 0x8000000000000000L) == 0) {
			return (a & (b ^ 0x8000000000000000L) & 0x8000000000000000L) != 0 ? Long.MIN_VALUE
					: a - b;
		} else {
			return ((a ^ 0x8000000000000000L) & b & 0x8000000000000000L) != 0 ? Long.MAX_VALUE
					: a - b;
		}
	}
	...
	static int SKP_ADD_POS_SAT32(int a, int b) {
		return ((((a) + (b)) & 0x80000000) != 0 ? Integer.MAX_VALUE
				: ((a) + (b)));
	}

	static long SKP_ADD_POS_SAT64(long a, long b) {
		return ((((a) + (b)) & 0x8000000000000000L) != 0 ? Long.MAX_VALUE
				: ((a) + (b)));
	}
	...
	static short SKP_LSHIFT_SAT16(short a, int shift) {
		return SKP_LSHIFT16(
				SKP_LIMIT_16(a, (short) (Short.MIN_VALUE >> shift),
						(short) (Short.MAX_VALUE >> shift)), shift);
	}

	static int SKP_LSHIFT_SAT32(int a, int shift) {
		return SKP_LSHIFT32(
				SKP_LIMIT(a, Integer.MIN_VALUE >> shift,
						Integer.MAX_VALUE >> shift), shift);
	}
	...
	static int SKP_NSHIFT_MUL_32_32(int a, int b) {
		return -(31 - (32 - Macros.SKP_Silk_CLZ32(Math.abs(a)) + (32 - Macros
				.SKP_Silk_CLZ32(Math.abs(b)))));
	}

	static int SKP_NSHIFT_MUL_16_16(short a, short b) {
		return -(15 - (16 - Macros.SKP_Silk_CLZ16((short) Math.abs(a)) + (16 - Macros
				.SKP_Silk_CLZ16((short) Math.abs(b)))));
	}
	...
	/**
	 * (((a) > 0) ? (a) : -(a)) Be careful, SKP_abs returns wrong when input
	 * equals to SKP_intXX_MIN
	 * 
	 * @param a
	 * @return
	 */
	...
	/**
	 * PSEUDO-RANDOM GENERATOR Make sure to store the result as the seed for the
	 * next call (also in between frames), otherwise result won't be random at
	 * all. When only using some of the bits, take the most significant bits by
	 * right-shifting. Do not just mask off the lowest bits. SKP_RAND(seed)
	 * (SKP_MLA_ovflw(907633515, (seed), 196314165))
	 * 
	 * @param seed
	 * @return
	 */
	...
	// ARMv3M+ 3 instruction cycles. use SMULL and ignore LSB registers.(except
	// xM)
	// #define SKP_SMMUL(a32, b32)
	// (SKP_int32)SKP_RSHIFT(SKP_SMLAL(SKP_SMULWB((a32), (b32)), (a32),
