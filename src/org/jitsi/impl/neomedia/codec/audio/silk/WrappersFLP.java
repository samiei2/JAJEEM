...
public class WrappersFLP {
	...
	static void SKP_Silk_NLSF_stabilize_FLP(float[] pNLSF, /*
		...
		for (i = 0; i < LPC_order; i++) {
			...
			ndelta_min_Q15[i] = SigProcFLP
					.SKP_float2int(pNDelta_min[i] * 32768.0f);
		}
		ndelta_min_Q15[LPC_order] = SigProcFLP
				.SKP_float2int(pNDelta_min[LPC_order] * 32768.0f);

		/* NLSF stabilizer, for a single input data vector */
		NLSFStabilize.SKP_Silk_NLSF_stabilize(NLSF_Q15, 0, ndelta_min_Q15,
				LPC_order);
	...
	static int SKP_Silk_VAD_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
		...
		ret = VAD.SKP_Silk_VAD_GetSA_Q8(psEnc.sCmn.sVAD, SA_Q8, SNR_dB_Q7,
				Quality_Bands_Q15, Tilt_Q15, pIn, pIn_offset,
	...
	static void SKP_Silk_NSQ_wrapper_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
			...
			int x_offset, byte q[], /* O Quantized pulse signal */
			int q_offset, final int useLBRR /* I LBRR flag */
	) {
		int i, j;
		float tmp_float;
		...
		/* Testing */
		short[] AR2_Q13 = new short[Define.NB_SUBFR
				* Define.SHAPE_LPC_ORDER_MAX];
		int[] LF_shp_Q14 = new int[Define.NB_SUBFR]; /*
													 * Packs two int16
													 * coefficients per int32
													 * value
													 */
		int Lambda_Q10;
		...
		for (i = 0; i < Define.NB_SUBFR * Define.SHAPE_LPC_ORDER_MAX; i++) {
			AR2_Q13[i] = (short) SigProcFIX.SKP_SAT16(SigProcFLP
					.SKP_float2int(psEncCtrl.AR2[i] * 8192.0f));
		}
		...
		// for( i = 0; i < Define.NB_SUBFR * Define.SHAPE_LPC_ORDER_MAX; i++ )
		// {
		// // AR2_Q13[ i ] = (short)SigProcFIX.SKP_SAT16(
		// SigProcFLP.SKP_float2int( psEncCtrl.AR2[ i ] * 8192.0f ) );
		// try
		// {
		// ar2_q13[i] = ar2_q13_datain.readShort();
		// AR2_Q13[i] = (short) (((ar2_q13[i] << 8) & 0xFF00) | ((ar2_q13[i] >>>
		// 8) & 0x00FF));
		// }
		// catch (IOException e)
		// {
		...
		// try
		// {
		// ar2_q13_datain_rand = new RandomAccessFile(new
		// File(ar2_q13_filename), "r");
		// try
		// {
		...
		for (i = 0; i < Define.NB_SUBFR; i++) {
			LF_shp_Q14[i] = (SigProcFLP
					.SKP_float2int(psEncCtrl.LF_AR_shp[i] * 16384.0f) << 16)
					| (0x0000FFFF & SigProcFLP
							.SKP_float2int(psEncCtrl.LF_MA_shp[i] * 16384.0f));
			Tilt_Q14[i] = SigProcFLP
					.SKP_float2int(psEncCtrl.Tilt[i] * 16384.0f);
			HarmShapeGain_Q14[i] = SigProcFLP
					.SKP_float2int(psEncCtrl.HarmShapeGain[i] * 16384.0f);
		}
		Lambda_Q10 = SigProcFLP.SKP_float2int(psEncCtrl.Lambda * 1024.0f);

		/* prediction and coding parameters */
		for (i = 0; i < Define.NB_SUBFR * Define.LTP_ORDER; i++) {
			LTPCoef_Q14[i] = (short) SigProcFLP
					.SKP_float2int(psEncCtrl.LTPCoef[i] * 16384.0f);
		}

		for (j = 0; j < Define.NB_SUBFR >> 1; j++) {
			for (i = 0; i < Define.MAX_LPC_ORDER; i++) {
				PredCoef_Q12[j][i] = (short) SigProcFLP
						.SKP_float2int(psEncCtrl.PredCoef[j][i] * 4096.0f);
			}
		}

		for (i = 0; i < Define.NB_SUBFR; i++) {
			tmp_float = SigProcFIX.SKP_LIMIT((psEncCtrl.Gains[i] * 65536.0f),
					2147483000.0f, -2147483000.0f);
		...
		SigProcFLP.SKP_float2short_array(x_16, 0, x, x_offset,
				psEnc.sCmn.frame_length);
		...
		// x_16[k] = (short)
		// (((x_16_test[k]<<8)&0xFF00)|((x_16_test[k]>>>8)&0x00FF));
		// }
		// catch (IOException e)
		// {
		// // TODO Auto-generated catch block
		...
		// try
		// {
		// x_16_datain_rand = new RandomAccessFile(new File(x_16_filename),
		// "r");
		// try
		// {
		...
		/* Call NSQ */
		short[] PredCoef_Q12_dim1_tmp = new short[PredCoef_Q12.length
				* PredCoef_Q12[0].length];
		int PredCoef_Q12_offset = 0;
		for (int PredCoef_Q12_i = 0; PredCoef_Q12_i < PredCoef_Q12.length; PredCoef_Q12_i++) {
			System.arraycopy(PredCoef_Q12[PredCoef_Q12_i], 0,
					PredCoef_Q12_dim1_tmp, PredCoef_Q12_offset,
					PredCoef_Q12[PredCoef_Q12_i].length);
			PredCoef_Q12_offset += PredCoef_Q12[PredCoef_Q12_i].length;
		}
		if (useLBRR != 0) {
			// psEnc.NoiseShapingQuantizer( psEnc.sCmn, psEncCtrl.sCmn,
			// psEnc.sNSQ_LBRR,
			// x_16, q, psEncCtrl.sCmn.NLSFInterpCoef_Q2, PredCoef_Q12[ 0 ],
			// LTPCoef_Q14, AR2_Q13,
			// HarmShapeGain_Q14, Tilt_Q14, LF_shp_Q14, Gains_Q16, Lambda_Q10,
			// LTP_scale_Q14 );\
			psEnc.NoiseShapingQuantizer(psEnc.sCmn, psEncCtrl.sCmn,
					psEnc.sNSQ_LBRR, x_16, q, psEncCtrl.sCmn.NLSFInterpCoef_Q2,
					PredCoef_Q12_dim1_tmp, LTPCoef_Q14, AR2_Q13,
			...
			psEnc.NoiseShapingQuantizer(psEnc.sCmn, psEncCtrl.sCmn, psEnc.sNSQ,
					x_16, q, psEncCtrl.sCmn.NLSFInterpCoef_Q2,
					PredCoef_Q12_dim1_tmp, LTPCoef_Q14, AR2_Q13,
					HarmShapeGain_Q14, Tilt_Q14, LF_shp_Q14, Gains_Q16,
					Lambda_Q10, LTP_scale_Q14);
			// psEnc.NoiseShapingQuantizer( &psEnc->sCmn, &psEncCtrl->sCmn,
