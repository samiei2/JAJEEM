...
class NSQDelDecStruct {
	...
	int sLPC_Q14[] = new int[Define.MAX_FRAME_LENGTH / Define.NB_SUBFR
...
public class NSQDelDec {
	static void SKP_Silk_NSQ_del_dec(SKP_Silk_encoder_state psEncC, /*
		...
		// TODO: use a local copy of the parameter short x[], which is supposed
		// to be input;
		short[] x_tmp = x.clone();
		int x_tmp_offset = 0;
		// TODO: use a local copy of the parameter byte[] q, which is supposed
		// to be output;
		byte[] q_tmp = q.clone();
		int q_tmp_offset = 0;

		/* Initialize delayed decision states */
		// SKP_memset( psDelDec, 0, psEncC.nStatesDelayedDecision * sizeof(
		// NSQ_del_dec_struct ) );
		// TODO:
		...
		for (k = 0; k < psEncC.nStatesDelayedDecision; k++) {
			...
			// SKP_memcpy( psDD.sLPC_Q14, NSQ.sLPC_Q14, NSQ_LPC_BUF_LENGTH *
			// sizeof( SKP_int32 ) );
			System.arraycopy(NSQ.sLPC_Q14, 0, psDD.sLPC_Q14, 0,
					Define.NSQ_LPC_BUF_LENGTH());
		}
		...
		decisionDelay = (Define.DECISION_DELAY < subfr_length ? Define.DECISION_DELAY
				: subfr_length);
		...
		if (psEncCtrlC.sigtype == Define.SIG_TYPE_VOICED) {
			for (k = 0; k < Define.NB_SUBFR; k++) {
				decisionDelay = (decisionDelay < (psEncCtrlC.pitchL[k]
						- Define.LTP_ORDER / 2 - 1) ? decisionDelay
						: (psEncCtrlC.pitchL[k] - Define.LTP_ORDER / 2 - 1));
			}
		}
		...
		for (k = 0; k < Define.NB_SUBFR; k++) {
			A_Q12 = PredCoef_Q12;
			A_Q12_offset = ((k >> 1) | (1 - LSF_interpolation_flag))
			...
			if (psEncCtrlC.sigtype == Define.SIG_TYPE_VOICED) {
				...
				if ((k & (3 - (LSF_interpolation_flag << 1))) == 0) {
					...
					start_idx = psEncC.frame_length - lag
							- psEncC.predictLPCOrder - Define.LTP_ORDER / 2;
					start_idx = SigProcFIX.SKP_LIMIT_int(start_idx, 0,
							psEncC.frame_length - psEncC.predictLPCOrder);

					// SKP_memset( FiltState, 0, psEncC.predictLPCOrder *
					// sizeof( SKP_int32 ) );
					Arrays.fill(FiltState, 0, psEncC.predictLPCOrder, 0);
					MA.SKP_Silk_MA_Prediction(NSQ.xq, start_idx + k
							* psEncC.subfr_length, A_Q12, A_Q12_offset,
							FiltState, sLTP, start_idx, psEncC.frame_length
			...
			SKP_Silk_nsq_del_dec_scale_states(NSQ, psDelDec, x_tmp,
					x_tmp_offset, x_sc_Q10, subfr_length, sLTP, sLTP_Q16, k,
					psEncC.nStatesDelayedDecision, smpl_buf_idx, LTP_scale_Q14,
					Gains_Q16, psEncCtrlC.pitchL);

			int smpl_buf_idx_ptr[] = new int[1];
			smpl_buf_idx_ptr[0] = smpl_buf_idx;
			SKP_Silk_noise_shape_quantizer_del_dec(NSQ, psDelDec,
					psEncCtrlC.sigtype, x_sc_Q10, q_tmp, q_tmp_offset, pxq,
					pxq_offset, sLTP_Q16, A_Q12, A_Q12_offset, B_Q14,
					B_Q14_offset, AR_shp_Q13, AR_shp_Q13_offset, lag,
					HarmShapeFIRPacked_Q14, Tilt_Q14[k], LF_shp_Q14[k],
					Gains_Q16[k], Lambda_Q10, offset_Q10, psEncC.subfr_length,
					subfr++, psEncC.shapingLPCOrder, psEncC.predictLPCOrder,
					psEncC.nStatesDelayedDecision, smpl_buf_idx_ptr,
					decisionDelay);
		...
		for (i = 0; i < decisionDelay; i++) {
			...
			q_tmp[q_tmp_offset + i - decisionDelay] = (byte) (psDD.Q_Q10[last_smple_idx] >> 10);
			pxq[pxq_offset + i - decisionDelay] = (short) SigProcFIX
					.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(Macros.SKP_SMULWW(
							psDD.Xq_Q10[last_smple_idx],
							psDD.Gain_Q16[last_smple_idx]), 10));
		...
		System.arraycopy(psDD.sLPC_Q14, psEncC.subfr_length, NSQ.sLPC_Q14, 0,
				Define.NSQ_LPC_BUF_LENGTH());
		...
		// psEncC.frame_length * sizeof( SKP_int16 ) );
		// SKP_memcpy( NSQ.sLTP_shp_Q10, &NSQ.sLTP_shp_Q10[ psEncC.frame_length
		// ], psEncC.frame_length * sizeof( SKP_int32 ) );
		System.arraycopy(NSQ.xq, psEncC.frame_length, NSQ.xq, 0,
				psEncC.frame_length);
		System.arraycopy(NSQ.sLTP_shp_Q10, psEncC.frame_length,
				NSQ.sLTP_shp_Q10, 0, psEncC.frame_length);
	...
	static void SKP_Silk_noise_shape_quantizer_del_dec(SKP_Silk_nsq_state NSQ, /*
			...
			final int x_Q10[], /* I */
			byte q[], /* O */
			int q_offset, short xq[], /* O */
			int xq_offset, int sLTP_Q16[], /* I/O LTP filter state */
			final short a_Q12[], /* I Short term prediction coefs */
			int a_Q12_offset, final short b_Q14[], /*
													 * I Long term prediction
													 * coefs
													 */
			int b_Q14_offset, final short AR_shp_Q13[], /* I Noise shaping coefs */
			int AR_shp_Q13_offset, int lag, /* I Pitch lag */
		...
		/*
		 * psSampleState is an two-dimension array of reference, which should be
		 * created manually.
		 */
		{
		...
		shp_lag_ptr_offset = NSQ.sLTP_shp_buf_idx - lag
				+ Define.HARM_SHAPE_FIR_TAPS / 2;
		...
		for (i = 0; i < length; i++) {
			...
			if (sigtype == Define.SIG_TYPE_VOICED) {
				/* Unrolled loop */
				LTP_pred_Q14 = Macros.SKP_SMULWB(
						pred_lag_ptr[pred_lag_ptr_offset + 0],
						b_Q14[b_Q14_offset + 0]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 1],
						b_Q14[b_Q14_offset + 1]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 2],
						b_Q14[b_Q14_offset + 2]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 3],
						b_Q14[b_Q14_offset + 3]);
				LTP_pred_Q14 = Macros.SKP_SMLAWB(LTP_pred_Q14,
						pred_lag_ptr[pred_lag_ptr_offset - 4],
						b_Q14[b_Q14_offset + 4]);
				pred_lag_ptr_offset++;
			} else {
				LTP_pred_Q14 = 0;
			}
			...
			if (lag > 0) {
				...
				n_LTP_Q14 = Macros.SKP_SMLAWT(n_LTP_Q14,
						shp_lag_ptr[shp_lag_ptr_offset - 1],
						HarmShapeFIRPacked_Q14);
			...
			for (k = 0; k < nStatesDelayedDecision; k++) {
				...
				// SKP_assert( ( (SKP_int64)a_Q12 & 3 ) == 0 ); /* check that
				// array starts at 4-byte aligned address */
				/* Partially unrolled */
				LPC_pred_Q10 = Macros.SKP_SMULWB(
						psLPC_Q14[psLPC_Q14_offset + 0],
						a_Q12[a_Q12_offset + 0]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 1],
						a_Q12[a_Q12_offset + 1]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 2],
						a_Q12[a_Q12_offset + 2]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 3],
						a_Q12[a_Q12_offset + 3]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 4],
						a_Q12[a_Q12_offset + 4]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 5],
						a_Q12[a_Q12_offset + 5]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 6],
						a_Q12[a_Q12_offset + 6]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 7],
						a_Q12[a_Q12_offset + 7]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 8],
						a_Q12[a_Q12_offset + 8]);
				LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
						psLPC_Q14[psLPC_Q14_offset - 9],
						a_Q12[a_Q12_offset + 9]);

				for (j = 10; j < predictLPCOrder; j++) {
					LPC_pred_Q10 = Macros.SKP_SMLAWB(LPC_pred_Q10,
							psLPC_Q14[psLPC_Q14_offset - j], a_Q12[a_Q12_offset
									+ j]);
				}

				/* Noise shape feedback */
				assert ((shapingLPCOrder & 1) == 0); /* check that order is even */
				// assert( ( (SKP_int64)AR_shp_Q13 & 3 ) == 0 ); /* check that
				// array starts at 4-byte aligned address */
				assert (shapingLPCOrder >= 12); /* check that unrolling works */
				/* Partially unrolled */
				n_AR_Q10 = Macros.SKP_SMULWB(psLPC_Q14[psLPC_Q14_offset + 0],
						AR_shp_Q13[AR_shp_Q13_offset + 0]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 1],
						AR_shp_Q13[AR_shp_Q13_offset + 1]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 2],
						AR_shp_Q13[AR_shp_Q13_offset + 2]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 3],
						AR_shp_Q13[AR_shp_Q13_offset + 3]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 4],
						AR_shp_Q13[AR_shp_Q13_offset + 4]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 5],
						AR_shp_Q13[AR_shp_Q13_offset + 5]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 6],
						AR_shp_Q13[AR_shp_Q13_offset + 6]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 7],
						AR_shp_Q13[AR_shp_Q13_offset + 7]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 8],
						AR_shp_Q13[AR_shp_Q13_offset + 8]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 9],
						AR_shp_Q13[AR_shp_Q13_offset + 9]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 10],
						AR_shp_Q13[AR_shp_Q13_offset + 10]);
				n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
						psLPC_Q14[psLPC_Q14_offset - 11],
						AR_shp_Q13[AR_shp_Q13_offset + 11]);

				for (j = 12; j < shapingLPCOrder; j++) {
					n_AR_Q10 = Macros.SKP_SMLAWB(n_AR_Q10,
							psLPC_Q14[psLPC_Q14_offset - j],
							AR_shp_Q13[AR_shp_Q13_offset + j]);
				}
				...
				n_LF_Q10 = (Macros.SKP_SMULWB(psDD.Shape_Q10[smpl_buf_idx[0]],
						LF_shp_Q14) << 2);
				n_LF_Q10 = Macros.SKP_SMLAWT(n_LF_Q10, psDD.LF_AR_Q12,
						LF_shp_Q14);

				/* Input minus prediction plus noise feedback */
				/*
				 * r = x[ i ] - LTP_pred - LPC_pred + n_AR + n_Tilt + n_LF +
				 * n_LTP
				 */
				...
				if (r_Q10 < -1536) {
					...
					r_Q10 = (r_Q10 - q1_Q10);
					rd1_Q10 = (Macros
							.SKP_SMLABB((-(q1_Q10 + offset_Q10) * Lambda_Q10),
									r_Q10, r_Q10) >> 10);
					rd2_Q10 = (rd1_Q10 + 1024);
					rd2_Q10 = (rd2_Q10 - SigProcFIX.SKP_ADD_LSHIFT32(
							Lambda_Q10, r_Q10, 1));
					q2_Q10 = (q1_Q10 + 1024);
				} else if (r_Q10 > 512) {
					q1_Q10 = (SigProcFIX.SKP_RSHIFT_ROUND(r_Q10, 10) << 10);
					r_Q10 = (r_Q10 - q1_Q10);
					rd1_Q10 = (Macros.SKP_SMLABB(
							((q1_Q10 + offset_Q10) * Lambda_Q10), r_Q10, r_Q10) >> 10);
					rd2_Q10 = (rd1_Q10 + 1024);
					rd2_Q10 = (rd2_Q10 - SigProcFIX.SKP_SUB_LSHIFT32(
							Lambda_Q10, r_Q10, 1));
					q2_Q10 = (q1_Q10 - 1024);
					...
					rd1_Q10 = (rd1_Q10 + SigProcFIX.SKP_SUB_RSHIFT32(
							SigProcFIX.SKP_ADD_LSHIFT32(Lambda_Q10, r_Q10, 1),
							rr_Q20, 9));
					q1_Q10 = -1024;
					q2_Q10 = 0;
				}
				...
				/* Add predictions */
				LPC_exc_Q10 = exc_Q10
						+ SigProcFIX.SKP_RSHIFT_ROUND(LTP_pred_Q14, 4);
			...
			smpl_buf_idx[0] = (smpl_buf_idx[0] - 1)
					& Define.DECISION_DELAY_MASK; /* Index to newest samples */
			last_smple_idx = (smpl_buf_idx[0] + decisionDelay)
					& Define.DECISION_DELAY_MASK; /*
												 * Index to decisionDelay old
												 * samples
												 */
			...
			if (RDmin_Q10 < RDmax_Q10) {
				...
				// SKP_memcpy( &psSampleState[ RDmax_ind ][ 0 ], &psSampleState[
				// RDmin_ind ][ 1 ], sizeof( NSQ_sample_struct ) );
				psSampleState[RDmax_ind][0] = (NSQ_sample_struct) psSampleState[RDmin_ind][1]
						.clone();
			}
			...
			if (subfr > 0 || i >= decisionDelay) {
				...
				xq[xq_offset + i - decisionDelay] = (short) SigProcFIX
						.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(Macros
								.SKP_SMULWW(psDD.Xq_Q10[last_smple_idx],
										psDD.Gain_Q16[last_smple_idx]), 10));
			...
			for (k = 0; k < nStatesDelayedDecision; k++) {
				...
				psDD.Seed = SigProcFIX.SKP_ADD_RSHIFT32(psDD.Seed,
						psSS[0].Q_Q10, 10);
		...
		for (k = 0; k < nStatesDelayedDecision; k++) {
			psDD = psDelDec[k];
			System.arraycopy(psDD.sLPC_Q14, length, psDD.sLPC_Q14, 0,
					Define.NSQ_LPC_BUF_LENGTH());
		}
	}
	...
	static void SKP_Silk_nsq_del_dec_scale_states(SKP_Silk_nsq_state NSQ, /*
			...
			int x_offset, int x_sc_Q10[], /* O Input scaled with 1/Gain in Q10 */
		...
		inv_gain_Q16 = (inv_gain_Q16 < Short.MAX_VALUE ? inv_gain_Q16
				: Short.MAX_VALUE);
		lag = pitchL[subfr];
		/*
		 * After rewhitening the LTP state is un-scaled. So scale with
		 * inv_gain_Q16
		 */
		...
		if (inv_gain_Q16 != NSQ.prev_inv_gain_Q16) {
			gain_adj_Q16 = Inlines.SKP_DIV32_varQ(inv_gain_Q16,
					NSQ.prev_inv_gain_Q16, 16);

			for (k = 0; k < nStatesDelayedDecision; k++) {
				psDD = psDelDec[k];

				/* Scale scalar states */
				psDD.LF_AR_Q12 = Macros
						.SKP_SMULWW(gain_adj_Q16, psDD.LF_AR_Q12);

				/* scale short term state */
				for (i = 0; i < Define.NSQ_LPC_BUF_LENGTH(); i++) {
					psDD.sLPC_Q14[Define.NSQ_LPC_BUF_LENGTH() - i - 1] = Macros
							.SKP_SMULWW(gain_adj_Q16,
									psDD.sLPC_Q14[Define.NSQ_LPC_BUF_LENGTH()
											- i - 1]);
				}
				for (i = 0; i < Define.DECISION_DELAY; i++) {
					psDD.Pred_Q16[i] = Macros.SKP_SMULWW(gain_adj_Q16,
							psDD.Pred_Q16[i]);
					psDD.Shape_Q10[i] = Macros.SKP_SMULWW(gain_adj_Q16,
							psDD.Shape_Q10[i]);
				}
			}
			...
			/*
			 * Calculate length to be scaled, Worst case: Next frame is voiced
			 * with max lag
			 */
			scale_length = length * Define.NB_SUBFR; /* aprox max lag */
			scale_length = scale_length
					- Macros.SKP_SMULBB(Define.NB_SUBFR - (subfr + 1), length); /*
																				 * subtract
																				 * samples
																				 * that
																				 * will
			...
			for (i = NSQ.sLTP_shp_buf_idx - scale_length; i < NSQ.sLTP_shp_buf_idx; i++) {
				NSQ.sLTP_shp_Q10[i] = Macros.SKP_SMULWW(gain_adj_Q16,
						NSQ.sLTP_shp_Q10[i]);
			}
		...
		/* Scale input */
		for (i = 0; i < length; i++) {
			x_sc_Q10[i] = (Macros.SKP_SMULBB(x[x_offset + i],
					(short) inv_gain_Q16) >> 6);
		}
	 ...
	 *            Index to LPC buffer
	 */
	static void SKP_Silk_copy_del_dec_state(NSQDelDecStruct DD_dst, /*
																	 * I Dst del
																	 * dec state
																	 */
		...
		System.arraycopy(DD_src.RandState, 0, DD_dst.RandState, 0,
				Define.DECISION_DELAY);
		System.arraycopy(DD_src.Q_Q10, 0, DD_dst.Q_Q10, 0,
				Define.DECISION_DELAY);
		System.arraycopy(DD_src.Pred_Q16, 0, DD_dst.Pred_Q16, 0,
				Define.DECISION_DELAY);
		System.arraycopy(DD_src.Shape_Q10, 0, DD_dst.Shape_Q10, 0,
				Define.DECISION_DELAY);
		System.arraycopy(DD_src.Xq_Q10, 0, DD_dst.Xq_Q10, 0,
				Define.DECISION_DELAY);

		System.arraycopy(DD_src.sLPC_Q14, LPC_state_idx, DD_dst.sLPC_Q14,
				LPC_state_idx, Define.NSQ_LPC_BUF_LENGTH());
