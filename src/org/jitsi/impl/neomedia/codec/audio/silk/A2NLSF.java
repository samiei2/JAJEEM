...
class A2NLSF_constants {
	...
	/*
	 * Flag for using 2x as many cosine sampling points, reduces the risk of
	 * missing a root
	 */
...
/**
 * Conversion between prediction filter coefficients and NLSFs. Requires the
 * order to be an even number. A piecewise linear approximation maps LSF <->
 * cos(LSF). Therefore the result is not accurate NLSFs, but the two. function
 * are accurate inverses of each other.
 * 
 * @author Jing Dai
 * @author Dingxin Xu
 */
public class A2NLSF extends A2NLSF_constants {
	 ...
	 */
	static int SKP_Silk_A2NLSF_eval_poly( /*
										 * return the polynomial evaluation, in
										 * QPoly
										 */
	...
	static void SKP_Silk_A2NLSF_init(int[] a_Q16, int[] P, int[] Q, final int dd) {
		...
		for (k = 0; k < dd; k++) {
			if (QPoly < 16) {
				P[k] = SigProcFIX.SKP_RSHIFT_ROUND(-a_Q16[dd - k - 1]
						- a_Q16[dd + k], 16 - QPoly); /* QPoly */
				Q[k] = SigProcFIX.SKP_RSHIFT_ROUND(-a_Q16[dd - k - 1]
						+ a_Q16[dd + k], 16 - QPoly); /* QPoly */
			} else if (QPoly == 16) {
	...
	/**
	 * Compute Normalized Line Spectral Frequencies (NLSFs) from whitening
	 * filter coefficients. If not all roots are found, the a_Q16 coefficients
	...
	static void SKP_Silk_A2NLSF(int[] NLSF, /*
											 * O Normalized Line Spectral
											 * Frequencies, Q15 (0 - (2^15-1)),
											 * [d]
											 */
		...
		int ylo, yhi, ymid;
		int nom, den;
		int[] P = new int[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC / 2 + 1];
		int[] Q = new int[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC / 2 + 1];
		int[][] PQ = new int[2][];
		int[] p;
		...
		while (true) {
			...
			if ((ylo <= 0 && yhi >= 0) || (ylo >= 0 && yhi <= 0)) {
				...
				if (Math.abs(ylo) < 65536) {
					...
					/*
					 * No risk of dividing by zero because abs(ylo - yhi) >=
					 * abs(ylo) >= 65536
					 */
					ffrac += ylo
							/ ((ylo - yhi) >> (8 - BIN_DIV_STEPS_A2NLSF_FIX));
				}
				if (OVERSAMPLE_COSINE_TABLE != 0) {
					NLSF[root_ix] = Math.min((k << 7) + ffrac,
							Typedef.SKP_int16_MAX);
				} else {
					NLSF[root_ix] = Math.min((k << 8) + ffrac,
							Typedef.SKP_int16_MAX);
				}
				...
				if (OVERSAMPLE_COSINE_TABLE != 0) {
					if (k > 2 * SigProcFIXConstants.LSF_COS_TAB_SZ_FIX) {
						i++;
						...
						/*
						 * Error: Apply progressively more bandwidth expansion
						 * and run again
						 */
						Bwexpander32.SKP_Silk_bwexpander_32(a_Q16, d,
								65536 - Macros.SKP_SMULBB(66, i)); // 66_Q16 =
																	// 0.001
						...
						if (ylo < 0) {
							/*
							 * Set the first NLSF to zero and move on to the
							 * next
							 */
							NLSF[0] = 0;
							p = Q; /* Pointer to polynomial */
					...
					if (k > SigProcFIXConstants.LSF_COS_TAB_SZ_FIX) {
						i++;
						if (i > MAX_ITERATIONS_A2NLSF_FIX) {
							/* Set NLSFs to white spectrum and exit */
							NLSF[0] = (1 << 15) / (d + 1);
