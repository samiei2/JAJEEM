		...
		for (k = 0; k < 3; k++) {
			...
			for (j = 0; j < Define.NB_SUBFR; j++) {
				...
				VQNearestNeighborFLP.SKP_Silk_VQ_WMat_EC_FLP(temp_idx, /*
																		 ...
																		 * vector
																		 */
						j, rate_dist_subfr_ptr, /*
												 * O best weighted quantization
												 * error + mu * rate
												 */
				...
				// &rate_dist_subfr, /* O best weighted quantization error + mu
				// * rate */
				// b_ptr, /* I input vector to be quantized */
			...
			/*
			 * Break early in low-complexity mode if rate distortion is below
			 * threshold
			 */
			if (lowComplexity != 0
					&& (rate_dist * 16384.0f < TablesLTP.SKP_Silk_LTP_gain_middle_avg_RD_Q14)) {
				break;
			}
		}
