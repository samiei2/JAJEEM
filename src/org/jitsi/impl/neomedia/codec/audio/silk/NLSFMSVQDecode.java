...
public class NLSFMSVQDecode {
	...
	static void SKP_Silk_NLSF_MSVQ_decode(int[] pNLSF_Q15, /*
		...
		Typedef.SKP_assert(0 <= NLSFIndices[0]
		...
		for (s = 1; s < psNLSF_CB.nStages; s++) {
			/* Check that each index is within valid range */
			Typedef.SKP_assert(0 <= NLSFIndices[s]
			...
			if (LPC_order == 16) {
				...
				/* Point to the first vector element */
				pCB_element = psNLSF_CB.CBStages[s].CB_NLSF_Q15;
				pCB_element_offset = Macros.SKP_SMULBB(NLSFIndices[s],
						LPC_order);
		...
		/* NLSF stabilization */
		NLSFStabilize.SKP_Silk_NLSF_stabilize(pNLSF_Q15, 0,
				psNLSF_CB.NDeltaMin_Q15, LPC_order);
	}
}
