...
public class ResamplerPrivateUp2HQ {
	/**
	 * Upsample by a factor 2, high quality. Uses 2nd order allpass filters for
	 * the 2x upsampling, followed by a notch filter just above Nyquist.
	 * 
	 * @param S
	 *            Resampler state [ 6 ].
	 * @param S_offset
	...
	static void SKP_Silk_resampler_private_up2_HQ(int[] S, /*
															 ...
															 * state [ 6 ]
															 */
			int S_offset, short[] out, /* O: Output signal [ 2 * len ] */
		...
		for (k = 0; k < len; k++) {
			...
			Y = in32 - S[S_offset];
			X = Macros.SKP_SMULWB(Y,
					ResamplerRom.SKP_Silk_resampler_up2_hq_0[0]);
			out32_1 = S[S_offset] + X;
			S[S_offset] = in32 + X;

			/* Second all-pass section for even output sample */
			Y = out32_1 - S[S_offset + 1];
			X = Macros.SKP_SMLAWB(Y, Y,
					ResamplerRom.SKP_Silk_resampler_up2_hq_0[1]);
			out32_2 = S[S_offset + 1] + X;
			S[S_offset + 1] = out32_1 + X;

			/* Biquad notch filter */
			out32_2 = Macros.SKP_SMLAWB(out32_2, S[S_offset + 5],
					ResamplerRom.SKP_Silk_resampler_up2_hq_notch[2]);
			out32_2 = Macros.SKP_SMLAWB(out32_2, S[S_offset + 4],
					ResamplerRom.SKP_Silk_resampler_up2_hq_notch[1]);
			out32_1 = Macros.SKP_SMLAWB(out32_2, S[S_offset + 4],
					ResamplerRom.SKP_Silk_resampler_up2_hq_notch[0]);
			S[S_offset + 5] = out32_2 - S[S_offset + 5];

			/* Apply gain in Q15, convert back to int16 and store to output */
			out[out_offset + 2 * k] = (short) SigProcFIX
					.SKP_SAT16(Macros.SKP_SMLAWB(256, out32_1,
							ResamplerRom.SKP_Silk_resampler_up2_hq_notch[3]) >> 9);

			/* First all-pass section for odd output sample */
			Y = in32 - S[S_offset + 2];
			X = Macros.SKP_SMULWB(Y,
					ResamplerRom.SKP_Silk_resampler_up2_hq_1[0]);
			...
			X = Macros.SKP_SMLAWB(Y, Y,
					ResamplerRom.SKP_Silk_resampler_up2_hq_1[1]);
			...
			/* Apply gain in Q15, convert back to int16 and store to output */
			out[out_offset + 2 * k + 1] = (short) SigProcFIX
					.SKP_SAT16(Macros.SKP_SMLAWB(256, out32_1,
							ResamplerRom.SKP_Silk_resampler_up2_hq_notch[3]) >> 9);
		}
	}
	...
	static void SKP_Silk_resampler_private_up2_HQ_wrapper(Object SS, /*
		...
		SKP_Silk_resampler_private_up2_HQ(S.sIIR, 0, out, out_offset, in,
				in_offset, len);
	}
}
