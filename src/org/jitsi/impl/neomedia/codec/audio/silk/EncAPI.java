...
public class EncAPI {
	 ...
	 * @return
	 */
	static int SKP_Silk_SDK_QueryEncoder(Object encState, /* I: State Vector */
		...
		encStatus.maxInternalSampleRate = Macros.SKP_SMULBB(
				psEnc.sCmn.maxInternal_fs_kHz, 1000);
	 ...
	 * @return
	 */
	static int SKP_Silk_SDK_InitEncoder(Object encState, /* I/O: State */
			SKP_SILK_SDK_EncControlStruct encStatus /* O: Control structure */
	) {
		SKP_Silk_encoder_state_FLP psEnc;
		int ret = 0;
	 ...
	 * @return
	 */
	static int SKP_Silk_SDK_Encode(Object encState, /* I/O: State */
			SKP_SILK_SDK_EncControlStruct encControl, /* I: Control structure */
			final short[] samplesIn, /* I: Speech sample input vector */
			int samplesIn_offset, int nSamplesIn, /*
												 * I: Number of samples in input
												 * vector
												 */
			byte[] outData, /* O: Encoded output vector */
			int outData_offset, short[] nBytesOut /*
												 * I/O: Number of bytes in
												 * outData (input: Max bytes)
												 */
	) {
		...
		if (((encControl.API_sampleRate != 8000)
				...
				&& (encControl.API_sampleRate != 44100) && (encControl.API_sampleRate != 48000))
		...
		max_internal_fs_kHz = encControl.maxInternalSampleRate / 1000; /*
																		 * convert
																		 * Hz ->
																		 * kHz
																		 */
		...
		if ((ret = ControlCodecFLP.SKP_Silk_control_encoder_FLP(psEnc,
				API_fs_Hz, max_internal_fs_kHz, PacketSize_ms, TargetRate_bps,
				PacketLoss_perc, UseInBandFEC, UseDTX, input_ms, Complexity)) != 0) {
			assert (false);
			return (ret);
		}
		/* Detect energy above 8 kHz */
		if (Math.min(API_fs_Hz, 1000 * max_internal_fs_kHz) == 24000
				&& psEnc.sCmn.sSWBdetect.SWB_detected == 0
				&& psEnc.sCmn.sSWBdetect.WB_detected == 0) {
			DetectSWBInput.SKP_Silk_detect_SWB_input(psEnc.sCmn.sSWBdetect,
					samplesIn, samplesIn_offset, nSamplesIn);
		}
		...
		while (true) {
			...
			if (API_fs_Hz == Macros.SKP_SMULBB(1000, psEnc.sCmn.fs_kHz)) {
				...
				/* Copy to buffer */
				System.arraycopy(samplesIn, samplesIn_offset,
						psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx,
						nSamplesFromInput);
			} else {
				nSamplesToBuffer = Math.min(nSamplesToBuffer, nSamplesIn
				...
				ret += Resampler.SKP_Silk_resampler(psEnc.sCmn.resampler_state,
						psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx, samplesIn,
				...
				// String inputbuf_filename =
				// "D:/gsoc/inputbuf-res/inputbuf-res";
			...
			if (psEnc.sCmn.inputBufIx >= psEnc.sCmn.frame_length) {
				...
				if (MaxBytesOut == 0) {
					...
					// if( ( ret =
					// Silk_encode_frame_FLP.SKP_Silk_encode_frame_FLP( psEnc,
					// outData, outData_offset,
					// MaxBytesOut_ptr, psEnc.sCmn.inputBuf,
					// psEnc.sCmn.inputBufIx ) ) != 0 )
					if ((ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP(psEnc,
							outData, outData_offset, MaxBytesOut_ptr,
							psEnc.sCmn.inputBuf, 0)) != 0) {
						assert (false);
					}
					...
					// Silk_encode_frame_FLP.SKP_Silk_encode_frame_FLP( psEnc,
					// outData, outData_offset,
					// nBytesOut, psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx) )
					// != 0 )
					if ((ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP(psEnc,
							outData, outData_offset, nBytesOut,
							psEnc.sCmn.inputBuf, 0)) != 0) {
						assert (false);
					}
