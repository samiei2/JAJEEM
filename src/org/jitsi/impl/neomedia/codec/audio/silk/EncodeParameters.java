...
public class EncodeParameters {
	 ...
	 */
	static void SKP_Silk_encode_parameters(SKP_Silk_encoder_state psEncC, /*
																		 * I/O
																		 * Encoder
																		 * state
																		 */
		...
		if (psEncC.nFramesInPayloadBuf == 0) {
			...
			RangeCoder.SKP_Silk_range_encoder(psRC, i,
		...
		if (psEncC.nFramesInPayloadBuf == 0) {
			...
			RangeCoder
					.SKP_Silk_range_encoder(
							psRC,
							typeOffset,
							TablesTypeOffset.SKP_Silk_type_offset_joint_CDF[psEncC.typeOffsetPrev],
							0);
		}
		...
		RangeCoder.SKP_Silk_range_encoder_multi(psRC, psEncCtrlC.NLSFIndices,
		...
		if (psEncCtrlC.sigtype == Define.SIG_TYPE_VOICED) {
			...
			if (psEncC.fs_kHz == 8) {
				...
				RangeCoder.SKP_Silk_range_encoder(psRC,
			...
			for (k = 0; k < Define.NB_SUBFR; k++) {
				RangeCoder
						.SKP_Silk_range_encoder(
								psRC,
								psEncCtrlC.LTPIndex[k],
								TablesLTP.SKP_Silk_LTP_gain_CDF_ptrs[psEncCtrlC.PERIndex],
								0);
			}
		...
		/* Encode seed */
		/***************/
		RangeCoder.SKP_Silk_range_encoder(psRC, psEncCtrlC.Seed,
				TablesOther.SKP_Silk_Seed_CDF, 0);

		/*********************************************/
		/* Encode quantization indices of excitation */
		/*********************************************/
		EncodePulses.SKP_Silk_encode_pulses(psRC, psEncCtrlC.sigtype,
				psEncCtrlC.QuantOffsetType, q, psEncC.frame_length);

		/*********************************************/
		/* Encode VAD flag */
		/*********************************************/
		RangeCoder.SKP_Silk_range_encoder(psRC, psEncC.vadFlag,
