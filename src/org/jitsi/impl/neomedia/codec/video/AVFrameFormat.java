...
public class AVFrameFormat extends VideoFormat {
	...
	public AVFrameFormat(Dimension size, float frameRate, int pixFmt,
		...
		if ((pixFmt == NOT_SPECIFIED) && (deviceSystemPixFmt != NOT_SPECIFIED)) {
			logger.warn("Specifying a device system-specific pixel format 0x"
					+ Long.toHexString(deviceSystemPixFmt & 0xffffffffL)
					+ " without a matching FFmpeg pixel format may"
					+ " eventually lead to a failure.", new Throwable());
		}
	...
	@Override
	public Object clone() {
		AVFrameFormat f = new AVFrameFormat(getSize(), getFrameRate(), pixFmt,
				deviceSystemPixFmt);
	...
	public Format intersects(Format format) {
		...
		if (intersection != null) {
			...
			avFrameFormatIntersection.pixFmt = ((pixFmt == NOT_SPECIFIED) && (format instanceof AVFrameFormat)) ? ((AVFrameFormat) format).pixFmt
					: pixFmt;
		}
		return intersection;
	}
	...
	public boolean matches(Format format) {
		...
		if (super.matches(format)) {
			if (format instanceof AVFrameFormat) {
				...
				matches = ((pixFmt == NOT_SPECIFIED)
						|| (avFrameFormat.pixFmt == NOT_SPECIFIED) || (pixFmt == avFrameFormat.pixFmt));
			} else {
				matches = true;
			}
		} else {
