 ...
 * process all valid streams. It works by concatenating packets' payload
 * (stripping the payload descriptor), until it encounters a packet with it's
 * Start of Partition bit set, at which point it outputs the concatenated data
 * and starts again.
 * 
 * @author Boris Grozev
 */
public class DePacketizer extends AbstractCodec2 {
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		boolean start = VP8PayloadDescriptor.isStartOfPartition(input,
		...
		int pdSize;
		try {
			pdSize = VP8PayloadDescriptor.getSize(input,
					inputBuffer.getOffset());
		} catch (Exception e) {
			outputBuffer.setDiscard(true);
		...
		if (logger.isTraceEnabled() && TRACE) {
			logger.trace("Packet: " + inputBuffer.getSequenceNumber()
					+ ", length=" + inputBuffer.getLength() + ", pdSize="
					+ pdSize + ", start=" + start + "\nPayload descriptor:");
			for (int i = inputBuffer.getOffset(); i < inputBuffer.getOffset()
					+ pdSize + 1; i++) {
				logger.trace("\t\t" + ((input[i] & 0x80) == 0 ? "0" : "1")
						+ ((input[i] & 0x40) == 0 ? "0" : "1")
						+ ((input[i] & 0x20) == 0 ? "0" : "1")
						+ ((input[i] & 0x10) == 0 ? "0" : "1")
						+ ((input[i] & 0x08) == 0 ? "0" : "1")
		...
		System.arraycopy(input, inputBuffer.getOffset() + pdSize, buffer,
				bufferPointer, len - pdSize);
		bufferPointer += len - pdSize;

		if (logger.isTraceEnabled() && TRACE) {
			logger.trace("Saving payload to buffer, seq num:"
					+ inputBuffer.getSequenceNumber() + ", bufferPointer="
					+ bufferPointer);
		}
	...
	static class VP8PayloadDescriptor {
		 ...
		 *         <tt>offset</tt> in <tt>input</tt>. The size is between 1 and
		 *         6.
		 * @throws Exception
		 *             if there isn't a valid Payload Descriptor structure at
		...
		/**
		 * Checks whether the 'start of partition' bit is set in the the Payload
