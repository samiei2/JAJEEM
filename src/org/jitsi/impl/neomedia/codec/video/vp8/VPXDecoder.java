...
public class VPXDecoder extends AbstractCodec2 {
	 ...
	 */
	private static final VideoFormat[] SUPPORTED_OUTPUT_FORMATS = new VideoFormat[] { new AVFrameFormat(
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		if (leftoverFrames) {
			...
			int ret = VPX
					.codec_decode(context, buf, buf_offset, buf_size, 0, 0);
			if (ret != VPX.CODEC_OK) {
				if (logger.isDebugEnabled()) {
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AVFrameFormat(inputVideoFormat.getSize(),
				inputVideoFormat.getFrameRate(), FFmpeg.PIX_FMT_YUV420P) };
	}
	...
	private AVFrame makeAVFrame(long img) {
		...
		FFmpeg.avframe_set_linesize(avframe.getPtr(), VPX.img_get_stride0(img),
				VPX.img_get_stride1(img), VPX.img_get_stride2(img));
	...
	private void updateOutputFormat(int width, int height, float frameRate) {
		if ((width > 0) && (height > 0)
			...
			this.width = width;
			this.height = height;
			outputFormat = new AVFrameFormat(new Dimension(width, height),
					frameRate, FFmpeg.PIX_FMT_YUV420P);
		}
	}
}
