...
public class VPXEncoder extends AbstractCodec2 {
	 ...
	 */
	private static final VideoFormat[] SUPPORTED_OUTPUT_FORMATS = new VideoFormat[] { new VideoFormat(
			Constants.VP8) };
	...
	protected void doOpen() throws ResourceUnavailableException {
		...
		int bitRate = NeomediaServiceUtils.getMediaServiceImpl()
				.getDeviceConfiguration().getVideoBitrate();
		...
		VPX.codec_enc_cfg_set_error_resilient(cfg, VPX.ERROR_RESILIENT_DEFAULT
				| VPX.ERROR_RESILIENT_PARTITIONS);
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (leftoverPackets) {
			...
			int result = VPX.codec_encode(context, img,
					(byte[]) inputBuffer.getData(), offsetY, offsetU, offsetV,
					frameCount, // pts
					1, // duration
					0, // flags
					VPX.DL_REALTIME);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new VideoFormat[] { new VideoFormat(Constants.VP8,
	...
	private void reinit() {
		...
		if (ret != VPX.CODEC_OK) {
			throw new RuntimeException(
					"Failed to re-initialize encoder, libvpx" + " error:\n"
							+ VPX.codec_err_to_string(ret));
		}
	}
