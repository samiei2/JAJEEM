...
public class SwScale extends AbstractCodec {
	...
	private static VideoFormat setSize(VideoFormat format, Dimension size) {
		...
		if (format instanceof RGBFormat) {
			...
			if ((pixelStride == Format.NOT_SPECIFIED) && (dataType != null)
					&& (bitsPerPixel != Format.NOT_SPECIFIED)) {
				pixelStride = dataType.equals(Format.byteArray) ? (bitsPerPixel / 8)
						: 1;
			}
			format = new RGBFormat(
					...
					pixelStride,
					((pixelStride == Format.NOT_SPECIFIED) || (size == null)) ? Format.NOT_SPECIFIED
							: (pixelStride * size.width) /* lineStride */,
					rgbFormat.getFlipped(), rgbFormat.getEndian());
		} else if (format instanceof YUVFormat) {
			YUVFormat yuvFormat = (YUVFormat) format;

			format = new YUVFormat(size,
			/* maxDataLength */Format.NOT_SPECIFIED, format.getDataType(),
					format.getFrameRate(), yuvFormat.getYuvType(),
					/* strideY */Format.NOT_SPECIFIED,
					/* strideUV */Format.NOT_SPECIFIED, 0,
					/* offsetU */Format.NOT_SPECIFIED,
					/* offsetV */Format.NOT_SPECIFIED);
		} else if (format != null) {
			logger.warn("SwScale outputFormat of type "
	...
	private VideoFormat[] supportedOutputFormats = new VideoFormat[] {
			new RGBFormat(), new YUVFormat(YUVFormat.YUV_420) };
	 ...
	 *            <tt>true</tt> to keep the width and height of YUV 420 output
	 *            even; otherwise, <tt>false</tt>
	 */
	protected SwScale(boolean fixOddYuv420Size) {
		this.fixOddYuv420Size = fixOddYuv420Size;

		inputFormats = new Format[] { new AVFrameFormat(), new RGBFormat(),
	...
	public int process(Buffer input, Buffer output) {
		...
		if (outputFormat instanceof RGBFormat) {
			...
			dstLength = outputWidth * outputHeight + 2
					* ((outputWidth + 1) / 2) * ((outputHeight + 1) / 2);
		} else {
			return BUFFER_PROCESSED_FAILED;
		}
		...
		if (src instanceof AVFrame) {
			...
			srcFmt = (inputFormat instanceof YUVFormat) ? FFmpeg.PIX_FMT_YUV420P
					: getFFmpegPixelFormat((RGBFormat) inputFormat);
			srcPicture = 0;
		}

		swsContext = FFmpeg.sws_getCachedContext(swsContext, inputWidth,
				inputHeight, srcFmt, outputWidth, outputHeight, dstFmt,
				FFmpeg.SWS_BICUBIC);

		if (srcPicture == 0) {
			FFmpeg.sws_scale(swsContext, src, srcFmt, inputWidth, inputHeight,
					0, inputHeight, dst, dstFmt, outputWidth, outputHeight);
		} else {
			FFmpeg.sws_scale(swsContext, srcPicture, 0, inputHeight, dst,
					dstFmt, outputWidth, outputHeight);
		}
	...
	public Format setInputFormat(Format format) {
		Format inputFormat = (format instanceof VideoFormat) ? super
				.setInputFormat(format) : null /*
												 * The input must be video, a
	...
	public Format setOutputFormat(Format format) {
		...
		if ((outputFormat != null) && logger.isDebugEnabled()) {
			logger.debug(getClass().getName() + " 0x"
					+ Integer.toHexString(hashCode()) + " set to output in "
					+ outputFormat);
		}
		return outputFormat;
	}
