...
public class Packetizer extends AbstractPacketizer {
	...
	static final Format[] SUPPORTED_OUTPUT_FORMATS = {
			new ParameterizedVideoFormat(Constants.H264_RTP,
	 ...
	 */
	private static int ff_avc_find_startcode(byte[] byteStream, int beginIndex,
			int endIndex) {
	...
	private Format[] getMatchingOutputFormats(Format input) {
		...
		return new Format[] { new ParameterizedVideoFormat(Constants.H264_RTP,
				size, Format.NOT_SPECIFIED, Format.byteArray, frameRate,
				ParameterizedVideoFormat.toMap(
						JNIEncoder.PACKETIZATION_MODE_FMTP, packetizationMode)) };
	}
	...
	private String getPacketizationMode(Format format) {
		...
		if (format instanceof ParameterizedVideoFormat) {
			packetizationMode = ((ParameterizedVideoFormat) format)
					.getFormatParameter(JNIEncoder.PACKETIZATION_MODE_FMTP);
		}
	...
	public int process(Buffer inBuffer, Buffer outBuffer) {
		...
		if (beginIndex < endIndex) {
			...
			for (int nextBeginIndex; (beginIndex < endIndex)
					&& ((nextBeginIndex = ff_avc_find_startcode(inData,
							beginIndex, endIndex)) <= endIndex); beginIndex = nextBeginIndex + 3) {
				...
				if (nalLength > 0) {
					nalsAdded = packetizeNAL(inData, beginIndex, nalLength)
							|| nalsAdded;
				}
			}
		}
		...
		return nalsAdded ? process(inBuffer, outBuffer)
	...
	public Format setOutputFormat(Format format) {
		...
		if (fmtps.get(JNIEncoder.PACKETIZATION_MODE_FMTP) == null) {
			fmtps.put(JNIEncoder.PACKETIZATION_MODE_FMTP,
