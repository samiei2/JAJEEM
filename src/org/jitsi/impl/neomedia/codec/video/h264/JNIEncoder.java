...
public class JNIEncoder extends AbstractCodec implements RTCPFeedbackListener {
	 ...
	 */
	public static final String[] AVAILABLE_PRESETS = { "ultrafast",
	 ...
	 */
	public static final String DEFAULT_PROFILE_PNAME = "net.java.sip.communicator.impl.neomedia.codec.video.h264."
			+ "defaultProfile";
	 ...
	 * speed to compression ratio. A slower preset will provide better
	 * compression (compression is quality per size).
	 */
	...
	/**
	 * Force encoder to send a key frame. First frame have to be a keyframe.
	 */
	...
	private Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		String[] packetizationModes = (this.packetizationMode == null) ? new String[] {
				"0", "1" }
		...
		for (int index = packetizationModes.length - 1; index >= 0; index--) {
			matchingOutputFormats[index] = new ParameterizedVideoFormat(
					Constants.H264, size,
					/* maxDataLength */Format.NOT_SPECIFIED, Format.byteArray,
					frameRate, ParameterizedVideoFormat.toMap(
							PACKETIZATION_MODE_FMTP, packetizationModes[index]));
		}
	...
	private boolean keyFrameRequest() {
		if (System.currentTimeMillis() > (lastKeyFrameRequestTime + PLI_INTERVAL)) {
	...
	public synchronized void open() throws ResourceUnavailableException {
		...
		String profile = (cfg == null) ? null : cfg.getString(
				DEFAULT_PROFILE_PNAME, DEFAULT_DEFAULT_PROFILE);
		...
		int bitRate = NeomediaServiceUtils.getMediaServiceImpl()
				.getDeviceConfiguration().getVideoBitrate() * 1000;
		...
		String preset = (cfg == null) ? DEFAULT_PRESET : cfg.getString(
				PRESET_PNAME, DEFAULT_PRESET);

		if (FFmpeg.avcodec_open2(avctx,
				avcodec,
				/*
				 * XXX crf=0 means lossless coding which is not supported by the
				 * baseline and main profiles. Consequently, we cannot specify
				 * it because we specify either the baseline or the main
				 * profile. Otherwise, x264 will detect the inconsistency in the
				 * specified parameters/options and FFmpeg will fail.
				 */
				// "crf" /* constant quality mode, constant ratefactor */, "0",
				"intra-refresh", useIntraRefresh ? "1" : "0", "partitions",
				"b8x8,i4x4,p8x8", "preset", preset, "thread_type", "slice",
				"tune", "zerolatency") < 0) {
		...
		FFmpeg.avframe_set_data(avFrame, rawFrameBuffer, sizeInBytes,
				sizeInBytes / 4);
	...
	public synchronized int process(Buffer inBuffer, Buffer outBuffer) {
		...
		// encode data
		int outputLength = FFmpeg.avcodec_encode_video(avctx, out, out.length,
				avFrame);
	 ...
	 * 
	 * @param event
	 *            an <tt>RTCPFeedbackEvent</tt> which specifies the details of
	 *            the notification event such as the feedback message type and
	 *            the payload type
	 */
	@Override
	...
	public Format setOutputFormat(Format format) {
		...
		if (!(format instanceof VideoFormat)
				|| (null == AbstractCodec2.matches(format,
		...
		outputFormat = new ParameterizedVideoFormat(videoFormat.getEncoding(),
				size,
				/* maxDataLength */Format.NOT_SPECIFIED, Format.byteArray,
