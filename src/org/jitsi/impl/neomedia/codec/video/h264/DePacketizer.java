...
public class DePacketizer extends AbstractCodec2 {
	 ...
	 */
	private static final long TIME_FROM_KEY_FRAME_TO_REQUEST_KEY_FRAME = ((JNIEncoder.IFRAME_INTERVAL * 4) / (JNIEncoder.DEFAULT_FRAME_RATE * 3)) * 1000;
	 ...
	 */
	private int dePacketizeFUA(byte[] in, int inOffset, int inLength,
			Buffer outBuffer) {
		...
		byte[] out = validateByteArraySize(outBuffer, outBuffer.getOffset()
				+ newOutLength + outputPaddingSize, true);
	 ...
	 *         {@link #process(Buffer, Buffer)}
	 */
	private int dePacketizeSingleNALUnitPacket(int nal_unit_type, byte[] in,
			int inOffset, int inLength, Buffer outBuffer) {
		this.nal_unit_type = nal_unit_type;

		int outOffset = outBuffer.getOffset();
		int newOutLength = NAL_PREFIX.length + inLength;
		byte[] out = validateByteArraySize(outBuffer, outOffset + newOutLength
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if ((nal_unit_type >= 1) && (nal_unit_type <= 23)) {
			fuaStartedAndNotEnded = false;
			ret = dePacketizeSingleNALUnitPacket(nal_unit_type, in, inOffset,
					inBuffer.getLength(), outBuffer);
		} else if (nal_unit_type == 28) // FU-A Fragmentation unit (FU)
		{
			...
			logger.warn("Dropping NAL unit of unsupported type "
					+ nal_unit_type);
			this.nal_unit_type = nal_unit_type;
	...
	private int reset(Buffer outBuffer) {
		 ...
		 * H.264 decoder.
		 */
		if (OUTPUT_INCOMPLETE_NAL_UNITS && fuaStartedAndNotEnded
		...
		while (true) {
			synchronized (this) {
				...
				if (requestKeyFrame) {
					 ...
					 * requesting it.
					 */
					long nextKeyFrameTime = lastKeyFrameTime
					...
					if (now >= nextKeyFrameTime) {
						 ...
						 */
						long nextRequestKeyFrameTime = lastRequestKeyFrameTime
			...
			if (keyFrameControl != null) {
				List<KeyFrameControl.KeyFrameRequester> keyFrameRequesters = keyFrameControl
					...
					public void run() {
						try {
							...
							synchronized (DePacketizer.this) {
								if (requestKeyFrameThread == Thread
										.currentThread()) {
									requestKeyFrameThread = null;
								}
							}
