...
public class HFlip extends AbstractCodec2 implements Effect {
	...
	protected synchronized int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (graph == 0) {
			...
			if (graphIsPending) {
				...
				if (graph == 0) {
					...
					String filters = VSRC_BUFFER_NAME + "=" + size.width + ":"
							+ size.height + ":" + pixFmt
							+ ":1:1000000:1:1,hflip," + VSINK_FFSINK_NAME;
					long log_ctx = 0;

					error = FFmpeg.avfilter_graph_parse(graph, filters, 0, 0,
							log_ctx);
					if (error == 0) {
						/*
						...
						buffer = FFmpeg.avfilter_graph_get_filter(graph,
								parsedFilterName);
						if (buffer == 0) {
							...
							parsedFilterName = String.format(
									parsedFilterNameFormat, 2,
									VSINK_FFSINK_NAME);
							ffsink = FFmpeg.avfilter_graph_get_filter(graph,
									parsedFilterName);
							if (ffsink == 0) {
								errorReason = "avfilter_graph_get_filter: "
										+ VSINK_FFSINK_NAME + "/"
								...
								error = FFmpeg.avfilter_graph_config(graph,
										log_ctx);
								if (error != 0) {
		...
		outputFilterBufferRef = FFmpeg.get_filtered_video_frame(inputFrame,
				this.width, this.height, this.pixFmt, buffer, ffsink,
				outputFrame);
		if (outputFilterBufferRef == 0) {
			/*
