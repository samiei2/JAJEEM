...
public class JNIEncoder extends AbstractCodec {
	 ...
	 */
	private static final Format[] DEFAULT_OUTPUT_FORMATS = { new VideoFormat(
			Constants.H263P) };
	...
	private Format[] getMatchingOutputFormats(Format in) {
		...
		return new VideoFormat[] { new VideoFormat(Constants.H263P,
				videoIn.getSize(), Format.NOT_SPECIFIED, Format.byteArray,
	...
	public synchronized int process(Buffer inBuffer, Buffer outBuffer) {
		...
		FFmpeg.memcpy(rawFrameBuffer, (byte[]) inBuffer.getData(),
				inBuffer.getOffset(), encFrameLen);
		...
		// encode data
		int outputLength = FFmpeg.avcodec_encode_video(avcontext, out,
				out.length, avFrame);
