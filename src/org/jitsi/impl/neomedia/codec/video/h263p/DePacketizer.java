...
public class DePacketizer extends AbstractCodec2 {
	 ...
	 */
	public DePacketizer() {
		super("H263+ DePacketizer", VideoFormat.class,
				new VideoFormat[] { new VideoFormat(Constants.H263P) });

		inputFormats = new VideoFormat[] { new VideoFormat(Constants.H263P_RTP) };
	}
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if ((lastSequenceNumber != -1)
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Dropped RTP packets upto sequenceNumber "
						+ lastSequenceNumber
		...
		System.arraycopy(in, inOffset + 2 + (vBit ? 1 : 0) + plen, out,
				outOffset + (pBit ? 2 : 0), dataLength - (pBit ? 2 : 0));
