...
public abstract class AbstractCodec2 extends AbstractCodec {
	...
	public static YUVFormat specialize(YUVFormat yuvFormat, Class<?> dataType) {
		...
		if ((offsetU == Format.NOT_SPECIFIED)
				&& (strideY != Format.NOT_SPECIFIED) && (size != null)) {
		...
		if ((offsetV == Format.NOT_SPECIFIED)
				...
				&& (strideUV != Format.NOT_SPECIFIED) && (size != null)) {
			offsetV = offsetU + strideUV * ((size.height + 1) / 2);
		}

		int maxDataLength = ((strideY != Format.NOT_SPECIFIED) && (strideUV != Format.NOT_SPECIFIED))
				&& (size != null) ? (strideY * size.height + 2 * strideUV
				* ((size.height + 1) / 2) + FFmpeg.FF_INPUT_BUFFER_PADDING_SIZE)
				: Format.NOT_SPECIFIED;

		return new YUVFormat(size, maxDataLength,
				(dataType == null) ? yuvFormat.getDataType() : dataType,
				yuvFormat.getFrameRate(), YUVFormat.YUV_420, strideY, strideUV,
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return (supportedOutputFormats == null) ? EMPTY_FORMATS
	 ...
	 */
	public static byte[] validateByteArraySize(Buffer buffer, int newSize,
			boolean arraycopy) {
