...
public class AudioLevelCalculator {
	 ...
	 * meter.
	 */
	private static final double MAX_SOUND_PRESSURE_LEVEL = 127 /*
																 * HUMAN
																 * TINNITUS
																 * (RINGING IN
																 * THE EARS)
	 ...
	 * 
	 * @param level
	 *            the level to animate
	 * @param minLevel
	 *            the minimum value of the range to which <tt>level</tt> belongs
	 * @param maxLevel
	 *            the maximum value of the range to which <tt>level</tt> belongs
	 * @param lastLevel
	 *            the last level which has been previously calculated
	 * @return the value which represents <tt>level</tt> with animation taken
	 *         into account
	 */
	private static int animateLevel(int level, int minLevel, int maxLevel,
			int lastLevel) {
	 ...
	 * 
	 * @param samples
	 *            the samples of the signal to calculate the signal power level
	 *            of
	 * @param offset
	 *            the offset that data starts.
	 * @param length
	 ...
	 * @param lastLevel
	 *            the last level we calculated.
	 * @return the power of the signal in dB SWL.
	 */
	public static int calculateSignalPowerLevel(byte[] samples, int offset,
			int length, int minLevel, int maxLevel, int lastLevel) {
		if (length == 0) {
			return 0;
		}
	 ...
	 * 
	 * @param samples
	 *            the samples of the signal to calculate the sound pressure
	 *            level of
	 * @param offset
	 *            the offset in <tt>samples</tt> in which the samples start
	 * @param length
	 *            the length in bytes of the samples in <tt>samples<tt>
	 * starting at <tt>offset</tt>
	 * @param minLevel
	 *            the minimum value of the level to be returned
	 * @param maxLevel
	 *            the maximum value of the level to be returned
	 * @param lastLevel
	 *            the last level which has been previously calculated
	 * @return the sound pressure level of the specified signal as a value in
	 *         the range between <tt>minLevel</tt> and <tt>maxLevel</tt>
	 */
	public static int calculateSoundPressureLevel(byte[] samples, int offset,
			int length, int minLevel, int maxLevel, int lastLevel) {
		double rms = 0;
		int sampleCount = 0;
	 ...
	 *         <tt>maxLevel</tt>, <tt>minLevel</tt> if <tt>level</tt> has a
	 *         value which is less than <tt>minLevel</tt>, or <tt>maxLevel</tt>
	 *         if <tt>level</tt> has a value which is greater than
	 *         <tt>maxLevel</tt>
	 */
