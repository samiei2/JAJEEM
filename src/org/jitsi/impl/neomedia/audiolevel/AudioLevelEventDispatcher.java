...
public class AudioLevelEventDispatcher {
	...
	private void run() {
		...
		while (true) {
			...
			int newLevel = AudioLevelCalculator.calculateSoundPressureLevel(
					data, 0, dataLength, SimpleAudioLevelListener.MIN_LEVEL,
					SimpleAudioLevelListener.MAX_LEVEL, lastLevel);
	...
	public synchronized void addData(Buffer buffer) {
		...
		if (dataLength > 0) {
			...
			if (bufferData != null) {
				System.arraycopy(bufferData, buffer.getOffset(), data, 0,
						dataLength);
			}
			...
			public void run() {
				try {
					...
					synchronized (AudioLevelEventDispatcher.this) {
						 ...
						 */
						if ((thread == null)
								&& ((listener != null) || ((cache != null) && (ssrc != -1)))
