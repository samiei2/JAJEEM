...
public class AudioLevelEffect extends ControlsAdapter implements Effect {
	 ...
	 */
	private final AudioLevelEventDispatcher eventDispatcher = new AudioLevelEventDispatcher(
			"AudioLevelEffect Dispatcher");
	...
	public AudioLevelEffect() {
		supportedAudioFormats = new Format[] { new AudioFormat(
				...
				AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, 16,
	...
	public Format[] getSupportedOutputFormats(Format input) {
		return new Format[] { new AudioFormat(AudioFormat.LINEAR,
				((AudioFormat) input).getSampleRate(), 16, 1,
				AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, 16,
				Format.NOT_SPECIFIED, Format.byteArray) };
	}
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (COPY_DATA_FROM_INPUT_TO_OUTPUT) {
			...
			System.arraycopy(inputBuffer.getData(), inputBuffer.getOffset(),
					bufferData, 0, inputBufferLength);
