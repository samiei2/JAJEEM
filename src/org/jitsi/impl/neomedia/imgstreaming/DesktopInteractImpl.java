...
import net.java.sip.communicator.impl.neomedia.imgstreaming.NativeScreenCapture;
...
public class DesktopInteractImpl implements DesktopInteract {
	...
	private static final Logger logger = Logger
			.getLogger(DesktopInteractImpl.class);
	 ...
	 * 
	 * @throws AWTException
	 *             if platform configuration does not allow low-level input
	 *             control
	 * @throws SecurityException
	 *             if Robot creation is not permitted
	 */
	 ...
	 * <tt>BufferedImage</tt>. It is done in order to limit slow operation such
	 * as converting ARGB images (uint32_t) to bytes especially for big big
	 * screen. For example a 1920x1200 desktop consumes 9 MB of memory for
	 * grabbing and another 9 MB array for conversion operation.
	 * 
	 * @param display
	 *            index of display
	 * @param output
	 *            output buffer to store bytes in. Be sure that output length is
	 *            sufficient
	 ...
	 *            index of display
	 * @param buffer
	 *            native output buffer to store bytes in. Be sure that output
	...
	public boolean captureScreen(int display, int x, int y, int width,
		...
		if (OSUtils.IS_LINUX || OSUtils.IS_MAC || OSUtils.IS_WINDOWS) {
			return NativeScreenCapture.grabScreen(display, x, y, width, height,
					output);
		}
		return false;
	}
	 ...
	 * @param height
	 *            capture height
	 * @return <tt>BufferedImage</tt> of a part of the desktop screen or null if
	 *         Robot problem
	 */
	@Override
