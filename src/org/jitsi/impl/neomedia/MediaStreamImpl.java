 ...
 * @author Boris Grozev
 */
public class MediaStreamImpl extends AbstractMediaStream implements
		ReceiveStreamListener, SendStreamListener, SessionListener,
		RemoteListener {
	/**
	...
	private static final Logger logger = Logger
			.getLogger(MediaStreamImpl.class);
	...
	public MediaStreamImpl(StreamConnector connector, MediaDevice device,
		...
		this.srtpControl = (srtpControl == null) ? NeomediaServiceUtils
				.getMediaServiceImpl().createZrtpControl() : srtpControl;
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Created " + getClass().getSimpleName()
	...
	protected void configureRTPManagerBufferControl(
			StreamRTPManager rtpManager, BufferControl bufferControl) {
	}
	...
	private TransformEngineChain createTransformEngineChain() {
		ArrayList<TransformEngine> engineChain = new ArrayList<TransformEngine>(
				4);
		...
		return new TransformEngineChain(
				engineChain.toArray(new TransformEngine[engineChain.size()]));
	}
	...
	public byte getActiveRTPExtensionID(RTPExtension rtpExtension) {
		synchronized (activeRTPExtensions) {
			Set<Map.Entry<Byte, RTPExtension>> extSet = this.activeRTPExtensions
					.entrySet();
	...
	private void createSendStreams() {
		...
		if (dataSource instanceof PushBufferDataSource) {
			PushBufferStream[] streams = ((PushBufferDataSource) dataSource)
					.getStreams();

			streamCount = (streams == null) ? 0 : streams.length;
		} else if (dataSource instanceof PushDataSource) {
			PushSourceStream[] streams = ((PushDataSource) dataSource)
					.getStreams();

			streamCount = (streams == null) ? 0 : streams.length;
		} else if (dataSource instanceof PullBufferDataSource) {
			PullBufferStream[] streams = ((PullBufferDataSource) dataSource)
					.getStreams();

			streamCount = (streams == null) ? 0 : streams.length;
		} else if (dataSource instanceof PullDataSource) {
			PullSourceStream[] streams = ((PullDataSource) dataSource)
					.getStreams();
		...
		for (int streamIndex = 0; streamIndex < streamCount; streamIndex++) {
			try {
				SendStream sendStream = rtpManager.createSendStream(dataSource,
						streamIndex);

				if (logger.isTraceEnabled()) {
					logger.trace("Created SendStream" + " with hashCode "
				...
				logger.error("Failed to create send stream for data source "
						+ dataSource + " and stream index " + streamIndex, ioe);
			} catch (UnsupportedFormatException ufe) {
				logger.error("Failed to create send stream for data source "
						+ dataSource + " and stream index " + streamIndex
						+ " because of failed format " + ufe.getFailedFormat(),
						ufe);
			}
		}
		...
		if (logger.isTraceEnabled()) {
			...
			int sendStreamCount = (sendStreams == null) ? 0 : sendStreams
					.size();
	...
	private void deviceSessionSsrcListChanged(PropertyChangeEvent evt) {
		...
		for (int i = 0, j = 0; (i < ssrcArray.length)
				&& (j < csrcArray.length - 1); i++) {
	...
	private void doSetTarget(MediaStreamTarget target) {
		...
		if (rtpConnectorTarget != null) {
			...
			boolean removeTargets = (oldDataAddr == null) ? (newDataAddr != null)
					: !oldDataAddr.equals(newDataAddr);

			if (!removeTargets) {
				InetSocketAddress oldControlAddr = rtpConnectorTarget
						.getControlAddress();

				removeTargets = (oldControlAddr == null) ? (newControlAddr != null)
		...
		if (target == null) {
			...
			try {
				...
				rtpConnector.addTarget(new SessionAddress(newDataAddr
						.getAddress(), newDataAddr.getPort(), controlInetAddr,
						controlPort));
				targetIsSet = true;
			} catch (IOException ioe) {
				// TODO
		...
		if (targetIsSet) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Set target of " + getClass().getSimpleName()
						+ " with hashCode " + hashCode() + " to " + target);
			}
		}
	}
	...
	private MediaDirection getDeviceDirection() {
		...
		return (deviceSession == null) ? MediaDirection.INACTIVE
	...
	public byte getDynamicRTPPayloadType(String encoding) {
		synchronized (dynamicRTPPayloadTypes) {
			for (Map.Entry<Byte, MediaFormat> entry : dynamicRTPPayloadTypes
					.entrySet()) {
	...
	public InetSocketAddress getRemoteControlAddress() {
		StreamConnector connector = (rtpConnector != null) ? rtpConnector
				.getConnector() : null;
	...
	protected void registerCustomCodecFormats(StreamRTPManager rtpManager) {
		synchronized (dynamicRTPPayloadTypes) {
			for (Map.Entry<Byte, MediaFormat> dynamicRTPPayloadType : dynamicRTPPayloadTypes
					.entrySet()) {
				@SuppressWarnings("unchecked")
				MediaFormatImpl<? extends Format> mediaFormatImpl = (MediaFormatImpl<? extends Format>) dynamicRTPPayloadType
						.getValue();
	...
	private void rtpConnectorInputStreamCreated(
			RTPConnectorInputStream inputStream, boolean data) {
		/*
		 ...
		 */
		try {
			firePropertyChange(MediaStreamImpl.class.getName()
					+ ".rtpConnector." + (data ? "data" : "control")
		...
		case UDP:
			rtpConnector = new RTPTransformUDPConnector(connector) {
				...
				protected TransformUDPInputStream createControlInputStream()
						throws IOException {
					TransformUDPInputStream s = super
		...
		case TCP:
			rtpConnector = new RTPTransformTCPConnector(connector) {
				...
				protected TransformTCPInputStream createControlInputStream()
						throws IOException {
					TransformTCPInputStream s = super
							.createControlInputStream();
	...
	public void setDevice(MediaDevice device) {
		...
		if ((deviceSession == null) || (deviceSession.getDevice() != device)) {
			...
			if (deviceSession != null) {
				...
				deviceSession
						.removePropertyChangeListener(deviceSessionPropertyChangeListener);

				// keep player active
				deviceSession
						.setDisposePlayerOnClose(!(deviceSession instanceof VideoMediaDeviceSession));
				deviceSession.close();
				deviceSession = null;
			} else {
				format = null;
	...
	public void setFormat(MediaFormat format) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Changing format of stream " + hashCode() + " from: "
					+ deviceSessionFormat + " to: " + format);
		}
	...
	private void start(MediaDirection direction) {
		...
		if (direction.allowsSending()
				&& ((startedDirection == null) || !startedDirection
						.allowsSending())) {
			/*
			...
			if (logger.isInfoEnabled()) {
				...
				logger.info(mediaType + " remote IP/port: "
	...
	private void startReceiveStreams() {
		...
		if (receiveStreams != null) {
			...
			for (ReceiveStream receiveStream : receiveStreams) {
				try {
					DataSource receiveStreamDataSource = receiveStream
							.getDataSource();
					...
					logger.warn("Failed to start receive stream "
							+ receiveStream, ioex);
				}
			}
		}
	...
	private void startSendStreams() {
		...
		if (sendStreams != null) {
			for (SendStream sendStream : sendStreams) {
				try {
					DataSource sendStreamDataSource = sendStream
							.getDataSource();
	...
	private void stop(MediaDirection direction) {
		...
		if ((MediaDirection.SENDRECV.equals(direction) || MediaDirection.SENDONLY
				.equals(direction))
				&& (MediaDirection.SENDRECV.equals(startedDirection) || MediaDirection.SENDONLY
						.equals(startedDirection))) {
			/*
		...
		if ((MediaDirection.SENDRECV.equals(direction) || MediaDirection.RECVONLY
				.equals(direction))
				&& (MediaDirection.SENDRECV.equals(startedDirection) || MediaDirection.RECVONLY
						.equals(startedDirection))) {
			stopReceiveStreams();
		...
		if (receiveStreams != null) {
			...
			for (ReceiveStream receiveStream : receiveStreams) {
				try {
					...
					logger.warn("Failed to stop receive stream "
							+ receiveStream, ioex);
				}
			}
		}
	...
	private Iterable<SendStream> stopSendStreams(boolean close) {
		...
		@SuppressWarnings("unchecked")
		Iterable<SendStream> sendStreams = rtpManager.getSendStreams();
		Iterable<SendStream> stoppedSendStreams = stopSendStreams(sendStreams,
				close);
	...
	private Iterable<SendStream> stopSendStreams(
			Iterable<SendStream> sendStreams, boolean close) {
		if (sendStreams == null) {
			return null;
		}
			...
			try {
				...
				if (close) {
					try {
						 ...
						 */
						logger.error("Failed to close send stream "
								+ sendStream, npe);
					}
				}
			} catch (IOException ioe) {
	...
	public void update(ReceiveStreamEvent event) {
		if (event instanceof NewReceiveStreamEvent) {
			 ...
			 */
			/*
			 * if(!zrtpRestarted) restartZrtpControl();
			 */
			...
			if (receiveStream != null) {
				...
				if (deviceSession != null) {
					TranscodingDataSource transcodingDataSource = deviceSession
					...
					try {
						...
						deviceSession.playbackDataSourceChanged(receiveStream
								.getDataSource());
					} catch (IOException e) {
	...
	public void update(RemoteEvent remoteEvent) {
		if (remoteEvent instanceof SenderReportEvent
			...
			if ((feedback != null)
				...
				Set<PacketLossAwareEncoder> plaes = deviceSession
						.getEncoderControls(PacketLossAwareEncoder.class);
			...
			if (logger.isInfoEnabled()) {
				...
				StringBuilder buff = new StringBuilder(
						StatisticsEngine.RTP_STAT_PREFIX);
				MediaType mediaType = getMediaType();
				String mediaTypeStr = (mediaType == null) ? "" : mediaType
						.toString();

				buff.append("Received a ")
						.append(senderReport ? "sender" : "receiver")
						.append(" report for ").append(mediaTypeStr)
						.append(" stream SSRC:").append(getLocalSourceID())
						.append(" [");
				if (senderReport) {
					buff.append("packet count:")
							.append(((SenderReport) report)
				...
				if (feedback != null) {
					buff.append(", interarrival jitter:").append(remoteJitter)
							.append(", lost packets:")
	...
	protected void setLocalSourceID(long localSourceID) {
		if (this.localSourceID != localSourceID) {
			...
			/*
			 * If a ZRTP engine is available, then let it know about the SSRC of
			 * the new SendStream. Currently, ZRTP supports only one SSRC per
			 * engine.
			 */
	...
	private void printFlowStatistics(StreamRTPManager rtpManager) {
		try {
			...
			buff.append("call stats for incoming ")
					...
					.append("\n").append(StatisticsEngine.RTP_STAT_PREFIX)
					.append("packets lost: ")
					.append(statisticsEngine.getLost()).append("\n")
					.append(StatisticsEngine.RTP_STAT_PREFIX)
	...
	private void printReceiveStreamStatistics() {
		...
		StringBuilder buff = new StringBuilder("\nReceive stream stats: "
				...
				.append(mediaStreamStatsImpl.getNbDiscarded()).append("\n")
				.append("Receive stream stats: " + "decoded with FEC: ")
