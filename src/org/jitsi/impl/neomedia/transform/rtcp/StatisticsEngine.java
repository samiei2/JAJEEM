...
public class StatisticsEngine implements TransformEngine, PacketTransformer {
	...
	/**
	 * The RTP statistics prefix we use for every log. Simplifies parsing and
	 * searching for statistics info in log files.
	 */
	...
	/**
	 * Number of sender reports send. Used only for logging and debug purposes.
	 */
	...
	/**
	 * Always returns <tt>null</tt> since this engine does not require any RTP
	 * transformations.
	 * 
	 * @return <tt>null</tt> since this engine does not require any RTP
	 *         transformations.
	 */
	@Override
	 ...
	 * stream for the MediaStreamStats. Returns the packet as we are listening
	 * just for sending packages.
	 * 
	 * @param pkt
		...
		try {
			...
			if (length != 0) {
				...
				if (header.getPacketType() == RTCPPacket.SR) {
					RTCPSenderReport senderReport = new RTCPSenderReport(data,
							offset, length);
					List<?> feedbackReports = senderReport.getFeedbackReports();

					if (feedbackReports.size() != 0) {
						RTCPFeedback feedback = (RTCPFeedback) feedbackReports
								.get(0);
			...
			if (t instanceof ThreadDeath) {
				...
				logger.error("Failed to analyze an incoming RTCP packet"
						+ " for the purposes of statistics.", t);
			}
		}
		...
		try {
			...
			if (header.getPacketType() == RTCPPacket.SR) {
				...
				if (feedbackReports.size() != 0) {
					...
					mediaStream.getMediaStreamStats().updateNewSentFeedback(
							feedback);
			...
			if (t instanceof ThreadDeath) {
				...
				logger.error("Failed to analyze an outgoing RTCP packet"
						+ " for the purposes of statistics.", t);
			}
		}
