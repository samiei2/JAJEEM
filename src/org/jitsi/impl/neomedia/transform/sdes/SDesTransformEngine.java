...
public class SDesTransformEngine implements TransformEngine {
	...
	private static SRTPContextFactory getTransformEngine(
		...
		return new SRTPContextFactory(getKey(attribute), getSalt(attribute),
				new SRTPPolicy(getEncryptionCipher(cs),
						cs.getEncKeyLength() / 8, getHashAlgorithm(cs),
						cs.getSrtpAuthKeyLength() / 8,
						cs.getSrtpAuthTagLength() / 8,
						cs.getSaltKeyLength() / 8), new SRTPPolicy(
