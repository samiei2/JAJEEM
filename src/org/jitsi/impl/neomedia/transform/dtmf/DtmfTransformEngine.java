 ...
 */
public class DtmfTransformEngine implements TransformEngine, PacketTransformer {
	/**
	 ...
	 */
	private static final DTMFRtpTone[] supportedTones = new DTMFRtpTone[] {
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		byte currentDtmfPayload = mediaStream
				.getDynamicRTPPayloadType(Constants.TELEPHONE_EVENT);
	...
	public RawPacket transform(RawPacket pkt) {
		...
		DtmfRawPacket dtmfPkt = new DtmfRawPacket(pkt.getBuffer(),
				pkt.getOffset(), pkt.getLength(), currentDtmfPayload);
		...
		if (toneTransmissionState == ToneTransmissionState.IDLE) {
			...
			synchronized (startStopToneMutex) {
				...
				|| (toneTransmissionState == ToneTransmissionState.END_REQUESTED && !lastMinimalDuration)) {
			currentDuration += getCurrentSpacingDuration();
			pktDuration = currentDuration;
		...
		dtmfPkt.init(toneCode, pktEnd, pktMarker, pktDuration, currentTimestamp);
		pkt = dtmfPkt;
	...
	public void stopSendingDTMF() {
		synchronized (startStopToneMutex) {
			...
			int stoppingTone = (toneTransmissionState == ToneTransmissionState.END_REQUESTED || toneTransmissionState == ToneTransmissionState.END_SEQUENCE_INITIATED) ? 1
					: 0;
	...
	private class DTMFDispatcher implements Runnable {
		...
		public void run() {
			...
			while (isRunning) {
				...
				if (temp != null
						&& ((lastReportedTone == null && !toEnd) || (lastReportedTone != null && toEnd))) {
