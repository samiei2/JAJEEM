...
import org.jitsi.impl.neomedia.ZrtpControlImpl;
...
public class SecurityEventManager extends ZrtpUserCallback {
	...
	private static final Logger logger = Logger
			.getLogger(SecurityEventManager.class);

	/**
	 * A warning <tt>String</tt> that we display to the user.
	 */
	public static final String WARNING_NO_RS_MATCH = getI18NString(
			"impl.media.security.WARNING_NO_RS_MATCH", null);

	/**
	 * A warning <tt>String</tt> that we display to the user.
	 */
	public static final String WARNING_NO_EXPECTED_RS_MATCH = getI18NString(
			"impl.media.security.WARNING_NO_EXPECTED_RS_MATCH", null);

	/**
	 * Indicates an audio session type.
	 */
	public static final int AUDIO_SESSION = CallPeerSecurityStatusEvent.AUDIO_SESSION;

	/**
	 * Indicates a video session type.
	 */
	public static final int VIDEO_SESSION = CallPeerSecurityStatusEvent.VIDEO_SESSION;
	 ...
	 * 
	 * @param type
	 *            the session type. The session type could be either
	...
	public void showMessage(ZrtpCodes.MessageSeverity sev, EnumSet<?> subCode) {
		...
		if (msgCode instanceof ZrtpCodes.InfoCodes) {
			...
			i18nMessage = getI18NString("impl.media.security.ZRTP_GENERIC_MSG",
					msgCode.toString());
		} else {
			// Other warnings are internal only, no user action required
			sendEvent = false;
		...
		if (sendEvent) {
			securityListener.securityMessageReceived(message, i18nMessage,
					severity);
		}
	 ...
	 * 
	 * @param sessionType
	 *            one of the <tt>CallPeerSecurityStatusEvent.XXX_SESSION</tt>
	 *            fields
	 * 
