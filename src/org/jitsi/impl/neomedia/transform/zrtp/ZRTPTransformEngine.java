 ...
 * 
 */
public class ZRTPTransformEngine implements TransformEngine, PacketTransformer,
		ZrtpCallback {
	/**
	...
	private static final Logger logger = Logger
			.getLogger(ZRTPTransformEngine.class);
	...
	public synchronized boolean initialize(String zidFilename,
		...
		if (!zf.isOpen()) {
			if (zidFilePath == null) {
				...
				String baseDir = ((home == null) || (home.length() < 1)) ? ""
						: (home + "/");
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		...
		/*
		 * Check if incoming packet is a ZRTP packet, if not treat it as normal
		 * RTP packet and handle it accordingly.
		 */
		if (!ZrtpRawPacket.isZrtpData(pkt)) {
			if (srtpInTransformer == null) {
				if (muted) {
		 ...
		 * 
		 * In any case return null because ZRTP packets must never reach the
		 * application.
		 */
		if (enableZrtp && started) {
			ZrtpRawPacket zPkt = new ZrtpRawPacket(pkt);
			if (!zPkt.checkCrc()) {
				securityEventManager.showMessage(
			...
			// Check if it is really a ZRTP packet, if not don't process it
				...
				byte[] extHeader = zPkt.readRegion(extHeaderOffset,
						RawPacket.EXT_HEADER_SIZE + zPkt.getExtensionLength()
								+ zPkt.getPayloadLength());
	 ...
	 * place, the initalize the header, counter, SSRC and crc.
	 * 
	 * @param data
	...
	public boolean checkSASSignature(byte[] sasHash) {
		return ((securityEventManager != null) ? securityEventManager
				.checkSASSignature(sasHash) : false);
	}
