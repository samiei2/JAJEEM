...
public class SRTCPCryptoContext {
	...
	// the methods must use this only to store some reults for immediate
	// processing
	...
	public SRTCPCryptoContext(long ssrcIn, byte[] masterK, byte[] masterS,
			SRTPPolicy policyIn) {
		ssrcCtx = ssrcIn;
		mki = null;
	...
	/**
	 * Transform a SRTCP packet into a RTCP packet. This method is called when a
	 * SRTCP packet was received.
	 * 
	 ...
	 * encryption and no authentication. Then the packet will be sent out
	 * untouched. However this is not encouraged. If no SRTCP feature is
	 ...
	 * @return true if the packet can be accepted false if authentication or
	 *         replay check failed
	 */
	...
	public void processPacketAESF8(RawPacket pkt, int index) {
		...
		final int payloadOffset = 8;
		final int payloadLength = pkt.getLength()
				- (4 + policy.getAuthTagLength());
		...
		if (authKey != null) {
			...
			case SRTPPolicy.SKEIN_AUTHENTICATION:
				...
				ParametersForSkein pfs = new ParametersForSkein(
						new KeyParameter(authKey), ParametersForSkein.Skein512,
						tagStore.length * 8);
				mac.init(pfs);
				break;
			}
