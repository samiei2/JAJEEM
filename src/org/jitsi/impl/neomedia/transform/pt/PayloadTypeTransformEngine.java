 ...
 * support of legacy protocols such as H.323 but we've been bumping with a
 * number of cases where multi-component pure SIP systems also need to behave
 * this way.
 * 
 * @author Damian Minkov
 */
public class PayloadTypeTransformEngine implements TransformEngine,
		PacketTransformer {
	/**
	 * The mapping we use to override payloads. By default it is empty and we do
	 * nothing, packets are passed through without modification. Maps source
	 * payload to target payload.
	 */
	 ...
	 * through the packet. If the <tt>RawPacket</tt> payload has entry in
	 * mappings to override, we override packet payload type.
	 * 
	 * @param pkt
	 *            the RTP <tt>RawPacket</tt> that we check for need to change
	 *            payload type.
	 * 
	 ...
	 * 
	 * @param pkt
	 *            the RTP <tt>RawPacket</tt> that we will pass through.
	 * 
