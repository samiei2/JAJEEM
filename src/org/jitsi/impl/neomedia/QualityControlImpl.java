...
class QualityControlImpl implements QualityControl {
	...
	private static final Logger logger = Logger
			.getLogger(QualityControlImpl.class);
	...
	private void setRemoteReceivePreset(QualityPreset preset)
		...
		if (preset.compareTo(getPreferredSendPreset()) > 0) {
			...
			if (logger.isInfoEnabled() && (preset != null)
				...
				logger.info("video send resolution: " + resolution.width + "x"
						+ resolution.height);
			}
		}
	}
	...
	private QualityPreset getPreferredSendPreset() {
		if (localSettingsPreset == null) {
			DeviceConfiguration devCfg = NeomediaServiceUtils
					.getMediaServiceImpl().getDeviceConfiguration();

			localSettingsPreset = new QualityPreset(devCfg.getVideoSize(),
					devCfg.getFrameRate());
		}
