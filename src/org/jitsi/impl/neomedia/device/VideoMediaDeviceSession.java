 ...
 * @author Boris Grozev
 */
public class VideoMediaDeviceSession extends MediaDeviceSession implements
		RTCPFeedbackCreateListener {
	/**
	...
	private static final Logger logger = Logger
			.getLogger(VideoMediaDeviceSession.class);
			...
			try {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Called Player#getVisualComponent() "
							+ "on unrealized player " + player, nre);
				}
			}
		}
	...
	/**
	 * RTCPFeedbackListener instance that will be passed to rtpConnectors to
	...
	/**
	 * A list with RTCPFeedbackCreateListener which will be notified when a
	 * RTCPFeedbackListener is created.
	 */
	private List<RTCPFeedbackCreateListener> rtcpFeedbackCreateListners = new LinkedList<RTCPFeedbackCreateListener>();
	 ...
	 */
	private final VideoNotifierSupport videoNotifierSupport = new VideoNotifierSupport(
			this, false);
	...
	private void controllerUpdateForCreateLocalVisualComponent(
			ControllerEvent ev, boolean hflip) {
		if (ev instanceof ConfigureCompleteEvent) {
			...
			playerSizeChange(scev.getSourceController(), VideoEvent.LOCAL,
	...
	protected DataSource createCaptureDevice() {
		...
		if (captureDevice != null) {
			...
			float frameRate;
			DeviceConfiguration deviceConfig = NeomediaServiceUtils
			...
			if (DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING.equals(protocol)) {
				...
				Dimension dim = VideoMediaStreamImpl.selectVideoSize(
						captureDevice, videoSize.width, videoSize.height);

				frameRate = deviceConfig.getFrameRate();

				// print initial video resolution, when starting video
				if (logger.isInfoEnabled() && dim != null) {
					logger.info("video send resolution: " + dim.width + "x"
							+ dim.height);
				}
			}

			FrameRateControl frameRateControl = (FrameRateControl) captureDevice
					.getControl(FrameRateControl.class.getName());

			if (frameRateControl != null) {
				float maxSupportedFrameRate = frameRateControl
				...
				if (logger.isInfoEnabled()) {
					logger.info("video send FPS: "
							+ (frameRate == -1 ? "default(no restriction)"
									: frameRate));
				}
			}
			...
			if (!(captureDevice instanceof SourceCloneable)) {
				DataSource cloneableDataSource = Manager
						.createCloneableDataSource(captureDevice);
	...
	protected Player createLocalPlayer(DataSource captureDevice) {
		DataSource dataSource = (captureDevice instanceof SourceCloneable) ? ((SourceCloneable) captureDevice)
				.createClone() : null;
		...
		if (dataSource != null) {
			...
			if (exception == null) {
				if (localPlayer != null) {
					 ...
					 * not seem natural.
					 */
					final boolean hflip = (captureDevice
							.getControl(ImgStreamingControl.class.getName()) == null);

					localPlayer.addControllerListener(new ControllerListener() {
						@Override
						public void controllerUpdate(ControllerEvent ev) {
							controllerUpdateForCreateLocalVisualComponent(ev,
									hflip);
						}
					});
	...
	protected Component createLocalVisualComponent() {
		...
		if ((captureDevice != null)
				&& (captureDevice.getControl(ImgStreamingControl.class
						.getName()) != null)) {
		...
		synchronized (localPlayerSyncRoot) {
			...
			localVisualComponent = (localPlayer == null) ? null
		...
		if (localVisualComponent != null) {
			fireVideoEvent(VideoEvent.VIDEO_ADDED, localVisualComponent,
					VideoEvent.LOCAL, false);
		}
	...
	private Component createLocalVisualComponentForDesktopStreaming() {
		...
		ImageIcon icon = (resources == null) ? null : resources
				.getImage(DESKTOP_STREAMING_ICON);
		Canvas canvas;
		...
		if (icon == null) {
			...
			canvas = new Canvas() {
				...
				public void paint(Graphics g) {
					...
					if (imgHeight > height) {
						scale = true;
						scaleFactor = Math.min(scaleFactor, height
								/ (float) imgHeight);
					}
					...
					g.drawImage(img, dstX, dstY, dstX + dstWidth, dstY
							+ dstHeight, 0, 0, imgWidth, imgHeight, this);
				}
			};
	...
	protected void disposeLocalVisualComponent(Component component) {
		if (component != null) {
			...
			if (DESKTOP_STREAMING_ICON.equals(component.getName())) {
				fireVideoEvent(VideoEvent.VIDEO_REMOVED, component,
						VideoEvent.LOCAL, false);
			} else {
				Player localPlayer;
	 ...
	 */
	protected boolean fireVideoEvent(int type, Component visualComponent,
			int origin, boolean wait) {
		if (logger.isTraceEnabled()) {
			logger.trace("Firing VideoEvent with type "
					+ VideoEvent.typeToString(type) + " and origin "
					+ VideoEvent.originToString(origin));
		}

		return videoNotifierSupport.fireVideoEvent(type, visualComponent,
				origin, wait);
	}
	...
	private Format getCaptureDeviceFormat() {
		...
		if (captureDevice != null) {
			...
			if (captureDevice instanceof CaptureDevice) {
				formatControls = ((CaptureDevice) captureDevice)
						.getFormatControls();
			}
			if ((formatControls == null) || (formatControls.length == 0)) {
				FormatControl formatControl = (FormatControl) captureDevice
	...
	public VideoFormat getSentVideoFormat() {
		...
		if (capture instanceof PullBufferDataSource) {
			PullBufferStream[] streams = ((PullBufferDataSource) capture)
					.getStreams();
			...
			try {
				for (TrackControl trackControl : trackControls) {
					...
					if ("h264/rtp".equalsIgnoreCase(fmjEncoding)) {
						...
						trackControl.setCodecChain(new Codec[] { depacketizer,
								decoder, playerScaler });
					} else {
						trackControl
								.setCodecChain(new Codec[] { playerScaler });
					}
					break;
				}
			} catch (UnsupportedPlugInException upiex) {
				logger.error("Failed to add SwScale or H.264 DePacketizer"
						+ " to codec chain", upiex);
				playerScaler = null;
			}
		}
	...
	protected void playerControllerUpdate(ControllerEvent ev) {
		...
		if ((ev instanceof SizeChangeEvent)
			...
			playerSizeChange(scev.getSourceController(), VideoEvent.REMOTE,
					scev.getWidth(), scev.getHeight());
		}
	}
	 ...
	 * @see SizeChangeEvent
	 */
	protected void playerSizeChange(final Controller sourceController,
			final int origin, final int width, final int height) {
		/*
		...
		if (visualComponent != null) {
			...
			try {
				...
				if ((prefSize == null)
						|| (prefSize.width < 1)
						|| (prefSize.height < 1)
						|| !VideoLayout.areAspectRatiosEqual(prefSize, width,
								height) || (prefSize.width < width)
						|| (prefSize.height < height)) {
					visualComponent.setPreferredSize(new Dimension(width,
							height));
				}
			} finally {
				fireVideoEvent(new SizeChangeVideoEvent(this, visualComponent,
						origin, width, height), false);
			}
		}
	}
	 ...
	 *            <tt>Component</tt>
	 */
	private void playerVisualComponentResized(Processor player,
			ComponentEvent ev) {
		if (playerScaler == null) {
			return;
		}
		...
		if (playerScalerOutputSize == null) {
			 ...
			 * anyway.
			 */
			int outputWidthDelta = outputSize.width
					- playerScalerOutputSize.width;
			int outputHeightDelta = outputSize.height
					- playerScalerOutputSize.height;
	...
	public void setFormat(MediaFormat format) {
		if (format instanceof VideoMediaFormat
			...
			FrameRateControl frameRateControl = (FrameRateControl) getCaptureDevice()
					.getControl(FrameRateControl.class.getName());
	...
	public void setKeyFrameControl(KeyFrameControl keyFrameControl) {
		if (this.keyFrameControl != keyFrameControl) {
			...
			if ((this.keyFrameControl != null) && (keyFrameRequester != null)) {
				this.keyFrameControl
						.addKeyFrameRequester(-1, keyFrameRequester);
			}
		}
	}
	...
	public void setOutputSize(Dimension size) {
		boolean equal = (size == null) ? (outputSize == null) : size
				.equals(outputSize);
	 ...
	 */
	@Override
	protected void setProcessorFormat(Processor processor,
		...
		if ((outputSize != null) && (outputSize.width > 0)
				&& (outputSize.height > 0)) {
			Dimension deviceSize = ((VideoFormat) getCaptureDeviceFormat())
					.getSize();
			Dimension videoFormatSize;

			if ((deviceSize != null)
					&& ((deviceSize.width > outputSize.width) || (deviceSize.height > outputSize.height))) {
			 ...
			 */
			format = new VideoFormat(videoFormat.getEncoding(),
					videoFormatSize, videoFormat.getMaxDataLength(),
					videoFormat.getDataType(), videoFormat.getFrameRate());
		} else {
			outputSize = null;
		}
	...
	protected Format setProcessorFormat(TrackControl trackControl,
		...
		if (!OSUtils.IS_ANDROID
			...
			// packetization-mode
			{
				Map<String, String> formatParameters = mediaFormat
						.getFormatParameters();
				String packetizationMode = (formatParameters == null) ? null
						: formatParameters
								.get(JNIEncoder.PACKETIZATION_MODE_FMTP);
			...
			{
				encoder.setAdditionalCodecSettings(mediaFormat
		...
		if (codecCount != 0) {
			...
			try {
				...
				logger.error("Failed to add SwScale/JNIEncoder to codec chain",
						upiex);
			}
		}
							...
							if (VideoMediaDeviceSession.this.usePLI) {
								try {
									new RTCPFeedbackPacket(
											...
											RTCPFeedbackEvent.PT_PS, localSSRC,
											remoteSSRC).writeTo(rtpConnector
											.getControlOutputStream());
									requested = true;
								} catch (IOException ioe) {
									/*
									 * Apart from logging the ioe, there are not
			...
			if (result == BUFFER_PROCESSED_OK) {
				...
				if (inputFormat != null) {
					...
					if ((size != null)
						...
						lastSize = size;
						playerSizeChange(player, VideoEvent.REMOTE,
				...
				if ((inputSize != null) && (inputSize.width > 0)) {
					...
					if ((outputSize != null)
						...
						int outputHeight = (int) (outputWidth
								* inputSize.height / (float) inputSize.width);
						int outputHeightDelta = outputHeight
								- outputSize.height;

						if ((outputHeightDelta < -1) || (outputHeightDelta > 1)) {
							super.setOutputSize(new Dimension(outputWidth,
									outputHeight));
						}
					}
				}
