 ...
 */
public class DeviceConfiguration extends PropertyChangeNotifier implements
	...
	public static final String PROP_AUDIO_SYSTEM_DEVICES = PROP_AUDIO_SYSTEM
			+ "." + DeviceSystem.PROP_DEVICES;
	...
	private static void fixRenderers() {
		...
		Vector<String> renderers = PlugInManager.getPlugInList(null, null,
		...
		if (OSUtils.IS_WINDOWS) {
			...
			if (renderers
					.contains("com.sun.media.renderer.video.LightWeightRenderer")
					|| renderers
							.contains("com.sun.media.renderer.video.AWTRenderer")) {
	...
	public DeviceConfiguration() {
		...
		setAudioSystemIsDisabled = (cfg != null)
				&& cfg.getBoolean(
						MediaServiceImpl.DISABLE_SET_AUDIO_SYSTEM_PNAME, false);
	...
	private void addDeviceSystemPropertyChangeListener() {
		...
		for (MediaType mediaType : MediaType.values()) {
			DeviceSystem[] deviceSystems = DeviceSystem
					.getDeviceSystems(mediaType);
				...
				if (cfg != null) {
					...
					if (locatorProtocol != null) {
						for (AudioSystem availableAudioSystem : availableAudioSystems) {
							if (locatorProtocol
									.equalsIgnoreCase(availableAudioSystem
											.getLocatorProtocol())) {
						...
						if (setAudioSystemIsDisabled && (audioSystem == null)) {
							audioSystem = AudioSystem
									.getAudioSystem(locatorProtocol);
						}
					}
				}
	...
	private CaptureDeviceInfo extractConfiguredVideoCaptureDevice(Format format) {
		@SuppressWarnings("unchecked")
		List<CaptureDeviceInfo> videoCaptureDevices = CaptureDeviceManager
				.getDeviceList(format);
		CaptureDeviceInfo videoCaptureDevice = null;

		if (videoCaptureDevices.size() > 0) {
			ConfigurationService cfg = LibJitsi.getConfigurationService();
			String videoDevName = (cfg == null) ? null : cfg
			...
			if ((videoCaptureDevice != null) && logger.isInfoEnabled()) {
				logger.info("Found " + videoCaptureDevice.getName() + " as a "
	...
	private void extractConfiguredVideoCaptureDevices() {
		ConfigurationService cfg = LibJitsi.getConfigurationService();
		String videoCaptureDeviceString = (cfg == null) ? null : cfg
				.getString(PROP_VIDEO_DEVICE);

		if (NoneAudioSystem.LOCATOR_PROTOCOL
				.equalsIgnoreCase(videoCaptureDeviceString)) {
			...
			Format[] formats = new Format[] { new AVFrameFormat(),
	...
	public CaptureDeviceInfo2 getAudioCaptureDevice() {
		...
		return (audioSystem == null) ? null : audioSystem
				.getSelectedDevice(AudioSystem.DataFlow.CAPTURE);
	}
	...
	public CaptureDeviceInfo getAudioNotifyDevice() {
		...
		return (audioSystem == null) ? null : audioSystem
				.getSelectedDevice(AudioSystem.DataFlow.NOTIFY);
	}
	...
	public AudioSystem[] getAvailableAudioSystems() {
		...
		if ((audioSystems == null) || (audioSystems.length == 0)) {
			...
			for (AudioSystem audioSystem : audioSystems) {
				if (!NoneAudioSystem.LOCATOR_PROTOCOL
						.equalsIgnoreCase(audioSystem.getLocatorProtocol())) {
					List<CaptureDeviceInfo2> captureDevices = audioSystem
							.getDevices(AudioSystem.DataFlow.CAPTURE);

					if ((captureDevices == null)
							|| (captureDevices.size() <= 0)) {
						if ((AudioSystem.FEATURE_NOTIFY_AND_PLAYBACK_DEVICES & audioSystem
								.getFeatures()) == 0) {
							continue;
						} else {
							List<CaptureDeviceInfo2> notifyDevices = audioSystem
									.getDevices(AudioSystem.DataFlow.NOTIFY);

							if ((notifyDevices == null)
									|| (notifyDevices.size() <= 0)) {
								List<CaptureDeviceInfo2> playbackDevices = audioSystem
										.getDevices(AudioSystem.DataFlow.PLAYBACK);
			...
			return (audioSystemsWithDevicesCount == audioSystems.length) ? audioSystems
					: audioSystemsWithDevices
							.toArray(new AudioSystem[audioSystemsWithDevicesCount]);
		}
	}
	...
	public List<CaptureDeviceInfo> getAvailableVideoCaptureDevices(
		...
		for (Format format : formats) {
			@SuppressWarnings("unchecked")
			Vector<CaptureDeviceInfo> cdis = CaptureDeviceManager
					.getDeviceList(format);
			...
			if (useCase != MediaUseCase.ANY) {
				for (CaptureDeviceInfo cdi : cdis) {
					MediaUseCase cdiUseCase = DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING
							.equalsIgnoreCase(cdi.getLocator().getProtocol()) ? MediaUseCase.DESKTOP
							: MediaUseCase.CALL;
	...
	public long getEchoCancelFilterLengthInMillis() {
		...
		if (cfg != null) {
			value = cfg.getLong(PROP_AUDIO_ECHOCANCEL_FILTER_LENGTH_IN_MILLIS,
					value);
		}
		return value;
	}
	...
	public void propertyChange(PropertyChangeEvent ev) {
		...
		if (AUDIO_CAPTURE_DEVICE.equals(propertyName)
			...
			if (audioSystem != null) {
				CaptureDeviceInfo oldValue = (CaptureDeviceInfo) ev
						.getOldValue();
				CaptureDeviceInfo newValue = (CaptureDeviceInfo) ev
						.getNewValue();
				CaptureDeviceInfo device = (oldValue == null) ? newValue
						: oldValue;

				// Fire an event on the selected device only if the event is
				// generated by the selected audio system.
				if ((device == null)
						|| device.getLocator().getProtocol()
								.equals(audioSystem.getLocatorProtocol())) {
			...
			if (ev.getSource() instanceof AudioSystem) {
				...
				@SuppressWarnings("unchecked")
				List<CaptureDeviceInfo> newValue = (List<CaptureDeviceInfo>) ev
						.getNewValue();

				firePropertyChange(PROP_AUDIO_SYSTEM_DEVICES, ev.getOldValue(),
						newValue);
			}
	...
	private void registerCustomRenderers() {
		...
		for (String customRenderer : CUSTOM_RENDERERS) {
			...
			if (customRenderer.startsWith(".")) {
				customRenderer = "org.jitsi.impl.neomedia"
						+ ".jmfext.media.renderer" + customRenderer;
			}
			if ((renderers == null) || !renderers.contains(customRenderer)) {
				try {
					Renderer customRendererInstance = (Renderer) Class.forName(
							customRenderer).newInstance();

					PlugInManager.addPlugIn(customRenderer,
							customRendererInstance.getSupportedInputFormats(),
							null, PlugInManager.RENDERER);
					commit = true;
				} catch (Throwable t) {
					logger.error("Failed to register custom Renderer "
							+ customRenderer + " with JMF.", t);
				}
			}
		}
		...
		@SuppressWarnings("unchecked")
		Vector<String> plugins = PlugInManager.getPlugInList(null, null,
				pluginType);
			...
			if (save) {
				...
				if (cfg != null) {
					if (this.audioSystem == null) {
						...
						cfg.setProperty(PROP_AUDIO_SYSTEM,
		...
		if (videoCaptureDevice != device) {
			...
			if (save) {
				...
				if (cfg != null) {
					cfg.setProperty(
							PROP_VIDEO_DEVICE,
							(videoCaptureDevice == null) ? NoneAudioSystem.LOCATOR_PROTOCOL
	...
	public void setVideoSize(Dimension videoSize) {
		...
		firePropertyChange(VIDEO_CAPTURE_DEVICE, videoCaptureDevice,
				videoCaptureDevice);
	}
}
