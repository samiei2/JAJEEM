...
public class AudioMediaDeviceImpl extends MediaDeviceImpl {
	...
	private static final Logger logger = Logger
			.getLogger(AudioMediaDeviceImpl.class);
	...
	protected synchronized CaptureDevice createCaptureDevice() {
		...
		if (getDirection().allowsSending()) {
			if (captureDeviceSharing == null) {
				...
				if (AudioSystem.LOCATOR_PROTOCOL_JAVASOUND
					...
					if (captureDevice != null) {
						...
						captureDevice = captureDeviceSharing
	...
	protected CaptureDevice superCreateCaptureDevice() {
		...
		if (captureDevice != null) {
			 ...
			 */
			try {
				FormatControl[] formatControls = captureDevice
						.getFormatControls();
				...
				if ((formatControls != null) && (formatControls.length != 0)) {
					for (FormatControl formatControl : formatControls) {
						...
						if (!supportedFormatToSet.matches(format)) {
							...
							double sampleRate = supportedFormatToSet
									.getSampleRate();
							int sampleSizeInBits = supportedFormatToSet
									.getSampleSizeInBits();
							...
							if ((channels != Format.NOT_SPECIFIED)
								...
								if (supportedFormatToSet.matches(formatToSet)) {
									formatControl
											.setFormat(supportedFormatToSet
													.intersects(formatToSet));
								}
							}
						}
					}
				...
				if (t instanceof ThreadDeath) {
					throw (ThreadDeath) t;
					/*
					 * We tried to default the captureDevice to a Format which
					 * does not exceed the maximum quality known to MediaUtils
					 * and we failed but it does not mean that the captureDevice
					 * will not be successfully used.
					 */
				}
			}
		}
