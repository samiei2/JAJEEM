...
public class WASAPISystem extends AudioSystem {
	...
	public static AudioFormat[] getFormatsToInitializeIAudioClient(
		...
		return new AudioFormat[] {
				/*
				 * Regardless of the differences in the states of the support of
				 * mono and stereo in the library at the time of this writing,
				 * try to initialize a new IAudioClient instance with a format
				 * which will not require conversion between mono and stereo.
				 */
				format,
				new AudioFormat(format.getEncoding(), format.getSampleRate(),
						format.getSampleSizeInBits(), channels,
	...
	public static void WAVEFORMATEX_fill(long waveformatex,
		...
		WASAPI.WAVEFORMATEX_fill(waveformatex, WAVE_FORMAT_PCM,
				(char) channels, sampleRate, sampleRate * nBlockAlign,
				nBlockAlign, (char) sampleSizeInBits,
				/* cbSize */(char) 0);
	}
	...
	protected void doInitialize() throws Exception {
		...
		if (iMMDeviceEnumerator == 0) {
			iMMDeviceEnumerator = CoCreateInstance(CLSID_MMDeviceEnumerator, 0,
		...
		long iMMDeviceCollection = IMMDeviceEnumerator_EnumAudioEndpoints(
				iMMDeviceEnumerator, eAll, DEVICE_STATE_ACTIVE);
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				long iMMDevice = IMMDeviceCollection_Item(iMMDeviceCollection,
						i);
				...
				try {
					doInitializeIMMDevice(iMMDevice, captureDevices,
							playbackDevices);
				} catch (Throwable t) {
	...
	private void doInitializeIMMDevice(long iMMDevice,
		...
		long iAudioClient = IMMDevice_Activate(iMMDevice, IID_IAudioClient,
				CLSCTX_ALL, 0);
		...
		if ((formats != null) && !formats.isEmpty()) {
			...
			try {
				...
				logger.warn("Failed to retrieve the PKEY_Device_FriendlyName"
						+ " of IMMDevice " + id, t);
			}
			...
			CaptureDeviceInfo2 cdi2 = new CaptureDeviceInfo2(name,
					new MediaLocator(LOCATOR_PROTOCOL + ":" + id),
					formats.toArray(new Format[formats.size()]), id,
					/* transportType */null,
			...
			case eRender:
				...
				break;
			default:
				logger.error("Failed to retrieve dataFlow from IMMEndpoint "
						+ id);
				break;
			}
		}
		...
		for (char nChannels = 1; nChannels <= 2; nChannels++) {
			for (int i = 0; i < Constants.AUDIO_SAMPLE_RATES.length; i++) {
				...
				for (char wBitsPerSample = 16; wBitsPerSample > 0; wBitsPerSample -= 8) {
					...
					WASAPI.WAVEFORMATEX_fill(waveformatex, WAVE_FORMAT_PCM,
							nChannels, nSamplesPerSec, nSamplesPerSec
									* nBlockAlign, nBlockAlign, wBitsPerSample,
							cbSize);

					long pClosestMatch = IAudioClient_IsFormatSupported(
							iAudioClient, AUDCLNT_SHAREMODE_SHARED,
							waveformatex);
					...
					try {
						 ...
						 */
						if (nChannels == 2) {
							supportedFormat = new AudioFormat(
									AudioFormat.LINEAR, nSamplesPerSec,
									wBitsPerSample,
									/* channels */1, AudioFormat.LITTLE_ENDIAN,
									AudioFormat.SIGNED,
						...
						supportedFormat = new AudioFormat(AudioFormat.LINEAR,
	 ...
	 * 
	 * @param id
	 *            the endpoing ID string which identifies the audio endpoint
	 *            device to be retrieved
	 * @return an <tt>IMMDevice</tt> instance which represents the audio
	 *         endpoint device that is identified by the specified enpoint ID
	 *         string
	 * @throws HResultException
	...
	private String getIMMDeviceFriendlyName(long iMMDevice)
		...
		try {
			deviceFriendlyName = IPropertyStore_GetString(iPropertyStore,
	 ...
	 */
	public long initializeIAudioClient(MediaLocator locator, DataFlow dataFlow,
			long eventHandle, long hnsBufferDuration, AudioFormat[] formats)
			throws HResultException {
			...
			try {
				...
				try {
					...
					for (int i = 0; i < formats.length; i++) {
						...
						long pClosestMatch = IAudioClient_IsFormatSupported(
								iAudioClient, shareMode, waveformatex);
					...
					int hresult = IAudioClient_Initialize(iAudioClient,
							shareMode, streamFlags, hnsBufferDuration,
							/* hnsPeriodicity */0, waveformatex,
							audioSessionGuid);
					...
					if (((streamFlags & AUDCLNT_STREAMFLAGS_EVENTCALLBACK) == AUDCLNT_STREAMFLAGS_EVENTCALLBACK)
							&& (eventHandle != 0)) {
	...
	protected void preInitialize() {
		...
		if (pNotify == null) {
			pNotify = new IMMNotificationClient() {
				@Override
				public void OnDefaultDeviceChanged(int flow, int role,
				...
				@Override
				public void OnDeviceStateChanged(String pwstrDeviceId,
						int dwNewState) {
					reinitialize(pwstrDeviceId);
				}
