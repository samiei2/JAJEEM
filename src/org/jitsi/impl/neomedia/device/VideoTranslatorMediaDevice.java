 ...
 * @author Boris Grozev
 */
public class VideoTranslatorMediaDevice extends AbstractMediaDevice implements
		MediaDeviceWrapper, VideoListener {
	/**
	...
	private synchronized void close(
		...
		if (deviceSession != null) {
			deviceSession.removeRTCPFeedbackCreateListner(streamDeviceSession);
		}
	...
	protected synchronized DataSource createOutputDataSource() {
		if (deviceSession == null) {
			...
			for (MediaStreamMediaDeviceSession streamDeviceSession : streamDeviceSessions) {
				MediaFormatImpl<? extends Format> streamFormat = streamDeviceSession
						.getFormat();

				if ((streamFormat != null) && (format == null)) {
					format = streamFormat;
				}
				startedDirection = startedDirection.or(streamDeviceSession
						.getStartedDirection());
			}
			...
			if (newDeviceSession instanceof VideoMediaDeviceSession) {
				...
				for (MediaStreamMediaDeviceSession streamDeviceSession : streamDeviceSessions) {
					deviceSession
							.addRTCPFeedbackCreateListner(streamDeviceSession);
				}
			}
			if (format != null) {
		...
		return (deviceSession == null) ? null : deviceSession
				.getOutputDataSource();
	}
	...
	private synchronized void updateDeviceSessionStartedDirection() {
		...
		for (MediaStreamMediaDeviceSession streamDeviceSession : streamDeviceSessions) {
			startDirection = startDirection.or(streamDeviceSession
					.getStartedDirection());
		}
