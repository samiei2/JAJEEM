...
public class PortAudioSystem extends AudioSystem {
	...
	public interface PaUpdateAvailableDeviceListListener extends EventListener {
		 ...
		 *             <tt>Throwable</tt> apart from <tt>ThreadDeath</tt> will
		 *             be ignored after it is logged for debugging purposes.
		 */
	...
	private static final Logger logger = Logger
			.getLogger(PortAudioSystem.class);

	/**
	 * The number of times that {@link #willPaOpenStream()} has been invoked
	 * without an intervening {@link #didPaOpenStream()} i.e. the number of
	 * PortAudio clients which are currently executing <tt>Pa_OpenStream</tt>
	 * and which are thus inhibiting <tt>Pa_UpdateAvailableDeviceList</tt>.
	 */
	...
	/**
	 * The number of times that {@link #willPaUpdateAvailableDeviceList()} has
	 * been invoked without an intervening
	 * {@link #didPaUpdateAvailableDeviceList()} i.e. the number of PortAudio
	 * clients which are currently executing
	 ...
	 * 
	 * @param listener
	 *            the <tt>PaUpdateAvailableDeviceListListener</tt> to be
	 *            notified before and after PortAudio's native function
	 *            <tt>Pa_UpdateAvailableDeviceList()</tt> is invoked
	 */
	public static void addPaUpdateAvailableDeviceListListener(
		...
		synchronized (paUpdateAvailableDeviceListListeners) {
			Iterator<WeakReference<PaUpdateAvailableDeviceListListener>> i = paUpdateAvailableDeviceListListeners
					.iterator();
			boolean add = true;
			...
			if (add) {
				paUpdateAvailableDeviceListListeners
						.add(new WeakReference<PaUpdateAvailableDeviceListListener>(
								listener));
			}
		}
	}
	 ...
	 * 
	 * @param will
	 *            <tt>true</tt> if PortAudio's native function
	 *            <tt>Pa_UpdateAvailableDeviceList()</tt> will be invoked or
	...
	private static void firePaUpdateAvailableDeviceListEvent(boolean will) {
		try {
			...
			synchronized (paUpdateAvailableDeviceListListeners) {
				ls = new ArrayList<WeakReference<PaUpdateAvailableDeviceListListener>>(
						paUpdateAvailableDeviceListListeners);
			}
				...
				if (l != null) {
					try {
						...
						if (t instanceof ThreadDeath) {
							...
							logger.error("PaUpdateAvailableDeviceListListener."
									+ (will ? "will" : "did")
									+ "PaUpdateAvailableDeviceList failed.", t);
						}
					}
				}
			}
	 ...
	 */
	private static double getSupportedSampleRate(boolean input,
			int deviceIndex, int channelCount, long sampleFormat) {
		long deviceInfo = Pa.GetDeviceInfo(deviceIndex);
		double supportedSampleRate;

		if (deviceInfo != 0) {
			double defaultSampleRate = Pa
					.DeviceInfo_getDefaultSampleRate(deviceInfo);

			if (defaultSampleRate >= MediaUtils.MAX_AUDIO_SAMPLE_RATE) {
				supportedSampleRate = defaultSampleRate;
			} else {
				long streamParameters = Pa.StreamParameters_new(deviceIndex,
						channelCount, sampleFormat, Pa.LATENCY_UNSPECIFIED);
				...
				if (streamParameters == 0) {
					...
					try {
						...
						boolean formatIsSupported = Pa.IsFormatSupported(
	...
	private void bubbleUpUsbDevices(List<CaptureDeviceInfo2> devices) {
		if (!devices.isEmpty()) {
			List<CaptureDeviceInfo2> nonUsbDevices = new ArrayList<CaptureDeviceInfo2>(
					devices.size());

			for (Iterator<CaptureDeviceInfo2> i = devices.iterator(); i
					.hasNext();) {
	...
	protected void doInitialize() throws Exception {
		...
		for (int deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++) {
			...
			int maxInputChannels = Pa
					.DeviceInfo_getMaxInputChannels(deviceInfo);
			int maxOutputChannels = Pa
			...
			if (cdi == null) {
				cdi = new CaptureDeviceInfo2(
						name,
						new MediaLocator(LOCATOR_PROTOCOL + ":#"
								+ locatorRemainder),
						new Format[] { new AudioFormat(
								AudioFormat.LINEAR,
								(maxInputChannels > 0) ? getSupportedSampleRate(
										true, deviceIndex, channels,
										sampleFormat) : Pa.DEFAULT_SAMPLE_RATE,
								sampleSizeInBits, channels,
								AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
								Format.NOT_SPECIFIED /* frameSizeInBits */,
								Format.NOT_SPECIFIED /* frameRate */,
								Format.byteArray) }, deviceUID, transportType,
						modelIdentifier);
			}
			...
			devicesChangedCallback = new Runnable() {
				...
				public void run() {
					try {
						...
						logger.warn("Failed to reinitialize PortAudio devices",
								t);
					}
				}
			};
	 ...
	 */
	private void matchDevicesByName(List<CaptureDeviceInfo2> captureDevices,
		...
		while (captureIter.hasNext()) {
			...
			if (captureName != null) {
				...
				if (captureName.length() != 0) {
					Iterator<CaptureDeviceInfo2> playbackIter = playbackDevices
							.iterator();
					...
					while (playbackIter.hasNext()) {
						...
						if (playbackName != null) {
							playbackName = pattern.matcher(playbackName)
									.replaceAll("");
