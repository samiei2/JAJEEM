...
public class Video4Linux2System extends DeviceSystem {
	...
	private static final Logger logger = Logger
			.getLogger(Video4Linux2System.class);
		...
		if (-1 != fd) {
			try {
				...
				if (0 != v4l2_capability) {
					try {
						if ((Video4Linux2.ioctl(fd,
								Video4Linux2.VIDIOC_QUERYCAP, v4l2_capability) != -1)
								&& ((Video4Linux2
										.v4l2_capability_getCapabilities(v4l2_capability) & Video4Linux2.V4L2_CAP_VIDEO_CAPTURE) == Video4Linux2.V4L2_CAP_VIDEO_CAPTURE)) {
							captureDeviceInfoIsAdded = register(deviceName, fd,
									v4l2_capability);
						}
					} finally {
	...
	protected void doInitialize() throws Exception {
		...
		for (int deviceMinorNumber = 0; deviceMinorNumber <= 63; deviceMinorNumber++) {
			captureDeviceInfoIsAdded = discoverAndRegister(baseDeviceName
					+ deviceMinorNumber)
