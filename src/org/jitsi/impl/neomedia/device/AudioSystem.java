...
public abstract class AudioSystem extends DeviceSystem {
	...
	public static AudioSystem[] getAudioSystems() {
		DeviceSystem[] deviceSystems = DeviceSystem
				.getDeviceSystems(MediaType.AUDIO);
		...
		return (audioSystems == null) ? null : audioSystems
				.toArray(new AudioSystem[audioSystems.size()]);
	}
					...
					if ((constructor != null)) {
						...
						try {
							renderer = (Renderer) constructor
									.newInstance(playback);
						} catch (Throwable t) {
							if (t instanceof ThreadDeath) {
								throw (ThreadDeath) t;
							} else {
								renderer = null;
								logger.error("Failed to initialize a new "
	...
	public InputStream getAudioInputStream(String uri) throws IOException {
		...
		URL url = (resources == null) ? null : resources
	...
	public Format getFormat(InputStream audioInputStream) {
		if ((audioInputStream instanceof AudioInputStream)) {
			AudioFormat audioInputStreamFormat = ((AudioInputStream) audioInputStream)
					.getFormat();
	...
	public CaptureDeviceInfo2 getSelectedDevice(DataFlow dataFlow) {
		return devices[dataFlow.ordinal()].getSelectedDevice(
				getLocatorProtocol(), getDevices(dataFlow));
	}
	...
	protected void postInitializeSpecificDevices(DataFlow dataFlow) {
		...
		CaptureDeviceInfo2 selectedActiveDevice = devices.getSelectedDevice(
				locatorProtocol, activeDevices);
	 ...
	 */
	public void setDevice(DataFlow dataFlow, CaptureDeviceInfo2 device,
			boolean save) {
		devices[dataFlow.ordinal()].setDevice(getLocatorProtocol(), device,
				save);
	}
