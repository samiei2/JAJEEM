...
public class MediaDeviceSession extends PropertyChangeNotifier {
	...
	private static final Logger logger = Logger
			.getLogger(MediaDeviceSession.class);
	...
	private static VideoFormat assertSize(VideoFormat sourceFormat) {
		...
		VideoFormat result = new VideoFormat(null,
				new Dimension(width, height), Format.NOT_SPECIFIED, null,
	...
	protected DataSource createCaptureDevice() {
		...
		if (!(captureDevice instanceof MuteDataSource)) {
			...
			if (captureDevice instanceof PushBufferDataSource) {
				captureDevice = new RewritablePushBufferDataSource(
						(PushBufferDataSource) captureDevice);
			} else if (captureDevice instanceof PullBufferDataSource) {
				captureDevice = new RewritablePullBufferDataSource(
		...
		if (exception != null) {
			...
			if (playerControllerListener == null) {
				playerControllerListener = new ControllerListener() {
					 ...
					 *            <tt>Controller</tt> which is the source of the
					 *            event and the very type of the event
	...
	protected Processor createProcessor() {
		...
		if (captureDevice != null) {
			...
			if (exception != null) {
				logger.error("Failed to create Processor for " + captureDevice,
						exception);
			} else {
				if (processorControllerListener == null) {
				...
				if (waitForState(processor, Processor.Configured)) {
					...
					if (processorControllerListener != null) {
						processor
								.removeControllerListener(processorControllerListener);
					}
					processor = null;
				}
			}
	...
	protected ContentDescriptor createProcessorContentDescriptor(
			Processor processor) {
		return (contentDescriptor == null) ? new ContentDescriptor(
	...
	private void disconnectCaptureDevice() {
		if (captureDevice != null) {
			...
			try {
				 ...
				 */
				logger.error("Failed to properly stop captureDevice "
						+ captureDevice, ioe);
			}
	 ...
	 */
	private static Format findFirstMatchingFormat(Format[] formats,
			Format format) {
		double formatSampleRate = (format instanceof AudioFormat) ? ((AudioFormat) format)
				.getSampleRate() : Format.NOT_SPECIFIED;
		ParameterizedVideoFormat parameterizedVideoFormat = (format instanceof ParameterizedVideoFormat) ? (ParameterizedVideoFormat) format
				: null;
			...
			if (match.isSameEncoding(format)) {
				...
				if (match instanceof AudioFormat) {
					if (formatSampleRate != Format.NOT_SPECIFIED) {
						double matchSampleRate = ((AudioFormat) match)
								.getSampleRate();
	...
	protected DataSource getConnectedCaptureDevice() {
		...
		if ((captureDevice != null) && !captureDeviceIsConnected) {
			...
			try {
				...
				logger.warn("Failed to setup an optimized media codec chain"
	...
	public Format getProcessorFormat() {
		...
		if ((processor != null) && (this.processor == processor)
			...
			for (TrackControl trackControl : processor.getTrackControls()) {
				...
				MediaType type = (jmfFormat instanceof VideoFormat) ? MediaType.VIDEO
						: MediaType.AUDIO;
	...
	public MediaFormatImpl<? extends Format> getFormat() {
		...
		if (logger.isDebugEnabled() && (processor != null)) {
			...
			Format format = (this.format == null) ? null : this.format
					.getFormat();
			boolean processorFormatMatchesFormat = (processorFormat == null) ? (format == null)
					: processorFormat.matches(format);

			if (!processorFormatMatchesFormat) {
				logger.debug("processorFormat != format; processorFormat= `"
	...
	public DataSource getOutputDataSource() {
		...
		if ((processor == null)
				|| ((processor.getState() < Controller.Realized) && !waitForState(
						processor, Controller.Realized))) {
			outputDataSource = null;
		} else {
			outputDataSource = processor.getDataOutput();
			if (logger.isTraceEnabled() && (outputDataSource != null)) {
				logger.trace("Processor with hashCode " + processor.hashCode()
						+ " provided "
	...
	public List<MediaFormat> getSupportedFormats() {
		...
		if ((processor != null) && (this.processor == processor)
			...
			for (TrackControl trackControl : processor.getTrackControls()) {
				...
				for (Format supportedFormat : trackControl
						.getSupportedFormats()) {
					switch (mediaType) {
					case AUDIO:
	...
	protected void processorControllerUpdate(ControllerEvent ev) {
		if (ev instanceof ConfigureCompleteEvent) {
			...
			if (processor != null) {
				try {
					processor
							.setContentDescriptor(createProcessorContentDescriptor(processor));
				} catch (NotConfiguredError nce) {
					logger.error(
			...
			for (FormatParametersAwareCodec fpac : getAllTrackControls(
					FormatParametersAwareCodec.class, processor)) {
	...
	protected void removeSSRC(long ssrc) {
		...
		if (index < ssrcList.length - 1) {
			System.arraycopy(ssrcList, index + 1, newSsrcList, index,
	...
	protected void setCaptureDeviceFormat(DataSource captureDevice,
		...
		if (format instanceof AudioFormat) {
			...
			double sampleRate = OSUtils.IS_ANDROID ? audioFormat
					.getSampleRate() : Format.NOT_SPECIFIED;
			...
			if ((channels != Format.NOT_SPECIFIED)
				...
				if (formatControl != null) {
					Format[] supportedFormats = formatControl
							.getSupportedFormats();
	...
	public void setFormat(MediaFormat format) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Set format " + this.format + " on "
		...
		if (processor != null) {
			...
			if (processorState == Processor.Configured) {
				setProcessorFormat(processor, this.format);
					...
					|| ((processorState > Processor.Configured) && !this.format
							.getFormat().equals(getProcessorFormat()))
					|| outputSizeChanged) {
		...
		for (int trackIndex = 0; trackIndex < trackControls.length; trackIndex++) {
			...
			case AUDIO:
				if (supportedFormats[0] instanceof AudioFormat) {
					supportedFormat = findFirstMatchingFormat(supportedFormats,
							format);
			...
			if (supportedFormat == null) {
				...
				Format setFormat = setProcessorFormat(trackControl,
						mediaFormat, supportedFormat);

				if (setFormat == null) {
					logger.error("Failed to set format of track " + trackIndex
							+ " to " + supportedFormat
							+ ". Processor is in state " + processor.getState());
				} else if (setFormat != supportedFormat) {
					logger.warn("Failed to change format of track "
							+ trackIndex + " from " + setFormat + " to "
							+ supportedFormat + ". Processor is in state "
							+ processor.getState());
				} else if (logger.isTraceEnabled()) {
					logger.trace("Set format of track " + trackIndex + " to "
							+ setFormat);
				}
			}
		}
		...
		synchronized (playbacks) {
			...
			if (playback == null) {
				if (playbackDataSource instanceof ReceiveStreamPushBufferDataSource) {
					ReceiveStream receiveStream = ((ReceiveStreamPushBufferDataSource) playbackDataSource)
							.getReceiveStream();
			...
			if (getPlayback(receiveStream) == null) {
				...
				if (receiveStreamDataSource != null) {
					if (receiveStreamDataSource instanceof PushBufferDataSource) {
						...
						logger.warn("Adding ReceiveStream with DataSource"
	...
	public void removeReceiveStream(ReceiveStream receiveStream) {
		synchronized (playbacks) {
			...
			if (playback != null) {
				...
				if (playback.dataSource != null) {
					logger.warn("Removing ReceiveStream"
	...
	protected void startProcessorInAccordWithDirection(Processor processor) {
		if (startedDirection.allowsSending()
				&& (processor.getState() != Controller.Started)) {
			processor.start();
			if (logger.isTraceEnabled()) {
	...
	public void stop(MediaDirection direction) {
		...
		case SENDRECV:
			if (direction.allowsReceiving()) {
				startedDirection = direction.allowsSending() ? MediaDirection.INACTIVE
	...
	public void copyPlayback(MediaDeviceSession deviceSession) {
		if (deviceSession.disposePlayerOnClose) {
			logger.error("Cannot copy playback"
	 ...
	 * @return A non-null <tt>Set</tt> of all <tt>controlType</tt>s found.
	 */
	public <T> Set<T> getDecoderControls(ReceiveStream receiveStream,
			Class<T> controlType) {
		...
		if (transcodingDataSource == null) {
			...
			return getAllTrackControls(controlType,
	 ...
	 * @return A non-null <tt>Set</tt> of all <tt>controlType</tt>s found.
	 */
	private <T> Set<T> getAllTrackControls(Class<T> controlType,
			Processor processor) {
		Set<T> controls = null;

		if ((processor != null)
				&& (processor.getState() >= Controller.Realized)) {
			TrackControl[] trackControls = processor.getTrackControls();
