...
public abstract class Devices {
	 ...
	 * 
	 * @param newsDeviceIdentifier
	 *            The identifier of the device to add int first active position
	 *            of the preferences.
	 * @param isSelected
	 ...
	 */
	public CaptureDeviceInfo2 getSelectedDevice(String locator,
		...
		if (activeDevices != null) {
			...
			for (int i = activeDevices.size() - 1; i >= 0; i--) {
				...
				if (!devicePreferences.contains(activeDevice
						.getModelIdentifier())) {
					// By default, select automatically the USB devices.
					boolean isSelected = activeDevice
							.isSameTransportType("USB");
					ConfigurationService cfg = LibJitsi
					...
					saveDevice(locator, property, activeDevice, isSelected);
				}
			}
			...
			synchronized (devicePreferences) {
				for (String devicePreference : devicePreferences) {
					for (CaptureDeviceInfo2 activeDevice : activeDevices) {
						...
						// device.
						if (devicePreference.equals(activeDevice
								.getModelIdentifier())) {
							return activeDevice;
						}
						...
						// "active" device.
								...
								.equals(NoneAudioSystem.LOCATOR_PROTOCOL)) {
							return null;
						}
					}
				}
	 ...
	 *            specifies the user's preference.
	 */
		...
		if (cfg != null) {
			...
			synchronized (devicePreferences) {
				...
				// This whole "else" block may be removed in the future.
					...
					String old_property = DeviceConfiguration.PROP_AUDIO_SYSTEM
							+ "." + locator + "." + property;
	 ...
	 *            The string representation of the locator.
	 * @param property
	 *            the name of the <tt>ConfigurationService</tt> property into
	 *            which the user's preference with respect to the specified
	 ...
	 *            True if the device is the selected one.
	 */
	private void saveDevice(String locator, String property,
			CaptureDeviceInfo2 device, boolean isSelected) {
		String selectedDeviceIdentifier = (device == null) ? NoneAudioSystem.LOCATOR_PROTOCOL
	 ...
	 */
	public void setDevice(String locator, CaptureDeviceInfo2 device,
			boolean save) {
		...
		if ((device == null) || !device.equals(this.device)) {
			...
			// Saves the new selected device in top of the user preferences.
			if (save) {
				saveDevice(locator, getPropDevice(), device, true);
			}
			this.device = device;
	 ...
	 * 
	 * @param devices
	 *            the list of <tt>CaptureDeviceInfo2</tt>s which are
	 *            active/plugged-in
	 */
	public void setDevices(List<CaptureDeviceInfo2> devices) {
		this.devices = (devices == null) ? null
	...
	private void writeDevicePreferences(String locator, String property) {
		...
		if (cfg != null) {
			property = DeviceConfiguration.PROP_AUDIO_SYSTEM + "." + locator
					+ "." + property + "_list";
