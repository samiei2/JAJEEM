...
public class QuickTimeSystem extends DeviceSystem {
	...
	private static final Logger logger = Logger
			.getLogger(QuickTimeSystem.class);
	...
	protected void doInitialize() throws Exception {
		...
		for (QTCaptureDevice inputDevice : inputDevices) {
			CaptureDeviceInfo device = new CaptureDeviceInfo(
					inputDevice.localizedDisplayName(), new MediaLocator(
							LOCATOR_PROTOCOL + ':' + inputDevice.uniqueID()),
					new Format[] { new AVFrameFormat(FFmpeg.PIX_FMT_ARGB),
							new RGBFormat() });
