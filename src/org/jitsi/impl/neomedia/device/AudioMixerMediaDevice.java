 ...
 * @author Emil Ivov
 */
public class AudioMixerMediaDevice extends AbstractMediaDevice implements
		MediaDeviceWrapper {
	/**
	...
	private static final Logger logger = Logger
			.getLogger(AudioMixerMediaDevice.class);
	...
	public void connect(DataSource captureDevice) throws IOException {
		...
		if (captureDevice instanceof PushBufferDataSourceDelegate<?>) {
			captureDevice = ((PushBufferDataSourceDelegate<?>) captureDevice)
					.getDataSource();
		}
	...
	private void fireLocalUserAudioLevelChanged(int level) {
		 ...
		 */
		int localUserAudioLevelListenerCount = localUserAudioLevelListeners
				.size();
	...
	private synchronized AudioMixer getAudioMixer() {
		if (audioMixer == null) {
			audioMixer = new AudioMixer(device.createCaptureDevice()) {
				@Override
				protected void connect(DataSource dataSource,
						DataSource inputDataSource) throws IOException {
					/*
				...
				protected void read(PushBufferStream stream, Buffer buffer,
					...
					if (dataSource == captureDevice) {
						 ...
						 * mix.
						 */
						ReceiveStream receiveStream = ((ReceiveStreamPushBufferDataSource) dataSource)
								.getReceiveStream();
						AudioLevelEventDispatcher streamEventDispatcher;

						synchronized (streamAudioLevelListeners) {
							streamEventDispatcher = streamAudioLevelListeners
									.get(receiveStream);
						}
	...
	public List<RTPExtension> getSupportedExtensions() {
		if (rtpExtensions == null) {
			...
			if (csrcAudioLevelURN != null) {
				rtpExtensions.add(new RTPExtension(csrcAudioLevelURN,
	...
	private class AudioMixerMediaDeviceSession extends MediaDeviceSession {
			...
			synchronized (localUserAudioLevelListenersSyncRoot) {
				...
				if (localUserAudioLevelListeners.isEmpty()) {
					localUserAudioLevelDispatcher
							.setAudioLevelListener(localUserAudioLevelDelegate);
				}

				// check if this listener has already been added.
				SimpleAudioLevelListenerWrapper wrapper = new SimpleAudioLevelListenerWrapper(
						l);
				...
				if (index != -1) {
					 ...
					 */
					localUserAudioLevelListeners = new ArrayList<SimpleAudioLevelListenerWrapper>(
		 ...
		 *            <tt>stream</tt>.
		 */
		void setStreamAudioLevelListener(ReceiveStream stream,
				SimpleAudioLevelListener listener) {
			synchronized (streamAudioLevelListeners) {
				AudioLevelEventDispatcher dispatcher = streamAudioLevelListeners
						.get(stream);
					...
					if (dispatcher == null) {
						dispatcher = new AudioLevelEventDispatcher(
								"Stream Audio Level Dispatcher"
										+ " (Mixer Edition)");
						dispatcher.setAudioLevelCache(audioLevelCache,
								stream.getSSRC());
		 ...
		 */
		@Override
		protected Renderer createRenderer(Player player,
				TrackControl trackControl) {
			Renderer renderer = super.createRenderer(player, trackControl);

			if (renderer != null) {
				AudioMediaDeviceSession.setVolumeControl(renderer,
						outputVolumeControl);
			}
		...
		void removeLocalUserAudioLevelListener(SimpleAudioLevelListener l) {
			synchronized (localUserAudioLevelListenersSyncRoot) {
				...
				if (index != -1) {
					SimpleAudioLevelListenerWrapper wrapper = localUserAudioLevelListeners
							.get(index);
	 ...
	 */
	private static class MediaStreamMediaDeviceSession extends
			AudioMediaDeviceSession implements PropertyChangeListener {
		/**
		...
		protected void playbackDataSourceAdded(DataSource playbackDataSource) {
			...
			audioMixerMediaDeviceSession
					.addPlaybackDataSource(playbackDataSource);
		}
		...
		protected void playbackDataSourceRemoved(DataSource playbackDataSource) {
			...
			audioMixerMediaDeviceSession
					.removePlaybackDataSource(playbackDataSource);
		}
		...
		public void setOutputVolumeControl(VolumeControl outputVolumeControl) {
			audioMixerMediaDeviceSession
					.setOutputVolumeControl(outputVolumeControl);
		}
		...
		public int getLastMeasuredAudioLevel(long csrc) {
			return ((AudioMixerMediaDevice) getDevice()).audioLevelCache
					.getLevel(csrc);
		}
