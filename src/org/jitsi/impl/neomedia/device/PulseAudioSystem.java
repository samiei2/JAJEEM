...
public class PulseAudioSystem extends AudioSystem {
	...
	public static PulseAudioSystem getPulseAudioSystem() {
		AudioSystem audioSystem = AudioSystem
				.getAudioSystem(PulseAudioSystem.LOCATOR_PROTOCOL);

		return (audioSystem instanceof PulseAudioSystem) ? (PulseAudioSystem) audioSystem
				: null;
	}
	...
	private void createContext() {
		...
		try {
			...
			try {
				...
				long context = PA.context_new_with_proplist(
				...
				try {
					...
					try {
						...
						try {
							int state = waitForContextState(context,
									PA.CONTEXT_READY);
	...
	public Renderer createRenderer(boolean playback) {
		...
		PulseAudioRenderer renderer = new PulseAudioRenderer(
	...
	public long createStream(int sampleRate, int channels, String mediaName,
			String mediaRole) throws IllegalStateException, RuntimeException {
		...
		long sampleSpec = PA.sample_spec_new(PA.SAMPLE_S16LE, sampleRate,
				channels);
		...
		try {
			...
			try {
				...
				long stream = PA.stream_new_with_proplist(context, null,
						sampleSpec, 0, proplist);
	...
	protected synchronized void doInitialize() throws Exception {
		...
		PA.source_info_cb_t sourceInfoListCallback = new PA.source_info_cb_t() {
			...
			public void callback(long c, long i, int eol) {
				try {
					if ((eol == 0) && (i != 0)) {
						sourceInfoListCallback(c, i, captureDevices,
		...
		PA.sink_info_cb_t sinkInfoListCallback = new PA.sink_info_cb_t() {
			...
			public void callback(long c, long i, int eol) {
				try {
					if ((eol == 0) && (i != 0)) {
						sinkInfoListCallback(c, i, playbackDevices,
		...
		lockMainloop();
		try {
			long o = PA.context_get_source_info_list(context,
		...
		if (!captureDeviceFormats.isEmpty()) {
			captureDevices.add(
					...
					new CaptureDeviceInfo2(NULL_DEV_CAPTURE_DEVICE_INFO_NAME,
							...
							captureDeviceFormats
									.toArray(new Format[captureDeviceFormats
											.size()]), null, null, null));
		}
		if (!playbackDevices.isEmpty()) {
			playbackDevices.add(0, new CaptureDeviceInfo2(
					NULL_DEV_CAPTURE_DEVICE_INFO_NAME, new MediaLocator(
							LOCATOR_PROTOCOL + ":"), null, null, null, null));
		}
	...
	private void populateContextProplist(long proplist) {
		...
		String applicationName = System
				.getProperty(Version.PNAME_APPLICATION_NAME);

		if (applicationName != null) {
			PA.proplist_sets(proplist, PA.PROP_APPLICATION_NAME,
					applicationName);
		}

		String applicationVersion = System
				.getProperty(Version.PNAME_APPLICATION_VERSION);

		if (applicationVersion != null) {
			PA.proplist_sets(proplist, PA.PROP_APPLICATION_VERSION,
					applicationVersion);
		}
	}
	...
	private void sinkInfoListCallback(long context, long sinkInfo,
		...
		deviceList.add(new CaptureDeviceInfo2(description, new MediaLocator(
				LOCATOR_PROTOCOL + ":" + name), null, null, null, null));
	}

	private void sourceInfoListCallback(long context, long sourceInfo,
			List<CaptureDeviceInfo2> deviceList, List<Format> formatList) {
		...
		int sampleSpecFormat = PA
				.source_info_get_sample_spec_format(sourceInfo);
		...
		AudioFormat audioFormat = new AudioFormat(AudioFormat.LINEAR, rate, 16,
		...
		if (!formatList.isEmpty()) {
			...
			deviceList.add(new CaptureDeviceInfo2(description,
					new MediaLocator(LOCATOR_PROTOCOL + ":" + name),
					sourceInfoFormatList
							.toArray(new Format[sourceInfoFormatList.size()]),
					null, null, null));
		}
	}
