...
public abstract class DeviceSystem extends PropertyChangeNotifier {
	...
	protected static List<CaptureDeviceInfo> filterDeviceListByLocatorProtocol(
		...
		if ((deviceList != null) && (deviceList.size() > 0)) {
			...
			while (deviceListIter.hasNext()) {
				...
				if ((locator == null)
						|| !locatorProtocol.equalsIgnoreCase(locator
								.getProtocol())) {
	...
	public static void initializeDeviceSystems() {
		...
		if (((cfg == null) || !cfg.getBoolean(
				MediaServiceImpl.DISABLE_AUDIO_SUPPORT_PNAME, false))
				&& !Boolean
						.getBoolean(MediaServiceImpl.DISABLE_AUDIO_SUPPORT_PNAME)) {
		...
		if ((cfg == null)
				|| !cfg.getBoolean(
						MediaServiceImpl.DISABLE_VIDEO_SUPPORT_PNAME, false)) {
			if (logger.isInfoEnabled()) {
	...
	private static void initializeDeviceSystems(String[] classNames) {
		synchronized (deviceSystems) {
			...
			for (String className : classNames) {
				...
				if (reinitialize
						&& ((deviceSystem.getFeatures() & FEATURE_REINITIALIZE) != 0)) {
					try {
						...
						if (t instanceof ThreadDeath) {
							...
							logger.warn("Failed to reinitialize " + className,
									t);
						}
					}
				}
	 ...
	 *             <tt>initialize()</tt> on the specified <tt>deviceSystem</tt>
	 */
	static void invokeDeviceSystemInitialize(final DeviceSystem deviceSystem,
	...
	protected void postInitialize() {
		try {
			...
			if (format != null) {
				...
				List<CaptureDeviceInfo> cdis = CaptureDeviceManager
						.getDeviceList(format);
				List<CaptureDeviceInfo> postInitializeDevices = new ArrayList<CaptureDeviceInfo>(
						cdis);

				if (preInitializeDevices != null) {
					for (Iterator<CaptureDeviceInfo> preIter = preInitializeDevices
							.iterator(); preIter.hasNext();) {
				 ...
				 */
				int preInitializeDeviceCount = (preInitializeDevices == null) ? 0
						: preInitializeDevices.size();
				int postInitializeDeviceCount = (postInitializeDevices == null) ? 0
						: postInitializeDevices.size();

				if ((preInitializeDeviceCount != 0)
						|| (postInitializeDeviceCount != 0)) {
					firePropertyChange(PROP_DEVICES, preInitializeDevices,
	...
	protected void preInitialize() {
		...
		if (format != null) {
			...
			if ((cdis != null) && (cdis.size() > 0)) {
				...
				for (CaptureDeviceInfo cdi : filterDeviceListByLocatorProtocol(
