...
public class VideoLayout extends FitLayout {
	 ...
	 */
	public static boolean areAspectRatiosEqual(Dimension size, int width,
			int height) {
	...
	public void layoutContainer(Container parent) {
		...
		if (!conference && (remoteCount == 1)) {
			...
			super.layoutContainer(parent,
					(local == null) ? Component.CENTER_ALIGNMENT
							: remoteAlignmentX);
			...
			for (int i = 0; i < remoteCount; i++) {
				...
				if (column == 0) {
					...
					if (row == rowsMinus1) {
						...
						if (available < columns) {
							bounds.x = (parentSize.width - available
									* bounds.width - (available - 1) * HGAP) / 2;
						}
					}
				} else {
				...
				super.layoutComponent(remotes.get(i), bounds,
		...
		if (local == null) {
			...
			if (!remotes.contains(local)) {
				...
				int localX;
				int localY;
				int height = Math.round(parentSize.height
						* LOCAL_TO_REMOTE_RATIO);
				int width = Math
						.round(parentSize.width * LOCAL_TO_REMOTE_RATIO);
				float alignmentX;
				...
				super.layoutComponent(local, new Rectangle(localX, localY,
						width, height), alignmentX, Component.BOTTOM_ALIGNMENT);
			}
			...
			if (closeButton != null) {
				...
				super.layoutComponent(
						closeButton,
						new Rectangle(local.getX() + local.getWidth()
								- closeButton.getWidth(), local.getY(),
	...
	public Dimension preferredLayoutSize(Container parent) {
		...
		if (!conference && (remoteCount == 1)) {
			...
			prefLayoutSize = new Dimension(prefLayoutWidth + columnsMinus1
					* HGAP, prefLayoutHeight);
		} else {
			prefLayoutSize = null;
		}
		...
		if (local != null) {
			...
			if (!remotes.contains(local) && (prefLayoutSize == null)) {
				...
				if (prefSize != null) {
					int prefHeight = Math.round(prefSize.height
							* LOCAL_TO_REMOTE_RATIO);
					int prefWidth = Math.round(prefSize.width
							* LOCAL_TO_REMOTE_RATIO);
