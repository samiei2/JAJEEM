...
public abstract class LibJitsi {
	 ...
	 * 
	 * @param serviceClass
	 *            the class of the service to be retrieved
	 * @return a service of the specified type if such a service is associated
	 *         with the library
	 * @throws IllegalStateException
	 *             if the library is not currently initialized
	 */
	 ...
	 * 
	 * @param context
	 *            an <tt>Object</tt>, if any, which represents a context in
	...
	private static void start(Object context) {
		 ...
		 */
		String implBaseClassName = LibJitsi.class.getName().replace(
				".service.", ".impl.");
		...
		for (int i = 0; i < implClassNameExtensions.length; i++) {
			...
			String implClassName = implBaseClassName
					+ implClassNameExtensions[i] + "Impl";
	 ...
	 *         known to this implementation of the <tt>libjitsi</tt> library;
