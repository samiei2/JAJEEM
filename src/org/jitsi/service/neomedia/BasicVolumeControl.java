...
public class BasicVolumeControl implements VolumeControl, GainControl {
	...
	private static final Logger logger = Logger
			.getLogger(BasicVolumeControl.class);
	 ...
	 */
	public static void applyGain(GainControl gainControl, byte[] buffer,
	 ...
	 */
	private static float getDbFromPowerRatio(float powerLevelRequired,
	...
	protected static float getDefaultVolumeLevel() {
		return MIN_VOLUME_LEVEL + (MAX_VOLUME_LEVEL - MIN_VOLUME_LEVEL)
	 ...
	 *         power level.
	 */
	private static float getPowerRatioFromDb(float gainInDb,
			float referencePowerLevel) {
	...
	public void addVolumeChangeListener(VolumeChangeListener listener) {
		synchronized (volumeChangeListeners) {
			Iterator<WeakReference<VolumeChangeListener>> i = volumeChangeListeners
					.iterator();
			...
			if (!contains) {
				volumeChangeListeners
						.add(new WeakReference<VolumeChangeListener>(listener));
			}
		}
	}
	...
	private void fireGainChange() {
		if (gainChangeListeners != null) {
			GainChangeEvent ev = new GainChangeEvent(this, mute, db,
					volumeLevel);
	...
	private void fireVolumeChange() {
		...
		synchronized (volumeChangeListeners) {
			...
			ls = new ArrayList<VolumeChangeListener>(
	...
	protected void loadVolume() {
		try {
			...
			if (cfg != null) {
				String volumeLevelString = cfg
						.getString(volumeLevelConfigurationPropertyName);
	...
	private float setVolumeLevel(float value) {
		...
		if (cfg != null) {
			cfg.setProperty(this.volumeLevelConfigurationPropertyName,
