...
public class DefaultStreamConnector implements StreamConnector {
	...
	private static final Logger logger = Logger
			.getLogger(DefaultStreamConnector.class);
	...
	private static synchronized MulticastSocket createDatagramSocket(
		...
		for (int i = 0; i < bindRetries; i++) {
			if ((minPort < 0) || (minPort > maxPort)) {
				...
				if (cfg != null) {
					minPort = cfg
							.getInt(MIN_PORT_NUMBER_PROPERTY_NAME, minPort);
				}
			}
	 ...
	 *            RTCP) traffic
	 */
	public DefaultStreamConnector(MulticastSocket dataSocket,
