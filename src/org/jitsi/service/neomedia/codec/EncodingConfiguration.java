...
public abstract class EncodingConfiguration {
	...
	private Set<MediaFormat> updateSupportedEncodings(MediaType type) {
		Set<MediaFormat> enabled = new TreeSet<MediaFormat>(encodingComparator);
	 ...
	 */
	protected abstract void setEncodingPreference(String encoding,
			double clockRate, int pref);
	...
	public void setPriority(MediaFormat encoding, int priority) {
		 ...
		 */
		setEncodingPreference(encodingEncoding, encoding.getClockRate(),
				priority);
	...
	public int getPriority(MediaFormat encoding) {
		 ...
		 */
		Integer priority = encodingPreferences
				.get(getEncodingPreferenceKey(encoding));
	 ...
	 * 
	 * @param type
	 *            the <tt>MediaType</tt> of the supported <tt>MediaFormat</tt>s
	 *            to get
	...
	public MediaFormat[] getEnabledEncodings(MediaType type) {
		...
		return supportedEncodings.toArray(new MediaFormat[supportedEncodings
				.size()]);
	}
	 ...
	 */
	protected abstract int compareEncodingPreferences(MediaFormat enc1,
			MediaFormat enc2);
	 ...
	 * 
	 * @param encoding
	 *            the <tt>MediaFormat</tt> to get the key in
		...
		for (Map.Entry<String, String> entry : properties.entrySet()) {
			...
			try {
				...
				if (encodingClockRateSeparator > -1) {
					...
					clockRate = Double.parseDouble(fmtName
