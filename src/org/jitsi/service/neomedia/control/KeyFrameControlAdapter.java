...
public class KeyFrameControlAdapter implements KeyFrameControl {
	 ...
	 */
	private List<KeyFrameRequestee> keyFrameRequestees = new ArrayList<KeyFrameRequestee>(
			0);
	 ...
	 * <tt>KeyFrameControl</tt>.
	 */
	private List<KeyFrameRequester> keyFrameRequesters = new ArrayList<KeyFrameRequester>(
			0);
	...
	public void addKeyFrameRequestee(int index,
		...
		synchronized (this) {
			if (!keyFrameRequestees.contains(keyFrameRequestee)) {
				List<KeyFrameRequestee> newKeyFrameRequestees = new ArrayList<KeyFrameRequestee>(
				 ...
				 */
				if (-1 == index) {
					if (keyFrameRequestee.getClass().getName()
							.contains(".neomedia.")) {
	...
	public void addKeyFrameRequester(int index,
		...
		synchronized (this) {
			if (!keyFrameRequesters.contains(keyFrameRequester)) {
				List<KeyFrameRequester> newKeyFrameRequesters = new ArrayList<KeyFrameRequester>(
				 ...
				 */
				if (-1 == index) {
					if (keyFrameRequester.getClass().getName()
							.contains(".neomedia.")) {
	...
	public List<KeyFrameRequestee> getKeyFrameRequestees() {
		synchronized (this) {
			if (unmodifiableKeyFrameRequestees == null) {
				unmodifiableKeyFrameRequestees = Collections
						.unmodifiableList(keyFrameRequestees);
			}
	...
	public List<KeyFrameRequester> getKeyFrameRequesters() {
		synchronized (this) {
			if (unmodifiableKeyFrameRequesters == null) {
				unmodifiableKeyFrameRequesters = Collections
						.unmodifiableList(keyFrameRequesters);
			}
