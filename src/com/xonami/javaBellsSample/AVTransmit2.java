...
public class AVTransmit2 {
	 ...
	 */
	private AVTransmit2(String localPortBase, String remoteHost,
	...
	private String start() throws Exception {
		...
		for (MediaType mediaType : mediaTypes) {
			...
			if (localPortBase == -1) {
				...
				connector = new DefaultStreamConnector(new DatagramSocket(
						localRTPPort), new DatagramSocket(localRTCPPort));
			}
			...
			mediaStream.setTarget(new MediaStreamTarget(new InetSocketAddress(
					remoteAddr, remoteRTPPort), new InetSocketAddress(
					remoteAddr, remoteRTCPPort)));
...
//		AVTransmit2 at = new AVTransmit2("1000", "192.168.0.15", "5000");
//
		LibJitsi.start();
		try {
