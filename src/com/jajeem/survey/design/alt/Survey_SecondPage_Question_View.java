		...
		webComboBox.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				if (currentSurvey != null) {
					currentQuestion = currentSurvey.getQuestionList().get(
					...
					if (webComboBox.getSelectedIndex() != -1
							&& surveyResponse.size() != 0) {
						for (int i = 0; i < surveyResponse.get(
								webComboBox.getSelectedIndex()).size(); i++) {
							SurveyResponse ex = surveyResponse.get(
							...
							if (currentSurvey.getQuestionList()
									.get(webComboBox.getSelectedIndex())
									.getType() == 0
									|| currentSurvey
											.getQuestionList()
											.get(webComboBox.getSelectedIndex())
								...
								webTable.setModel(new DefaultTableModel(
										new Object[][] {}, new String[] { "#",
												"Rates" }));
								webTable.getColumnModel().getColumn(0)
										.setPreferredWidth(35);
								int totalNumofStudents = studentList.size();
								for (int k = 1; k <= 5; k++) { // for each
																// choice of the
									int rate = 0;
									for (int j = 0; j < surveyResponse.get(
											webComboBox.getSelectedIndex())
											.size(); j++) { // for each students
															// answer of that
															// question
										StudentOption = "";
										if (currentSurvey
										...
										if (k == 4
												&& StudentOption
														.contains("Fourth Option")) {
											rate++;
										}
										if (k == 5
												&& StudentOption
														.contains("Fifth Option")) {
											rate++;
										}
									}
									((DefaultTableModel) webTable.getModel()).addRow(new Object[] {
											...
											String.format(
													"%.2f",
													(((double) rate / (double) totalNumofStudents)) * 100)
													+ " ("
													+ rate
													+ " of "
													+ totalNumofStudents + ")" });
								...
								webTable.getColumnModel().getColumn(0)
										.setPreferredWidth(35);
								for (int k = 0; k < surveyResponse.get(
										webComboBox.getSelectedIndex()).size(); k++) {
			...
			if (question.getId().equals(currentQuestion.getId())) {// if the
				...
				if (currentSurvey.getQuestionList().get(index).getType() == 0
					...
					for (int i = 1; i <= 5; i++) { // for each choice of the
						int rate = 0;
						for (int j = 0; j < surveyResponse.get(index).size(); j++) { // for
																						// each
																						// students
																						// answer
																						// of
																						...
																						// question
							StudentOption = "";
							if (currentSurvey.getQuestionList().get(index)
									.getType() == 0) {
								if (surveyResponse.get(index).get(j)
										.getQuestion().getStudentAnswer()[0]) {
									StudentOption = "First Option";
								}
								if (surveyResponse.get(index).get(j)
										.getQuestion().getStudentAnswer()[1]) {
									StudentOption = "Second Option";
								}
								if (surveyResponse.get(index).get(j)
										.getQuestion().getStudentAnswer()[2]) {
									StudentOption = "Third Option";
								}
								if (surveyResponse.get(index).get(j)
										.getQuestion().getStudentAnswer()[3]) {
									StudentOption = "Fourth Option";
								}
								if (surveyResponse.get(index).get(j)
										.getQuestion().getStudentAnswer()[4]) {
									StudentOption = "Fifth Option";
								}
								if (StudentOption == "") {
									StudentOption = "None Selected";
								}
							} else if (currentSurvey.getQuestionList()
									.get(index).getType() == 1) {
								if (surveyResponse.get(index).get(j)
										.getQuestion().getStudentAnswer()[0]) {
									StudentOption += "First Option ";
								...
								StudentOption = surveyResponse.get(index)
										.get(j).getQuestion()
										.getStudentTextAnswer();
							}
							if (i == 1
									&& StudentOption.contains("First Option")) {
								rate++;
							}
							if (i == 2
									&& StudentOption.contains("Second Option")) {
								rate++;
							}
							if (i == 3
									&& StudentOption.contains("Third Option")) {
								rate++;
							}
							if (i == 4
									&& StudentOption.contains("Fourth Option")) {
								rate++;
							}
							if (i == 5
									&& StudentOption.contains("Fifth Option")) {
								rate++;
							}
						}
