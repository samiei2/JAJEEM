...
public class SetAuthenticateCommandHanlder implements ICommandHandler {
	...
	public void run(Command cmd) throws NumberFormatException,
		...
		synchronized (authenticateLock) {
			if(Session.getLoggedInStudents().containsValue(((AuthenticateCommand) cmd).getUsername()))
			{
				DuplicateLoginCommand dup = new DuplicateLoginCommand(InetAddress.getLocalHost()
					.getHostAddress(), ((AuthenticateCommand) cmd).getFrom(),
				...
				if(grant)
					Session.getLoggedInStudents().put(((AuthenticateCommand) cmd).getFrom(),((AuthenticateCommand) cmd).getUsername());
			}
		}
