...
public class Query {
	...
	public static String courses() {
		return "SELECT CONCAT(I.FIRSTNAME,' ',I.LASTNAME) INTRUCTORNAME,C.NAME, C.CLASSTYPE,C.LEVEL,C.SESSION, "
				+ "FORMATDATETIME(DATEADD('SECOND', C.STARTDATE/1000, DATE '1970-01-01'), 'EEE, d MMM yyyy', 'en', 'GMT') COURSESTARTDATE, "
				+ "CASE WHEN LENGTH(DAY1)=0 THEN '' ELSE CONCAT(C.DAY1,' (',CASE WHEN LENGTH(STARTTIME1)>2 THEN "
				+ "			CONCAT(SUBSTR(STARTTIME1,1,2),':',SUBSTR(STARTTIME1,3,2)) ELSE "
				+ "			STARTTIME1 END, '-', CASE WHEN LENGTH(ENDTIME1)>2 THEN "
				+ "			CONCAT(SUBSTR(ENDTIME1,1,2),':',SUBSTR(ENDTIME1,3,2)) ELSE "
				+ "			ENDTIME1 END, ')') END DAYTIME1, "
				+ "CASE WHEN LENGTH(DAY2)=0 THEN '' ELSE CONCAT(C.DAY2,' (',CASE WHEN LENGTH(STARTTIME2)>2 THEN "
				+ "			CONCAT(SUBSTR(STARTTIME2,1,2),':',SUBSTR(STARTTIME2,3,2)) ELSE "
				+ "			STARTTIME2 END, '-', CASE WHEN LENGTH(ENDTIME2)>2 THEN "
				+ "			CONCAT(SUBSTR(ENDTIME2,1,2),':',SUBSTR(ENDTIME2,3,2)) ELSE "
				+ "			ENDTIME2 END, ')') END DAYTIME2, "
				+ "CASE WHEN LENGTH(DAY3)=0 THEN '' ELSE CONCAT(C.DAY3,' (',CASE WHEN LENGTH(STARTTIME3)>2 THEN "
				+ "			CONCAT(SUBSTR(STARTTIME3,1,2),':',SUBSTR(STARTTIME3,3,2)) ELSE "
				+ "			STARTTIME3 END, '-', CASE WHEN LENGTH(ENDTIME3)>2 THEN "
				+ "			CONCAT(SUBSTR(ENDTIME3,1,2),':',SUBSTR(ENDTIME3,3,2)) ELSE "
				+ "			ENDTIME3 END, ')') END DAYTIME3, "
				+ "CASE WHEN LENGTH(DAY4)=0 THEN '' ELSE CONCAT(C.DAY4,' (',CASE WHEN LENGTH(STARTTIME4)>2 THEN "
				+ "			CONCAT(SUBSTR(STARTTIME4,1,2),':',SUBSTR(STARTTIME4,3,2)) ELSE "
				+ "			STARTTIME4 END, '-', CASE WHEN LENGTH(ENDTIME4)>2 THEN "
				+ "			CONCAT(SUBSTR(ENDTIME4,1,2),':',SUBSTR(ENDTIME4,3,2)) ELSE "
				+ "			ENDTIME4 END, ')') END DAYTIME4, "
				+ "CASE WHEN LENGTH(DAY5)=0 THEN '' ELSE CONCAT(C.DAY5,' (',CASE WHEN LENGTH(STARTTIME5)>2 THEN "
				+ "			CONCAT(SUBSTR(STARTTIME5,1,2),':',SUBSTR(STARTTIME5,3,2)) ELSE "
				+ "			STARTTIME5 END, '-', CASE WHEN LENGTH(ENDTIME5)>2 THEN "
				+ "			CONCAT(SUBSTR(ENDTIME5,1,2),':',SUBSTR(ENDTIME5,3,2)) ELSE "
				+ "			ENDTIME5 END, ')') END DAYTIME5 "
				+ "FROM COURSE C JOIN INSTRUCTOR I ON (C.INSTRUCTORID=I.ID)";
	}

	public static String students() {
		return "SELECT USERNAME, FIRSTNAME, MIDDLENAME, LASTNAME, LANGUAGE FROM STUDENT "
				+ "ORDER BY LASTNAME, FIRSTNAME";
	}

	public static String instructors() {
		return "SELECT USERNAME, FIRSTNAME, MIDDLENAME, LASTNAME, LANGUAGE FROM INSTRUCTOR "
				+ "ORDER BY LASTNAME, FIRSTNAME";
	}

	public static String studentByCourse(int courseId) {
		return "SELECT CONCAT(S.FIRSTNAME,' ',S.LASTNAME) STUDENTNAME,CONCAT(I.FIRSTNAME,' ',I.LASTNAME) INTRUCTORNAME,C.NAME, C.CLASSTYPE,C.LEVEL,C.SESSION, "
				+ " FORMATDATETIME(DATEADD('SECOND', C.STARTDATE/1000, DATE '1970-01-01'), 'EEE, d MMM yyyy', 'en', 'GMT') COURSESTARTDATE, "
				+ " CASE WHEN LENGTH(C.DAY1)=0 THEN '' ELSE CONCAT(C.DAY1,' (',CASE WHEN LENGTH(C.STARTTIME1)>2 THEN "
				+ "			CONCAT(SUBSTR(C.STARTTIME1,1,2),':',SUBSTR(C.STARTTIME1,3,2)) ELSE "
				+ "			C.STARTTIME1 END, '-', CASE WHEN LENGTH(C.ENDTIME1)>2 THEN "
				+ "			CONCAT(SUBSTR(C.ENDTIME1,1,2),':',SUBSTR(C.ENDTIME1,3,2)) ELSE "
				+ "			C.ENDTIME1 END, ')') END DAYTIME1, "
				+ " CASE WHEN LENGTH(C.DAY2)=0 THEN '' ELSE CONCAT(C.DAY2,' (',CASE WHEN LENGTH(C.STARTTIME2)>2 THEN "
				+ "		CONCAT(SUBSTR(C.STARTTIME2,1,2),':',SUBSTR(C.STARTTIME2,3,2)) ELSE "
				+ "		C.STARTTIME2 END, '-', CASE WHEN LENGTH(C.ENDTIME2)>2 THEN "
				+ "		CONCAT(SUBSTR(C.ENDTIME2,1,2),':',SUBSTR(C.ENDTIME2,3,2)) ELSE "
				+ "		C.ENDTIME2 END, ')') END DAYTIME2, "
				+ " CASE WHEN LENGTH(C.DAY3)=0 THEN '' ELSE CONCAT(C.DAY3,' (',CASE WHEN LENGTH(C.STARTTIME3)>2 THEN "
				+ "	CONCAT(SUBSTR(C.STARTTIME3,1,2),':',SUBSTR(C.STARTTIME3,3,2)) ELSE "
				+ "		C.STARTTIME3 END, '-', CASE WHEN LENGTH(C.ENDTIME3)>2 THEN "
				+ "		CONCAT(SUBSTR(C.ENDTIME3,1,2),':',SUBSTR(C.ENDTIME3,3,2)) ELSE "
				+ "		C.ENDTIME3 END, ')') END DAYTIME3, "
				+ " CASE WHEN LENGTH(C.DAY4)=0 THEN '' ELSE CONCAT(C.DAY4,' (',CASE WHEN LENGTH(C.STARTTIME4)>2 THEN "
				+ "			CONCAT(SUBSTR(C.STARTTIME4,1,2),':',SUBSTR(C.STARTTIME4,3,2)) ELSE "
				+ "			C.STARTTIME4 END, '-', CASE WHEN LENGTH(C.ENDTIME4)>2 THEN "
				+ "			CONCAT(SUBSTR(C.ENDTIME4,1,2),':',SUBSTR(C.ENDTIME4,3,2)) ELSE "
				+ "			C.ENDTIME4 END, ')') END DAYTIME4, "
				+ " CASE WHEN LENGTH(C.DAY5)=0 THEN '' ELSE CONCAT(C.DAY5,' (',CASE WHEN LENGTH(C.STARTTIME5)>2 THEN "
				+ "			CONCAT(SUBSTR(C.STARTTIME5,1,2),':',SUBSTR(C.STARTTIME5,3,2)) ELSE "
				+ "			C.STARTTIME5 END, '-', CASE WHEN LENGTH(C.ENDTIME5)>2 THEN "
				+ "			CONCAT(SUBSTR(C.ENDTIME5,1,2),':',SUBSTR(C.ENDTIME5,3,2)) ELSE "
				+ "			C.ENDTIME5 END, ')') END DAYTIME5 "
				+ " FROM STUDENTCOURSE SC JOIN COURSE C ON (SC.COURSEID=C.ID) JOIN STUDENT S ON (SC.STUDENTID=S.ID) "
				+ " JOIN INSTRUCTOR I ON (C.INSTRUCTORID=I.ID) "
				+ " WHERE SC.COURSEID=" + courseId;
	}

	public static String courseByInstructor(int instructorId) {
		return "SELECT CONCAT(I.FIRSTNAME,' ',I.LASTNAME) INTRUCTORNAME,C.NAME, C.CLASSTYPE,C.LEVEL,C.SESSION, "
				+ "FORMATDATETIME(DATEADD('SECOND', C.STARTDATE/1000, DATE '1970-01-01'), 'EEE, d MMM yyyy', 'en', 'GMT') COURSESTARTDATE, "
				+ "CASE WHEN LENGTH(DAY1)=0 THEN '' ELSE CONCAT(C.DAY1,' (',CASE WHEN LENGTH(STARTTIME1)>2 THEN "
				+ "			CONCAT(SUBSTR(STARTTIME1,1,2),':',SUBSTR(STARTTIME1,3,2)) ELSE "
	...
	public static String courseByStudent(int studentId) {
		return "SELECT CONCAT(S.FIRSTNAME,' ',S.LASTNAME) STUDENTNAME,CONCAT(I.FIRSTNAME,' ',I.LASTNAME) INTRUCTORNAME,C.NAME, C.CLASSTYPE,C.LEVEL,C.SESSION, "
				+ "FORMATDATETIME(DATEADD('SECOND', C.STARTDATE/1000, DATE '1970-01-01'), 'EEE, d MMM yyyy', 'en', 'GMT') COURSESTARTDATE, "
				+ "CASE WHEN LENGTH(C.DAY1)=0 THEN '' ELSE CONCAT(C.DAY1,' (',CASE WHEN LENGTH(C.STARTTIME1)>2 THEN "
				+ "			CONCAT(SUBSTR(C.STARTTIME1,1,2),':',SUBSTR(C.STARTTIME1,3,2)) ELSE "
