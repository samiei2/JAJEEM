...
public class ConversationService implements IConversationService {
	...
	@Override
	public void create(String leader, String[] groupList, int port)
			throws NumberFormatException, Exception {
		/*
		 * ServerService serverService = new ServerService();
		 * 
		 * StartCaptureCommand leaderCmd = new StartCaptureCommand(leader,
		 * port); serverService.send(leaderCmd);
		 * 
		 * StartViewerCommand viewCommand = new StartViewerCommand("", port,
		 * "");
		 * 
		 * for(int i =0; i < groupList.length; i++) {
		 * viewCommand.setHost(groupList[i]); viewCommand.setLeader(leader);
