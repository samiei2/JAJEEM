...
public class ContentPacketExtension extends AbstractPacketExtension {
	 ...
	 * 
	 * @param creator
	 *            indicates which party originally generated the content type
	 * @param disposition
	 *            indicates how the content definition is to be interpreted by
	 *            the recipient
	 * @param name
	 *            the content type according to the creator
	 * @param senders
	 *            the parties in the session will be generating content.
	 */
	public ContentPacketExtension(CreatorEnum creator, String disposition,
			String name, SendersEnum senders) {
		super(null, ELEMENT_NAME);
	...
	/**
	 * Sets the value of the disposition argument if it exists or <tt>null</tt>
	 * if it doesn't. The disposition argument indicates how the content
	 * definition is to be interpreted by the recipient. The meaning of this
	 * attribute matches the <tt>"Content-Disposition"</tt> header as defined in
	 * RFC 2183 and applied to SIP by RFC 3261. The value of this attribute
	 * SHOULD be one of the values registered in the IANA Mail Content
	 * Disposition Values and Parameters Registry. The default value of this
	 * attribute is "session".
	 * 
	 * @param disposition
	...
	/**
	 * Returns the value of the name argument if it exists or <tt>null</tt> if
	 * it doesn't. The name argument is a unique identifier for the content type
	 * according to the creator, which MAY have meaning to a human user in order
	 * to differentiate this content type from other content types (e.g., two
	 * content types containing video media could differentiate between
	 * "room-pan" and "slides"). If there are two content types with the same
	 * value for the 'name' attribute, they shall understood as alternative
	...
	/**
	 * Sets the value of the name argument if it exists or <tt>null</tt> if it
	 * doesn't. The name argument is a unique identifier for the content type
	 * according to the creator, which MAY have meaning to a human user in order
	 * to differentiate this content type from other content types (e.g., two
	 * content types containing video media could differentiate between
	 * "room-pan" and "slides"). If there are two content types with the same
	 ...
	 * 
	 * @param name
	 *            the value of this content's <tt>name</tt> argument.
	 */
	 ...
	 * 
	 * @param senders
	 *            a {@link SendersEnum} instance indicating the the parties that
	 *            will be generating content in this session.
	 */
