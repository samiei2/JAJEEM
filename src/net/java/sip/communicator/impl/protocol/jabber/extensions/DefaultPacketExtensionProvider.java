 ...
 * 
 * @param <C>
 *            Class that the packets we will be parsing belong to
 * @author Emil Ivov
 */
public class DefaultPacketExtensionProvider<C extends AbstractPacketExtension>
	 ...
	 * 
	 * @param c
	 *            the {@link Class} that the packets we will be parsing belong
	 *            to.
	 */
	 ...
	 * 
	 * @param parser
	 *            an XML parser positioned at the packet's starting element.
	 * 
	...
	public C parseExtension(XmlPullParser parser) throws Exception {
		...
		for (int i = 0; i < attrCount; i++) {
			packetExtension.setAttribute(parser.getAttributeName(i),
		...
		while (!done) {
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.finest("Will parse " + elementName + " ns=" + namespace
						+ " class="
			...
			if (eventType == XmlPullParser.START_TAG) {
				PacketExtensionProvider provider = (PacketExtensionProvider) ProviderManager
						.getInstance().getExtensionProvider(elementName,
								namespace);
				...
				if (provider == null) {
					...
					PacketExtension childExtension = provider
