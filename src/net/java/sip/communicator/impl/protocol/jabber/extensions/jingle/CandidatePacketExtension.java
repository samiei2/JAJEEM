 ...
 * @author Emil Ivov
 */
public class CandidatePacketExtension extends AbstractPacketExtension implements
		Comparable<CandidatePacketExtension> {
	/**
	 ...
	 * 
	 * @param elementName
	 *            the element name that this instance should be using.
	 */
	...
	/**
	 * Returns this canditate's generation. A generation is an index, starting
	 ...
	 * 
	 * @param id
	 *            this candidates's unique identifier <tt>String</tt>
	 */
	 ...
	 * 
	 * @param network
	 *            the network index indicating the interface that the candidate
	 *            belongs to.
	 */
	 ...
	 * 
	 * @param relAddr
	 *            this candidate's related address as described by ICE's RFC
	 *            5245.
	 */
	 ...
	 * candidates. All allowable values are enumerated in the
	...
	public int compareTo(CandidatePacketExtension candidatePacketExtension) {
		...
		if (this.getType() != candidatePacketExtension.getType()) {
			CandidateType[] types = { CandidateType.host, CandidateType.local,
					CandidateType.prflx, CandidateType.srflx,
					CandidateType.stun, CandidateType.relay };
